...
**Pano** is a modern WinForms grid/list/card control for .NET Windows desktop apps. It started as an ObjectListView-style replacement and grew into a themed, virtualized, multi-view data presentation control.

> Current release: **1.0.52.2 Community Preview**

Pano is still in Community Preview. The API is usable, but feedback from real projects may still shape naming, defaults and module boundaries.

> Note: In this preview, some internal project, namespace, file and class names still use the original `Pano` / `PanoControl` naming for compatibility. These names may be gradually migrated in a future release.
...
Pano/
├─ src/
│  └─ Pano/                 # Core DLL, legacy internal project name
├─ samples/
│  └─ Taylan.Pano.TestApp/         # Showcase, examples and developer center
...
All samples live in samples/Taylan.Pano.TestApp. The core DLL intentionally does not contain Example Center or Developer Center forms.
...
Pano v1.0.52.2 includes a documentation screenshot workflow inside samples/Taylan.Pano.TestApp.
...
docs/screenshots/pano-screenshot-manifest.json
docs/screenshots/pano-screenshots.md
docs/screenshots/pano-docx-insert-map.json

To append generated screenshots into the Word guide:

python tools/docs/insert_screenshots_into_docx.py \
  --docx docs/Pano_Professional_Developer_User_Guide.docx \
...
the core library currently remains under src/Taylan.Pano,
showcase and developer examples currently remain under samples/Taylan.Pano.TestApp,
...
dotnet pack src/Taylan.Pano/Taylan.Pano.csproj -c Release -o artifacts/nuget /p:PanoRepositoryUrl=https://github.com/Manisa79/Pano
Naming roadmap

The public repository and product name are now Pano.
...
A future release may introduce Pano-native API names while keeping compatibility aliases for existing preview users.
