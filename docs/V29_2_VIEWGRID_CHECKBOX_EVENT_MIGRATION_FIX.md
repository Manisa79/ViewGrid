# V29.2 ViewGrid checkbox event migration fix

Bu paket, OLV/ListView tarafındaki `ItemCheck` kullanımının `FastViewGridControl` üzerinde doğru event modeliyle karşılanması için migration örneğini günceller.

## Net karar

`FastViewGridControl` içinde `ItemCheck` yoktur ve eklenmemelidir. ViewGrid tarafında checkbox değişimi için önerilen yol:
...
`ViewGridCellClickEventArgs` içine:

```csharp
bool IsCheckBoxColumn { get; }
```

`ViewGridCellEditEventArgs` içine:
...
private void GlvPanel_CellValueChanged(object? sender, ViewGridCellEditEventArgs e)
{
...
- Kaynakta `FastViewGridControl.ItemCheck` veya `glvPanel.ItemCheck` benzeri hatalı kullanım bulunmadı.
- Kalan `.ItemCheck +=` kullanımları `CheckedListBox` tabanlı popup/chooser listelerine aittir; bunlar ViewGrid event'i değildir ve doğrudur.
