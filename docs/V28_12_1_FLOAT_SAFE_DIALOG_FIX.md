# Taylan.Pano v28.12.1 - Float Safe Dialog/Icon Fix

Bu bakım paketi v28.11 ve v28.12 değişikliklerini çalıştırmayı engelleyen derleme hatasını düzeltir.

## Düzeltmeler

- `PanoDialogIconFactory.Blend(...)` metodu `float` yerine `double` oran kabul edecek şekilde güncellendi.
- Koyu/açık tema uyumlu Pano dialog başlık ikonlarında `double -> float` dönüşüm hatası giderildi.
