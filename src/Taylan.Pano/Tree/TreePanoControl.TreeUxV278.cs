...
public enum TreePanoSearchBehavior
{
    SelectOnly,
    ExpandAncestors,
...
public partial class TreePanoControl
{
    ...
    [Category("Pano Tree - UX v27.8")]
    [DefaultValue(TreePanoSearchBehavior.ExpandAncestors)]
    public TreePanoSearchBehavior TreeSearchBehavior { get; set; } = TreePanoSearchBehavior.ExpandAncestors;
    ...
    public bool ApplyTreeSearch(string text)
    {
        ...
        if (TreeSearchExpandsAncestors || TreeSearchBehavior != TreePanoSearchBehavior.SelectOnly)
            ExpandAncestors(match);

        if (TreeSearchBehavior == TreePanoSearchBehavior.ExpandAncestorsAndDescendants)
