...
/// <summary>Pano uyumlu kolon ikon delegate imzası. BrightIdeasSoftware bağımlılığı yoktur.</summary>
public delegate object? PanoImageGetterDelegate(object rowObject);

/// <summary>Pano uyumlu kolon text delegate imzası. BrightIdeasSoftware bağımlılığı yoktur.</summary>
public delegate string? PanoAspectToStringConverterDelegate(object? value);

/// <summary>Pano uyumluluğu için kolon içi button boyutlama modu.</summary>
public enum PanoColumnButtonSizing
{
    FixedBounds = 0,
    TextBounds = 1,
    CellBounds = 2
...
public enum PanoAutoFilterMode
{
    Default = 0,
    Smart = 1,
    Text = 2,
...
public enum PanoFilterOperator
{
    Contains = 0,
    StartsWith = 1,
    EndsWith = 2,
...
[TypeConverter(typeof(PanoColumnTypeConverter))]
public class PanoColumn : Component
{
    ...
    [Description("Hücre içi scroll açıkken görünür maksimum satır sayısı. 0 ise Pano varsayılan MaxCellTextLines kullanılır.")]
    [DefaultValue(0)]
    ...
    public PanoColumn()
    {
        Name = PanoColumnNameHelper.CreateDefaultName(1);
        DefaultWidth = 120;
        DefaultVisible = true;
    }
    public PanoColumn(string title, string aspectName, int width = 120)
    {
        Header = title;
        AspectName = aspectName;
        Name = PanoColumnNameHelper.CreateNameFromAspectOrText(aspectName, title);
        Width = width;
        DefaultWidth = width;
        DefaultVisible = true;
    }
    ...
    /// <summary>
    /// Pano uyumlu kolon kimliği. PropertyGrid'de ayrıca görünmez;
    ...
    // Pano / DataGridView geçişini kolaylaştıran alias'lar.
    [Category("Pano - Compatibility"), Browsable(false)]
    ...
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)] public PanoAspectToStringConverterDelegate? AspectToStringConverter { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(null)] public string? AspectToStringFormat { get; set; }
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)] public PanoAspectToStringConverterDelegate? ValueToStringConverter { get => AspectToStringConverter; set => AspectToStringConverter = value; }
    [Category("Pano"), DefaultValue(120)]
    public int Width
    {
        get => _width;
        set
    ...
    [Category("Pano"), DefaultValue(true)]
    ...
    /// Kolon Pano kolon düzenleyicisinde görünür kalır; kullanıcı isterse geri alabilir.
    /// Kolon koleksiyonda kalır; AspectName/AspectGetter üzerinden veri erişimi bozulmaz.
    /// </summary>
    [Browsable(true)]
    [Category("Pano - Behavior")]
    [DisplayName("Private Column")]
    [DefaultValue(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
    [Description("Kolonu dahili/gizli kolon yapar. True yapıldığında kolon tasarım/runtime grid yüzeyinden, runtime kolon seçicisinden, export ve print akışından çıkar; Pano kolon düzenleyicisinde görünür kalır.")]
    ...
    [Category("Pano - Smart Filter"), DefaultValue(PanoAutoFilterMode.Default)]
    ...
    public PanoAutoFilterMode AutoFilterMode { get; set; } = PanoAutoFilterMode.Default;

    [Category("Pano - Smart Filter"), DefaultValue(PanoFilterOperator.Contains)]
    [Description("Akıllı filtre penceresinde bu kolon için varsayılan operatör.")]
    public PanoFilterOperator DefaultFilterOperator { get; set; } = PanoFilterOperator.Contains;

    [Category("Pano - Smart Filter"), DefaultValue(false)]
    ...
    [Category("Pano - Search"), DefaultValue(null)]
    ...
    [Category("Pano - Layout"), DefaultValue(false)]
    ...
    [Category("Pano - Summary"), DefaultValue(PanoAggregateMode.None)]
    [Description("Footer/summary alanında bu kolon için hesaplanacak özet türü.")]
    public PanoAggregateMode Aggregate { get; set; } = PanoAggregateMode.None;

    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public Func<IEnumerable<object>, PanoColumn, object?>? CustomAggregateGetter { get; set; }

    [Category("Pano - Column Manager"), DefaultValue(120)] public int DefaultWidth { get; set; } = 120;
    [Category("Pano - Column Manager"), DefaultValue(true)] public bool DefaultVisible { get; set; } = true;
    [Category("Pano - Behavior"), DefaultValue(true)] public bool Sortable { get; set; } = true;
    [Category("Pano - Behavior"), DefaultValue(true)] public bool Filterable { get; set; } = true;
    [Category("Pano - Behavior"), DefaultValue(true)] public bool AllowResize { get; set; } = true;
    [Category("Pano - Behavior"), DefaultValue(true)] public bool AllowReorder { get; set; } = true;
    [Category("Pano - Behavior"), DefaultValue(true)] public bool AllowGroup { get; set; } = true;
    [Category("Pano - Behavior"), DefaultValue(true)] public bool AllowColumnChooser { get; set; } = true;
    [Category("Pano - Behavior"), DefaultValue(true)]
    ...
    [Category("Pano - Layout"), DefaultValue(-1)] public int MinimumWidth { get; set; } = -1;
    [Category("Pano - Layout"), DefaultValue(-1)] public int MaximumWidth { get; set; } = -1;
    [Category("Pano - Layout"), DefaultValue(-1)] public int DisplayIndex { get => _displayIndex; set => _displayIndex = value; }
    [Category("Pano - Layout"), DefaultValue(false)] public bool WordWrap { get; set; }
    [Category("Pano - Layout"), DefaultValue(0)]
    [Description("WordWrap aktifken bu kolon için gösterilecek en fazla metin satırı. 0: Pano genel MaxCellTextLines değerini kullanır.")]
    ...
    [Category("Pano - Compatibility"), DefaultValue(true)] public bool Searchable { get; set; } = true;
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    [EditorBrowsable(EditorBrowsableState.Never)]
    ...
    [Category("Pano - Compatibility"), DefaultValue(false)] public bool IsTileViewColumn { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(0)] public int FreeSpaceProportion { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(null)] public string? ToolTipText { get; set; }
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)] public object? Tag { get; set; }

    // Pano migration aliases. Designer'da kolon bazlı yönetilebilir.
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    [EditorBrowsable(EditorBrowsableState.Never)]
    ...
    // Pano kolon editörü uyumluluk alanları.
    // Bunların bir kısmı Pano tarafında doğrudan çizim davranışına bağlanır,
    ...
    public bool CheckBoxes
    {
        ...
        set
        {
            ...
            else if (Kind == PanoColumnKind.CheckBox)
            {
                HeaderCheckBox = false;
                Kind = PanoColumnKind.Text;
            }
        }
    }
    [Category("Pano - Compatibility"), DefaultValue(false)] public bool TriStateCheckBoxes { get; set; }
    [Category("Pano - CheckBox"), DefaultValue(false)]
    ...
    [Category("Pano - CheckBox"), DefaultValue("")]
    ...
    [Category("Pano - Compatibility"), DefaultValue(false)] public bool HeaderCheckBoxDisabled { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(true)] public bool HeaderCheckBoxUpdatesRowCheckBoxes { get; set; } = true;
    [Category("Pano - Compatibility"), DefaultValue(false)] public bool CellEditUseWholeCell { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(false)] public bool EnableButtonWhenItemIsDisabled { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(false)] public bool Hyperlink { get => Kind == PanoColumnKind.Hyperlink; set { if (value) Kind = PanoColumnKind.Hyperlink; } }
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    [EditorBrowsable(EditorBrowsableState.Never)]
    ...
    [Category("Pano - Compatibility"), DefaultValue(false)] public bool ShowTextInHeader { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(true)] public bool UseInitialLetterForGroup { get; set; } = true;
    [Category("Pano - Compatibility"), DefaultValue(true)] public bool AutoCompleteEditor { get; set; } = true;
    [Category("Pano - Compatibility"), DefaultValue(System.Windows.Forms.AutoCompleteMode.Append)] public System.Windows.Forms.AutoCompleteMode AutoCompleteEditorMode { get; set; } = System.Windows.Forms.AutoCompleteMode.Append;
    [Category("Pano - Button"), DefaultValue("")]
    ...
    [Category("Pano - Compatibility"), DefaultValue(-1)] public int ButtonMaxWidth { get; set; } = -1;
    [Category("Pano - Compatibility"), DefaultValue(typeof(Padding), "0, 0, 0, 0")] public Padding ButtonPadding { get; set; } = Padding.Empty;
    [Category("Pano - Compatibility"), DefaultValue(typeof(Size), "0, 0")] public Size ButtonSize { get; set; } = Size.Empty;
    [Category("Pano - Compatibility"), DefaultValue(PanoColumnButtonSizing.TextBounds)] public PanoColumnButtonSizing ButtonSizing { get; set; } = PanoColumnButtonSizing.TextBounds;
    [Category("Pano - Compatibility"), DefaultValue(typeof(Padding), "0, 0, 0, 0")] public Padding CellPadding { get; set; } = Padding.Empty;
    [Category("Pano - Compatibility"), DefaultValue(ContentAlignment.MiddleCenter)] public ContentAlignment CellVerticalAlignment { get; set; } = ContentAlignment.MiddleCenter;
    [Category("Pano - Compatibility"), DefaultValue(null)] public string? GroupWithItemCountFormat { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(null)] public string? GroupWithItemCountSingularFormat { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(null)] public Font? HeaderFont { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(null)] public object? HeaderFormatStyle { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(null)] public string? HeaderImageKey { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(-1)] public int HeaderImageIndex { get; set; } = -1;
    [Category("Pano - Compatibility"), DefaultValue(null)] public string? HeaderToolTipText { get; set; }
    [Category("Pano - Compatibility"), DefaultValue(null)] public object? Renderer { get; set; }

    [Category("Pano"), DefaultValue(false)] public bool Editable { get; set; }
    [Category("Pano - Database"), DefaultValue(false)] public bool ReadOnly { get; set; }
    [Category("Pano - Database"), DefaultValue(false)] public bool Required { get; set; }
    [Category("Pano - Database"), DefaultValue(0)] public int MaxLength { get; set; }
    [Category("Pano - Editing"), DefaultValue(Taylan.Pano.Editing.PanoCellEditorKind.Auto)] public Taylan.Pano.Editing.PanoCellEditorKind EditorType { get; set; } = Taylan.Pano.Editing.PanoCellEditorKind.Auto;
    [Category("Pano"), DefaultValue(PanoColumnKind.Text)] public PanoColumnKind Kind { get; set; } = PanoColumnKind.Text;
    [Category("Pano"), DefaultValue(ContentAlignment.MiddleLeft)] public ContentAlignment TextAlign { get; set; } = ContentAlignment.MiddleLeft;

    // v25.18 Pano header visuals: eski liste kontrolündeki kolon checkbox, dikey başlık, renkli başlık ve başlık ikonu davranışları.
    [Category("Pano - Header"), DefaultValue(false)]
    ...
    [Category("Pano - Header"), DefaultValue(CheckState.Unchecked)] public CheckState HeaderCheckState { get; set; } = CheckState.Unchecked;
    [Category("Pano - Header"), DefaultValue(false)] public bool HeaderCheckBoxThreeState { get; set; }
    [Category("Pano - Header"), DefaultValue(false)] public bool HeaderTextVertical { get; set; }
    [Category("Pano - Header"), DefaultValue(0)] public int HeaderTextAngle { get; set; }
    [Category("Pano - Header"), DefaultValue(ContentAlignment.MiddleLeft)] public ContentAlignment HeaderTextAlign { get; set; } = ContentAlignment.MiddleLeft;
    [Category("Pano - Header"), DefaultValue(typeof(Color), "Empty")] public Color HeaderForeColor { get; set; } = Color.Empty;
    [Category("Pano - Header"), DefaultValue(typeof(Color), "Empty")] public Color HeaderBackColor { get; set; } = Color.Empty;
    [Category("Pano - Header"), DefaultValue(null)] public Image? HeaderImage { get; set; }
    [Category("Pano - Header"), DefaultValue(ContentAlignment.MiddleLeft)] public ContentAlignment HeaderImageAlign { get; set; } = ContentAlignment.MiddleLeft;
    [Category("Pano - Header"), DefaultValue(true)] public bool HeaderImageBeforeText { get; set; } = true;
    [Category("Pano - Header"), DefaultValue(16)] public int HeaderImageSize { get; set; } = 16;

    // Pano migration aliases. Yeni kullanımda Header* adları tercih edilir.
    [Category("Pano - Header"), Browsable(false)] public bool HeaderUsesCheckBox { get => HeaderCheckBox; set => HeaderCheckBox = value; }
    [Category("Pano - Header"), Browsable(false)] public bool HeaderIsVertical { get => HeaderTextVertical; set => HeaderTextVertical = value; }
    [Category("Pano - Header"), Browsable(false)] public Color HeaderTextColor { get => HeaderForeColor; set => HeaderForeColor = value; }
    [Category("Pano - Header"), Browsable(false)] public Color HeaderColor { get => HeaderBackColor; set => HeaderBackColor = value; }
    [Category("Pano - Header"), Browsable(false)] public Image? HeaderIcon { get => HeaderImage; set => HeaderImage = value; }

    [Category("Pano - Header Icons"), DefaultValue(Taylan.Pano.Core.PanoFilterIconStyle.Inherit)]
    public Taylan.Pano.Core.PanoFilterIconStyle FilterIconStyle { get; set; } = Taylan.Pano.Core.PanoFilterIconStyle.Inherit;

    [Category("Pano - Header Icons"), DefaultValue(null)]
    public Image? FilterIcon { get; set; }

    [Category("Pano - Header Icons"), DefaultValue(Taylan.Pano.Core.PanoSortGlyphStyle.Inherit)]
    public Taylan.Pano.Core.PanoSortGlyphStyle SortGlyphStyle { get; set; } = Taylan.Pano.Core.PanoSortGlyphStyle.Inherit;
    ...
    [Category("Pano - Pinning")]
    [DefaultValue(Taylan.Pano.Core.PanoColumnPinMode.None)]
    [Description("Excel/AG Grid style column pinning metadata. Renderers can keep pinned columns fixed left/right.")]
    public Taylan.Pano.Core.PanoColumnPinMode PinMode { get; set; } = Taylan.Pano.Core.PanoColumnPinMode.None;

    [Category("Pano - Sparkline")]
    [DefaultValue(false)]
    ...
    [Category("Pano"), DefaultValue(5)] public int MaxRating { get; set; } = 5;
    [Category("Pano"), DefaultValue("★")] public string RatingSymbol { get; set; } = "★";
    [Category("Pano"), DefaultValue(",")] public string TagSeparator { get; set; } = ",";
    [Category("Pano"), DefaultValue(false)] public bool FillFreeSpace { get; set; }
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)] public Image? Image { get; set; }
    /// <summary>
    /// Pano uyumlu kolon bazlı ikon üretici. Image, Icon, Bitmap, string ImageList key,
    /// int ImageList index veya null dönebilir. Eski Pano kodundaki Image döndüren delegate'ler de çalışır.
    /// </summary>
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)] public PanoImageGetterDelegate? ImageGetter { get; set; }

    /// <summary>Pano benzeri sabit ImageList key/index. ImageGetter yoksa kullanılır.</summary>
    [Category("Pano - Compatibility"), DefaultValue(null)] public object? ImageKey { get; set; }

    [Category("Pano - Compatibility"), DefaultValue(-1)] public int ImageIndex { get; set; } = -1;

    /// <summary>Pano benzeri state image üretici. StateImageList veya SmallImageList üzerinden çözümlenir.</summary>
    [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)] public PanoImageGetterDelegate? StateImageGetter { get; set; }

    [Category("Pano - Compatibility"), DefaultValue("")] public string ImageAspectName { get; set; } = string.Empty;
    ...
    [Category("Pano - Editing"), DefaultValue(-999999999)] public decimal NumericMinimum { get; set; } = -999999999m;
    [Category("Pano - Editing"), DefaultValue(999999999)] public decimal NumericMaximum { get; set; } = 999999999m;
    [Category("Pano - Editing"), DefaultValue(0)] public int NumericDecimalPlaces { get; set; }
    [Category("Pano - Editing"), DefaultValue(Taylan.Pano.Editing.PanoDateTimeEditorFormat.Short)] public Taylan.Pano.Editing.PanoDateTimeEditorFormat DateTimeFormat { get; set; } = Taylan.Pano.Editing.PanoDateTimeEditorFormat.Short;
    [Category("Pano - Editing"), DefaultValue("dd.MM.yyyy HH:mm")] public string DateTimeCustomFormat { get; set; } = "dd.MM.yyyy HH:mm";
    [Category("Pano - Editing"), DefaultValue(true)] public bool DateTimeNullable { get; set; } = true;
    ...
    private object? TryEvaluateFormula(object row)
    {
        try
        {
            return new PanoExpressionEngine().Evaluate(row, Formula);
        }
        catch
        {
            return null;
    ...
    /// <summary>
    /// Designer collection list display. Matches the Pano convention:
    /// olvColumnName (AspectName). For Pano this becomes glvColumnName (AspectName).
    /// </summary>
    public override string ToString() => DisplayText;
}

public sealed class PanoColumnTypeConverter : ExpandableObjectConverter
{
    ...
    {
        if (destinationType == typeof(string) && value is PanoColumn column)
