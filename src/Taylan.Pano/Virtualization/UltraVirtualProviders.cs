...
/// <summary>
/// Server/query backed provider contract. PanoControl asks the provider to apply the
...
public interface IQueryRowProvider : IRowProvider
{
    ...
    void ApplyView(PanoFilterSet filters, PanoColumn[] columns, PanoColumn? sortColumn, bool sortDescending);
    bool TryGetDistinctValues(PanoColumn column, PanoFilterSet filters, PanoColumn[] columns, int maxValues, string? searchText, out IReadOnlyList<string> values);
}
...
public sealed class DbQueryRowProvider : IQueryRowProvider, IAsyncPreloadRowProvider, IDisposable
{
    ...
    private void AppendWhere(StringBuilder sql, List<(string name, object? value)> parameters, PanoFilterSet filters, PanoColumn[] columns, string? ignoredAspectName)
    {
    ...
    private static bool IsKnownColumn(string? name, PanoColumn[] columns)
