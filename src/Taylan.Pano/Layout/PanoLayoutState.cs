...
namespace Taylan.Pano.Layout;

public sealed class PanoLayoutState
{
    public List<PanoColumnLayout> Columns { get; set; } = new();
    ...
    public List<PanoColumnFilter> ColumnFilters { get; set; } = new();
    ...
    public static PanoLayoutState Capture(IEnumerable<PanoColumn> columns, string globalFilter, string? sortAspect, bool sortDescending)
    {
        var state = new PanoLayoutState { GlobalFilter = globalFilter ?? string.Empty, SortAspectName = sortAspect, SortDescending = sortDescending };
        int order = 0;
        foreach (var c in columns)
            state.Columns.Add(new PanoColumnLayout { AspectName = c.AspectName, Text = c.Header, Width = c.Width, Visible = c.Visible, DisplayIndex = order++ });
        return state;
    }

    public void Apply(IList<PanoColumn> columns)
    {
    ...
    public static PanoLayoutState FromJson(string json) => JsonSerializer.Deserialize<PanoLayoutState>(json) ?? new PanoLayoutState();
}

public sealed class PanoLayoutProfile
{
    ...
    public string PanoVersion { get; set; } = Taylan.Pano.Core.PanoVersionInfo.Version;
    public DateTime CreatedAt { get; set; } = DateTime.Now;
    public DateTime UpdatedAt { get; set; } = DateTime.Now;
    public PanoLayoutState State { get; set; } = new();

    public string ToJson() => JsonSerializer.Serialize(this, new JsonSerializerOptions { WriteIndented = true });
    public static PanoLayoutProfile FromJson(string json) => JsonSerializer.Deserialize<PanoLayoutProfile>(json) ?? new PanoLayoutProfile();
}

public static class PanoProfileMigrator
{
    public static PanoLayoutProfile FromLegacyColumnProfileJson(string json, string name)
    {
        using JsonDocument doc = JsonDocument.Parse(json);
        PanoLayoutProfile profile = new PanoLayoutProfile { Name = name, DisplayName = name, ProfileVersion = 29 };
        ...
        if (doc.RootElement.TryGetProperty("Columns", out JsonElement columns) && columns.ValueKind == JsonValueKind.Array)
        {
            ...
            {
                PanoColumnLayout layout = new PanoColumnLayout();
        ...
        {
            profile.State.Columns.Add(new PanoColumnLayout
            {
                AspectName = key,
                Text = key,
...
public sealed class PanoColumnLayout
{
