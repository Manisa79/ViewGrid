...
{
    private readonly TextBox _search = new() { Dock = DockStyle.Fill, PlaceholderText = PanoText.ColumnSearchPlaceholder, BorderStyle = BorderStyle.FixedSingle };
    ...
    private readonly Button _all = new() { Text = PanoText.ShowAll, Width = 112, Height = 30 };
    private readonly Button _none = new() { Text = PanoText.HideAll, Width = 112, Height = 30 };
    private readonly Button _ok = new() { Text = PanoText.OK, DialogResult = DialogResult.OK, Width = 92, Height = 30 };
    private readonly Button _cancel = new() { Text = PanoText.Cancel, DialogResult = DialogResult.Cancel, Width = 92, Height = 30 };
    ...
    private readonly IList<PanoColumn> _columns;
    private readonly PanoTheme _theme;
    private readonly Dictionary<PanoColumn, bool> _pendingStates = new();
    ...
    public ColumnChooserForm(IList<PanoColumn> columns) : this(columns, WindowsThemeService.CurrentTheme())
    {
    }

    public ColumnChooserForm(IList<PanoColumn> columns, PanoTheme? theme)
    {
        _columns = columns;
        _theme = theme ?? WindowsThemeService.CurrentTheme();

        Text = PanoText.ColumnChooserTitle;
        Width = 450;
        Height = 460;
        MinimumSize = new Size(380, 360);
        KeyPreview = true;
        StartPosition = FormStartPosition.CenterParent;
        PanoDialogChrome.ConfigureStandardDialog(this, _theme, new Size(380, 360), sizeable: true, iconKind: PanoDialogIconKind.Column);
        AcceptButton = _ok;
    ...
    {
        PanoDialogChrome.ConfigureOpenPanoDialogs(this, _theme);
    ...
    private static string GetColumnText(PanoColumn c)
    {
    ...
    private static PanoColumn? GetColumnFromItem(object? item)
    {
        if (item is ColumnChooserItem chooserItem) return chooserItem.Column;
        return item as PanoColumn;
    }

    private sealed class ColumnChooserItem
    {
        public ColumnChooserItem(PanoColumn column, string text)
        {
            Column = column;
            Text = text;
        }
