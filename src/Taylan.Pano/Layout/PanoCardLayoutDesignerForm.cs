...
public sealed class PanoCardLayoutDesignerForm : Form
{
    ...
    private readonly PanoCardLayoutDefinition _definition;
    private bool _syncing;

    public PanoCardLayoutDefinition Result => _definition;

    public PanoCardLayoutDesignerForm(IEnumerable<PanoColumn> columns, PanoCardLayoutDefinition? current = null, PanoTheme? theme = null)
    {
        _definition = current ?? PanoCardLayoutDefinition.FromColumns(columns);
        Text = "Pano Card Layout Designer";
        StartPosition = FormStartPosition.CenterParent;
        Size = new Size(760, 520);
        MinimumSize = new Size(620, 420);
        Font = new Font("Segoe UI", 9F);
        PanoDialogChrome.ConfigureStandardDialog(this, theme ?? WindowsThemeService.CurrentTheme(), new Size(620, 420), sizeable: true, iconKind: PanoDialogIconKind.Designer);
        AcceptButton = _ok;
        CancelButton = _cancel;

        _role.Items.AddRange(Enum.GetNames(typeof(PanoCardFieldRole)));
        _density.Items.AddRange(Enum.GetNames(typeof(PanoCardLayoutDensity)));
        ...
        _density.SelectedIndexChanged += (_, _) => { if (Enum.TryParse<PanoCardLayoutDensity>(Convert.ToString(_density.SelectedItem), out var d)) _definition.Density = d; };
        _up.Click += (_, _) => MoveSelected(-1);
        _down.Click += (_, _) => MoveSelected(1);

        ApplyTheme(theme ?? PanoTheme.DarkTheme());
        Refill();
    ...
    private PanoCardLayoutField? SelectedField => _fields.SelectedItem as PanoCardLayoutField;
    ...
    private void LoadSelectedField()
    {
        ...
        try
        {
            _role.SelectedItem = field?.Role.ToString() ?? PanoCardFieldRole.Body.ToString();
    ...
    private void SaveSelectedField()
    {
        ...
        if (Enum.TryParse<PanoCardFieldRole>(Convert.ToString(_role.SelectedItem), out var role)) field.Role = role;
