...
public sealed class PanoQueryEngine
{
    ...
    private bool EvaluateTerm(object row, string raw)
    {
        ...
        if (colon.Success)
        {
            object? left = PanoExpressionEngine.GetValue(row, colon.Groups["name"].Value.Trim());
        ...
        foreach (string op in operators)
        {
            ...
            object? left = PanoExpressionEngine.GetValue(row, name);
