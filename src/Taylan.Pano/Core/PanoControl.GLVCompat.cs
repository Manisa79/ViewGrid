...
public sealed class PanoItemCheckedEventArgs : EventArgs
{
    public PanoItemCheckedEventArgs(object model, int rowIndex, CheckState checkState)
    {
        Model = model;
        RowObject = model;
        RowIndex = rowIndex;
...
public sealed class PanoObjectsChangedEventArgs : EventArgs
{
    public PanoObjectsChangedEventArgs(int objectCount, int visibleCount)
    {
...
public sealed class PanoDataModelEventArgs : EventArgs
{
    public PanoDataModelEventArgs(object? model, int rowIndex)
    {
        Model = model;
        RowObject = model;
        RowIndex = rowIndex;
...
public partial class PanoControl
{
    ...
    public event EventHandler<PanoItemCheckedEventArgs>? ItemChecked;

    [Category("Pano - Compatibility")]
    public event EventHandler<PanoItemCheckedEventArgs>? ObjectChecked;

    [Category("Pano - Compatibility")]
    public event EventHandler<PanoObjectsChangedEventArgs>? ObjectsChanged;

    [Category("Pano - Compatibility")]
    public event EventHandler<PanoDataModelEventArgs>? ObjectAdded;

    [Category("Pano - Compatibility")]
    public event EventHandler<PanoDataModelEventArgs>? ObjectRemoved;
    ...
    [Browsable(false)]
    public PanoColumn? PrimarySortColumnCompat => _sortColumn;

    [Browsable(false)]
    public PanoColumn? SortColumn
    {
        get => _sortColumn;
    ...
    private PanoObjectCollection? _items;

    /// <summary>
    /// GLV/ListView compatibility collection.
    /// Enables common migration code such as pano.Items.Count, Items.Add, Items.Clear.
    /// For Pano the collection is a live facade over the current model/tile provider.
    /// </summary>
    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public PanoObjectCollection Items => _items ??= new PanoObjectCollection(this);
    ...
    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public PanoObjectCollection Item => Items;
    ...
    /// <summary>GLV compatibility: current model objects. Setting this refreshes Pano.</summary>
    [Browsable(false)]
    ...
    /// <summary>Pano/GLV compatibility: current visible item count after filtering/sorting/grouping.</summary>
    ...
    /// <summary>
    /// Not: Pano v25.62 ile params SetObjects(row1, row2, ...) overload'u kaldırıldı.
    ...
    /// Pano's real scrollbar/layout synchronization lives in RefreshView().
    ...
    public PanoColumn? GetColumn(string aspectNameOrText)
    {
    ...
    public GLVColumn GetOrCreateGLVColumn(string name, string aspectName, string? text = null, int width = 120)
    {
        ...
        {
            Name = string.IsNullOrWhiteSpace(name) ? PanoColumnNameHelper.CreateNameFromAspectOrText(aspectName, text) : name
        };
        Columns.Add(column);
        RebuildColumns();
        return column;
    ...
    /// <summary>Live facade that mimics ListView/Pano Items for migration convenience.</summary>
    public sealed class PanoObjectCollection : IList<object>
    {
        private readonly PanoControl _owner;
        internal PanoObjectCollection(PanoControl owner) => _owner = owner;
    ...
    public PanoColumn? GetColumn(int visibleColumnIndex)
    {
    ...
    public PanoColumn? GetColumnByName(string name) => string.IsNullOrWhiteSpace(name) ? null : Columns.ByName(name);

    public PanoColumn? GetColumnByAspect(string aspectName) => string.IsNullOrWhiteSpace(aspectName) ? null : Columns.ByAspectName(aspectName);
    ...
    public PanoColumn? GetColumnAt(int x) => HitColumn(x);
    ...
    {
        var col = Columns.FirstOrDefault(x => x.Kind == PanoColumnKind.CheckBox);
    ...
    {
        var args = new PanoItemCheckedEventArgs(model, rowIndex, state);
    ...
    {
        ObjectsChanged?.Invoke(this, new PanoObjectsChangedEventArgs(ObjectCount, FilteredCount));
    }

    internal void OnObjectAdded(object model, int rowIndex)
    {
        ObjectAdded?.Invoke(this, new PanoDataModelEventArgs(model, rowIndex));
        OnObjectsChanged();
    }

    internal void OnObjectRemoved(object? model, int rowIndex)
    {
        ObjectRemoved?.Invoke(this, new PanoDataModelEventArgs(model, rowIndex));
        OnObjectsChanged();
    }
