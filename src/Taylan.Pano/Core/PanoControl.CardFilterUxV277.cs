...
public enum PanoCardFilterUxPlacement
{
    TopBar,
    FloatingButton,
...
public partial class PanoControl
{
    ...
    [Category("Pano - Card Filter UX v27.7")]
    [DefaultValue(true)]
    ...
    [Category("Pano - Card Filter UX v27.7")]
    [DefaultValue(PanoCardFilterUxPlacement.TopBar)]
    public PanoCardFilterUxPlacement CardFilterUxPlacement { get; set; } = PanoCardFilterUxPlacement.TopBar;
    ...
    private void InitializeCardViewFilterUx()
    {
        ...
        _cardFilterColumnBox = new ComboBox
        {
            ...
            Anchor = AnchorStyles.Left | AnchorStyles.Top,
            DisplayMember = nameof(PanoColumn.Header)
        };
    ...
    private void UpdateCardViewFilterUxLayout()
    {
        ...
        bool showBar = allowedByMode && ShowQuickFilterBar && CardFilterUxPlacement != PanoCardFilterUxPlacement.FloatingButton;
        bool showFloating = allowedByMode && ShowFloatingFilterButton && !MoveFilterButtonToTopBar && CardFilterUxPlacement != PanoCardFilterUxPlacement.TopBar;
    ...
    {
        if (_cardFilterBar == null) return;
        var theme = global::Taylan.Pano.Theming.PanoThemeAccessibility.Normalize(_theme);
        Color surface = theme.PanelBackColor;
        Color barBack = global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureSurfaceContrast(Blend(surface, theme.BackColor, theme.IsDark ? 0.10 : 0.03), theme.BackColor, theme.IsDark);
        Color fore = global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(theme.ForeColor, barBack, 4.5d);
        Color controlBack = global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureSurfaceContrast(theme.TextBackColor, barBack, theme.IsDark);
        Color controlFore = global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(theme.ForeColor, controlBack, 4.5d);
        Color buttonBack = global::Taylan.Pano.Theming.PanoThemeAccessibility.ButtonBack(theme, primary: false);
        Color buttonFore = global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(theme.ForeColor, buttonBack, 4.5d);
        Color buttonBorder = global::Taylan.Pano.Theming.PanoThemeAccessibility.ButtonBorder(theme, primary: false);
        ...
        if (_cardFilterFloatingButton != null)
        {
            ...
            _cardFilterFloatingButton.ForeColor = global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(Color.White, theme.AccentColor, 4.5d);
            _cardFilterFloatingButton.FlatStyle = FlatStyle.Flat;
            _cardFilterFloatingButton.FlatAppearance.BorderColor = global::Taylan.Pano.Theming.PanoThemeAccessibility.ButtonBorder(theme, primary: true);
        }
    }
    ...
    private void SyncCardFilterColumnList()
    {
        ...
        var current = _cardFilterColumnBox.SelectedItem as PanoColumn;
        var visibleFilterable = Columns.VisibleColumns.Where(c => c.Filterable).ToList();
        if (_cardFilterColumnBox.Items.Count == visibleFilterable.Count && visibleFilterable.SequenceEqual(_cardFilterColumnBox.Items.Cast<PanoColumn>()))
            return;
    ...
    private void BuildActiveFilterChips()
    {
        ...
        foreach (var f in _filters.Filters.Where(x => x.Enabled).ToList())
        {
            ...
            string value = f.Mode == PanoFilterMode.ValueList && f.SelectedValues != null
        ...
        if (_cardFilterChipPanel.Controls.Count == 0)
        {
            var empty = new Label
            {
                ...
                ForeColor = global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(
                    global::Taylan.Pano.Theming.PanoThemeAccessibility.MutedText(_cardFilterChipPanel.BackColor == Color.Empty ? _theme.PanelBackColor : _cardFilterChipPanel.BackColor, _theme.ForeColor),
    ...
    {
        if (_cardFilterColumnBox?.SelectedItem is PanoColumn col)
        {
    ...
    [Category("Pano - Card Filter UX v27.8")]
    [DefaultValue(true)]
