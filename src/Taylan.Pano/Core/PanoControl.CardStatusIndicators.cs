...
public partial class PanoControl
{
    ...
    private bool ShouldDrawCardStatusTopBar()
    {
        ...
        return ViewMode is PanoViewMode.Tile
    ...
    private Color? ResolveCardStatusColor(object row)
    {
        ...
        {
            // Consumer delegates should not break Pano painting.
        }
        ...
        if (statusValue == null)
        {
            PanoColumn? col = FindCardStatusColumn();
            if (col != null)
            {
    ...
    private PanoColumn? FindCardStatusColumn()
    {
        if (!string.IsNullOrWhiteSpace(CardStatusAspectName))
        {
            PanoColumn? exact = Columns.FirstOrDefault(c => string.Equals(c.AspectName, CardStatusAspectName, StringComparison.OrdinalIgnoreCase));
        ...
        {
            PanoColumn? visibleExact = Columns.VisibleColumns.FirstOrDefault(c => string.Equals(c.AspectName, aspect, StringComparison.OrdinalIgnoreCase));
            if (visibleExact != null) return visibleExact;
        }

        foreach (string aspect in preferredAspects)
        {
            PanoColumn? anyExact = Columns.FirstOrDefault(c => string.Equals(c.AspectName, aspect, StringComparison.OrdinalIgnoreCase));
