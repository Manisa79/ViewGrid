...
{
    [Category("Pano - v27 User Features")]
    [DefaultValue(PanoScenario.DataTable)]
    [Description("Hazır Pano kullanım senaryosu. Runtime menüden veya designer property penceresinden değiştirilebilir.")]
    public PanoScenario ActiveScenario { get; set; } = PanoScenario.DataTable;
    ...
    [Category("Pano - v27 User Features")]
    [DefaultValue("")]
    [Description("Kullanıcı presetlerinin saklanacağı klasör. Boşsa LocalAppData/Pano/States kullanılır.")]
    ...
    /// </summary>
    public void ApplyScenario(PanoScenario scenario)
    ...
    /// </summary>
    public void ApplyScenario(PanoScenario scenario, bool updateActiveScenario)
    {
        ...
        if (ScenarioAppliesViewDefaults)
        {
            ...
            PanoScenarioExtensions.ApplyScenario(this, scenario);
        }
    ...
    public string ResolveStateFilePath(string fallbackName = "pano-state")
    {
    ...
    private string GetUserStatePresetFolder()
    {
        ...
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), appName, "Pano", "States");
    }
    ...
    private void AddScenarioMenu(ToolStripItemCollection items)
    {
        ...
        foreach (PanoScenario scenario in Enum.GetValues(typeof(PanoScenario)))
        {
            ...
            item.Click += (_, _) => ApplyScenario((PanoScenario)item.Tag!, updateActiveScenario: true);
    ...
    private void SaveStateWithDialog()
    {
        ...
        {
            Filter = "Pano state (*.json)|*.json|Tüm dosyalar (*.*)|*.*",
            FileName = MakeSafeFileName(string.IsNullOrWhiteSpace(Name) ? "pano-state" : Name) + ".json",
            AddExtension = true,
            DefaultExt = "json"
        };
    ...
    {
        string? presetName = PanoPresetNameDialog.ShowDialog(FindForm(), "Preset adı", string.IsNullOrWhiteSpace(Name) ? "PanoPreset" : Name);
        if (!string.IsNullOrWhiteSpace(presetName))
            SaveStatePreset(presetName);
    }
}

internal sealed class PanoPresetNameDialog : Form
{
    ...
    private PanoPresetNameDialog(string caption, string defaultValue)
    {
        Text = caption;
    ...
    {
        using var form = new PanoPresetNameDialog(caption, defaultValue);
