...
public partial class PanoControl
{
    ...
    {
        if (!ShowDropIndicator || _smartDropTargetViewIndex < 0 || _smartDropPosition == PanoDropPosition.None) return;
        int header = ShowHeader ? HeaderHeight : 0;
        int y = header + (_smartDropTargetViewIndex - _scrollY) * RowHeight;
        if (_smartDropPosition == PanoDropPosition.After) y += RowHeight;
