...
public partial class PanoControl
{
    ...
    [DefaultValue(PanoMenuIconMode.BuiltIn)]
    [Description("Popup/context menülerde ikon gösterimini belirler. None: ikon yok, BuiltIn: Pano ikonları, Custom: kullanıcı ikonları, BuiltInThenCustom: özel ikon varsa onu, yoksa yerleşik ikon.")]
    public PanoMenuIconMode MenuIconMode { get; set; } = PanoMenuIconMode.BuiltIn;

    [Category("Pano - Menu Icons")]
    [DefaultValue(PanoMenuIconSize.Small16)]
    public PanoMenuIconSize MenuIconSize { get; set; } = PanoMenuIconSize.Small16;
    ...
    [Category("Pano - Menu Icons")]
    [DefaultValue(true)]
    [Description("Kullanıcı ContextMenuStrip verdiğinde Pano otomatik eklenen alt menüye ve desteklenen öğelere ikon uygular.")]
    ...
    {
        if (items == null || MenuIconMode == PanoMenuIconMode.None) return;
    ...
    private Image? ResolveMenuIcon(string key)
    {
        ...
        if (MenuIconMode is PanoMenuIconMode.Custom or PanoMenuIconMode.BuiltInThenCustom)
        {
            ...
            if (MenuIconMode == PanoMenuIconMode.Custom) return null;
        }
    ...
    private Image? TryGetCustomMenuIconFromImageList(string key)
    {
        ...
        bool dark = MenuIconThemeMode == PanoMenuIconTheme.Dark || (MenuIconThemeMode == PanoMenuIconTheme.Auto && _theme.IsDark);
        bool light = MenuIconThemeMode == PanoMenuIconTheme.Light || (MenuIconThemeMode == PanoMenuIconTheme.Auto && !_theme.IsDark);
