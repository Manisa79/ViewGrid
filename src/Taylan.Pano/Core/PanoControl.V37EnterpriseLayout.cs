...
public partial class PanoControl
{
    ...
    [Category("Pano - V37 Enterprise Layout")]
    [DefaultValue(false)]
    ...
    [Category("Pano - V37 Enterprise Layout")]
    [DefaultValue(PanoV37LayoutScope.User)]
    [Description("Layout profilinin kullanıcı, proje, makine veya senaryo bazında ele alınacağını belirtir.")]
    public PanoV37LayoutScope LayoutScope { get; set; } = PanoV37LayoutScope.User;
    ...
    public PanoLayoutPresetInfo CreateEnterpriseLayoutSnapshot(string name)
    {
        var preset = new PanoLayoutPresetInfo
        {
        ...
        if (LayoutIncludeColumns)
        {
            ...
            {
                preset.Columns.Add(new PanoColumnLayoutInfo
                {
                    Name = column.Name,
    ...
    public bool LoadEnterpriseLayoutFromJson(string json)
    {
        ...
        PanoLayoutPresetInfo? preset;
        try
        {
            preset = JsonSerializer.Deserialize<PanoLayoutPresetInfo>(json);
        }
        catch
        {
            return false;
        ...
        try
        {
            if (LayoutIncludeViewMode && Enum.TryParse<PanoViewMode>(preset.ViewMode, out var mode))
                SetViewMode(mode);
