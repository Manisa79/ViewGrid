...
{
    private PanoPluginCollection? _plugins;
    private readonly List<PanoConditionalRule> _conditionalRules = new();
    private readonly System.Windows.Forms.Timer _liveRefreshTimer = new();
    private PanoLiveUpdateMode _liveUpdateMode;
    ...
    public event EventHandler<PanoCardActionClickEventArgs>? CardActionClick;

    [Category("Pano - Enterprise")]
    [DefaultValue(false)]
    ...
    [Category("Pano - Live")]
    [DefaultValue(PanoLiveUpdateMode.Off)]
    public PanoLiveUpdateMode LiveUpdateMode
    {
        get => _liveUpdateMode;
        set
        {
    ...
    [Category("Pano - Live")]
    [DefaultValue(PanoChangedRowAnimation.Flash)]
    public PanoChangedRowAnimation ChangedRowAnimation { get; set; } = PanoChangedRowAnimation.Flash;

    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public IList<PanoConditionalRule> ConditionalRules => _conditionalRules;

    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public PanoPluginCollection Plugins => _plugins ??= new PanoPluginCollection(this);

    private void ConfigureLiveRefreshTimer()
    {
        _liveRefreshTimer.Stop();
        _liveRefreshTimer.Tick -= LiveRefreshTimerOnTick;
        if (_liveUpdateMode != PanoLiveUpdateMode.TimerRefresh || DesignMode || IsDisposed) return;
    ...
    public void AddConditionalRule(PanoConditionalRule rule)
    {
    ...
    public void ApplyLayoutProfile(PanoLayoutProfile profile)
    {
    ...
    public PanoLayoutProfile CaptureLayoutProfile(string name)
    {
        return new PanoLayoutProfile
        {
            Name = name,
            DisplayName = name,
    ...
    public IReadOnlyList<PanoSmartSearchToken> ParseSmartSearch(string query)
    {
        List<PanoSmartSearchToken> result = new();
        ...
        foreach (string raw in query.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            result.Add(new PanoSmartSearchToken { ColumnKey = columnKey, Text = token, Exclude = exclude });
        }
        return result;
    }
    ...
    {
        IReadOnlyList<PanoSmartSearchToken> tokens = ParseSmartSearch(query);
        if (tokens.Count == 0) return true;

        foreach (PanoSmartSearchToken token in tokens)
        {
    ...
    private bool SmartSearchTokenMatches(object row, PanoSmartSearchToken token)
    {
        IEnumerable<PanoColumn> searchColumns = Columns.VisibleColumns.Where(c => c.Searchable);
        ...
        foreach (PanoColumn column in searchColumns)
        {
    ...
    {
        if (row == null || ChangedRowAnimation == PanoChangedRowAnimation.None) return;
        int index = IndexOfObject(row);
        if (index < 0) return;
        HighlightRow(index, color ?? _theme.AccentColor, 1200, reason);
    }

    public object? CalculateAggregate(PanoColumn column)
    {
        if (column == null) throw new ArgumentNullException(nameof(column));
        List<object> rows = GetObjects().ToList();
        if (column.Aggregate == PanoAggregateMode.Custom)
        ...
        {
            PanoAggregateMode.Count => rows.Count,
            PanoAggregateMode.Sum => nums.Sum(),
            PanoAggregateMode.Average => nums.Count == 0 ? 0 : nums.Average(),
            PanoAggregateMode.Min => nums.Count == 0 ? 0 : nums.Min(),
            PanoAggregateMode.Max => nums.Count == 0 ? 0 : nums.Max(),
            _ => null
        };
    }
}
