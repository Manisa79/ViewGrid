...
public partial class PanoControl
{
    ...
    /// tekrar görünüyordu. Pano/ListView benzeri davranışta mevcut kolonlar korunur,
    ...
    private void MergeColumnsFromDataTable(DataTable table)
    {
        foreach (DataColumn dc in table.Columns)
        {
            ...
            if (existing != null)
            {
                ...
                existing.Kind = existing.Kind == PanoColumnKind.Text ? GetKindForDataColumn(dc) : existing.Kind;
                existing.EditorType = existing.EditorType == PanoCellEditorKind.Auto ? GetEditorForDataColumn(dc) : existing.EditorType;
    ...
    private static PanoColumn CreateColumnFromDataColumn(DataColumn dc)
    {
        var kind = GetKindForDataColumn(dc);
        var col = new PanoColumn(dc.Caption == dc.ColumnName ? dc.ColumnName : dc.Caption, dc.ColumnName, GetDefaultWidth(dc.DataType))
        {
            Kind = kind,
        ...
        if (kind == PanoColumnKind.CheckBox) col.Width = 80;
        return col;
    }

    private static PanoColumnKind GetKindForDataColumn(DataColumn dc)
    {
        var type = Nullable.GetUnderlyingType(dc.DataType) ?? dc.DataType;
        if (type == typeof(bool)) return PanoColumnKind.CheckBox;
        if (type == typeof(DateTime)) return PanoColumnKind.Date;
        if (IsNumericType(type)) return PanoColumnKind.Numeric;
        return PanoColumnKind.Text;
    }

    private static PanoCellEditorKind GetEditorForDataColumn(DataColumn dc)
    {
        var type = Nullable.GetUnderlyingType(dc.DataType) ?? dc.DataType;
        if (type == typeof(bool)) return PanoCellEditorKind.CheckBox;
        if (type == typeof(DateTime)) return PanoCellEditorKind.DateTime;
        if (IsNumericType(type)) return PanoCellEditorKind.Numeric;
        return PanoCellEditorKind.TextBox;
    }
