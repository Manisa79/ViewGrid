...
/// <summary>
/// Pano migration presets kept for source compatibility with older MasterData/Pano screens.
/// New code can still use Taylan.Pano.Presets.PanoPresets directly.
/// </summary>
public enum PanoPreset
{
    Default,
    ClassicPano,
    LogView,
...
{
    public FormatCellEventArgs(int rowIndex, int columnIndex, object rowObject, PanoColumn column, object? cellValue, ListViewItem.ListViewSubItem subItem)
    {
        RowIndex = rowIndex;
...
{
    /// <summary>
    /// Pano compatible row formatting event. Set e.Item.BackColor / ForeColor / Font.
    /// Pano applies BackColor and ForeColor while painting the visible row.
    /// </summary>
    ...
    [Category("Pano - Compatibility")]
    [DefaultValue(true)]
    [Description("When true Pano shows its built-in body context menu if BodyContextMenuStrip is null.")]
    public bool UseBuiltInBodyContextMenu { get; set; } = true;

    [Category("Pano - Compatibility")]
    [DefaultValue(null)]
    [Description("Pano style body context menu. Header menus remain Pano's filter/sort menu.")]
    ...
    [Category("Pano - Compatibility")]
    [DefaultValue(true)]
    [Description("True ise Text kolonlarında da ImageGetter/StateImageGetter sonucu Pano gibi metnin solunda çizilir.")]
    ...
    {
        get => false;
        set { /* Pano zaten owner-draw çalışır; Pano migration için no-op. */ }
    }
    ...
    [Browsable(false)]
    public PanoColumnCollection AllColumns => Columns;

    public void ApplyPreset(PanoPreset preset)
    {
        switch (preset)
        {
            case PanoPreset.AoiFailList:
                PanoPresets.ApplyAoiFailList(this);
                break;
            case PanoPreset.DatabaseEditor:
                PanoPresets.ApplyDatabaseEditorDefaults(this);
                break;
            case PanoPreset.LogView:
                ApplyClassicPanoPreset();
                RowHeight = Math.Max(RowHeight, 24);
                AlternateRows = false;
                ShowGridLines = false;
                EmptyListMessage = "Log kaydı yok";
                break;
            case PanoPreset.LineOverview:
                ApplyClassicPanoPreset();
                RowHeight = Math.Max(RowHeight, 34);
                RowColorPreset = Taylan.Pano.Theming.PanoRowColorPreset.StatusPills;
                break;
            case PanoPreset.CompareList:
                ApplyClassicPanoPreset();
                ShowGridLines = true;
                AlternateRows = true;
                ...
                break;
            case PanoPreset.ClassicPano:
            case PanoPreset.Default:
            default:
                ApplyClassicPanoPreset();
                break;
        }
    ...
    {
        if (Enum.TryParse<PanoPreset>(presetName ?? string.Empty, true, out var preset))
            ApplyPreset(preset);
        else
            ApplyPreset(PanoPreset.ClassicPano);
    }

    private void ApplyClassicPanoPreset()
    {
        ShowHeader = true;
        ShowGridLines = true;
        AlternateRows = true;
    ...
    private void ApplyFormatCellCompatibility(int viewIndex, object row, PanoColumn col, object? value, ref Color cellBack, ref Color cellFore)
    {
        if (!UseCellFormatEvents) return;
        ...
        {
            // Pano migration formatting code must never break painting.
        }
    }

    /// <summary>
    /// Pano compatible helper. Scrolls the row that owns the given model into view.
    /// Usage: grid.EnsureModelVisible(newLog);
    /// </summary>
    public void EnsureModelVisible(object? model) => EnsureObjectVisible(model);

    /// <summary>Pano compatible alias for EnsureModelVisible.</summary>
    ...
    /// <summary>Pano style selection check by model object.</summary>
    ...
    // GetItemCount() is implemented in PanoControl.GLVCompat.cs and returns ViewCount.
    ...
    public void AutoResizeColumn(PanoColumn col, ColumnHeaderAutoResizeStyle headerAutoResize)
    {
    ...
    private void AutoResizeColumnToHeader(PanoColumn col, bool refresh = true)
    {
    ...
    private void AutoResizeColumnToContentOnly(PanoColumn col, bool refresh = true)
    {
        if (col == null || !Columns.Contains(col)) return;
    ...
    /// <summary>Pano compatible alias: replaces current objects and rebuilds visible rows.</summary>
    ...
    // SelectObject, SelectObjects and DeselectObject are implemented in PanoControl.cs.
    ...
    private bool TryShowGLVBodyContextMenu(Point location)
    {
        ...
        {
            ApplyPanoThemeToMenu(BodyContextMenuStrip);
