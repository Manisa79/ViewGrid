...
using Taylan.Pano.Undo;
using Taylan.Pano.Details;
using Taylan.Pano.Theming;
using Taylan.Pano.Virtualization;
using Taylan.Pano.Localization;
using Taylan.Pano.Design;

namespace Taylan.Pano.Core;

public enum PanoEmptyStateSignatureAlignment
{
    BottomRight,
    BottomLeft,
    TopRight,
...
public enum PanoTileCheckBoxPosition
{
    TopLeft,
    TopRight,
    BottomLeft,
...
public enum PanoTileCheckBoxVisibilityMode
{
    Always,
    Hover,
    Selected,
...
public enum PanoMediaImageScaleMode
{
    Contain,
    Cover,
    Stretch
...
public enum PanoDetailCardLayout
{
    Standard,
    Compact,
    PropertyGrid,
...
[ToolboxBitmap(typeof(PanoControl), "PanoControl.bmp")]
[Description("Pano / PanoControl: Pano List View modern WinForms list/grid control.")]
public partial class PanoControl : Control
{
    ...
    private PanoColumn? _activeColumn;
    private readonly SortedSet<int> _selectedRows = new();
    private readonly HashSet<object> _checkedRows = new();
    private readonly Dictionary<PanoColumn, HashSet<object>> _internalCheckRowsByColumn = new();
    private int _selectionAnchorRow = -1;
    private PanoColumn? _resizingColumn;
    private int _resizeStartX;
    private int _resizeStartWidth;
    private PanoColumn? _dragColumn;
    ...
    private readonly PanoFilterSet _filters = new();
    private readonly List<int> _viewIndexes = new();
    private readonly List<PanoDisplayRow> _displayRows = new();
    private readonly HashSet<string> _collapsedGroups = new(StringComparer.CurrentCultureIgnoreCase);
    private PanoColumn? _sortColumn;
    private bool _sortDesc;
    private Control? _editor;
    private ICellEditor? _activeCellEditor;
    private (int row, PanoColumn col)? _editingCell;
    private PanoTheme _theme = WindowsThemeService.CurrentTheme();
    private bool _applyingTheme;
    private readonly List<PanoConditionalFormat> _conditionalFormats = new();
    private readonly List<PanoSummaryItem> _summaries = new();
    private readonly PanoUndoService _undo = new();
    private PanoRowDetailsProvider? _detailsProvider;
    private int _expandedRow = -1;
    private Control? _detailsControl;
    private bool _viewIsDirect;
    private int _dataVersion;
    private Dictionary<PanoColumn, string>? _summaryTextCache;
    private readonly PanoRenderOptions _renderOptions = new();
    ...
    private PanoMenuDismissMessageFilter? _activeMenuDismissFilter;
    ...
    private PanoViewMode _viewMode = PanoViewMode.Details;
    private PanoDataMode _mode = PanoDataMode.Object;
    ...
    public PanoControl()
    {
        _menuOptions = new PanoMenuCustomizationOptions(this);
        _menuIcons = new PanoMenuIconCustomizationOptions(this);
        MenuIconMode = PanoMenuIconMode.BuiltInThenCustom;
        SetStyle(ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.ResizeRedraw | ControlStyles.UserPaint, true);
        Columns = new PanoColumnCollection();
        ...
        WindowsThemeService.ThemeChanged += (_,__) => { if (FollowWindowsTheme || ThemePreset == PanoThemePreset.System) ApplySelectedTheme(); };
    ...
    [Category("Pano")]
    [Description("Pano kolon koleksiyonu. Designer penceresi GLV tarzı GLVColumn düzenleme sağlar.")]
    [Browsable(false)]
    [Editor(typeof(PanoColumnCollectionEditor), typeof(UITypeEditor))]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
    public PanoColumnCollection Columns { get; }

    [DefaultValue(true)] public bool FollowWindowsTheme { get; set; } = true;
    private PanoThemePreset _themePreset = PanoThemePreset.System;
    [Category("Pano - Theme"), DefaultValue(PanoThemePreset.System)]
    public PanoThemePreset ThemePreset
    {
        get => _themePreset;
        set
        {
            _themePreset = value;
            if (value != PanoThemePreset.System) FollowWindowsTheme = false;
            ApplySelectedTheme();
        }
    }
    [Category("Pano - Theme"), DefaultValue(true)]
    ...
    private PanoDesignTimeThemePreview _designTimeThemePreview = PanoDesignTimeThemePreview.Auto;
    [Category("Pano - Design Time"), DefaultValue(PanoDesignTimeThemePreview.Auto)]
    [Description("Designer önizlemesi için Auto, Light, Dark veya Fluent tema seçimi.")]
    public PanoDesignTimeThemePreview DesignTimeThemePreview
    {
    ...
    [Category("Pano - Design Time"), DefaultValue(true)]
    [Description("Visual Studio tasarım zamanında Pano'nin açık/VS designer uyumlu tema ile çizilmesini sağlar. Runtime temasını etkilemez.")]
    public bool EnableDesignTimeThemeSync { get; set; } = true;

    [Category("Pano - Design Time"), DefaultValue(false)]
    ...
    [Category("Pano - Design Time"), DefaultValue(true)]
    [Description("Designer'da Pano'ye bağlı context menülerin de açık/VS designer uyumlu tema ile çizilmesini sağlar.")]
    ...
    [Category("Pano - Appearance"), DefaultValue(PanoRowColorPreset.ThemeDefault)]
    public PanoRowColorPreset RowColorPreset { get; set; } = PanoRowColorPreset.ThemeDefault;

    [Category("Pano - Appearance"), DefaultValue("State")]
    public string RowColorAspectName { get; set; } = "State";

    [Category("Pano - Appearance"), DefaultValue(0.18)]
    ...
    [Category("Pano - Filtering"), DefaultValue(true)]
    ...
    [Category("Pano - Filtering"), DefaultValue(26)] public int ColumnFilterButtonWidth { get; set; } = 26;

    [Category("Pano - Header Icons"), DefaultValue(PanoFilterIconStyle.Funnel)]
    [Description("Kolon filtre butonunda kullanılacak default simge: Funnel, Dot veya CustomImage.")]
    public PanoFilterIconStyle FilterIconStyle { get; set; } = PanoFilterIconStyle.Funnel;

    [Category("Pano - Header Icons"), DefaultValue(null)]
    [Description("FilterIconStyle=CustomImage iken kullanılacak global filtre simgesi.")]
    public Image? CustomFilterIcon { get; set; }

    [Category("Pano - Header Icons"), DefaultValue(true)]
    [Description("Sıralama yapılan kolonda sort simgesini gösterir.")]
    public bool ShowColumnSortGlyphs { get; set; } = true;

    [Category("Pano - Header Icons"), DefaultValue(PanoSortGlyphStyle.Chevron)]
    [Description("Sort simge stili: Chevron, Triangle veya CustomImage.")]
    public PanoSortGlyphStyle SortGlyphStyle { get; set; } = PanoSortGlyphStyle.Chevron;
    ...
    [Category("Pano - Compatibility"), DefaultValue(null)]
    [Description("Pano uyumlu küçük ikon listesi. Kolon ImageGetter string key veya int index döndürürse buradan çözülür.")]
    public ImageList? SmallImageList { get; set; }

    [Category("Pano - Compatibility"), DefaultValue(null)]
    [Description("Tile/LargeIcons modları için Pano uyumlu büyük ikon listesi.")]
    public ImageList? LargeImageList { get; set; }

    [Category("Pano - Compatibility"), DefaultValue(null)]
    [Description("Check/state ikonları için Pano uyumlu state image listesi.")]
    public ImageList? StateImageList { get; set; }

    [Category("Pano - Sorting"), DefaultValue(true)]
    ...
    [Category("Pano - Sorting"), DefaultValue(5000)]
    [Description("Bu kayıt sayısından sonra sort işlemi async yürütülür.")]
    public int AsyncSortThreshold { get; set; } = 5000;

    [Category("Pano - Sorting"), DefaultValue(true)]
    [Description("Sort sırasında Pano'nin kısa bekleme göstergisi çizmesine izin verir.")]
    public bool ShowSortBusyIndicator { get; set; } = true;

    [Category("Pano - Sorting"), DefaultValue(0)]
    ...
    [Category("Pano - Context Menu"), DefaultValue(true)]
    ...
    [DefaultValue(null)] public string EmptyListMessage { get; set; } = PanoText.EmptyList;
    [Category("Pano - Appearance"), DefaultValue(true)]
    ...
    [Category("Pano - Appearance"), DefaultValue(true)] public bool AutoHideScrollBarsWhenNotNeeded { get; set; } = true;
    [Category("Pano - Appearance"), DefaultValue(true)] public bool EnableModernEmptyState { get; set; } = true;

    [Category("Pano - Appearance")]
    [DefaultValue(true)]
    [Description("Boş liste kartında Pano sürüm imzasını gösterir. Varsayılan olarak sağ altta görünür.")]
    public bool ShowEmptyStateSignature { get; set; } = true;

    [Category("Pano - Appearance")]
    [DefaultValue(PanoEmptyStateSignatureAlignment.BottomRight)]
    [Description("Boş liste kartındaki Pano sürüm imzasının konumunu belirler.")]
    public PanoEmptyStateSignatureAlignment EmptyStateSignatureAlignment { get; set; } = PanoEmptyStateSignatureAlignment.BottomRight;
    ...
    [Category("Pano - Appearance"), DefaultValue(true)] public bool EnableRowHoverGlow { get; set; } = true;
    [Category("Pano - Localization"), DefaultValue(PanoLanguage.Auto)] public PanoLanguage Language { get => PanoText.Language; set { PanoText.Language = value; Invalidate(); } }
    public void LoadLocalizationFile(string jsonFilePath) { PanoText.LoadCustomTranslations(jsonFilePath); Invalidate(); }
    ...
    [Category("Pano - DetailCard")]
    [DefaultValue(PanoDetailCardLayout.Standard)]
    ...
    public PanoDetailCardLayout DetailCardLayout { get; set; } = PanoDetailCardLayout.Standard;
    ...
    public bool ShowDetailCardColumnHeaders
    {
        ...
        set
        {
            ...
            if (ViewMode == PanoViewMode.DetailCard || ViewMode == PanoViewMode.PropertyCard)
                Invalidate();
        }
    }

    [Category("Pano - Editing"), DefaultValue(false)] public bool EnableCellEditing { get; set; } = false;
    [Category("Pano - Editing"), DefaultValue(false)] public bool CellEditActivationOnDoubleClick { get; set; } = false;
    [Category("Pano - Editing"), DefaultValue(Keys.None)] public Keys CellEditActivationKey { get; set; } = Keys.None;
    [Category("Pano - Editing"), DefaultValue(true)] public bool AllowEditAllCells { get; set; } = true;

    [Category("Pano - Context Menu"), DefaultValue(false)]
    ...
    [Category("Pano - Context Menu"), DefaultValue("")]
    ...
    [Category("Pano - Filtering"), DefaultValue(true)] public bool FastFilterMenuForHugeLists { get; set; } = true;
    [Category("Pano - Filtering"), DefaultValue(300)] public int FastFilterMenuInitialScanRows { get; set; } = 300;
    [Category("Pano - Filtering"), DefaultValue(1000000)] public int FastFilterMenuSearchScanRows { get; set; } = 1000000;

    [Category("Pano - Header"), DefaultValue(5000)]
    ...
    [Category("Pano - Filtering"), DefaultValue(true)] public bool TypedFilterSearchesAllRows { get; set; } = true;
    [Category("Pano - Filtering"), DefaultValue(2000)] public int MaxEmbeddedFilterVisibleValues { get; set; } = 2000;
    [Category("Pano - Filtering"), DefaultValue(true)] public bool AsyncLoadFullFilterValues { get; set; } = true;
    [Category("Pano - Filtering"), DefaultValue(1000000)] public int MaxAsyncFilterDistinctScanRows { get; set; } = 1000000;
    [Category("Pano - Filtering"), DefaultValue(300)] public int FastFilterPopupPreviewRows { get; set; } = 300;
    [Category("Pano - Filtering"), DefaultValue(75000)] public int FastFilterIndexedProviderThreshold { get; set; } = 75000;
    [Category("Pano - Smart Filter"), DefaultValue(true)] public bool EnableSmartFilterEngine { get; set; } = true;
    [Category("Pano - Smart Filter"), DefaultValue(true)] public bool BuildSmartFilterIndexInBackground { get; set; } = true;
    [Category("Pano - Smart Filter"), DefaultValue(35)] public int SmartFilterSearchDebounceMs { get; set; } = 35;
    [Category("Pano - Smart Filter"), DefaultValue(500)] public int SmartFilterPopupValueLimit { get; set; } = 500;
    [Category("Pano - Smart Filter"), DefaultValue(2000000)] public int SmartFilterMaxScanRows { get; set; } = 2000000;
    [Category("Pano - Smart Filter"), DefaultValue(true)] public bool SmartFilterSearchAllRows { get; set; } = true;
    [Category("Pano - Smart Filter"), DefaultValue(true)] public bool SmartFilterTopValuesFirst { get; set; } = true;
    [Category("Pano - Virtualization"), DefaultValue(300)] public int CachePreloadExtraRows { get; set; } = 300;
    ...
    [Category("Pano - Keyboard"), DefaultValue(900)] public int IncrementalSearchResetMs { get; set; } = 900;
    [Category("Pano - Highlight"), DefaultValue(true)] public bool EnableHighlightEngine { get; set; } = true;
    [Category("Pano - Highlight"), DefaultValue(7000)] public int DefaultHighlightDurationMs { get; set; } = 7000;
    ...
    [Category("Pano - Column Manager"), DefaultValue(true)] public bool EnableColumnAutoResizeOnDoubleClick { get; set; } = true;
    [Category("Pano - Column Manager"), DefaultValue(600)] public int AutoResizeMaxWidth { get; set; } = 600;
    [Category("Pano - Column Manager"), DefaultValue(true)] public bool AutoResizeIncludeHeader { get; set; } = true;
    [Category("Pano - Column Manager"), DefaultValue(2000)] public int AutoResizeSampleRows { get; set; } = 2000;
    [Category("Pano - Column Manager"), DefaultValue(true)]
    ...
    [Category("Pano - Images"), DefaultValue(true)]
    ...
    [Category("Pano - Images"), DefaultValue(24)]
    ...
    [Category("Pano - Column Manager"), DefaultValue(true)] public bool AllowColumnReorder { get; set; } = true;
    [Category("Pano - Column Manager"), DefaultValue(false)] public bool AutoSaveColumnLayout { get; set; } = false;
    [Category("Pano - Column Manager"), DefaultValue(true)] public bool ShowColumnReorderPreview { get; set; } = true;
    [Category("Pano - Column Manager"), DefaultValue(null)] public string? ColumnLayoutStorageKey { get; set; }
    ...
    [Category("Pano - Filtering"), DefaultValue(PanoFilterMenuMode.PopupMenu)]
    ...
    public PanoFilterMenuMode FilterMenuMode { get; set; } = PanoFilterMenuMode.PopupMenu;
    [Category("Pano - Filtering"), DefaultValue(true)] public bool ShowFilterStyleSelectorInContextMenu { get; set; } = true;

    [Category("Pano - Context Menu"), DefaultValue(PanoHeaderContextMenuBehavior.Full)]
    ...
    public PanoHeaderContextMenuBehavior HeaderContextMenuBehavior { get; set; } = PanoHeaderContextMenuBehavior.Full;


    [Category("Pano - Context Menu"), DefaultValue(PanoMenuProfile.Full)]
    [Description("Pano'nin hazır menü profilini belirler. None: Pano kendi sağ tık menüsünü göstermez. Custom: grup ve tekil ayarlar aynen kullanılır.")]
    public PanoMenuProfile MenuProfile { get; set; } = PanoMenuProfile.Full;

    [Category("Pano - Context Menu"), DefaultValue(PanoMenuGroups.All)]
    [Description("Başlık/kolon menüsünde hangi özellik gruplarının görüneceğini belirler.")]
    public PanoMenuGroups HeaderMenuGroups { get; set; } = PanoMenuGroups.All;

    [Category("Pano - Context Menu"), DefaultValue(PanoMenuGroups.All)]
    [Description("Liste/satır sağ tık menüsünde hangi özellik gruplarının görüneceğini belirler.")]
    public PanoMenuGroups BodyMenuGroups { get; set; } = PanoMenuGroups.All;

    [Category("Pano - Context Menu"), DefaultValue(true)]
    [Description("Pano'nin kolon başlığı sağ tık menüsünü kullanır. False ise header tarafında kendi ContextMenuStrip'inizi kullanabilirsiniz.")]
    public bool UseBuiltInHeaderMenu { get; set; } = true;

    [Category("Pano - Context Menu"), DefaultValue(true)]
    [Description("Pano'nin liste/satır sağ tık menüsünü kullanır. False ise body tarafında kendi ContextMenuStrip'inizi kullanabilirsiniz.")]
    public bool UseBuiltInBodyMenu { get; set; } = true;

    [Category("Pano - Context Menu"), DefaultValue(null)]
    [Description("Verilirse kolon başlığında Pano menüsü yerine bu özel ContextMenuStrip açılır.")]
    public ContextMenuStrip? HeaderMenuOverride { get; set; }

    [Category("Pano - Context Menu"), DefaultValue(null)]
    [Description("Verilirse liste/satır alanında Pano menüsü yerine bu özel ContextMenuStrip açılır.")]
    public ContextMenuStrip? BodyMenuOverride { get; set; }

    [Category("Pano - Context Menu"), DefaultValue(true)] public bool CloseHeaderContextMenuBeforeOpeningFilterPopup { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuFilterItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuSortItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuFreezeItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuAutoSizeItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuLayoutItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuGroupingItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowGroupHeaderContextMenu { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuToggleThisGroupItem { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuExpandAllItem { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuCollapseAllItem { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuClearGroupingItem { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowGroupMenuOnlyThisGroupItem { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuColumnChooserItem { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuModeItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuViewModeItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuFilterStyleItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuThemeItems { get; set; } = true;
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuStateItems { get => ShowStateMenuItems; set => ShowStateMenuItems = value; }
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowHeaderMenuScenarioItems { get => ShowScenarioMenuItems; set => ShowScenarioMenuItems = value; }
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuModeItems { get => ShowHeaderMenuModeItems; set => ShowHeaderMenuModeItems = value; }
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuViewModeItems { get => ShowHeaderMenuViewModeItems; set => ShowHeaderMenuViewModeItems = value; }
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuFilterStyleItems { get => ShowHeaderMenuFilterStyleItems; set => ShowHeaderMenuFilterStyleItems = value; }
    [Category("Pano - Context Menu"), DefaultValue(true)] public bool ShowMergedMenuThemeItems { get => ShowHeaderMenuThemeItems; set => ShowHeaderMenuThemeItems = value; }
    ...
    [Category("Pano - Context Menu"), DefaultValue(true)]
    [Description("Kullanıcının verdiği ContextMenuStrip içine Pano'nin temel liste özellikleri alt menüsünü otomatik ekler.")]
    public bool MergeBuiltInMenuWithUserContextMenu { get; set; } = true;

    [Category("Pano - Context Menu"), DefaultValue("Liste özellikleri")]
    [Description("ContextMenuStrip ile birleştirilen Pano alt menüsünün başlığı.")]
    public string BuiltInMenuMergeText { get; set; } = string.Empty;

    [Category("Pano - Context Menu"), DefaultValue(PanoMenuMergePlacement.Bottom)]
    [Description("Pano menüsü kullanıcı ContextMenuStrip içine nereye eklensin: Top, Bottom, BeforeFirstSeparator veya AfterFirstSeparator.")]
    public PanoMenuMergePlacement BuiltInMenuMergePlacement { get; set; } = PanoMenuMergePlacement.Bottom;

    [Category("Pano - Context Menu"), DefaultValue(PanoMenuMergePresentation.SubMenu)]
    [Description("Pano menüsü kullanıcı menüsüne nasıl eklensin: tek alt menü, direkt inline veya grup alt menüleri.")]
    public PanoMenuMergePresentation BuiltInMenuMergePresentation { get; set; } = PanoMenuMergePresentation.SubMenu;

    [Category("Pano - Context Menu"), DefaultValue(PanoMenuGroups.All)]
    [Description("Kullanıcı menüsüyle merge edilirken hangi Pano grupları eklensin.")]
    public PanoMenuGroups MergedMenuGroups { get; set; } = PanoMenuGroups.All;

    [Category("Pano - Context Menu"), DefaultValue("Mode,ViewMode,Filter,Sort,ColumnChooser,Clipboard,AutoSize,Layout,Grouping,FilterStyle,Theme")]
    [Description("Merge edilen Pano gruplarının görüntü sırası. Örn: Filter,ColumnChooser,Clipboard,AutoSize,Theme")]
    ...
    [Category("Pano - Context Menu"), DefaultValue(true)]
    [Description("Kullanıcı menüsüyle Pano menüsü arasına otomatik ayraç ekler.")]
    public bool BuiltInMenuMergeSeparator { get; set; } = true;

    [Category("Pano - Keyboard"), DefaultValue(true)] public bool KeyboardSelectFirstRowOnFocus { get; set; } = true;
    [Category("Pano - Keyboard"), DefaultValue(true)] public bool KeyboardContextMenuKeyOpensMenu { get; set; } = true;





    [Category("Pano - Filter Popup v27.3.1"), DefaultValue(true)]
    ...
    [Category("Pano - Filter Popup v27.3.1")]
    ...
    [Category("Pano - Filter Popup v28.10"), DefaultValue(true)]
    ...
    [Category("Pano - Filter Popup v28.12.2"), DefaultValue(true)]
    ...
    [Category("Pano - Rendering v27.3"), DefaultValue(true)]
    ...
    [Category("Pano - ProgressBar"), DefaultValue(false)]
    ...
    [Category("Pano - Printing"), DefaultValue(5000)]
    ...
    [Category("Pano - Printing"), DefaultValue("PanoControl")]
    public string PrintTitle { get; set; } = "PanoControl";
    ...
    [Category("Pano - Grouping"), DefaultValue(true)] public bool AllowGroupCollapse { get; set; } = true;
    [Category("Pano - Grouping"), DefaultValue(true)] public bool DrawGroupCollapseGlyph { get; set; } = true;
    [Category("Pano - Mode"), DefaultValue(PanoDataMode.Object)]
    [Description("PanoControl ana çalışma modu: Object, DataTable, Virtual, Tree veya Tile.")]
    public PanoDataMode Mode
    {
        get => _mode;
        set
        {
    ...
    [Category("Pano - View Modes")]
    [DefaultValue(PanoViewMode.Details)]
    [Description("Pano'nin gelişmiş görünüm modu. Details/List/Tile/LargeIcons gibi modları yönetir.")]
    public PanoViewMode ViewMode { get => _viewMode; set => SetViewMode(value); }
    ...
    public System.Windows.Forms.View View
    {
        ...
        {
            PanoViewMode.Poster => System.Windows.Forms.View.LargeIcon,
            PanoViewMode.MediaTile => System.Windows.Forms.View.Tile,
            PanoViewMode.Gallery => System.Windows.Forms.View.LargeIcon,
            PanoViewMode.FilmStrip => System.Windows.Forms.View.Tile,
            PanoViewMode.ExtraLargeIcons => System.Windows.Forms.View.LargeIcon,
            PanoViewMode.LargeIcons => System.Windows.Forms.View.LargeIcon,
            PanoViewMode.MediumIcons => System.Windows.Forms.View.SmallIcon,
            PanoViewMode.List => System.Windows.Forms.View.List,
            PanoViewMode.Tile => System.Windows.Forms.View.Tile,
            PanoViewMode.LargeCard => System.Windows.Forms.View.Tile,
            PanoViewMode.DashboardCard => System.Windows.Forms.View.Tile,
            PanoViewMode.RowCard => System.Windows.Forms.View.Tile,
            PanoViewMode.DetailCard => System.Windows.Forms.View.Tile,
            PanoViewMode.IconGrid => System.Windows.Forms.View.LargeIcon,
            PanoViewMode.GroupedList => System.Windows.Forms.View.Details,
            PanoViewMode.GroupCard => System.Windows.Forms.View.Tile,
            PanoViewMode.PropertyCard => System.Windows.Forms.View.Tile,
            PanoViewMode.KpiDashboard => System.Windows.Forms.View.Tile,
            PanoViewMode.HeatMap => System.Windows.Forms.View.Tile,
            PanoViewMode.MiniChart => System.Windows.Forms.View.Tile,
            PanoViewMode.RowPreview => System.Windows.Forms.View.Tile,
            PanoViewMode.Kanban => System.Windows.Forms.View.Tile,
            PanoViewMode.Timeline => System.Windows.Forms.View.Tile,
            PanoViewMode.MasterDetail => System.Windows.Forms.View.Details,
            PanoViewMode.DenseList => System.Windows.Forms.View.Details,
            _ => System.Windows.Forms.View.Details
        };
        set => SetViewMode(value switch
        {
            System.Windows.Forms.View.LargeIcon => PanoViewMode.LargeIcons,
            System.Windows.Forms.View.SmallIcon => PanoViewMode.MediumIcons,
            System.Windows.Forms.View.List => PanoViewMode.List,
            System.Windows.Forms.View.Tile => PanoViewMode.Tile,
            _ => PanoViewMode.Details
        });
    }
    ...
    private PanoMediaImageScaleMode _mediaImageScaleMode = PanoMediaImageScaleMode.Contain;
    private bool _mediaImageRoundedCorners = true;
    private bool _tileCheckBoxes;
    private PanoTileCheckBoxPosition _tileCheckBoxPosition = PanoTileCheckBoxPosition.TopRight;
    ...
    private PanoTileCheckBoxVisibilityMode _tileCheckBoxVisibilityMode = PanoTileCheckBoxVisibilityMode.Always;
    ...
    [Category("Pano - Tile / Card CheckBox")]
    [DefaultValue(false)]
    ...
    [Category("Pano - Tile / Card CheckBox")]
    [DefaultValue(PanoTileCheckBoxVisibilityMode.Always)]
    ...
    public PanoTileCheckBoxVisibilityMode TileCheckBoxVisibilityMode
    {
    ...
    public int LargeCardPreferredHeight
    {
        ...
        set
        {
            ...
            if (ViewMode == PanoViewMode.LargeCard) ResetTileRowHeightToPreferredIfNeeded();
            UpdateScrollbars();
            Invalidate();
        }
    }
    ...
    [Category("Pano - Poster Mode")]
    [DefaultValue(PanoMediaImageScaleMode.Contain)]
    ...
    public PanoMediaImageScaleMode MediaImageScaleMode
    {
    ...
    [Category("Pano - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomRowBackColor { get => _customBackColor; set { _customBackColor = value; Invalidate(); } }
    [Category("Pano - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomAlternateRowBackColor { get => _customAlternateBackColor; set { _customAlternateBackColor = value; Invalidate(); } }
    [Category("Pano - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomHotRowBackColor { get => _customHotBackColor; set { _customHotBackColor = value; Invalidate(); } }
    [Category("Pano - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomSelectionBackColor { get => _customSelectionBackColor; set { _customSelectionBackColor = value; Invalidate(); } }
    [Category("Pano - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomSelectionForeColor { get => _customSelectionForeColor; set { _customSelectionForeColor = value; Invalidate(); } }
    [Category("Pano - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomGroupBackColor { get => _customGroupBackColor; set { _customGroupBackColor = value; Invalidate(); } }
    [Category("Pano - Appearance"), DefaultValue(typeof(Color), "")] public Color CustomGroupForeColor { get => _customGroupForeColor; set { _customGroupForeColor = value; Invalidate(); } }
    [Category("Pano - Highlight"), DefaultValue(typeof(Color), "")] public Color SearchHighlightBackColor { get => _highlightBackColor; set { _highlightBackColor = value; Invalidate(); } }
    [Category("Pano - Highlight"), DefaultValue(typeof(Color), "")] public Color SearchHighlightBorderColor { get => _highlightBorderColor; set { _highlightBorderColor = value; Invalidate(); } }
    [Category("Pano - Highlight"), DefaultValue(typeof(Color), "")] public Color SearchHighlightForeColor { get => _highlightForeColor; set { _highlightForeColor = value; Invalidate(); } }
    ...
    [Category("Pano - Theme"), DefaultValue(true)]
    [Description("True ise tema/FormatRow/FormatCell/RowForeColorGetter kaynaklı düşük kontrastlı yazı renklerini arka plana göre otomatik okunur hale getirir. FastPanoControl ve log ekranlarında açık temada beyaz yazı gibi sorunları engeller.")]
    ...
    [Category("Pano - Theme"), DefaultValue(true)]
    [Description("True ise PanoControl/FastPanoControl/DataListView/TreePanoControl kolon başlığı renkleri aynı tema paletinden çizilir. Eski designer veya Pano uyumluluğundan gelen koyu/açık HeaderBackColor farklarını normalize eder.")]
    public bool AutoApplyThemeToColumnHeaders { get; set; } = true;

    [Category("Pano - Theme"), DefaultValue(true)]
    [Description("True ise header, filter/sort glyph, checkbox, grid çizgisi ve satır renkleri tema değiştiğinde tüm Pano türevlerinde aynı görsel profile çekilir.")]
    public bool UseUnifiedThemeVisuals { get; set; } = true;

    [Category("Pano - Theme"), DefaultValue(true)]
    [Description("Built-in, Pano uyumluluk ve kullanıcı tarafından verilen ContextMenuStrip menülerini Pano teması ile otomatik boyar.")]
    public bool AutoApplyThemeToContextMenus { get; set; } = true;
    [DefaultValue(false)] public bool EnableRowDetails { get; set; }
    [Browsable(false)] public IList<PanoConditionalFormat> ConditionalFormats => _conditionalFormats;
    [Browsable(false)] public IList<PanoSummaryItem> Summaries => _summaries;
    [Browsable(false)] public PanoUndoService UndoService => _undo;
    ...
    /// <summary>Pano migration alias.</summary>
    ...
    [Category("Pano - Interaction"), DefaultValue(true)]
    ...
    [Category("Pano - Keyboard"), DefaultValue(true)]
    ...
    [Browsable(false)] public PanoFilterSet Filters => _filters;
    ...
    /// <summary>Predicate tabanlı Pano uyumlu filtre alias'ı.</summary>
    [Browsable(false)]
    ...
    public void SetRowDetailsProvider(PanoRowDetailsProvider? provider) => _detailsProvider = provider;
    public void ClearRowDetailsProvider() => _detailsProvider = null;

    public PrintDocument CreatePrintDocument(string? title = null)
    {
        var state = new PanoPrintState { Title = string.IsNullOrWhiteSpace(title) ? PrintTitle : title! };
    ...
    public void ShowPrintPreview(string? title = null)
    {
        ...
        using var preview = new PrintPreviewDialog
        {
            ...
            Text = string.IsNullOrWhiteSpace(title) ? PrintTitle + " - " + PanoText.PrintPreviewSuffix : title + " - " + PanoText.PrintPreviewSuffix,
            UseAntiAlias = true
        };
        preview.Shown += (_, __) =>
        {
    ...
    private sealed class PanoPrintState
    {
        public string Title = "PanoControl";
        public int RowIndex;
        public List<object> Rows = new();
        public List<PanoColumn> Columns = new();
        public int PageNo;
    }

    private void PreparePrintState(PanoPrintState state)
    {
        state.RowIndex = 0;
        state.PageNo = 0;
    ...
    private void PrintPage(PrintPageEventArgs e, PanoPrintState state)
    {
        state.PageNo++;
        ...
        while (state.RowIndex < state.Rows.Count && y + rowHeight + footerHeight <= bounds.Bottom)
        {
            ...
            for (int i = 0; i < state.Columns.Count; i++)
            {
                ...
                if (PrintRenderProgressBars && col.Kind == PanoColumnKind.ProgressBar)
                {
                    DrawPrintProgress(g, r, ToInt(value), fore, back, grid, accent, printFont);
                }
                else
                {
                    string text = col.Kind == PanoColumnKind.ProgressBar ? ToInt(value) + "%" : Convert.ToString(value) ?? string.Empty;
        ...
        string footer = $"PanoControl {PanoVersionInfo.Version}  •  Sayfa {state.PageNo}";
    ...
    private static List<Rectangle> CalculatePrintColumnRects(IReadOnlyList<PanoColumn> columns, int left, int width, bool fitToPageWidth)
    {
        ...
        {
            var c = columns[i];
            bool checkLike = c.Kind == PanoColumnKind.CheckBox;
            minWidths[i] = checkLike ? 38 : c.Kind == PanoColumnKind.ProgressBar ? 74 : 46;
    ...
    private PanoColumn? GetActiveCheckBoxColumn()
    {
        if (_checkBoxes)
            return GetCompatibilityCheckBoxHostColumn();
        return Columns.FirstOrDefault(x => x.Kind == PanoColumnKind.CheckBox || x.CellCheckBox);
    }

    private static bool UsesInternalCheckStore(PanoColumn col)
    {
        ...
        if (col.Kind == PanoColumnKind.CheckBox)
        ...
        // keep the check state in Pano's internal store instead of trying to convert the
    ...
    private HashSet<object> GetInternalCheckRows(PanoColumn col)
    {
    ...
    private CheckState GetRowCheckState(object row, PanoColumn col)
    {
    ...
    private void SetRowCheckState(object row, PanoColumn col, CheckState state)
    {
    ...
    public void AutoResizeColumnToContent(PanoColumn col, int maxRows = -1, int padding = 24)
    {
        AutoResizeColumnToContent(col, maxRows, padding, refresh: true);
    }

    private void AutoResizeColumnToContent(PanoColumn col, int maxRows, int padding, bool refresh)
    {
    ...
    public void MoveColumn(PanoColumn column, int targetVisibleIndex)
    {
    ...
    public string GetColumnLayoutFilePath()
    {
        ...
        if (string.IsNullOrWhiteSpace(key)) key = GetType().FullName ?? "PanoControl";
        foreach (var ch in Path.GetInvalidFileNameChars()) key = key.Replace(ch, '_');
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PanoControl", "Layouts");
        Directory.CreateDirectory(dir);
        return Path.Combine(dir, key + ".json");
    }

    public PanoLayoutState CaptureColumnLayout()
    {
        var state = PanoLayoutState.Capture(Columns, _filters.GlobalText, _sortColumn?.AspectName, _sortDesc);
    ...
    public void ApplyColumnLayout(PanoLayoutState state)
    {
    ...
    public bool LoadColumnLayout()
    {
        try
        {
            ...
            ApplyColumnLayout(PanoLayoutState.FromJson(File.ReadAllText(file)));
            return true;
        }
        catch
        {
    ...
    public string GetColumnLayoutProfileFilePath(string profileName)
    {
        ...
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PanoControl", "Layouts", key);
        Directory.CreateDirectory(dir);
    ...
    public event EventHandler<PanoCellClickEventArgs>? CellClick;
    public event EventHandler<PanoCellClickEventArgs>? ButtonClick;
    public event EventHandler<PanoCellClickEventArgs>? HyperlinkClick;
    public event EventHandler<PanoCellClickEventArgs>? ItemActivate;
    public event EventHandler<PanoCellEditEventArgs>? CellValueChanged;
    public event EventHandler? SelectionChanged;

    [Browsable(false)]
    [EditorBrowsable(EditorBrowsableState.Never)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public PanoTheme Theme
    {
        get => _theme;
        set => ApplyTheme(value ?? WindowsThemeService.CurrentTheme());
    }

    public void ApplyTheme(PanoTheme theme)
    {
        _applyingTheme = true;
        try
        {
            theme = EnforceThemeAccessibility
                ? global::Taylan.Pano.Theming.PanoThemeAccessibility.Normalize(theme)
                : theme;
            _theme = theme;
    ...
    public void ApplyPanoThemeToMenu(ContextMenuStrip? menu)
    {
        ...
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(menu, _theme);
    }
    ...
    private void ApplyThemeToAllAttachedContextMenus()
    {
        ...
        ApplyPanoThemeToMenu(ContextMenuStrip);
        ApplyPanoThemeToMenu(HeaderMenuOverride);
        ApplyPanoThemeToMenu(BodyMenuOverride);
    }

    public void ApplyThemePreset(PanoThemePreset preset)
    {
        ThemePreset = preset;
    }
    ...
    private PanoTheme ResolveRuntimeTheme()
    {
        if (AutoThemeFromParent && ThemePreset == PanoThemePreset.System)
            return ResolveAutoThemeFromParent(useFluent: false);

        return PanoTheme.FromPreset(ThemePreset);
    }
    ...
    protected override void OnParentChanged(EventArgs e)
    {
        ...
        else if (AutoThemeFromParent && ThemePreset == PanoThemePreset.System)
        {
            ApplySelectedTheme();
        }
    }
    ...
    private void ApplyDesignTimePreviewTheme()
    {
        ...
        PanoTheme theme = ResolveDesignTimeTheme();
        ApplyTheme(theme);
    }

    private PanoTheme ResolveDesignTimeTheme()
    {
        PanoTheme theme = DesignTimeThemePreview switch
        {
            PanoDesignTimeThemePreview.Light => PanoTheme.LightTheme(),
            PanoDesignTimeThemePreview.Dark => PanoTheme.DarkTheme(),
            PanoDesignTimeThemePreview.Fluent => DesignTimeFollowParentTheme
                ? ResolveAutoThemeFromParent(useFluent: true)
                : PanoTheme.FluentLightTheme(),
            _ => DesignTimeFollowParentTheme
                ? ResolveAutoThemeFromParent(useFluent: false)
                : PanoTheme.LightTheme()
        };

        return NormalizeDesignTimeTheme(theme);
    }

    private PanoTheme NormalizeDesignTimeTheme(PanoTheme theme)
    {
        if (theme == null) return PanoTheme.LightTheme();
        ...
        if (DesignTimeThemePreview != PanoDesignTimeThemePreview.Dark && !theme.IsDark)
        {
    ...
    private PanoTheme ResolveAutoThemeFromParent(bool useFluent)
    {
        if (AutoThemeFromParent)
        {
            ...
            {
                var fromParent = PanoTheme.FromParentColor(parent.BackColor, parent.ForeColor);
                if (useFluent)
                    return fromParent.IsDark ? PanoTheme.FluentDarkTheme() : PanoTheme.FluentLightTheme();
                return fromParent;
            }
        }

        var systemTheme = PanoTheme.FromPreset(ThemePreset);
        if (useFluent)
            return systemTheme.IsDark ? PanoTheme.FluentDarkTheme() : PanoTheme.FluentLightTheme();
        return systemTheme;
    }
    ...
    protected override void OnBackColorChanged(EventArgs e)
    {
        ...
        if (IsInDesignMode && DesignTimeThemePreview == PanoDesignTimeThemePreview.Auto)
            ApplyDesignTimePreviewTheme();
    }
    ...
    private void EnsureDesignTimeSampleData()
    {
        ...
        try
        {
            if (Columns.Count == 0)
            {
                Columns.Add(new PanoColumn("Durum", "State", 90) { Editable = true });
                Columns.Add(new PanoColumn("Ad", "Name", 170) { Editable = true, FillFreeSpace = true });
                Columns.Add(new PanoColumn("Oran", "Rate", 80) { Editable = true, TextAlign = ContentAlignment.MiddleRight });
                Columns.Add(new PanoColumn("Puan", "Rating", 80) { Kind = PanoColumnKind.Rating, Editable = true });
            }
    ...
    {
        if (_mode != PanoDataMode.Tile) _mode = PanoDataMode.Object;
    ...
    private void ApplyModeVisualDefaults(PanoDataMode mode)
    {
        switch (mode)
        {
            case PanoDataMode.Tile:
                if (_viewMode != PanoViewMode.Tile) SetViewMode(PanoViewMode.Tile);
                break;
            case PanoDataMode.Tree:
                if (_viewMode != PanoViewMode.Details) SetViewMode(PanoViewMode.Details);
                break;
            default:
                if (_viewMode == PanoViewMode.Tile) SetViewMode(PanoViewMode.Details);
                break;
        }
    }

    public void SetViewMode(PanoViewMode mode)
    {
        _viewMode = mode;
        RememberCurrentViewModeForActiveScenario();
        if (mode == PanoViewMode.Poster)
            ApplyPosterModeDefaults(false);
        if (IsCardLikeViewMode(mode))
            _mode = PanoDataMode.Tile;
        else if (_mode == PanoDataMode.Tile)
            _mode = PanoDataMode.Object;

        if (mode == PanoViewMode.GroupedList && !EnableGrouping)
        {
        ...
        bool detailsLikeMode = mode == PanoViewMode.Details || mode == PanoViewMode.DenseList || mode == PanoViewMode.GroupedList || mode == PanoViewMode.MasterDetail;
        ...
        if (detailsLikeMode || mode == PanoViewMode.List)
        {
            int detailsHeight = mode == PanoViewMode.DenseList ? 22 : (mode == PanoViewMode.List ? 30 : 28);
        ...
        else
        {
            ...
            {
                PanoViewMode.Poster => Math.Max(180, PosterPreferredHeight),
                PanoViewMode.MediaTile => Math.Max(132, TilePreferredHeight),
                PanoViewMode.Gallery => Math.Max(170, TilePreferredHeight),
                PanoViewMode.FilmStrip => Math.Max(150, TilePreferredHeight),
                PanoViewMode.ExtraLargeIcons => 96,
                PanoViewMode.LargeIcons => 72,
                PanoViewMode.MediumIcons => 48,
                PanoViewMode.Tile => TilePosterMode ? Math.Max(180, TilePreferredHeight) : Math.Max(72, TilePreferredHeight),
                PanoViewMode.IconGrid => Math.Max(92, TilePreferredHeight),
                PanoViewMode.DashboardCard => Math.Max(150, Math.Max(TilePreferredHeight, LargeCardPreferredHeight - 12)),
                PanoViewMode.KpiDashboard => Math.Max(130, TilePreferredHeight),
                PanoViewMode.HeatMap => Math.Max(118, TilePreferredHeight),
                PanoViewMode.MiniChart => Math.Max(108, TilePreferredHeight),
                PanoViewMode.RowPreview => Math.Max(96, TilePreferredHeight),
                PanoViewMode.RowCard => Math.Max(118, TilePreferredHeight),
                PanoViewMode.PropertyCard => GetDetailCardPreferredHeight(),
                PanoViewMode.DetailCard => GetDetailCardPreferredHeight(),
                PanoViewMode.GroupCard => Math.Max(150, Math.Max(TilePreferredHeight, LargeCardPreferredHeight - 20)),
                PanoViewMode.Kanban => Math.Max(168, Math.Max(TilePreferredHeight, LargeCardPreferredHeight)),
                PanoViewMode.Timeline => Math.Max(132, TilePreferredHeight),
                PanoViewMode.LargeCard => Math.Max(LargeCardPreferredHeight, Math.Max(132, TilePreferredHeight)),
                _ => 28
            };
        }
        RefreshCardViewFilterUx();
    ...
    private static bool IsCardLikeViewMode(PanoViewMode mode)
        => mode is PanoViewMode.Tile
            or PanoViewMode.Poster
            or PanoViewMode.MediaTile
            or PanoViewMode.Gallery
            or PanoViewMode.FilmStrip
            or PanoViewMode.LargeCard
            or PanoViewMode.ExtraLargeIcons
            or PanoViewMode.LargeIcons
            or PanoViewMode.MediumIcons
            or PanoViewMode.DashboardCard
            or PanoViewMode.RowCard
            or PanoViewMode.DetailCard
            or PanoViewMode.IconGrid
            or PanoViewMode.GroupCard
            or PanoViewMode.PropertyCard
            or PanoViewMode.KpiDashboard
            or PanoViewMode.HeatMap
            or PanoViewMode.MiniChart
            or PanoViewMode.RowPreview
            or PanoViewMode.Kanban
            or PanoViewMode.Timeline;

    public static string GetViewModeDisplayName(PanoViewMode mode)
        => mode switch
        {
            PanoViewMode.Details => "Detay Liste",
            PanoViewMode.DenseList => "Yoğun Liste",
            PanoViewMode.List => "Basit Liste",
            PanoViewMode.Tile => "Kart Görünümü",
            PanoViewMode.LargeCard => "Geniş Kart",
            PanoViewMode.DashboardCard => "Dashboard Kart",
            PanoViewMode.RowCard => "Satır Kart",
            PanoViewMode.DetailCard => "DetailCard",
            PanoViewMode.MediaTile => "MediaTile",
            PanoViewMode.Gallery => "Gallery",
            PanoViewMode.FilmStrip => "FilmStrip",
            PanoViewMode.IconGrid => "İkon Grid",
            PanoViewMode.GroupedList => "Gruplu Liste",
            PanoViewMode.GroupCard => "GroupCard",
            PanoViewMode.PropertyCard => "PropertyCard",
            PanoViewMode.KpiDashboard => "KPI Dashboard",
            PanoViewMode.HeatMap => "HeatMap",
            PanoViewMode.MiniChart => "MiniChart",
            PanoViewMode.RowPreview => "RowPreview",
            PanoViewMode.Kanban => "Kanban",
            PanoViewMode.Timeline => "Zaman Akışı",
            PanoViewMode.MasterDetail => "Master-Detail",
            PanoViewMode.Poster => "Poster Görünümü",
            PanoViewMode.ExtraLargeIcons => "Poster Kart",
            PanoViewMode.LargeIcons => "Büyük İkon",
            PanoViewMode.MediumIcons => "Kompakt İkon",
            _ => mode.ToString()
        };
    ...
    public void SetColumnFilter(PanoColumnFilter filter)
    {
        _filters.Set(filter);
        BuildViewIndex();
        QueueAutoSaveUserLayout();
    ...
    public void ShowColumnChooser()
    {
        ...
        {
            MessageBox.Show((IWin32Window?)FindForm() ?? this, ex.ToString(), "Pano kolon seçici hatası", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    public void LoadLayout(string path)
    {
        ...
        var state = PanoLayoutState.FromJson(File.ReadAllText(path));
        SanitizeLayoutFiltersIfNeeded(state);
        ApplyLayout(state);
    }

    private void SanitizeLayoutFiltersIfNeeded(PanoLayoutState state)
    {
        if (state == null || PersistColumnFilters) return;

        state.GlobalFilter = string.Empty;
        state.ColumnFilters = new List<PanoColumnFilter>();
    }
    public PanoLayoutState CaptureLayout()
    {
        var state = PanoLayoutState.Capture(Columns, PersistColumnFilters ? _filters.GlobalText : string.Empty, _sortColumn?.AspectName, _sortDesc);
        state.ColumnFilters = PersistColumnFilters
            ? _filters.Filters.Select(f => new PanoColumnFilter
            {
    ...
    public void ApplyLayout(PanoLayoutState state)
    {
        if (state == null) return;
        state.Apply(Columns);
        _filters.Clear();
        ...
        if (PersistVisualPreferences)
        {
            if (!string.IsNullOrWhiteSpace(state.FilterMenuMode) && Enum.TryParse<PanoFilterMenuMode>(state.FilterMenuMode, out var filterMode)) FilterMenuMode = filterMode;
            if (!string.IsNullOrWhiteSpace(state.FilterIconStyle) && Enum.TryParse<PanoFilterIconStyle>(state.FilterIconStyle, out var filterIconStyle)) FilterIconStyle = filterIconStyle;
            if (!string.IsNullOrWhiteSpace(state.SortGlyphStyle) && Enum.TryParse<PanoSortGlyphStyle>(state.SortGlyphStyle, out var sortGlyphStyle)) SortGlyphStyle = sortGlyphStyle;
            ...
            if (!string.IsNullOrWhiteSpace(state.ViewMode) && Enum.TryParse<PanoViewMode>(state.ViewMode, out var viewMode)) ViewMode = viewMode;
            if (!string.IsNullOrWhiteSpace(state.ColumnChooserMenuMode) && Enum.TryParse<PanoColumnChooserMenuMode>(state.ColumnChooserMenuMode, out var chooserMode)) ColumnChooserMenuMode = chooserMode;
            ...
            if (!string.IsNullOrWhiteSpace(state.MenuIconMode) && Enum.TryParse<PanoMenuIconMode>(state.MenuIconMode, out var menuIconMode)) MenuIconMode = menuIconMode;
            if (!string.IsNullOrWhiteSpace(state.MenuIconSize) && Enum.TryParse<PanoMenuIconSize>(state.MenuIconSize, out var menuIconSize)) MenuIconSize = menuIconSize;
            if (!string.IsNullOrWhiteSpace(state.CustomMenuIconFolder)) CustomMenuIconFolder = state.CustomMenuIconFolder;
        }
        BuildViewIndex();
    }

    public string ExportVisibleCsv(string path, char separator = ';') => PanoExporter.SaveCsv(path, Columns, GetVisibleObjects(), separator);
    public string ExportVisibleExcel(string path, string worksheetName = "PanoControl") => PanoExporter.SaveExcelXml(path, Columns, GetVisibleObjects(), worksheetName);
    public string ExportVisiblePdf(string path, string title = "PanoControl") => PanoExporter.SavePdf(path, Columns, GetVisibleObjects(), title);
    public string ExportVisiblePdf(string path, PanoPdfExportOptions options)
    {
        options ??= new PanoPdfExportOptions();
        if (options.Mode == PanoPdfExportMode.Auto)
        {
            options.Mode = ViewMode is PanoViewMode.DashboardCard or PanoViewMode.LargeCard or PanoViewMode.RowCard or PanoViewMode.DetailCard or PanoViewMode.Tile or PanoViewMode.Kanban or PanoViewMode.Poster or PanoViewMode.MediaTile or PanoViewMode.FilmStrip
                ? PanoPdfExportMode.Card
                : PanoPdfExportMode.Table;
        }
        ...
        return PanoExporter.SavePdf(path, Columns, GetVisibleObjects(), options);
    }
    ...
    public void CopySelectionAsJsonToClipboard()
    {
        ...
        var rows = (_selectedRows.Count > 0 ? SelectedObjects : (SelectedObject == null ? Array.Empty<object>() : new[] { SelectedObject })).ToList();
        if (rows.Count == 0) return;
        Clipboard.SetText(PanoExporter.ToJson(Columns, rows));
    }

    public string ExportSelectedCsv(string path, char separator = ';') => PanoExporter.SaveCsv(path, Columns, SelectedObjects, separator);
    public string ExportSelectedExcel(string path, string worksheetName = "PanoSelection") => PanoExporter.SaveExcelXml(path, Columns, SelectedObjects, worksheetName);
    public string ExportVisibleJson(string path) => PanoExporter.SaveJson(path, Columns, GetVisibleObjects());
    public string ExportSelectedJson(string path) => PanoExporter.SaveJson(path, Columns, SelectedObjects);

    public IReadOnlyList<PanoColumnAnalytics> GetVisibleAnalytics(int maxDistinctPerColumn = 8, int maxRows = 50000)
    {
        var rows = EnumerateVisibleObjects(maxRows).ToList();
        var result = new List<PanoColumnAnalytics>();
        foreach (var col in Columns.VisibleColumns)
            result.Add(PanoColumnAnalytics.From(col, rows, maxDistinctPerColumn));
        return result;
    }

    public PanoColumnAnalytics? GetColumnAnalytics(string aspectName, int maxDistinct = 12, int maxRows = 100000)
    {
        ...
        return col == null ? null : PanoColumnAnalytics.From(col, EnumerateVisibleObjects(maxRows).ToList(), maxDistinct);
    }
    ...
    private Dictionary<PanoColumn, string> BuildSummaryTextCache()
    {
        var result = new Dictionary<PanoColumn, string>();
    ...
    private void BuildDisplayRows()
    {
        ...
        foreach (int realIndex in sourceIndexes)
        {
            ...
            if (string.IsNullOrWhiteSpace(key)) key = PanoText.EmptyValue;
        ...
        foreach (var pair in groups)
        {
            ...
            _displayRows.Add(PanoDisplayRow.Group(pair.Key, pair.Value.Count, collapsed));
            if (!collapsed)
            {
                foreach (int real in pair.Value) _displayRows.Add(PanoDisplayRow.Data(real));
            }
        }
        _viewIsDirect = false;
    }
    ...
    private readonly record struct PanoSortEntry(int Index, object? PrimaryKey, object? SecondaryKey);
    ...
    private int GetMinimumRowHeightForCurrentTileView()
    {
        return ViewMode switch
        {
            ...
            PanoViewMode.Gallery => Math.Max(170, TilePreferredHeight),
            PanoViewMode.FilmStrip => Math.Max(150, TilePreferredHeight),
            PanoViewMode.ExtraLargeIcons => TilePosterMode ? Math.Max(180, TilePreferredHeight) : 96,
            PanoViewMode.LargeIcons => TilePosterMode ? Math.Max(180, TilePreferredHeight) : 72,
            PanoViewMode.MediumIcons => 48,
            PanoViewMode.Tile => TilePosterMode ? Math.Max(180, TilePreferredHeight) : Math.Max(72, TilePreferredHeight),
            PanoViewMode.IconGrid => Math.Max(92, TilePreferredHeight),
            PanoViewMode.DashboardCard => Math.Max(150, Math.Max(TilePreferredHeight, LargeCardPreferredHeight - 12)),
    ...
    private int GetDetailCardPreferredHeight()
    {
        ...
        if (DetailCardLayout is PanoDetailCardLayout.Media or PanoDetailCardLayout.PosterLeft)
    ...
    private int GetTileWidth()
    {
        ...
        {
            PanoViewMode.Poster => Math.Max(160, PosterPreferredWidth),
            PanoViewMode.MediaTile => Math.Max(156, TilePreferredWidth),
            PanoViewMode.Gallery => Math.Max(180, TilePreferredWidth),
            PanoViewMode.FilmStrip => Math.Max(260, TilePreferredWidth),
            PanoViewMode.ExtraLargeIcons => TilePosterMode ? 160 : 270,
            PanoViewMode.LargeIcons => TilePosterMode ? 150 : 240,
            PanoViewMode.MediumIcons => TilePosterMode ? 140 : 210,
            PanoViewMode.Tile => TilePreferredWidth,
            PanoViewMode.IconGrid => Math.Max(160, TilePreferredWidth - 30),
            PanoViewMode.DashboardCard => Math.Max(320, TilePreferredWidth + 120),
            PanoViewMode.KpiDashboard => Math.Max(220, TilePreferredWidth),
            PanoViewMode.HeatMap => Math.Max(160, TilePreferredWidth - 20),
            PanoViewMode.MiniChart => Math.Max(300, TilePreferredWidth + 80),
            PanoViewMode.RowPreview => Math.Max(560, LargeCardPreferredWidth),
            PanoViewMode.RowCard => Math.Max(600, LargeCardPreferredWidth),
            PanoViewMode.PropertyCard => Math.Max(600, ClientSize.Width - VBarWidth - 12),
            PanoViewMode.DetailCard => Math.Max(600, ClientSize.Width - VBarWidth - 12),
            PanoViewMode.GroupCard => Math.Max(300, TilePreferredWidth + 90),
            PanoViewMode.Kanban => Math.Max(300, TilePreferredWidth + 90),
            PanoViewMode.Timeline => Math.Max(620, LargeCardPreferredWidth),
            PanoViewMode.LargeCard => Math.Max(LargeCardPreferredWidth, TilePreferredWidth + 140),
    ...
    private int GetTilesPerRow()
    {
        ...
        if (ViewMode is PanoViewMode.RowCard or PanoViewMode.RowPreview or PanoViewMode.DetailCard or PanoViewMode.PropertyCard or PanoViewMode.Timeline or PanoViewMode.FilmStrip) return 1;
    ...
    {
        // v25.83: Pano uyumlu CheckBoxes modunda eski/boş selector kolonları
    ...
    private void DrawColumnHeaderContent(Graphics g, Rectangle r, PanoColumn col)
    {
    ...
    private Rectangle GetHeaderCheckBoxRect(PanoColumn col)
    {
    ...
    private void ToggleHeaderCheckBox(PanoColumn col)
    {
        ...
        if ((col.Kind == PanoColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col)) && col.HeaderCheckBoxUpdatesRowCheckBoxes && next != CheckState.Indeterminate)
        {
    ...
    private PanoCellEditEventArgs CreateHeaderCheckBoxEditEventArgs(PanoColumn col, object? newValue)
    {
        int rowIndex = -1;
        ...
        return new PanoCellEditEventArgs(rowIndex, rowObject!, col, newValue);
    }

    private void UpdateCompatibilityHeaderCheckState(PanoColumn? col)
    {
        if (col == null || !col.HeaderCheckBox) return;
        if (!(col.Kind == PanoColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col))) return;
    ...
    private Color GetEffectiveHeaderBackColor(PanoColumn? col = null)
    {
    ...
    private Color GetEffectiveHeaderForeColor(Color headerBack, PanoColumn? col = null)
    {
    ...
    private bool ShouldShowColumnFilterButton(PanoColumn col)
        => ShowHeader && ShowFilterMenu && ShowColumnFilterButtons && col != null && col.Filterable && col.Visible;

    private Rectangle GetColumnFilterButtonRect(Rectangle headerBounds, PanoColumn col)
    {
    ...
    private Rectangle GetColumnFilterButtonRect(PanoColumn col)
    {
    ...
    private void DrawColumnFilterButton(Graphics g, Rectangle headerBounds, PanoColumn col)
    {
        ...
        var style = col.FilterIconStyle == PanoFilterIconStyle.Inherit ? FilterIconStyle : col.FilterIconStyle;
        var customIcon = col.FilterIcon ?? CustomFilterIcon;
        if (style == PanoFilterIconStyle.CustomImage && customIcon != null)
        {
            DrawHeaderIconImage(g, customIcon, r);
        }
        else if (style == PanoFilterIconStyle.Dot)
        {
        ...
        if (active && style != PanoFilterIconStyle.Dot)
        {
    ...
    private void DrawColumnSortGlyph(Graphics g, Rectangle headerBounds, PanoColumn col)
    {
        ...
        var style = col.SortGlyphStyle == PanoSortGlyphStyle.Inherit ? SortGlyphStyle : col.SortGlyphStyle;
        var customIcon = _sortDesc ? (col.SortDescendingIcon ?? CustomSortDescendingIcon) : (col.SortAscendingIcon ?? CustomSortAscendingIcon);
        if (style == PanoSortGlyphStyle.CustomImage && customIcon != null)
        {
            DrawHeaderIconImage(g, customIcon, r);
            return;
        }
        ...
        if (style == PanoSortGlyphStyle.Triangle)
        {
            var pts = _sortDesc
    ...
    {
        if (RowColorPreset == PanoRowColorPreset.ThemeDefault)
            return currentBack;
        ...
        {
            PanoRowColorPreset.SubtleZebra => viewIndex % 2 == 1
                ? Blend(_theme.AlternateBackColor, _theme.AccentColor, _theme.IsDark ? 0.06 : 0.035)
                : currentBack,
            PanoRowColorPreset.SoftAccent => Blend(currentBack, _theme.AccentColor, _theme.IsDark ? strength * 0.75 : strength * 0.55),
            PanoRowColorPreset.AOIRisk => string.IsNullOrWhiteSpace(key) ? currentBack : Blend(currentBack, accent, _theme.IsDark ? strength * 1.10 : strength),
            PanoRowColorPreset.StatusPills => string.IsNullOrWhiteSpace(key) ? currentBack : Blend(currentBack, accent, _theme.IsDark ? strength * 0.85 : strength * 0.70),
            PanoRowColorPreset.SeverityBands => Blend(currentBack, accent, _theme.IsDark ? strength * 1.20 : strength * 0.95),
            PanoRowColorPreset.PastelCards => Blend(currentBack, accent, _theme.IsDark ? strength * 0.65 : strength * 0.45),
            PanoRowColorPreset.FocusGlow => viewIndex == _hotRow
    ...
    private PanoColumn? GetTileCheckBoxColumn()
    {
        if (!TileCheckBoxes) return null;

        bool IsCheckColumn(PanoColumn c)
            => c.Kind == PanoColumnKind.CheckBox || c.CellCheckBox || IsCompatibilityCheckBoxHostColumn(c);
    ...
    private Rectangle GetTileCheckBoxRect(Rectangle cardBounds)
    {
        ...
        {
            case PanoTileCheckBoxPosition.TopLeft:
                x = left;
                y = top;
                break;

            case PanoTileCheckBoxPosition.BottomLeft:
                x = left;
                y = bottom;
                break;

            case PanoTileCheckBoxPosition.BottomRight:
                x = right;
                y = bottom;
                break;

            case PanoTileCheckBoxPosition.TopRight:
            default:
                x = right;
                y = top;
                break;
    ...
    private bool ShouldDisplayTileCheckBox(int viewIndex, object row, PanoColumn checkColumn)
    {
        bool isSelected = _selectedRows.Contains(viewIndex) || viewIndex == _selectedRow;
        ...
        {
            PanoTileCheckBoxVisibilityMode.Hover => isHot,
            PanoTileCheckBoxVisibilityMode.Selected => isSelected,
            PanoTileCheckBoxVisibilityMode.HoverOrSelected => isHot || isSelected,
            PanoTileCheckBoxVisibilityMode.CheckedOrHoverOrSelected => isChecked || isHot || isSelected,
            _ => true
        };
    }

    private void DrawTileCheckBoxAdorner(Graphics g, Rectangle cardBounds, object row, PanoColumn checkColumn)
    {
    ...
    private bool TryGetTileCheckBoxHit(Point location, int viewIndex, out PanoColumn? checkColumn)
    {
        checkColumn = null;
    ...
    private void DrawTileCard(Graphics g, Rectangle r, int viewIndex, object row)
    {
        ...
        PanoCardVisualInfo? cardVisualInfo = ResolveCardVisualInfo(row, cardStatusColor);
        ...
        else if (ViewMode == PanoViewMode.Timeline)
        {
        ...
        if (ViewMode == PanoViewMode.KpiDashboard)
        {
        ...
        if (ViewMode == PanoViewMode.HeatMap)
        {
        ...
        if (ViewMode == PanoViewMode.MiniChart)
        {
        ...
        {
            PanoViewMode.Poster => 44,
            PanoViewMode.Gallery => 44,
            PanoViewMode.MediaTile => 38,
            PanoViewMode.FilmStrip => 42,
            PanoViewMode.ExtraLargeIcons => 42,
            PanoViewMode.IconGrid => 40,
            PanoViewMode.MediumIcons => 24,
            PanoViewMode.LargeCard or PanoViewMode.DashboardCard or PanoViewMode.GroupCard or PanoViewMode.RowCard or PanoViewMode.RowPreview or PanoViewMode.DetailCard or PanoViewMode.PropertyCard or PanoViewMode.Timeline or PanoViewMode.KpiDashboard or PanoViewMode.HeatMap or PanoViewMode.MiniChart => 28,
            _ => 32
        };
        int textX = ViewMode == PanoViewMode.Timeline ? r.Left + 24 : r.Left + 10;
        ...
        if (showTileCheckBox && TileCheckBoxReserveTextArea && (TileCheckBoxPosition == PanoTileCheckBoxPosition.TopRight || TileCheckBoxPosition == PanoTileCheckBoxPosition.BottomRight))
            textW = Math.Max(20, textW - TileCheckBoxSize - 12);
        if (showTileCheckBox && TileCheckBoxReserveTextArea && (TileCheckBoxPosition == PanoTileCheckBoxPosition.TopLeft || TileCheckBoxPosition == PanoTileCheckBoxPosition.BottomLeft) && iconCol == null)
        {
        ...
        if (mediaLayout && iconCol != null)
        {
            ...
            if (poster != null)
            {
                ...
                if (ViewMode == PanoViewMode.FilmStrip)
                {
                    y = r.Top + 16;
                    textX = pr.Right + 14;
        ...
        {
            PanoViewMode.LargeCard => Math.Max(1, LargeCardMaxTextLines),
            PanoViewMode.DashboardCard => Math.Max(4, Math.Min(8, LargeCardMaxTextLines)),
            PanoViewMode.KpiDashboard => Math.Max(2, Math.Min(4, LargeCardMaxTextLines)),
            PanoViewMode.HeatMap => Math.Max(2, Math.Min(4, LargeCardMaxTextLines)),
            PanoViewMode.MiniChart => Math.Max(3, Math.Min(5, LargeCardMaxTextLines)),
            PanoViewMode.RowPreview => Math.Max(1, Math.Min(3, LargeCardMaxTextLines)),
            PanoViewMode.GroupCard => Math.Max(4, Math.Min(7, LargeCardMaxTextLines)),
            PanoViewMode.RowCard => Math.Max(4, Math.Min(9, LargeCardMaxTextLines)),
            PanoViewMode.DetailCard or PanoViewMode.PropertyCard => Math.Max(1, Columns.VisibleColumns.Count(c => c.Width > 0)),
            PanoViewMode.Kanban => Math.Max(4, Math.Min(7, LargeCardMaxTextLines)),
            PanoViewMode.Timeline => Math.Max(5, Math.Min(10, LargeCardMaxTextLines)),
            PanoViewMode.Tile => Math.Max(1, TileMaxTextLines),
            PanoViewMode.Poster => Math.Max(2, Math.Min(5, TileMaxTextLines)),
            PanoViewMode.Gallery => Math.Max(2, Math.Min(5, TileMaxTextLines)),
            PanoViewMode.MediaTile => Math.Max(2, Math.Min(5, TileMaxTextLines)),
            PanoViewMode.FilmStrip => Math.Max(4, Math.Min(7, TileMaxTextLines)),
            PanoViewMode.IconGrid => 2,
            PanoViewMode.ExtraLargeIcons => 4,
            PanoViewMode.LargeIcons => 3,
            _ => 2
        };
    ...
    {
        var visible = Columns.VisibleColumns.Where(c => c.Kind != PanoColumnKind.CheckBox && c.Kind != PanoColumnKind.Icon && c.Kind != PanoColumnKind.Image && c.Width > 0).ToList();
    ...
    private void DrawDetailCardContent(Graphics g, Rectangle r, int viewIndex, object row, Color fore, Color back, PanoCardVisualInfo? cardVisualInfo, Color? cardStatusColor, bool reserveCheckBoxArea)
    {
        ...
        if (detailImageColumn != null &&
            (DetailCardLayout is PanoDetailCardLayout.Media or PanoDetailCardLayout.PosterLeft ||
             ViewMode is PanoViewMode.DetailCard or PanoViewMode.PropertyCard))
        {
        ...
        var textColumns = visible.Where(c => c.Kind != PanoColumnKind.Icon && c.Kind != PanoColumnKind.Image).ToList();
        ...
        int left = r.Left + 16;
        int rightPadding = 16 + (reserveCheckBoxArea && TileCheckBoxReserveTextArea && (TileCheckBoxPosition == PanoTileCheckBoxPosition.TopRight || TileCheckBoxPosition == PanoTileCheckBoxPosition.BottomRight) ? TileCheckBoxSize + 12 : 0);
        ...
        bool propertyMode = ViewMode == PanoViewMode.PropertyCard;
        ...
        foreach (var col in visible)
        {
            ...
            string value;
            if (col.Kind == PanoColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col))
            {
                value = GetRowCheckState(row, col) == CheckState.Checked ? "Evet" : "Hayır";
            }
            else if (col.Kind == PanoColumnKind.Icon || col.Kind == PanoColumnKind.Image)
            {
    ...
    private void DrawMediaDetailCardContent(Graphics g, Rectangle r, int viewIndex, object row, Color fore, Color back, PanoCardVisualInfo? cardVisualInfo, Color? cardStatusColor, bool reserveCheckBoxArea, List<PanoColumn> visible, PanoColumn imageColumn)
    {
        var textColumns = visible.Where(c => c != imageColumn && c.Kind != PanoColumnKind.CheckBox && c.Width > 0).ToList();
        var titleCol = textColumns.FirstOrDefault(c => c.Editable)
        ...
        int imageW = DetailCardLayout == PanoDetailCardLayout.PosterLeft
            ? Math.Max(132, Math.Min(240, DetailCardMediaImageWidth))
            : Math.Max(96, Math.Min(190, DetailCardMediaImageWidth));
        int imageH = DetailCardLayout == PanoDetailCardLayout.PosterLeft
    ...
    private string BuildMediaCompactInfo(object row, List<PanoColumn> columns)
    {
    ...
    private PanoColumn? FindDetailCardImageColumn(List<PanoColumn> columns, object row)
    {
    ...
    private Image? ResolveDetailCardImage(PanoColumn column, object row)
    {
    ...
    private static Image? TryGetColumnValueImage(PanoColumn column, object row)
    {
        object? value;
        try
        {
    ...
    private void DrawMediaImage(Graphics g, Image image, Rectangle bounds)
    {
        ...
        {
            case PanoMediaImageScaleMode.Cover:
                DrawImageCover(g, image, bounds);
                break;
            case PanoMediaImageScaleMode.Stretch:
    ...
    private Color EnsureReadableTextOn(Color back, Color preferredFore)
    {
        ...
        return global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(preferredFore, back, 4.5d);
    }
    ...
    private void DrawCell(Graphics g, Rectangle r, object row, PanoColumn col, Color fore, Color cellBack, bool isSelected, int viewIndex)
    {
        ...
        bool cellCheckBoxHost = col.CellCheckBox && col.Kind != PanoColumnKind.CheckBox && !compatibilityCheckHost;
    ...
    private void DrawComboBoxCell(Graphics g, Rectangle r, string text, PanoColumn col, Color fore, Color cellBack)
    {
    ...
    private void DrawTextWithFilterHighlight(Graphics g, Rectangle r, string text, PanoColumn col, Color fore, Color cellBack)
    {
        ...
        if (columnFilter?.Mode == PanoFilterMode.Contains && !string.IsNullOrWhiteSpace(columnFilter.Text))
    ...
    private bool ShouldDrawMultilineCell(PanoColumn col)
    {
        if (IsTileView) return false;
        if (!AllowMultilineCells && !col.WordWrap) return false;
        if (col.Kind != PanoColumnKind.Text && col.Kind != PanoColumnKind.Hyperlink) return false;
        return col.WordWrap || AllowMultilineCells;
    }

    private int ResolveMultilineCellMaxLines(PanoColumn col)
    {
        if (col.MaxTextLines > 0) return Math.Clamp(col.MaxTextLines, 1, 12);
        return Math.Clamp(MaxCellTextLines, 1, 12);
    }

    private void DrawMultilineCellText(Graphics g, Rectangle r, string text, PanoColumn col, Color fore)
    {
    ...
    private string BuildCellOverflowKey(int viewIndex, PanoColumn col)
    {
    ...
    private void DrawCellOverflowAdorners(Graphics g, Rectangle cellRect, Rectangle textRect, PanoColumn col, Color fore, int offset, int maxOffset)
    {
    ...
    private void ShowCellOverflowReaderPopup(int rowIndex, PanoColumn col, object row)
    {
    ...
    private static string ResolveButtonCellText(PanoColumn col, object? value)
    {
    ...
    private void DrawButton(Graphics g, Rectangle r, string text, PanoColumn? col = null)
    {
        var br = Rectangle.Inflate(r, -1, -3);
        if (col != null)
        {
            ...
            if (col.ButtonSizing == PanoColumnButtonSizing.TextBounds && !string.IsNullOrEmpty(text))
            {
    ...
    private void DrawRating(Graphics g, Rectangle r, int value, PanoColumn col, Color fore, Color cellBack)
    {
    ...
    private Image? ResolveHeaderImage(PanoColumn col)
    {
    ...
    private Image? ResolveColumnImage(PanoColumn col, object row, bool preferLarge = false)
    {
    ...
    private Image? ResolveColumnStateImage(PanoColumn col, object row)
    {
    ...
    private bool ColumnMayDrawCellImage(PanoColumn col)
    {
        if (col == null) return false;
        if (col.Kind == PanoColumnKind.Image || col.Kind == PanoColumnKind.Icon) return true;
    ...
    private Rectangle DrawColumnImagesIfNeeded(Graphics g, Rectangle r, object row, PanoColumn col)
    {
        int x = r.X;
    ...
    private void DrawIconText(Graphics g, Rectangle r, object row, PanoColumn col, object? val, Color fore)
    {
    ...
    private void DrawTags(Graphics g, Rectangle r, string text, PanoColumn col, Color cellBack)
    {
        int x = r.X;
    ...
    private void DrawEmpty(Graphics g)
    {
        ...
        string title = string.IsNullOrWhiteSpace(EmptyListMessage) ? PanoText.EmptyList : EmptyListMessage;
        ...
        Color subFore = global::Taylan.Pano.Theming.PanoDialogThemeApplier.EnsureReadableTextColor(_theme.MutedForeColor, _theme.PanelBackColor == Color.Empty ? _theme.BackColor : _theme.PanelBackColor);
        TextRenderer.DrawText(g, PanoText.SearchPlaceholder, Font, subRect, subFore, TextFormatFlags.Left | TextFormatFlags.Top | TextFormatFlags.EndEllipsis);
    ...
    {
        if (!ShowEmptyStateSignature || EmptyStateSignatureAlignment == PanoEmptyStateSignatureAlignment.Hidden) return;
        ...
        {
            case PanoEmptyStateSignatureAlignment.BottomLeft:
                x = card.Left + pad;
                y = card.Bottom - textHeight - 6;
                break;
            case PanoEmptyStateSignatureAlignment.TopRight:
                x = card.Right - textWidth - pad;
                y = card.Top + 6;
                break;
            case PanoEmptyStateSignatureAlignment.TopLeft:
                x = card.Left + pad;
                y = card.Top + 6;
                break;
            case PanoEmptyStateSignatureAlignment.CenterBottom:
                x = card.Left + (card.Width - textWidth) / 2;
                y = card.Bottom - textHeight - 6;
                break;
            case PanoEmptyStateSignatureAlignment.BottomRight:
            default:
                x = card.Right - textWidth - pad;
                y = card.Bottom - textHeight - 6;
                break;
        ...
        Color baseFore = global::Taylan.Pano.Theming.PanoDialogThemeApplier.EnsureReadableTextColor(_theme.MutedForeColor, surface);
    ...
    {
        string versionText = "Pano";
        try
        {
            var version = typeof(PanoControl).Assembly.GetName().Version;
            if (version != null)
                versionText = $"Pano v{version.Major}.{version.Minor}.{version.Build}.{version.Revision}";
        }
        catch
        {
        }
    ...
    {
        return global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(preferred, back, 4.5d);
    }
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        ...
        if (e.Button == MouseButtons.Left && TryGetTileCheckBoxHit(e.Location, rowIndex, out var tileCheckColumn) && tileCheckColumn != null)
        {
            ...
            var tileArgs = new PanoCellClickEventArgs(rowIndex, row, tileCheckColumn);
        ...
        if (e.Button == MouseButtons.Right)
        {
            ...
            // already selected rows. This mirrors ListView/Pano behavior and
        ...
        var cellArgs = new PanoCellClickEventArgs(rowIndex, row, col);
        ...
        else if (leftButton && (col.Kind == PanoColumnKind.CheckBox || col.Kind == PanoColumnKind.ToggleSwitch)) ToggleBool(row, col);
        else if (leftButton && col.Kind == PanoColumnKind.Rating) SetRatingFromClick(e.X, row, col);
        else if (allowMouseAction && col.Kind == PanoColumnKind.Button) ButtonClick?.Invoke(this, cellArgs);
        else if (leftButton && col.Kind == PanoColumnKind.Hyperlink) HyperlinkClick?.Invoke(this, cellArgs);
        else if (leftButton && col.Kind == PanoColumnKind.ComboBox && EnableCellEditing && CanEditColumn(col)) BeginEdit(rowIndex, col);
        Invalidate();
    }
    protected override void OnDoubleClick(EventArgs e)
    {
        ...
        if (activeRow != null && col != null) ItemActivate?.Invoke(this, new PanoCellClickEventArgs(rowIndex, activeRow, col));
    ...
    private void ShowConfiguredFilterMenuForColumn(PanoColumn col, Point clientLocation)
    {
        if (FilterMenuMode == PanoFilterMenuMode.PopupMenu || UseEmbeddedHeaderFilterMenu)
        {
            ShowPopupFilterDropDown(col, clientLocation, includeOpenWindowItem: true);
            return;
        }

        if (FilterMenuMode == PanoFilterMenuMode.Both)
        {
            ShowPopupFilterDropDown(col, clientLocation, includeOpenWindowItem: true);
            return;
        }
    ...
    private void ShowPopupFilterDropDown(PanoColumn col, Point clientLocation, bool includeOpenWindowItem)
    {
        ...
        {
            Renderer = new global::Taylan.Pano.Theming.SmartMenuRenderer(_theme),
            BackColor = _theme.PanelBackColor,
            ForeColor = _theme.ForeColor,
            AutoClose = true,
        ...
        if (includeOpenWindowItem)
        {
            ...
            menu.Items.Add(PanoText.SeparateFilterWindow, null, (_,__) =>
            {
    ...
    private string BuildFilterPopupSizeMemoryKey(PanoColumn col)
    {
    ...
    private Size GetInitialFilterPopupSize(PanoColumn col, Size minimumSize, Size maximumSize, string sizeMemoryKey, bool allowRememberedSize)
    {
    ...
    private Size GetInitialFilterWindowSize(PanoColumn col, Size minimumSize, Size maximumSize, string sizeMemoryKey, bool allowRememberedSize)
    {
        if (allowRememberedSize && FilterPopupRememberSize && _floatingFilterPopupSizeMemory.TryGetValue(sizeMemoryKey, out Size rememberedSize))
            return ClampFilterPopupSize(rememberedSize, minimumSize, maximumSize);
    ...
    private void ShowFilterMenuForColumn(PanoColumn col, Point clientLocation)
    {
    ...
    public void SortBy(PanoColumn? column, bool descending = false)
    {
    ...
    private void BuildViewIndexSmartSort()
    {
        ...
        Task.Run(() =>
        {
            var entries = new List<PanoSortEntry>(Math.Min(scanCount, 250_000));
            for (int i = 0; i < scanCount; i++)
            {
                ...
                entries.Add(new PanoSortEntry(i, primaryKey, secondaryKey));
            }
    ...
    public void ClearSort(PanoColumn? column = null)
    {
    ...
    public void BeginEdit(int viewRowIndex, PanoColumn col)
    {
    ...
    private static ICellEditor CreateBuiltInEditor(PanoColumn col)
    {
        var type = col.EditorType;
        if (type == PanoCellEditorKind.Auto)
        {
            type = col.Kind switch
            {
                PanoColumnKind.CheckBox or PanoColumnKind.ToggleSwitch => PanoCellEditorKind.CheckBox,
                PanoColumnKind.ComboBox => PanoCellEditorKind.ComboBox,
                PanoColumnKind.Numeric or PanoColumnKind.Rating or PanoColumnKind.ProgressBar => PanoCellEditorKind.Numeric,
                PanoColumnKind.Date => PanoCellEditorKind.DateTime,
                _ => PanoCellEditorKind.TextBox
            };
        }

        return type switch
        {
            PanoCellEditorKind.CheckBox => new CheckBoxCellEditor(),
            PanoCellEditorKind.ComboBox => new ComboBoxCellEditor(col.ComboBoxItems),
            PanoCellEditorKind.Numeric => new NumericCellEditor(),
            PanoCellEditorKind.DateTime => new DateTimeCellEditor(),
    ...
    private void SetCellValueWithUndo(int viewRowIndex, object row, PanoColumn col, object? value)
    {
        var old = col.GetValue(row);
        col.PutValue(row, value);
        if (EnableUndoRedo) _undo.Push(new Taylan.Pano.Undo.PanoCellChange { RowObject = row, Column = col, OldValue = old, NewValue = value });
        CellValueChanged?.Invoke(this, new PanoCellEditEventArgs(viewRowIndex, row, col, value));
        InvalidateDataCaches(keepVersion: true);
        Invalidate();
    }
    private void ToggleBool(object row, PanoColumn col)
    {
        if (col.Kind == PanoColumnKind.CheckBox || col.CellCheckBox || IsCompatibilityCheckBoxHostColumn(col))
        {
            ...
            CellValueChanged?.Invoke(this, new PanoCellEditEventArgs(_selectedRow, row, col, newState == CheckState.Checked));
    ...
    private void SetRatingFromClick(int x, object row, PanoColumn col)
    {
    ...
    private string GetSmartFilterIndexKey(PanoColumn col) => col.AspectName + "|" + col.Header + "|" + _dataVersion.ToString(System.Globalization.CultureInfo.InvariantCulture);

    private void BeginBuildSmartFilterIndex(PanoColumn col)
    {
    ...
    private Point GetDefaultFilterPopupClientAnchor(PanoColumn col)
    {
        if (ShowHeader && (ViewMode == PanoViewMode.Details || ViewMode == PanoViewMode.DenseList || ViewMode == PanoViewMode.GroupedList || ViewMode == PanoViewMode.MasterDetail))
    ...
    private Point NormalizeFilterPopupClientAnchor(PanoColumn col, Point requestedClientAnchor)
    {
    ...
    public void ShowHeaderContextMenuForColumn(PanoColumn col, Point clientLocation)
    {
    ...
    private IReadOnlyList<string> GetDistinctColumnValues(PanoColumn col, int maxScanRows, string? searchText = null)
    {
    ...
    private List<string> ComputeDistinctColumnValuesSnapshot(PanoColumn col, int maxScanRows, string? searchText = null, bool useTopValueOrder = false)
    {
        var allColumns = Columns.ToArray();
    ...
    private PanoColumn? HitColumn(int x) { if (IsTileView) return Columns.VisibleColumns.FirstOrDefault(c => c.Editable) ?? Columns.VisibleColumns.FirstOrDefault(); int xx = -_scrollX; foreach (var c in Columns.VisibleColumns) { if (x >= xx && x < xx + c.Width) return c; xx += c.Width; } return null; }
    private PanoColumn? HitColumnResizeEdge(int x)
    {
        int xx = -_scrollX;
        foreach (var c in Columns.VisibleColumns)
        {
    ...
    private int GetColumnLeft(PanoColumn col) { int x=-_scrollX; foreach (var c in Columns.VisibleColumns) { if (c == col) return x; x += c.Width; } return 0; }

    private void SetResizingColumnWidth(PanoColumn column, int requestedWidth)
    {
    ...
    private int GetColumnMinimumResizeWidth(PanoColumn column)
    {
        return Math.Max(28, column.MinimumWidth > 0 ? column.MinimumWidth : 28);
    }

    private void AbsorbHorizontalOverflowFromFillColumns(PanoColumn resizedColumn)
    {
        var visible = Columns.VisibleColumns.ToList();
    ...
    public Rectangle GetCellBounds(int viewRowIndex, PanoColumn col)
    {
        int top = GetRowsTopOffset();
        if (IsTileView)
        {
    ...
    protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
    {
        ...
        if (HandlePanoKeyboardAccessibilityShortcut(keyData)) return true;
        ...
        if (keyData == Keys.Delete) { CellClick?.Invoke(this, new PanoCellClickEventArgs(_selectedRow, SelectedObject ?? this, Columns.VisibleColumns.FirstOrDefault() ?? new PanoColumn("", "", 0))); return true; }
    ...
    private bool TryActivateSelectedButtonCell()
    {
        ...
        var row = GetViewRow(_selectedRow);
        var col = _activeColumn != null && _activeColumn.Kind == PanoColumnKind.Button
            ? _activeColumn
            : Columns.VisibleColumns.FirstOrDefault(c => c.Kind == PanoColumnKind.Button);
        if (row == null || col == null) return false;
        ButtonClick?.Invoke(this, new PanoCellClickEventArgs(_selectedRow, row, col));
        return true;
    }
    ...
    private bool TryToggleSelectedCheckBoxesFromKeyboard()
    {
        ...
        foreach (var row in targetRows)
        {
            ...
            CellValueChanged?.Invoke(this, new PanoCellEditEventArgs(IndexOfObject(row), row, col, nextState == CheckState.Checked));
    ...
    private bool CanEditColumn(PanoColumn? col)
    {
        if (col == null) return false;
    ...
    private ToolStripMenuItem CreateEmbeddedFilterMenuItem(PanoColumn col)
    {
        var root = new ToolStripMenuItem(PanoText.Filtering);
        ...
        root.DropDownItems.Add(PanoText.SortAscending, null, (_,__) => { SortBy(col, false); CloseActiveMenusSafe(); }).Enabled = col.Sortable;
        root.DropDownItems.Add(PanoText.SortDescending, null, (_,__) => { SortBy(col, true); CloseActiveMenusSafe(); }).Enabled = col.Sortable;
        root.DropDownItems.Add(PanoText.Unsort, null, (_,__) => { ClearSort(col); CloseActiveMenusSafe(); }).Enabled = _sortColumn == col;
        ...
        var caption = new Label { Text = PanoText.ColumnFilterTitle(col.Header), Left = 8, Top = 6, Width = 272, Height = 20, ForeColor = filterPanelFore, BackColor = filterPanelBack, Font = new Font(Font, FontStyle.Bold) };
        var search = new TextBox { PlaceholderText = PanoText.SearchPlaceholder, Left = 8, Top = 30, Width = 272, BackColor = filterControlBack, ForeColor = filterControlFore, BorderStyle = BorderStyle.FixedSingle };
        var useTextFilter = new CheckBox { Text = PanoText.ApplyTypedTextAsFilter, Left = 8, Top = 58, Width = 272, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat };
        if (TypedFilterSearchesAllRows && FastFilterMenuForHugeLists && _provider.Count > 20_000) useTextFilter.Checked = true;
        var selectAll = new CheckBox { Text = PanoText.SelectAll, Left = 8, Top = 84, Width = 272, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat, ThreeState = true, AutoCheck = false };
        ...
        var previewInfo = new Label { Text = IsHugeFastFilterPreview() ? PanoText.FastPreviewInfo : string.Empty, Left = 8, Top = 304, Width = 272, Height = 16, ForeColor = filterMutedFore, BackColor = filterPanelBack };
        ...
        var limitedLabel = new Label { Text = PanoText.FastPreviewLimitedInfo, Left = 28, Top = 4, Width = 212, Height = 34, BackColor = limitedInfo.BackColor, ForeColor = Color.Black };
        ...
        var clear = new Button { Text = PanoText.Clear, Left = 112, Top = 374, Width = 78, Height = 28, BackColor = filterControlBack, ForeColor = filterControlFore, FlatStyle = FlatStyle.Flat };
        var apply = new Button { Text = PanoText.Apply, Left = 202, Top = 374, Width = 78, Height = 28, BackColor = _theme.AccentColor, ForeColor = GetReadableTextColor(_theme.AccentColor), FlatStyle = FlatStyle.Flat };
        ...
        bool IsChecked(string value) => current?.Mode == PanoFilterMode.ValueList && current.SelectedValues != null ? current.SelectedValues.Contains(value) : true;
        void Refill()
        {
            ...
            if (previewInfo != null)
                previewInfo.Text = fullValuesLoaded ? string.Format("{0:N0}", values.Count) : PanoText.FastPreviewInfo;
            if (limitedInfo != null)
        ...
        search.TextChanged += (_,__) =>
        {
            ...
            if (!fullValuesLoaded && !string.IsNullOrWhiteSpace(search.Text) && previewInfo != null)
                previewInfo.Text = PanoText.SearchingPreviewResults;
        ...
        apply.Click += (_,__) =>
        {
            ...
            {
                SetColumnFilter(new PanoColumnFilter { AspectName = col.AspectName, Mode = PanoFilterMode.Contains, Text = q, Enabled = true });
                CloseActiveMenusSafe();
                return;
            }
            ...
            {
                SetColumnFilter(new PanoColumnFilter { AspectName = col.AspectName, Mode = PanoFilterMode.ValueList, SelectedValues = selected, Enabled = true });
            }
            CloseActiveMenusSafe();
        };
        ...
        root.DropDownItems.Add(new ToolStripSeparator());
        root.DropDownItems.Add(PanoText.ClearAllFilters, null, (_,__) => { ClearFilters(); CloseActiveMenusSafe(); });
        root.DropDownItems.Add(PanoText.SeparateFilterWindow, null, (_,__) => { ShowFilterMenuForColumn(col, new Point(GetColumnLeft(col), HeaderHeight)); CloseActiveMenusSafe(); });
        return root;
    }
    ...
    {
        return global::Taylan.Pano.Theming.PanoThemeAccessibility.EnsureReadableTextColor(_theme.ForeColor, backColor, 4.5d);
    }

    private Color GetFilterPopupPanelBack() => global::Taylan.Pano.Theming.PanoDialogThemeApplier.NormalizePanelBack(_theme);
    private Color GetFilterPopupControlBack() => global::Taylan.Pano.Theming.PanoDialogThemeApplier.NormalizeControlBack(_theme);
    private Color GetFilterPopupPanelFore() => global::Taylan.Pano.Theming.PanoDialogThemeApplier.NormalizePanelFore(_theme);
    private Color GetFilterPopupControlFore() => global::Taylan.Pano.Theming.PanoDialogThemeApplier.NormalizeControlFore(_theme);
    private Color GetFilterPopupMutedFore(Color back) => global::Taylan.Pano.Theming.PanoDialogThemeApplier.EnsureReadableTextColor(_theme.MutedForeColor, back);
    ...
    private void ApplyFilterPopupTheme(Control root)
    {
        ...
        foreach (Control control in EnumerateFilterPopupControls(root))
        {
            ...
            if (control is Label label)
            {
                ...
                label.ForeColor = global::Taylan.Pano.Theming.PanoDialogThemeApplier.EnsureReadableTextColor(label.ForeColor == Color.Empty ? panelFore : label.ForeColor, back);
                continue;
            }
            ...
            if (control is Panel)
            {
                ...
                control.ForeColor = global::Taylan.Pano.Theming.PanoDialogThemeApplier.EnsureReadableTextColor(control.ForeColor == Color.Empty ? panelFore : control.ForeColor, control.BackColor);
                continue;
            }
    ...
    private ToolStripMenuItem CreateStableFilterLauncherMenuItem(ContextMenuStrip ownerMenu, PanoColumn col)
    {
        var launcher = new ToolStripMenuItem(PanoText.Filtering);
        bool opening = false;
    ...
    private Control CreateFloatingFilterActionLabel(string icon, string text, int left, int top, int width, bool enabled, bool active, Action clickAction)
    {
        ...
        Color activeBack = PanoDialogThemeApplier.Blend(panelBack, _theme.AccentColor, _theme.IsDark ? 0.38 : 0.16);
        Color hoverBack = PanoDialogThemeApplier.Blend(panelBack, _theme.AccentColor, _theme.IsDark ? 0.25 : 0.10);
    ...
    private void ShowFloatingEmbeddedFilterForm(ContextMenuStrip? ownerMenu, PanoColumn col, Point screenLocation)
    {
        if (_activeFilterPopupForm != null && !_activeFilterPopupForm.IsDisposed)
        {
            try { _activeFilterPopupForm.Close(); } catch { }
        ...
        var sortAsc = CreateFloatingFilterActionLabel("▲", PanoText.SortAscending, 12, 10, 310, col.Sortable, _sortColumn == col && !_sortDesc, () => { SortBy(col, false); popup.Close(); });
        var sortDesc = CreateFloatingFilterActionLabel("▼", PanoText.SortDescending, 12, 34, 310, col.Sortable, _sortColumn == col && _sortDesc, () => { SortBy(col, true); popup.Close(); });
        var unsort = CreateFloatingFilterActionLabel("↕", PanoText.Unsort, 12, 58, 310, _sortColumn == col, false, () => { ClearSort(col); popup.Close(); });
        var sep1 = new Panel { Left = 10, Top = 84, Width = 326, Height = 1, BackColor = _theme.BorderColor };
        var caption = new Label { Text = PanoText.ColumnFilterTitle(col.Header), Left = 16, Top = 100, Width = 310, Height = 22, ForeColor = filterPanelFore, BackColor = filterPanelBack, Font = new Font(Font, FontStyle.Bold) };
        var search = new TextBox { PlaceholderText = PanoText.SearchPlaceholder, Left = 16, Top = 126, Width = 300, Height = 24, BackColor = filterControlBack, ForeColor = filterControlFore, BorderStyle = BorderStyle.FixedSingle };
        var useTextFilter = new CheckBox { Text = PanoText.ApplyTypedTextAsFilter, Left = 16, Top = 158, Width = 310, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat };
        if (TypedFilterSearchesAllRows && FastFilterMenuForHugeLists && _provider.Count > 20_000) useTextFilter.Checked = true;
        var selectAll = new CheckBox { Text = PanoText.SelectAll, Left = 16, Top = 184, Width = 310, Height = 22, ForeColor = filterControlFore, BackColor = filterPanelBack, FlatStyle = FlatStyle.Flat, ThreeState = true, AutoCheck = false };
        ...
        var previewInfo = new Label { Text = IsHugeFastFilterPreview() ? PanoText.FastPreviewInfo : string.Empty, Left = 16, Top = 394, Width = 300, Height = 16, ForeColor = filterMutedFore, BackColor = filterPanelBack };
        ...
        var limitedLabel = new Label { Text = PanoText.FastPreviewLimitedInfo, Left = 30, Top = 4, Width = 238, Height = 38, BackColor = limitedInfo.BackColor, ForeColor = Color.Black };
        ...
        limitedClose.Click += (_,__) => limitedInfo.Visible = false;
        limitedInfo.Controls.AddRange(new Control[] { limitedIcon, limitedLabel, limitedClose });
        var clear = new Button { Text = PanoText.Clear, Left = 130, Top = 470, Width = 78, Height = 30, BackColor = filterControlBack, ForeColor = filterControlFore, FlatStyle = FlatStyle.Flat };
        var apply = new Button { Text = PanoText.Apply, Left = 220, Top = 470, Width = 78, Height = 30, BackColor = _theme.AccentColor, ForeColor = GetReadableTextColor(_theme.AccentColor), FlatStyle = FlatStyle.Flat };
        var sep2 = new Panel { Left = 10, Top = 510, Width = 326, Height = 1, BackColor = _theme.BorderColor };
        var openWindow = CreateFloatingFilterActionLabel("▽", PanoText.SeparateFilterWindow, 12, 520, 310, true, false, () => { popup.Close(); BeginInvoke(new Action(() => ShowFilterMenuForColumn(col, PointToClient(popup.Location)))); });
        var clearAll = CreateFloatingFilterActionLabel("×", PanoText.ClearAllFilters, 12, 542, 310, true, current != null, () => { ClearFilters(); popup.Close(); });
        ...
        apply.Click += (_,__) =>
        {
            ...
            else SetColumnFilter(new PanoColumnFilter { AspectName = col.AspectName, Mode = PanoFilterMode.ValueList, SelectedValues = selected, Enabled = true });
            CloseAllMenus();
        };
    ...
    {
        RemoveMenuDismissMessageFilter();
        _activeMenuDismissFilter = new PanoMenuDismissMessageFilter(
            isInsideAllowedArea: () =>
            {
                var p = Control.MousePosition;
                bool insideOwner = ownerMenu != null && !ownerMenu.IsDisposed && ownerMenu.Bounds.Contains(p);
    ...
    private bool IsMenuGroupVisible(PanoMenuGroups group, bool header)
    {
        if (MenuProfile == PanoMenuProfile.None) return false;
        PanoMenuGroups groups = header ? HeaderMenuGroups : BodyMenuGroups;
        if ((groups & group) != group) return false;

        return MenuProfile switch
        {
            PanoMenuProfile.Full => true,
            PanoMenuProfile.Custom => true,
            PanoMenuProfile.Standard => group is PanoMenuGroups.Filter or PanoMenuGroups.Sort or PanoMenuGroups.AutoSize or PanoMenuGroups.ColumnChooser or PanoMenuGroups.ViewMode or PanoMenuGroups.State or PanoMenuGroups.Scenario or PanoMenuGroups.Clipboard or PanoMenuGroups.Editing or PanoMenuGroups.RowDetails,
            PanoMenuProfile.Minimal => group is PanoMenuGroups.Filter or PanoMenuGroups.Sort or PanoMenuGroups.AutoSize or PanoMenuGroups.Clipboard,
            PanoMenuProfile.ReadOnly => group is PanoMenuGroups.Filter or PanoMenuGroups.Sort or PanoMenuGroups.AutoSize or PanoMenuGroups.ColumnChooser or PanoMenuGroups.ViewMode or PanoMenuGroups.State or PanoMenuGroups.Scenario or PanoMenuGroups.Clipboard or PanoMenuGroups.RowDetails,
            _ => true
        };
    }

    public void ApplyMenuProfile(PanoMenuProfile profile)
    {
        MenuProfile = profile;
        switch (profile)
        {
            case PanoMenuProfile.None:
                UseBuiltInHeaderMenu = false;
                UseBuiltInBodyMenu = false;
                HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.None;
                break;
            case PanoMenuProfile.Full:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = PanoMenuGroups.All;
                BodyMenuGroups = PanoMenuGroups.All;
                break;
            case PanoMenuProfile.Standard:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = PanoMenuGroups.Filter | PanoMenuGroups.Sort | PanoMenuGroups.AutoSize | PanoMenuGroups.ColumnChooser | PanoMenuGroups.ViewMode | PanoMenuGroups.State | PanoMenuGroups.Scenario;
                BodyMenuGroups = PanoMenuGroups.Clipboard | PanoMenuGroups.Editing | PanoMenuGroups.RowDetails | PanoMenuGroups.AutoSize | PanoMenuGroups.ColumnChooser | PanoMenuGroups.ViewMode | PanoMenuGroups.State | PanoMenuGroups.Scenario;
                break;
            case PanoMenuProfile.Minimal:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = PanoMenuGroups.Filter | PanoMenuGroups.Sort | PanoMenuGroups.AutoSize;
                BodyMenuGroups = PanoMenuGroups.Clipboard | PanoMenuGroups.AutoSize;
                break;
            case PanoMenuProfile.ReadOnly:
                UseBuiltInHeaderMenu = true;
                UseBuiltInBodyMenu = true;
                HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.Full;
                HeaderMenuGroups = PanoMenuGroups.Filter | PanoMenuGroups.Sort | PanoMenuGroups.AutoSize | PanoMenuGroups.ColumnChooser | PanoMenuGroups.ViewMode | PanoMenuGroups.State | PanoMenuGroups.Scenario;
                BodyMenuGroups = PanoMenuGroups.Clipboard | PanoMenuGroups.RowDetails | PanoMenuGroups.AutoSize | PanoMenuGroups.ColumnChooser | PanoMenuGroups.ViewMode | PanoMenuGroups.State | PanoMenuGroups.Scenario;
                break;
        }
        Invalidate();
    }

    public void ShowMenuGroup(PanoMenuGroups group, bool visible, bool header = true, bool body = true)
    {
        ...
        if (MenuProfile != PanoMenuProfile.None) MenuProfile = PanoMenuProfile.Custom;
    }

    private void ShowBuiltInContextMenu(Point location)
    {
        bool isHeaderMenu = ShowHeader && location.Y < HeaderHeight;
        if (MenuProfile == PanoMenuProfile.None) return;
        ...
        if (ShowHeader && location.Y < HeaderHeight)
        {
            ...
            if (HeaderContextMenuBehavior == PanoHeaderContextMenuBehavior.None) return;
            if (HeaderContextMenuBehavior == PanoHeaderContextMenuBehavior.FilterOnly)
            {
        ...
        if (ShowHeader && location.Y < HeaderHeight)
        {
            ...
            if (col != null)
            {
                if (ShowHeaderMenuFilterItems && IsMenuGroupVisible(PanoMenuGroups.Filter, true) && ShowFilterMenu && col.Filterable)
                {
                    if (FilterMenuMode == PanoFilterMenuMode.PopupMenu || FilterMenuMode == PanoFilterMenuMode.Both || UseEmbeddedHeaderFilterMenu)
                    {
                    ...
                    if (FilterMenuMode == PanoFilterMenuMode.ModalWindow || FilterMenuMode == PanoFilterMenuMode.Both)
                    {
                        menu.Items.Add(PanoText.FilterWindow(col.Header), null, (_,__) =>
                        {
                    ...
                    if (ShowHeaderMenuFilterStyleItems && IsMenuGroupVisible(PanoMenuGroups.FilterStyle, true) && ShowFilterStyleSelectorInContextMenu) AddFilterStyleMenu(menu);
                    ...
                    if (ShowQuickClearFilterInHeaderMenu)
                    {
                        ...
                        if (colFilter != null)
                            menu.Items.Add(PanoText.ClearColumnFilter, null, (_,__) => { _filters.Clear(col.AspectName); BuildViewIndex(); Invalidate(); QueueAutoSaveUserLayout(); });
                        if (HasActiveFilters)
                            menu.Items.Add(PanoText.ClearAllFilters, null, (_,__) => ClearFilters());
                    }
                    menu.Items.Add(new ToolStripSeparator());
                }
                if (ShowHeaderMenuSortItems && IsMenuGroupVisible(PanoMenuGroups.Sort, true))
                {
                    AddSortMenu(menu.Items, col);
                    menu.Items.Add(new ToolStripSeparator());
                }
                if (ShowHeaderMenuFreezeItems && IsMenuGroupVisible(PanoMenuGroups.Freeze, true))
                {
                    AddFreezeMenu(menu.Items, col);
                }
                if (ShowHeaderMenuAutoSizeItems && IsMenuGroupVisible(PanoMenuGroups.AutoSize, true))
                {
                    AddAutoSizeMenu(menu.Items, col);
                }
                if (ShowHeaderMenuLayoutItems && IsMenuGroupVisible(PanoMenuGroups.Layout, true))
                {
                    AddLayoutMenu(menu.Items);
                }
                if (ShowHeaderMenuFreezeItems || ShowHeaderMenuAutoSizeItems || ShowHeaderMenuLayoutItems)
                    menu.Items.Add(new ToolStripSeparator());
                if (ShowHeaderMenuGroupingItems && IsMenuGroupVisible(PanoMenuGroups.Grouping, true))
                {
                    AddGroupingMenu(menu.Items, col);
                }
                if (ShowViewModeMenuItems && IsMenuGroupVisible(PanoMenuGroups.ViewMode, true)) AddViewModeMenu(menu);
                if ((IsMenuGroupVisible(PanoMenuGroups.Scenario, true) && ShowScenarioMenuItems) || (IsMenuGroupVisible(PanoMenuGroups.State, true) && ShowStateMenuItems))
                {
                    menu.Items.Add(new ToolStripSeparator());
                    if (IsMenuGroupVisible(PanoMenuGroups.Scenario, true) && ShowScenarioMenuItems) AddScenarioMenu(menu.Items);
                    if (IsMenuGroupVisible(PanoMenuGroups.State, true) && ShowStateMenuItems) AddStateMenu(menu.Items);
                }
                if (ShowHeaderMenuColumnChooserItem && IsMenuGroupVisible(PanoMenuGroups.ColumnChooser, true))
                {
                    menu.Items.Add(new ToolStripSeparator());
        ...
        else
        {
            ...
            if (IsMenuGroupVisible(PanoMenuGroups.Clipboard, false))
            {
                menu.Items.Add(PanoText.Copy, null, (_,__) => CopySelectionToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                menu.Items.Add(PanoText.CopyCell, null, (_,__) => CopySelectedCellToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                menu.Items.Add(PanoText.CopySelectionJson, null, (_,__) => CopySelectionAsJsonToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                if (IsMenuGroupVisible(PanoMenuGroups.Analytics, false))
                    menu.Items.Add(PanoText.CopyMiniAnalytics, null, (_,__) => CopyMiniAnalyticsToClipboard()).Enabled = EnableClipboard && ViewCount > 0;
                menu.Items.Add(PanoText.Paste, null, (_,__) => PasteTextToSelectedCell(Clipboard.GetText())).Enabled = EnableClipboard && SelectedObject != null && MenuProfile != PanoMenuProfile.ReadOnly;
            }
            if (IsMenuGroupVisible(PanoMenuGroups.Editing, false) && MenuProfile != PanoMenuProfile.ReadOnly)
                AddEditCellMenuItem(menu.Items);
            menu.Items.Add(new ToolStripSeparator());
            if (IsMenuGroupVisible(PanoMenuGroups.Filter, false))
                menu.Items.Add(PanoText.ClearFilters, null, (_,__) => ClearFilters()).Enabled = HasActiveFilters;
            if (ShowHeaderMenuFilterStyleItems && ShowFilterStyleSelectorInContextMenu && IsMenuGroupVisible(PanoMenuGroups.FilterStyle, false))
                AddFilterStyleMenu(menu);
            if (ShowAdvancedFilterMenuItems) AddAdvancedFilterMenu(menu.Items);
                    AddCardLayoutDesignerMenu(menu.Items);
            if (ShowViewModeMenuItems && IsMenuGroupVisible(PanoMenuGroups.ViewMode, false)) AddViewModeMenu(menu);
            if ((IsMenuGroupVisible(PanoMenuGroups.Scenario, false) && ShowScenarioMenuItems) || (IsMenuGroupVisible(PanoMenuGroups.State, false) && ShowStateMenuItems))
            {
                if (IsMenuGroupVisible(PanoMenuGroups.Scenario, false) && ShowScenarioMenuItems) AddScenarioMenu(menu.Items);
                if (IsMenuGroupVisible(PanoMenuGroups.State, false) && ShowStateMenuItems) AddStateMenu(menu.Items);
            }
            if (IsMenuGroupVisible(PanoMenuGroups.AutoSize, false))
                AddAutoSizeMenu(menu.Items, null);
            if (IsMenuGroupVisible(PanoMenuGroups.Layout, false))
                AddLayoutMenu(menu.Items);
            if (IsMenuGroupVisible(PanoMenuGroups.Grouping, false))
                AddGroupingMenu(menu.Items, null);
            if (IsMenuGroupVisible(PanoMenuGroups.ColumnChooser, false)) AddColumnChooserMenu(menu.Items);
            if (ShowHeaderMenuThemeItems && IsMenuGroupVisible(PanoMenuGroups.Theme, false)) AddThemeMenu(menu.Items);
            if (IsMenuGroupVisible(PanoMenuGroups.Analytics, false) && !IsMenuGroupVisible(PanoMenuGroups.Clipboard, false))
                menu.Items.Add(PanoText.CopyMiniAnalytics, null, (_,__) => CopyMiniAnalyticsToClipboard()).Enabled = EnableClipboard && ViewCount > 0;
            menu.Items.Add(new ToolStripSeparator());
            if (EnableRowDetails && IsMenuGroupVisible(PanoMenuGroups.RowDetails, false))
                menu.Items.Add(PanoText.RowDetails, null, (_,__) => ToggleRowDetails(_selectedRow)).Enabled = SelectedObject != null;
            }
        }
    ...
    public void AttachBuiltInMenuTo(ContextMenuStrip userMenu, Point? clientLocation = null, string? menuText = null)
    {
        ...
        const string mergeTag = "__PanoBuiltInMergedMenu__";
        RemoveMergedPanoMenuItems(userMenu.Items, mergeTag);

        var text = string.IsNullOrWhiteSpace(menuText)
            ? (string.IsNullOrWhiteSpace(BuiltInMenuMergeText) ? PanoText.BuiltInMenuMergeText : BuiltInMenuMergeText.Trim())
            : menuText.Trim();

        var location = clientLocation ?? PointToClient(Cursor.Position);
        var mergedItems = CreatePanoMergedMenuItems(text, location, mergeTag);
        ...
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(userMenu, _theme);
    }

    private static void RemoveMergedPanoMenuItems(ToolStripItemCollection items, string mergeTag)
    {
    ...
    private int GetMergeInsertIndex(ToolStripItemCollection items)
    {
        ...
        {
            case PanoMenuMergePlacement.Top:
                return 0;
            case PanoMenuMergePlacement.BeforeFirstSeparator:
                for (int i = 0; i < items.Count; i++)
                    if (items[i] is ToolStripSeparator) return i;
                return items.Count;
            case PanoMenuMergePlacement.AfterFirstSeparator:
                for (int i = 0; i < items.Count; i++)
                    if (items[i] is ToolStripSeparator) return Math.Min(items.Count, i + 1);
                return items.Count;
            case PanoMenuMergePlacement.Bottom:
            default:
                return items.Count;
        }
    }

    private List<ToolStripItem> CreatePanoMergedMenuItems(string text, Point location, string mergeTag)
    {
        var result = new List<ToolStripItem>();

        if (BuiltInMenuMergePresentation == PanoMenuMergePresentation.SubMenu)
        {
            var root = CreatePanoMergedMenu(text, location);
        ...
        foreach (var group in GetMergedMenuGroupOrder())
        {
            ...
            if (BuiltInMenuMergePresentation == PanoMenuMergePresentation.GroupedSubMenus)
            {
                var groupMenu = CreatePanoMergedGroupMenu(group, location);
    ...
    private IEnumerable<PanoMenuGroups> GetMergedMenuGroupOrder()
    {
        var used = new HashSet<PanoMenuGroups>();
        foreach (var token in (MergedMenuGroupOrder ?? string.Empty).Split(new[] { ',', ';', '|', ' ' }, StringSplitOptions.RemoveEmptyEntries))
        {
            if (Enum.TryParse(token.Trim(), true, out PanoMenuGroups group) && group != PanoMenuGroups.None && group != PanoMenuGroups.All && used.Add(group))
                yield return group;
        }

        PanoMenuGroups[] defaults =
        {
            PanoMenuGroups.Mode,
            PanoMenuGroups.ViewMode,
            PanoMenuGroups.Filter,
            PanoMenuGroups.Sort,
            PanoMenuGroups.ColumnChooser,
            PanoMenuGroups.Clipboard,
            PanoMenuGroups.AutoSize,
            PanoMenuGroups.Freeze,
            PanoMenuGroups.Layout,
            PanoMenuGroups.Grouping,
            PanoMenuGroups.Editing,
            PanoMenuGroups.RowDetails,
            PanoMenuGroups.Analytics,
            PanoMenuGroups.FilterStyle,
            PanoMenuGroups.Theme
        };
    ...
    private bool IsMergedMenuGroupVisible(PanoMenuGroups group, Point location)
    {
    ...
    private ToolStripMenuItem CreatePanoMergedGroupMenu(PanoMenuGroups group, Point location)
    {
        ...
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(menu.DropDown, _theme);
        return menu;
    }

    private string GetMergedGroupText(PanoMenuGroups group)
    {
        return group switch
        {
            PanoMenuGroups.Filter => PanoText.Filtering,
            PanoMenuGroups.Sort => PanoText.Sorting,
            PanoMenuGroups.Freeze => PanoText.Freeze,
            PanoMenuGroups.AutoSize => PanoText.ColumnWidth,
            PanoMenuGroups.Layout => PanoText.Layout,
            PanoMenuGroups.Grouping => PanoText.Grouping,
            PanoMenuGroups.ColumnChooser => PanoText.Columns,
            PanoMenuGroups.Mode => PanoText.Mode,
            PanoMenuGroups.ViewMode => PanoText.View,
            PanoMenuGroups.FilterStyle => PanoText.FilterStyle,
            PanoMenuGroups.Theme => PanoText.Theme,
            PanoMenuGroups.Clipboard => PanoText.Clipboard,
            PanoMenuGroups.Editing => PanoText.Editing,
            PanoMenuGroups.RowDetails => PanoText.RowDetails,
            PanoMenuGroups.Analytics => PanoText.Analytics,
            _ => group.ToString()
        };
    }

    private void AddMergedGroupItems(List<ToolStripItem> result, PanoMenuGroups group, Point location, object? tag)
    {
    ...
    private void AddMergedGroupItems(ToolStripItemCollection items, PanoMenuGroups group, Point location)
    {
        bool header = ShowHeader && location.Y < HeaderHeight;
        ...
        switch (group)
        {
            case PanoMenuGroups.Mode:
                if (ShowHeaderMenuModeItems) AddModeMenu(items);
                break;
            case PanoMenuGroups.ViewMode:
                if (ShowHeaderMenuViewModeItems) AddViewModeMenuItems(items);
                break;
            case PanoMenuGroups.Filter:
                if (col != null && ShowHeaderMenuFilterItems && ShowFilterMenu && col.Filterable)
                {
                    if (FilterMenuMode != PanoFilterMenuMode.ModalWindow || UseEmbeddedHeaderFilterMenu)
                        items.Add(CreateEmbeddedFilterMenuItem(col));
                    items.Add(PanoText.FilterWindow(col.Header), null, (_,__) => ShowFilterMenuForColumn(col, location)).Enabled = true;
                    if (ShowQuickClearFilterInHeaderMenu)
                    {
                        if (_filters.Get(col.AspectName) != null)
                            items.Add(PanoText.ClearColumnFilter, null, (_,__) => { _filters.Clear(col.AspectName); BuildViewIndex(); Invalidate(); QueueAutoSaveUserLayout(); });
                        if (HasActiveFilters)
                            items.Add(PanoText.ClearAllFilters, null, (_,__) => ClearFilters());
                    }
                }
                else
                {
                    items.Add(PanoText.ClearFilters, null, (_,__) => ClearFilters()).Enabled = HasActiveFilters;
                }
                break;
            case PanoMenuGroups.FilterStyle:
                ...
                break;
            case PanoMenuGroups.Sort:
                if (col != null && ShowHeaderMenuSortItems) AddSortMenu(items, col);
                break;
            case PanoMenuGroups.Freeze:
                if (col != null && ShowHeaderMenuFreezeItems) AddFreezeMenu(items, col);
                break;
            case PanoMenuGroups.AutoSize:
                if (ShowHeaderMenuAutoSizeItems) AddAutoSizeMenu(items, col);
                break;
            case PanoMenuGroups.Layout:
                if (ShowHeaderMenuLayoutItems) AddLayoutMenu(items);
                break;
            case PanoMenuGroups.Grouping:
                if (ShowHeaderMenuGroupingItems) AddGroupingMenu(items, col);
                break;
            case PanoMenuGroups.ColumnChooser:
                if (ShowHeaderMenuColumnChooserItem) AddColumnChooserMenu(items);
                break;
            case PanoMenuGroups.Theme:
                if (ShowHeaderMenuThemeItems) AddThemeMenu(items);
                break;
            case PanoMenuGroups.State:
                if (ShowStateMenuItems) AddStateMenu(items);
                break;
            case PanoMenuGroups.Scenario:
                if (ShowScenarioMenuItems) AddScenarioMenu(items);
                break;
            case PanoMenuGroups.Clipboard:
                items.Add(PanoText.Copy, null, (_,__) => CopySelectionToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                items.Add(PanoText.CopyCell, null, (_,__) => CopySelectedCellToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                items.Add(PanoText.CopySelectionJson, null, (_,__) => CopySelectionAsJsonToClipboard()).Enabled = EnableClipboard && SelectedObject != null;
                items.Add(PanoText.Paste, null, (_,__) => PasteTextToSelectedCell(Clipboard.GetText())).Enabled = EnableClipboard && SelectedObject != null && MenuProfile != PanoMenuProfile.ReadOnly;
                break;
            case PanoMenuGroups.Editing:
                if (MenuProfile != PanoMenuProfile.ReadOnly)
                    AddEditCellMenuItem(items);
                break;
            case PanoMenuGroups.RowDetails:
                if (EnableRowDetails)
                    items.Add(PanoText.RowDetails, null, (_,__) => ToggleRowDetails(_selectedRow)).Enabled = SelectedObject != null;
                break;
            case PanoMenuGroups.Analytics:
                items.Add(PanoText.CopyMiniAnalytics, null, (_,__) => CopyMiniAnalyticsToClipboard()).Enabled = EnableClipboard && ViewCount > 0;
                break;
        }
    }
    ...
    {
        return string.IsNullOrWhiteSpace(EditCellMenuText) ? PanoText.EditCell : EditCellMenuText.Trim();
    }
    ...
    public void AttachBuiltInHeaderMenuTo(ContextMenuStrip userMenu, PanoColumn? column = null, string? menuText = null)
    {
        if (userMenu == null || userMenu.IsDisposed) return;
        ...
        AttachBuiltInMenuTo(userMenu, location, menuText ?? PanoText.BuiltInHeaderMenu);
    }
    ...
    private ToolStripMenuItem CreatePanoMergedMenu(string text, Point location)
    {
    ...
    private void AddViewModeMenuItem(ToolStripItemCollection items, PanoViewMode mode)
    {
    ...
    {
        var viewMenu = new ToolStripMenuItem(PanoText.ViewMode)
        {
            ToolTipText = "Pano görünümünü tablo, kart, medya, dashboard veya workflow senaryosuna göre değiştirir."
        };
        ...
        {
            ApplyV40AnalyticsProfile(PanoV40AnalyticsPreset.KpiDashboard);
        });
        ...
        {
            ApplyV40AnalyticsProfile(PanoV40AnalyticsPreset.FactoryOverview);
        });
        AddPresetMenuItem(presets.DropDownItems, "Timeline akışı", "Olay, iş emri veya ticket zaman akışı görünümüne geçer.", () =>
        {
            ApplyV40AnalyticsProfile(PanoV40AnalyticsPreset.Timeline);
        });
        ...
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Müzik", PanoMediaSmartPreset.Music, "Albüm kapağı, now playing ve equalizer odaklı MediaTile.");
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Film", PanoMediaSmartPreset.Movie, "Afiş/poster, kalite rozeti ve playback overlay odaklı görünüm.");
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Fotoğraf", PanoMediaSmartPreset.Photo, "Galeri, cover ölçekleme ve görsel katalog modu.");
        AddMediaPresetMenuItem(mediaPresets.DropDownItems, "Doküman", PanoMediaSmartPreset.Document, "PDF/doküman önizleme için contain ölçekli DetailCard.");
        ...
        AddDashboardWidgetMenuItem(dashboard.DropDownItems, "KPI", PanoDashboardWidgetKind.Kpi, PanoViewMode.KpiDashboard);
        AddDashboardWidgetMenuItem(dashboard.DropDownItems, "HeatMap", PanoDashboardWidgetKind.HeatMap, PanoViewMode.HeatMap);
        AddDashboardWidgetMenuItem(dashboard.DropDownItems, "MiniChart", PanoDashboardWidgetKind.Chart, PanoViewMode.MiniChart);
        dashboard.DropDownItems.Add(new ToolStripSeparator());
        dashboard.DropDownItems.Add("Performans özeti", null, (_, __) =>
        {
            MessageBox.Show(FindForm(), GetPerformanceSummary(), "Pano Performance", MessageBoxButtons.OK, MessageBoxIcon.Information);
            CloseActiveMenusSafe();
        });
        dashboard.DropDownItems.Add("Diagnostics Center", CreateMenuDotIcon(Color.FromArgb(120, 155, 255)), (_, __) =>
        {
            ShowPanoDiagnosticsCenter();
            CloseActiveMenusSafe();
        });
        ...
        AddViewModeGroup(viewMenu.DropDownItems, "Tablo ve liste", "Yoğun veri, klasik tablo ve sade liste görünümleri.",
            PanoViewMode.Details,
            PanoViewMode.DenseList,
            PanoViewMode.List,
            PanoViewMode.GroupedList);

        AddViewModeGroup(viewMenu.DropDownItems, "Kart ve detay", "Ticket, ürün, master/detail ve okunabilir kart görünümleri.",
            PanoViewMode.Tile,
            PanoViewMode.LargeCard,
            PanoViewMode.RowCard,
            PanoViewMode.RowPreview,
            PanoViewMode.DetailCard,
            PanoViewMode.PropertyCard,
            PanoViewMode.MasterDetail);

        AddViewModeGroup(viewMenu.DropDownItems, "Medya ve galeri", "Müzik, film, resim, kapak ve katalog senaryoları.",
            PanoViewMode.Poster,
            PanoViewMode.Gallery,
            PanoViewMode.MediaTile,
            PanoViewMode.FilmStrip,
            PanoViewMode.IconGrid,
            PanoViewMode.ExtraLargeIcons,
            PanoViewMode.LargeIcons,
            PanoViewMode.MediumIcons);

        AddViewModeGroup(viewMenu.DropDownItems, "Dashboard ve analiz", "KPI, dashboard, heatmap ve mini trend görünümleri.",
            PanoViewMode.DashboardCard,
            PanoViewMode.KpiDashboard,
            PanoViewMode.HeatMap,
            PanoViewMode.MiniChart);

        AddViewModeGroup(viewMenu.DropDownItems, "Workflow", "Kanban, timeline ve grup kart akışları.",
            PanoViewMode.Kanban,
            PanoViewMode.Timeline,
            PanoViewMode.GroupCard);

        RemoveRedundantMenuSeparators(viewMenu.DropDownItems);
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(viewMenu.DropDown, _theme);
        return viewMenu;
    }

    private void AddMediaPresetMenuItem(ToolStripItemCollection items, string text, PanoMediaSmartPreset preset, string tooltip)
    {
        ...
        {
            PanoMediaSmartPreset.Music => Color.FromArgb(120, 155, 255),
            PanoMediaSmartPreset.Movie => Color.FromArgb(180, 120, 255),
            PanoMediaSmartPreset.Photo => Color.FromArgb(90, 190, 150),
            PanoMediaSmartPreset.Document => Color.FromArgb(220, 170, 80),
            _ => _theme.AccentColor
        });
        item.ToolTipText = tooltip;
    }

    private void AddDashboardWidgetMenuItem(ToolStripItemCollection items, string text, PanoDashboardWidgetKind kind, PanoViewMode viewMode)
    {
        ...
        {
            PanoViewMode.KpiDashboard => Color.FromArgb(92, 200, 128),
            PanoViewMode.HeatMap => Color.FromArgb(230, 120, 80),
            PanoViewMode.MiniChart => Color.FromArgb(90, 170, 230),
            _ => _theme.AccentColor
        });
    ...
    private void AddViewModeGroup(ToolStripItemCollection items, string text, string tooltip, params PanoViewMode[] modes)
    {
        ...
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(group.DropDown, _theme);
        items.Add(group);
    }
    ...
    {
        var modeMenu = new ToolStripMenuItem(PanoText.Mode);
        foreach (PanoDataMode m in Enum.GetValues(typeof(PanoDataMode)))
        {
            var local = m;
            AddCheckedMenuItem(modeMenu.DropDownItems, PanoText.PanoDataModeName(local.ToString()), Mode == local, (_,__) => { Mode = local; RefreshView(); CloseActiveMenusSafe(); });
        }
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(modeMenu.DropDown, _theme);
        items.Add(modeMenu);
    }

    private void AddThemeMenu(ToolStripItemCollection items)
    {
        var themeMenu = new ToolStripMenuItem(PanoText.Theme);
        foreach (PanoThemePreset p in Enum.GetValues(typeof(PanoThemePreset)))
        {
            var local = p;
            AddCheckedMenuItem(themeMenu.DropDownItems, PanoText.ThemePresetName(local.ToString()), ThemePreset == local, (_,__) => { ThemePreset = local; RefreshView(); CloseActiveMenusSafe(); });
        }
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(themeMenu.DropDown, _theme);
        items.Add(themeMenu);
    }
    ...
    private void AddGroupHeaderContextMenuItems(ContextMenuStrip menu, int rowIndex)
    {
        ...
        if (ShowGroupMenuToggleThisGroupItem)
            menu.Items.Add(collapsed ? PanoText.ExpandThisGroup : PanoText.CollapseThisGroup, null, (_, __) => ToggleGroup(groupKey));

        if (ShowGroupMenuOnlyThisGroupItem)
            menu.Items.Add(PanoText.ShowOnlyThisGroup, null, (_, __) => ShowOnlyGroup(groupKey));

        if (ShowGroupMenuToggleThisGroupItem || ShowGroupMenuOnlyThisGroupItem)
            menu.Items.Add(new ToolStripSeparator());

        if (ShowGroupMenuCollapseAllItem)
            menu.Items.Add(PanoText.CollapseAllGroups, null, (_, __) => CollapseAllGroups()).Enabled = EnableGrouping;

        if (ShowGroupMenuExpandAllItem)
            menu.Items.Add(PanoText.ExpandAllGroups, null, (_, __) => ExpandAllGroups()).Enabled = EnableGrouping;
        ...
        if (ShowGroupMenuClearGroupingItem)
            menu.Items.Add(PanoText.ClearGrouping, null, (_, __) => ClearGrouping()).Enabled = EnableGrouping;
    }


    private void AddSortMenu(ToolStripItemCollection items, PanoColumn? col)
    {
        var root = new ToolStripMenuItem(PanoText.Sorting);
        if (col != null)
        {
            root.DropDownItems.Add(PanoText.SortAscending, null, (_,__) => { SortBy(col, false); }).Enabled = col.Sortable;
            root.DropDownItems.Add(PanoText.SortDescending, null, (_,__) => { SortBy(col, true); }).Enabled = col.Sortable;
            root.DropDownItems.Add(PanoText.UnsortColumn, null, (_,__) => ClearSort(col)).Enabled = _sortColumn == col;
            root.DropDownItems.Add(new ToolStripSeparator());
        }
        root.DropDownItems.Add(PanoText.ClearSort, null, (_,__) => ClearSort(null)).Enabled = _sortColumn != null;
        RemoveRedundantMenuSeparators(root.DropDownItems);
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }

    private void AddFreezeMenu(ToolStripItemCollection items, PanoColumn? col)
    {
        var root = new ToolStripMenuItem(PanoText.FrozenColumn);
        if (col != null)
            root.DropDownItems.Add(PanoText.FreezeColumn, null, (_,__) => { FrozenColumnCount = Math.Max(FrozenColumnCount, Columns.VisibleColumns.ToList().IndexOf(col) + 1); RefreshView(); });
        root.DropDownItems.Add(PanoText.ClearFrozenColumns, null, (_,__) => { FrozenColumnCount = 0; RefreshView(); }).Enabled = FrozenColumnCount > 0;
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }

    private void AddAutoSizeMenu(ToolStripItemCollection items, PanoColumn? col)
    {
        var root = new ToolStripMenuItem(PanoText.ColumnWidth);
        if (col != null)
            root.DropDownItems.Add(PanoText.AutoSizeColumn, null, (_,__) => AutoResizeColumn(col));
        root.DropDownItems.Add(PanoText.AutoSizeAllColumns, null, (_,__) => AutoResizeAllColumnsToContent());
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }
    ...
    {
        var root = new ToolStripMenuItem(PanoText.Layout);
        root.DropDownItems.Add(PanoText.SaveColumnLayout, null, (_,__) => SaveColumnLayout());
        root.DropDownItems.Add(PanoText.LoadColumnLayout, null, (_,__) => LoadColumnLayout());
        root.DropDownItems.Add(PanoText.SaveDefaultLayoutProfile, null, (_,__) => SaveColumnLayoutProfile("Default"));
        var profileMenu = new ToolStripMenuItem(PanoText.LayoutProfiles);
        ...
        root.DropDownItems.Add(PanoText.ResetColumnLayout, null, (_,__) => ResetColumnLayout());
        root.DropDownItems.Add(new ToolStripSeparator());
        root.DropDownItems.Add(PanoText.SaveUserLayout, null, (_,__) => SaveUserLayout());
        root.DropDownItems.Add(PanoText.LoadUserLayout, null, (_,__) => LoadUserLayout());
        root.DropDownItems.Add(PanoText.ResetUserLayout, null, (_,__) => ResetUserLayout());
        root.DropDownItems.Add(PanoText.ExportUserLayout, null, (_,__) => ExportUserLayoutWithDialog());
        root.DropDownItems.Add(PanoText.ImportUserLayout, null, (_,__) => ImportUserLayoutWithDialog());
        RemoveRedundantMenuSeparators(root.DropDownItems);
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }

    private void AddGroupingMenu(ToolStripItemCollection items, PanoColumn? col)
    {
        var root = new ToolStripMenuItem(PanoText.Grouping);
        if (col != null)
        {
            root.DropDownItems.Add(PanoText.GroupByColumn(col.Header), null, (_,__) => SetGroupBy(col.AspectName)).Enabled = col.AllowGroup;
            root.DropDownItems.Add(new ToolStripSeparator());
        }
        root.DropDownItems.Add(PanoText.CollapseAllGroups, null, (_,__) => CollapseAllGroups()).Enabled = EnableGrouping;
        root.DropDownItems.Add(PanoText.ExpandAllGroups, null, (_,__) => ExpandAllGroups()).Enabled = EnableGrouping;
        root.DropDownItems.Add(PanoText.ClearGrouping, null, (_,__) => ClearGrouping()).Enabled = EnableGrouping;
        RemoveRedundantMenuSeparators(root.DropDownItems);
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(root.DropDown, _theme);
        items.Add(root);
    }
    ...
    {
        var filterStyleMenu = new ToolStripMenuItem(PanoText.FilterStyle);
        filterStyleMenu.DropDown.Renderer = new global::Taylan.Pano.Theming.SmartMenuRenderer(_theme);
        AddCheckedMenuItem(filterStyleMenu.DropDownItems, PanoText.PopupMenu, FilterMenuMode == PanoFilterMenuMode.PopupMenu, (_,__) => { FilterMenuMode = PanoFilterMenuMode.PopupMenu; QueueAutoSaveUserLayout(); CloseActiveMenusSafe(); });
        AddCheckedMenuItem(filterStyleMenu.DropDownItems, PanoText.ModalWindow, FilterMenuMode == PanoFilterMenuMode.ModalWindow, (_,__) => { FilterMenuMode = PanoFilterMenuMode.ModalWindow; QueueAutoSaveUserLayout(); CloseActiveMenusSafe(); });
        AddCheckedMenuItem(filterStyleMenu.DropDownItems, PanoText.Both, FilterMenuMode == PanoFilterMenuMode.Both, (_,__) => { FilterMenuMode = PanoFilterMenuMode.Both; QueueAutoSaveUserLayout(); CloseActiveMenusSafe(); });
        global::Taylan.Pano.Theming.SmartMenuRenderer.ApplyTo(filterStyleMenu.DropDown, _theme);
    ...
    public void AutoResizeColumn(PanoColumn col) => AutoResizeColumnToContent(col);
...
internal sealed class PanoMenuDismissMessageFilter : IMessageFilter
{
    ...
    public PanoMenuDismissMessageFilter(Func<bool> isInsideAllowedArea, Action closeAll)
    {
...
internal readonly struct PanoDisplayRow
{
    private PanoDisplayRow(bool isGroup, int realIndex, string groupKey, string caption, int count, bool collapsed)
    {
    ...
    public static PanoDisplayRow Data(int realIndex) => new(false, realIndex, string.Empty, string.Empty, 0, false);
    public static PanoDisplayRow Group(string caption, int count, bool collapsed) => new(true, -1, caption, $"{caption} ({count})", count, collapsed);
}

public class PanoCellClickEventArgs : EventArgs
{
    public PanoCellClickEventArgs(int rowIndex, object rowObject, PanoColumn column) { RowIndex = rowIndex; RowObject = rowObject; Column = column; }
    public int RowIndex { get; }
    public object RowObject { get; }
    public PanoColumn Column { get; }

    /// <summary>True ise tıklanan hücre Pano checkbox/toggle veya GLV compatibility checkbox hücresidir.</summary>
    public bool IsCheckBoxColumn => Column.Kind == PanoColumnKind.CheckBox || Column.Kind == PanoColumnKind.ToggleSwitch || Column.CellCheckBox;
}
public sealed class PanoCellEditEventArgs : PanoCellClickEventArgs
{
    public PanoCellEditEventArgs(int rowIndex, object rowObject, PanoColumn column, object? newValue) : base(rowIndex,rowObject,column) => NewValue = newValue;
