...
{
    [Category("Pano - Profiles")]
    [DefaultValue(true)]
    [Description("v29 profil sistemi. Tek kaynak PanoLayoutProfile kullanılır; eski kolon profil JSON dosyaları otomatik migrate edilebilir.")]
    ...
    public string GetLayoutProfileFilePath(string profileName, string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PanoControl", "Profiles", key, scope);
        Directory.CreateDirectory(dir);
        return Path.Combine(dir, safeProfile + ".panoprofile");
    }
    ...
    public void SaveLayoutProfile(string profileName, string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        PanoLayoutProfile profile = CaptureLayoutProfile(profileName);
    ...
    public bool LoadLayoutProfile(string profileName, string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        if (!File.Exists(file))
        {
            ...
            PanoLayoutProfile migrated = PanoProfileMigrator.FromLegacyColumnProfileJson(File.ReadAllText(legacy), profileName);
            ApplyLayoutProfile(migrated);
            return true;
        }

        PanoLayoutProfile profile = PanoLayoutProfile.FromJson(File.ReadAllText(file));
    ...
    public IReadOnlyList<string> GetLayoutProfileNames(string? roleName = null, string? userName = null, string? machineName = null)
    {
        ...
        return Directory.GetFiles(dir, "*.panoprofile")
    ...
    public PanoLayoutProfile ImportLayoutProfile(string filePath, bool apply = true)
    {
        if (string.IsNullOrWhiteSpace(filePath)) throw new ArgumentException("File path cannot be empty.", nameof(filePath));
        PanoLayoutProfile profile = PanoLayoutProfile.FromJson(File.ReadAllText(filePath));
    ...
    public int MigrateLegacyProfiles(bool deleteLegacyAfterMigration = false)
    {
        ...
        foreach (string name in GetColumnLayoutProfileNames())
        {
            ...
            try
            {
                PanoLayoutProfile profile = PanoProfileMigrator.FromLegacyColumnProfileJson(File.ReadAllText(legacy), name);
