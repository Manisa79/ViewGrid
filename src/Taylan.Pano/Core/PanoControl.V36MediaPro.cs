...
public enum PanoMediaMissingCoverBehavior
{
    Placeholder,
    Initials,
    Hidden,
...
public partial class PanoControl
{
    ...
    [Category("Pano - V36 Media Pro")]
    [DefaultValue(PanoMediaMissingCoverBehavior.Placeholder)]
    [Description("Kapak bulunamadığında placeholder/initials/gizle/rozet-only davranışı.")]
    public PanoMediaMissingCoverBehavior MissingCoverBehavior { get; set; } = PanoMediaMissingCoverBehavior.Placeholder;
    ...
    public void ApplyV36MediaProPack()
    {
        ...
        if (MediaPlaceholderImage == null)
            MediaPlaceholderImage = CreateMediaPlaceholderBitmap("PANO", _theme.AccentColor);
    ...
    public Image? ResolveMediaImagePro(object row)
    {
        ...
        if (string.IsNullOrWhiteSpace(path))
            return MissingCoverBehavior == PanoMediaMissingCoverBehavior.Hidden ? null : MediaPlaceholderImage;
    ...
    public Bitmap CreateMediaPlaceholderBitmap(string label, Color? baseColor = null)
    {
        ...
        using var bg = new System.Drawing.Drawing2D.LinearGradientBrush(new Rectangle(0, 0, bmp.Width, bmp.Height), Taylan.Pano.Theming.PanoThemeAccessibility.Blend(color, Color.White, 0.15d), Taylan.Pano.Theming.PanoThemeAccessibility.Blend(color, Color.Black, 0.45d), 55f);
        ...
        TextRenderer.DrawText(g, "Pano Media Pro", SystemFonts.CaptionFont, new Rectangle(0, 426, bmp.Width, 30), Color.Gainsboro, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
        return bmp;
    }
