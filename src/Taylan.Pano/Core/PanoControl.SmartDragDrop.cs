...
public enum PanoExternalDropKind
{
    Unknown,
    Text,
    Files,
...
public sealed class PanoRowDragStartingEventArgs : CancelEventArgs
{
    public PanoRowDragStartingEventArgs(IReadOnlyList<object> rows, int sourceViewIndex)
    {
        Rows = rows;
...
public sealed class PanoRowDropValidatingEventArgs : CancelEventArgs
{
    public PanoRowDropValidatingEventArgs(IReadOnlyList<object> rows, int targetViewIndex, PanoDropPosition position)
    {
        Rows = rows;
    ...
    public PanoDropPosition Position { get; set; }
}

public sealed class PanoRowDroppedEventArgs : EventArgs
{
    public PanoRowDroppedEventArgs(IReadOnlyList<object> rows, int targetViewIndex, PanoDropPosition position)
    {
        Rows = rows;
        TargetViewIndex = targetViewIndex;
        Position = position;
    ...
    public IReadOnlyList<object> Rows { get; }
    public int TargetViewIndex { get; }
    public PanoDropPosition Position { get; }
}

public sealed class PanoExternalDropEventArgs : CancelEventArgs
{
    public PanoExternalDropEventArgs(PanoExternalDropKind kind, object? payload, int targetViewIndex, PanoDropPosition position)
    {
        Kind = kind;
        Payload = payload;
        TargetViewIndex = targetViewIndex;
    ...
    public PanoExternalDropKind Kind { get; }
...
public partial class PanoControl
{
    ...
    private PanoDropPosition _smartDropPosition = PanoDropPosition.None;

    [Category("Pano - Smart Drag Drop")]
    [DefaultValue(false)]
    ...
    public event EventHandler<PanoRowDragStartingEventArgs>? RowDragStarting;
    public event EventHandler<PanoRowDropValidatingEventArgs>? RowDropValidating;
    public event EventHandler<PanoRowDroppedEventArgs>? RowDropped;
    public event EventHandler<PanoExternalDropEventArgs>? ExternalDropReceived;
    ...
    private void SmartDragDrop_MouseMove(object? sender, MouseEventArgs e)
    {
        ...
        var args = new PanoRowDragStartingEventArgs(rows, _smartDragStartViewIndex);
    ...
    private void SmartDragDrop_DragLeave(object? sender, EventArgs e)
    {
        ...
        _smartDropPosition = PanoDropPosition.None;
        Invalidate();
    }
    ...
    private void SmartDragDrop_DragDrop(object? sender, DragEventArgs e)
    {
        ...
        {
            var validate = new PanoRowDropValidatingEventArgs(payload.Rows, _smartDropTargetViewIndex, _smartDropPosition);
            ...
            RowDropped?.Invoke(this, new PanoRowDroppedEventArgs(payload.Rows, validate.TargetViewIndex, validate.Position));
        ...
        if (AllowExternalDrop)
        {
            ...
            if (external != null)
            {
                ...
                {
                    var result = MessageBox.Show(FindForm(), "Bu veriler listeye bırakılsın mı?", "Pano Drag Drop", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
    ...
    private void UpdateSmartDropTarget(Point pt)
    {
        ...
        if (rowIndex >= 0)
        {
            ...
            _smartDropPosition = delta < RowHeight / 2 ? PanoDropPosition.Before : PanoDropPosition.After;
        }
        Invalidate();
    }
    ...
    private void ReorderRowsFromSmartDrop(IReadOnlyList<object> rows, int targetViewIndex, PanoDropPosition position)
    {
        ...
        if (position == PanoDropPosition.After) insertIndex++;
    ...
    private PanoExternalDropEventArgs? CreateExternalDropArgs(IDataObject? data, int targetViewIndex, PanoDropPosition position)
    {
        if (data == null) return null;
        if (data.GetDataPresent(DataFormats.FileDrop))
            return new PanoExternalDropEventArgs(PanoExternalDropKind.Files, data.GetData(DataFormats.FileDrop), targetViewIndex, position);
        if (data.GetDataPresent(DataFormats.Text))
            return new PanoExternalDropEventArgs(PanoExternalDropKind.Text, data.GetData(DataFormats.Text), targetViewIndex, position);
        return new PanoExternalDropEventArgs(PanoExternalDropKind.Unknown, data, targetViewIndex, position);
    }
    ...
    {
        public SmartRowDragPayload(PanoControl source, IReadOnlyList<object> rows)
        {
            Source = source;
            Rows = rows;
        }

        public PanoControl Source { get; }
        public IReadOnlyList<object> Rows { get; }
    }
}
