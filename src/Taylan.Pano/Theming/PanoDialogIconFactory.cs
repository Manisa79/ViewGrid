...
/// <summary>
/// Creates small theme-aware icons for Pano owned tool windows. The icons are
/// intentionally generated in code so the library does not need external image
/// resources and stays designer/package friendly.
/// </summary>
public static class PanoDialogIconFactory
{
    public static Icon Create(PanoDialogIconKind kind, PanoTheme theme, int size = 32)
    {
    ...
    private static void DrawGlyph(Graphics g, PanoDialogIconKind kind, Rectangle r, Pen pen, Pen accentPen, Brush accentBrush, Brush textBrush, PanoTheme theme)
    {
        int x = r.X;
        int y = r.Y;
        int w = r.Width;
        ...
        switch (kind)
        {
            case PanoDialogIconKind.Filter:
                Point[] funnel =
                {
                    new(x + w / 4, y + h / 4),
                    new(x + 3 * w / 4, y + h / 4),
            ...
            case PanoDialogIconKind.Export:
                ...
                break;
            case PanoDialogIconKind.Designer:
