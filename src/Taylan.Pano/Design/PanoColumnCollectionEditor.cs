...
/// </summary>
public sealed class PanoColumnCollectionEditor : UITypeEditor
{
    // WinForms designer / PanoControlDesigner tarafı bazen editor
    ...
    public PanoColumnCollectionEditor()
    {
    }

    public PanoColumnCollectionEditor(Type collectionType)
    {
    ...
    {
        if (value is not PanoColumnCollection columns)
            return value;

        PanoColumnCollectionEditor.EditColumns(
            columns,
            context?.Instance,
        ...
        bool centerScreen = false)
    {
        ...
        using var tx = host?.CreateTransaction("PanoControl kolonları düzenlendi");
        try
        {
            ...
            using var form = new PanoColumnEditorForm(columns, host);
            if (centerScreen)
            {
    ...
    private static void EnsureDesignerSitedColumns(PanoColumnCollection columns, IDesignerHost? host)
    {
        ...
        for (int i = 0; i < columns.Count; i++)
        {
            ...
            desiredName = PanoColumnNameHelper.EnsureUnique(desiredName, columns.Where((_, idx) => idx != i));

            current.Name = desiredName;
        }
    }

    internal static void CopyColumn(PanoColumn source, PanoColumn target)
    {
    ...
    internal static GLVColumn CreateDesignerColumn(PanoColumnCollection columns, IDesignerHost? host)
    {
        var name = CreateUniqueName(columns, host, PanoColumnNameHelper.GetNextDefaultOrdinal(columns));
    ...
    private static string CreateUniqueName(PanoColumnCollection columns, IDesignerHost? host, int startIndex)
    {
        ...
        while (true)
        {
            var name = PanoColumnNameHelper.CreateDefaultName(i);
...
internal sealed class PanoColumnEditorForm : System.Windows.Forms.Form
{
    private readonly PanoColumnCollection _columns;
    ...
    private readonly HashSet<PanoColumn> _createdColumns = new();
    private bool _committed;
    private bool _restored;

    public PanoColumnEditorForm(PanoColumnCollection columns, IDesignerHost? host)
    {
        _columns = columns;
        _host = host;
        ...
        Width = 760;
        Height = 520;
        PanoDialogChrome.ConfigureStandardDialog(this, WindowsThemeService.CurrentTheme(), new System.Drawing.Size(640, 420), sizeable: true, iconKind: PanoDialogIconKind.Column);
        ...
        // DisplayMember kullanmıyoruz; ListBox item text'i doğrudan PanoColumn.ToString()
    ...
    private bool IsOriginalColumn(PanoColumn column)
    ...
    private void DestroyDesignerComponent(PanoColumn column)
    {
    ...
    {
        // v26.64: PanoColumn artık Component tabanlı olduğu için Visual Studio
        // zaten gerçek Design > (Name) alanını kendisi sağlar. PropertyGrid'i
        // doğrudan kolona bağlamak çift (Name) satırını engeller; PanoColumn.Name
        // property’si Browsable(false) olduğu için sadece VS'nin Component adı görünür.
        _propertyGrid.SelectedObject = _list.SelectedItem is PanoColumn column
            ? column
            : null;
        UpdateButtons();
    }
    ...
    {
        var column = PanoColumnCollectionEditor.CreateDesignerColumn(_columns, _host);
    ...
    {
        if (_list.SelectedItem is not PanoColumn column) return;
...
internal sealed class PanoColumnPropertyGridObject : CustomTypeDescriptor
{
    private readonly PanoColumn _column;

    public PanoColumnPropertyGridObject(PanoColumn column)
    ...
    public override PropertyDescriptorCollection GetProperties(Attribute[]? attributes)
    {
        ...
        {
            if (string.Equals(property.Name, nameof(PanoColumn.Name), StringComparison.Ordinal))
            {
                if (!nameAdded)
                {
                    result.Add(new PanoColumnNamePropertyDescriptor(property));
                    nameAdded = true;
                }
                continue;
            }
        ...
        if (!nameAdded)
            result.Insert(0, new PanoColumnNamePropertyDescriptor(null));
...
internal sealed class PanoColumnNamePropertyDescriptor : PropertyDescriptor
{
    private readonly PropertyDescriptor? _inner;

    public PanoColumnNamePropertyDescriptor(PropertyDescriptor? inner)
        : base(nameof(PanoColumn.Name), null)
    {
        _inner = inner;
    }

    public override Type ComponentType => typeof(PanoColumn);
    ...
    {
        var column = component as PanoColumn;
    ...
    {
        if (component is PanoColumn column)
...
{
    public OriginalColumnEntry(PanoColumn column, ColumnSnapshot snapshot)
    {
        Column = column;
        Snapshot = snapshot;
    }
...
internal sealed class ColumnSnapshot
    {
        ...
        public static ColumnSnapshot From(PanoColumn column)
        {
        ...
        public void ApplyTo(PanoColumn column)
        {
