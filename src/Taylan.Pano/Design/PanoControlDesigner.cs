...
namespace Taylan.Pano.Design;

/// <summary>
/// Visual Studio Designer smart-tag support for PanoControl.
/// It exposes the most frequently changed ListView/Pano-like settings at design time.
/// </summary>
public sealed class PanoControlDesigner : Microsoft.DotNet.DesignTools.Designers.ControlDesigner
{
    private DesignerActionListCollection? _actionLists;
    private DesignerVerbCollection? _verbs;
    private PanoControlActionList? _registeredActionList;
    ...
    public override DesignerVerbCollection Verbs
    {
        get
        {
            ...
            {
                new DesignerVerb("PanoControl: Pano Kolon Editörünü Aç...", (_, _) => CreateActionList().EditColumns()),
                new DesignerVerb("PanoControl: Varsayılan Kolon Ekle", (_, _) => CreateActionList().AddDefaultColumns()),
                new DesignerVerb("PanoControl: Checkbox Seçimini Aç", (_, _) => CreateActionList().AddCheckBoxColumn()),
                new DesignerVerb("PanoControl: Ana Kapsayıcıda Yerleştir", (_, _) => CreateActionList().DockFill()),
                new DesignerVerb("PanoControl: Aktif Senaryoyu Uygula", (_, _) => CreateActionList().ApplyActiveScenario()),
                new DesignerVerb("PanoControl: Designer Açık Temayı Uygula", (_, _) => CreateActionList().UseLightTheme()),
                new DesignerVerb("PanoControl: Önizlemeyi Yenile", (_, _) => CreateActionList().RefreshPreview())
            };
            return _verbs;
        }
    }
    ...
    public override void Initialize(IComponent component)
    {
        ...
        if (component is not PanoControl)
            return;

        _registeredActionList = new PanoControlActionList(component);
        _actionLists = null;
    ...
    public override void InitializeNewComponent(IDictionary defaultValues)
    {
        ...
        if (Component is PanoControl pano)
        {
            pano.ShowHeader = true;
            pano.ShowGridLines = true;
            pano.FullRowSelect = true;
            pano.UseUnifiedThemeVisuals = true;
            pano.AutoApplyThemeToColumnHeaders = true;
    ...
    private PanoControlActionList CreateActionList()
    {
        if (_registeredActionList != null)
            return _registeredActionList;

        if (Component == null)
            throw new InvalidOperationException("PanoControl designer component is not initialized.");
...
public sealed class PanoControlActionList : DesignerActionList
{
    private readonly global::Taylan.Pano.Core.PanoControl _control;
    ...
    public PanoControlActionList(IComponent component) : base(component)
    {
        _control = (global::Taylan.Pano.Core.PanoControl)component;
    ...
    public PanoScenario ActiveScenario
    {
    ...
    public bool SelectionHeaderCheckBox
    {
        ...
        set
        {
            ...
            using var tx = host?.CreateTransaction(value ? "PanoControl header checkbox açıldı" : "PanoControl header checkbox kapatıldı");
    ...
    public PanoColumnCollection Columns
    {
        get => _control.Columns;
        set
        {
            // Koleksiyon nesnesi PanoControl içinde sabittir; Visual Studio SmartTag
    ...
    public PanoHeaderContextMenuBehavior HeaderContextMenuBehavior
    {
    ...
    private void EditColumnsCore()
    {
        ...
        try
        {
            ...
            // PanoColumnCollectionEditor formunu açar. VS native CollectionEditor
            // fallback'i kaldırıldı; böylece iki farklı editor/sıra/serialization yolu
            // birbirine karışmaz.
            if (PanoColumnCollectionEditor.EditColumns(_control.Columns, _control, columnsProperty, provider, centerScreen: true))
            {
    ...
    {
        // v25.80: Pano uyumlu seçim checkbox'ı ayrı kolon oluşturmaz.
        // İlk görünür veri kolonunun içinde çizilir. Bu nedenle SmartTag artık
        // yeni kolon eklemek yerine CheckBoxes özelliğini açar.
        var host = GetDesignerHost();
        using var tx = host?.CreateTransaction("PanoControl seçim checkbox'ı açıldı");
    ...
    {
        var host = GetDesignerHost();
        using var tx = host?.CreateTransaction("PanoControl varsayılan kolonlar eklendi");
        RaiseChanging(nameof(_control.Columns));
    ...
    {
        var baseName = PanoColumnNameHelper.CreateNameFromAspectOrText(aspectName, text, _control.Columns.Count + 1);
    ...
    {
        using var tx = GetDesignerHost()?.CreateTransaction("PanoControl kolonları temizlendi");
        RaiseChanging(nameof(_control.Columns));
    ...
    public void SetObjectMode() => SetProperty(nameof(_control.Mode), PanoDataMode.Object);
    public void SetDataTableMode() => SetProperty(nameof(_control.Mode), PanoDataMode.DataTable);
    public void SetVirtualMode() => SetProperty(nameof(_control.Mode), PanoDataMode.Virtual);
    public void SetTreeMode() => SetProperty(nameof(_control.Mode), PanoDataMode.Tree);
    public void SetTileMode() => SetProperty(nameof(_control.Mode), PanoDataMode.Tile);
    public void UseAutoTheme() => SetProperty(nameof(_control.DesignTimeThemePreview), PanoDesignTimeThemePreview.Auto);
    public void UseLightTheme() => SetProperty(nameof(_control.DesignTimeThemePreview), PanoDesignTimeThemePreview.Light);
    public void UseDarkTheme() => SetProperty(nameof(_control.DesignTimeThemePreview), PanoDesignTimeThemePreview.Dark);

    public void MenuFull() => ApplyMenuProfile(PanoMenuProfile.Full);
    public void MenuStandard() => ApplyMenuProfile(PanoMenuProfile.Standard);
    public void MenuMinimal() => ApplyMenuProfile(PanoMenuProfile.Minimal);
    public void MenuReadOnly() => ApplyMenuProfile(PanoMenuProfile.ReadOnly);
    public void MenuNone() => ApplyMenuProfile(PanoMenuProfile.None);

    private void ApplyMenuProfile(PanoMenuProfile profile)
    {
    ...
    public override DesignerActionItemCollection GetSortedActionItems()
    {
        ...
        {
            new DesignerActionMethodItem(this, nameof(EditColumns), "Pano Kolon Editörünü Aç...", "1. Kolonlar", "Pano'ye özgü kolon düzenleme penceresini açar.", true),
            ...
            new DesignerActionPropertyItem(nameof(UseBuiltInHeaderMenu), "Başlık Menüsü", "5. Menü / Filtre", "Pano'nin dahili başlık menüsünü açar/kapatır."),
            new DesignerActionPropertyItem(nameof(UseBuiltInBodyMenu), "Gövde Menüsü", "5. Menü / Filtre", "Pano'nin dahili gövde menüsünü açar/kapatır."),
            ...
            new DesignerActionPropertyItem(nameof(ThemePreset), "Tema Preseti:", "7. Tema / Designer", "Pano tema presetini seçer."),
    ...
    private sealed class PanoTypeDescriptorContext : ITypeDescriptorContext
    {
        private readonly object _instance;
        private readonly PropertyDescriptor? _propertyDescriptor;
        private readonly IServiceProvider? _serviceProvider;

        public PanoTypeDescriptorContext(object instance, PropertyDescriptor? propertyDescriptor, IServiceProvider? serviceProvider = null)
        {
            _instance = instance;
