...
/// <summary>
/// Optional provider hook: huge/async providers can notify ViewGridControl when a cached page is ready.
/// </summary>
...
/// <summary>
/// Optional fast path for huge virtual/query providers. It lets ViewGridControl update or summarize
/// checkbox state without materializing millions of rows on the UI thread.
/// </summary>
public interface IBulkCheckStateProvider
{
    bool TrySetAllCheckStates(ViewGridColumn column, CheckState state);
    bool TryGetCheckStateSummary(ViewGridColumn column, out int checkedCount, out int uncheckedCount);
}

/// <summary>
/// Optional fast path for huge/virtual data sources. If implemented, ViewGridControl can ask the
...
{
    bool TryBuildViewIndexes(ViewGridFilterSet filters, ViewGridColumn[] columns, ViewGridColumn? sortColumn, bool sortDescending, int maxFallbackScanRows, out IReadOnlyList<int> indexes);
}
...
public sealed class ListRowProvider : IIndexedRowProvider
{
    ...
    /// </summary>
    public bool TryBuildViewIndexes(ViewGridFilterSet filters, ViewGridColumn[] columns, ViewGridColumn? sortColumn, bool sortDescending, int maxFallbackScanRows, out IReadOnlyList<int> indexes)
    {
