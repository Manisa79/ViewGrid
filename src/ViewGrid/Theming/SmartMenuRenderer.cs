...
/// <summary>
/// Theme-aware ToolStrip renderer used by ViewGrid popup menus, merged user menus and nested drop-down menus.
...
public sealed class SmartMenuRenderer : ToolStripProfessionalRenderer
{
    ...
    public SmartMenuRenderer(ViewGridTheme theme) : base(new SmartMenuColorTable(theme))
    {
        _theme = theme;
    }

    public static void ApplyTo(ToolStrip? strip, ViewGridTheme theme)
    {
    ...
    public static void ApplyToItem(ToolStripItem item, ViewGridTheme theme)
    {
    ...
    private static void OnDropDownOpening(object? sender, EventArgs e)
    {
        ...
        if (item.DropDown.Tag is not ViewGridTheme theme) return;
...
{
    private readonly ViewGridTheme _theme;
    public SmartMenuColorTable(ViewGridTheme theme) => _theme = theme;
