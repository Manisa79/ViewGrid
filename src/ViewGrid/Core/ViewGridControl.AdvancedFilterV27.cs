...
{
    [Category("ViewGrid - v27.4 Advanced Filter")]
    [DefaultValue(true)]
    ...
    [Category("ViewGrid - v27.4 Advanced Filter")]
    [DefaultValue(ViewGridFilterLogic.And)]
    ...
    public ViewGridFilterLogic AdvancedFilterLogic
    {
        get => _filters.Logic;
        set
        {
    ...
    [Category("ViewGrid - v27.4 Advanced Filter")]
    [DefaultValue("")]
    [Description("Filtre presetlerinin saklanacağı klasör. Boşsa LocalAppData altında ViewGrid/FilterPresets kullanılır.")]
    ...
    {
        using var form = new ViewGridAdvancedFilterBuilderForm(Columns, CreateFilterPresetSnapshot(ActiveFilterPresetName), _theme);
    ...
    public ViewGridFilterPreset CreateFilterPresetSnapshot(string name = "")
    {
        return new ViewGridFilterPreset
        {
            Name = string.IsNullOrWhiteSpace(name) ? ActiveFilterPresetName : name,
            Description = "ViewGrid filter preset",
            Logic = _filters.Logic,
            GlobalText = _filters.GlobalText,
            Filters = _filters.Filters.Select(x => new ViewGridColumnFilterPresetItem
            {
    ...
    public void ApplyFilterPreset(ViewGridFilterPreset? preset)
    {
    ...
    public void SaveFilterPreset(ViewGridFilterPreset preset)
    {
        if (preset == null) throw new ArgumentNullException(nameof(preset));
        if (string.IsNullOrWhiteSpace(preset.Name))
            preset.Name = string.IsNullOrWhiteSpace(Name) ? "ViewGridFilterPreset" : Name;
    ...
    public bool LoadFilterPreset(string presetName)
    {
        ...
        var preset = JsonSerializer.Deserialize<ViewGridFilterPreset>(File.ReadAllText(path));
    ...
    public void ApplyBuiltInFilterPreset(string presetName)
    {
        ...
        var preset = new ViewGridFilterPreset { Name = presetName, Logic = ViewGridFilterLogic.And };

        if (key.Contains("OPEN") || key.Contains("AÇIK"))
            preset.Filters.Add(new ViewGridColumnFilterPresetItem { AspectName = "Status", Mode = ViewGridFilterMode.Contains, Text = "Open" });
        else if (key.Contains("EKSİK") || key.Contains("MISSING"))
            preset.Filters.Add(new ViewGridColumnFilterPresetItem { AspectName = "Status", Mode = ViewGridFilterMode.Contains, Text = "Eksik" });
        else if (key.Contains("WAIT"))
            preset.Filters.Add(new ViewGridColumnFilterPresetItem { AspectName = "Status", Mode = ViewGridFilterMode.Contains, Text = "Waiting" });
        else if (key.Contains("SAP"))
            preset.Filters.Add(new ViewGridColumnFilterPresetItem { AspectName = "Type", Mode = ViewGridFilterMode.Contains, Text = "SAP" });
        else if (key.Contains("BOM"))
            preset.Filters.Add(new ViewGridColumnFilterPresetItem { AspectName = "Type", Mode = ViewGridFilterMode.Contains, Text = "BOM" });
        else
    ...
    private void AddAdvancedFilterMenu(ToolStripItemCollection items)
    {
        ...
        var andItem = new ToolStripMenuItem("AND mantığı") { Checked = AdvancedFilterLogic == ViewGridFilterLogic.And };
        andItem.Click += (_, _) => AdvancedFilterLogic = ViewGridFilterLogic.And;
        var orItem = new ToolStripMenuItem("OR mantığı") { Checked = AdvancedFilterLogic == ViewGridFilterLogic.Or };
        orItem.Click += (_, _) => AdvancedFilterLogic = ViewGridFilterLogic.Or;
    ...
    {
        string? presetName = ViewGridPresetNameDialog.ShowDialog(FindForm(), "Filtre preset adı", string.IsNullOrWhiteSpace(ActiveFilterPresetName) ? "ViewGridFilter" : ActiveFilterPresetName);
        if (!string.IsNullOrWhiteSpace(presetName))
    ...
    private string GetFilterPresetFolder()
    {
        ...
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), appName, "ViewGrid", "FilterPresets");
    }
