...
{
    private CancellationTokenSource? _fastViewGridAsyncLoadCts;

    /// <summary>
    /// FastViewGridControl için önerilen en hızlı ve stabil profil.
    ...
    public void PrebuildSmartFilterIndexes(params string[] aspectNames)
    {
        ...
        IEnumerable<ViewGridColumn> targets = aspectNames == null || aspectNames.Length == 0
    ...
    public Task SetObjectsAsyncSafe<T>(Func<CancellationToken, Task<IEnumerable<T>>> loader, bool applyPerformanceProfile = true, CancellationToken token = default)
    {
        ...
        _fastViewGridAsyncLoadCts?.Cancel();
        _fastViewGridAsyncLoadCts?.Dispose();
        _fastViewGridAsyncLoadCts = CancellationTokenSource.CreateLinkedTokenSource(token);
        var linked = _fastViewGridAsyncLoadCts.Token;
    ...
    /// </summary>
    public void RefreshFastViewGridCore(bool prebuildFilterIndexes = true)
    {
