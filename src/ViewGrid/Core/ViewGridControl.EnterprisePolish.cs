...
public partial class ViewGridControl
{
    ...
    [Category("ViewGrid - Enterprise"), DefaultValue(true)]
    ...
    [Category("ViewGrid - Enterprise"), DefaultValue(typeof(Color), "Empty")]
    public Color ValidationErrorColor { get; set; } = Color.Empty;

    [Category("ViewGrid - Enterprise"), DefaultValue("Sıralanıyor...")]
    public string SortBusyTitle { get; set; } = "Sıralanıyor...";

    [Category("ViewGrid - Enterprise"), DefaultValue("Lütfen bekleyin, liste arka planda hazırlanıyor.")]
    public string SortBusyDetail { get; set; } = "Lütfen bekleyin, liste arka planda hazırlanıyor.";

    [Category("ViewGrid - Enterprise"), DefaultValue(460)]
    public int SortBusyOverlayWidth { get; set; } = 460;

    [Category("ViewGrid - Enterprise"), DefaultValue(138)]
    public int SortBusyOverlayHeight { get; set; } = 138;

    [Category("ViewGrid - Enterprise"), DefaultValue(125)]
    ...
    public event EventHandler<ViewGridValidationEventArgs>? CellValidationNeeded;

    public void ShowCommandPalette(string? initialSearch = null)
    {
        if (!EnableCommandPalette) return;
        using var form = new ViewGridCommandPaletteForm(this, initialSearch ?? string.Empty);
        form.ShowDialog((IWin32Window?)FindForm() ?? this);
    }

    public void SetCellError(object rowObject, ViewGridColumn column, string? message)
    {
    ...
    public string? GetCellError(object rowObject, ViewGridColumn column)
    {
        if (rowObject == null || column == null) return null;
        var args = new ViewGridValidationEventArgs(rowObject, column);
    ...
    public string ExportVisibleHtml(string path, string title = "ViewGridControl") => SaveHtml(path, title, Columns.VisibleColumns.ToArray(), GetVisibleObjects());
    public string ExportSelectedHtml(string path, string title = "ViewGridSelection") => SaveHtml(path, title, Columns.VisibleColumns.ToArray(), SelectedObjects);

    public string ExportVisibleCsvWithDialog()
    {
        using var dlg = new SaveFileDialog { Title = "ViewGrid CSV dışa aktar", Filter = "CSV (*.csv)|*.csv|Tüm dosyalar (*.*)|*.*", FileName = "viewgrid-visible.csv" };
    ...
    {
        using var dlg = new SaveFileDialog { Title = "ViewGrid Excel dışa aktar", Filter = "Excel XML (*.xml)|*.xml|Tüm dosyalar (*.*)|*.*", FileName = "viewgrid-visible.xml" };
    ...
    {
        using var dlg = new SaveFileDialog { Title = "ViewGrid seçimi CSV dışa aktar", Filter = "CSV (*.csv)|*.csv|Tüm dosyalar (*.*)|*.*", FileName = "viewgrid-selection.csv" };
        return dlg.ShowDialog(this) == DialogResult.OK ? ExportSelectedCsv(dlg.FileName) : string.Empty;
    }

    private static string SaveHtml(string path, string title, IReadOnlyList<ViewGridColumn> columns, IEnumerable<object> rows)
    {
    ...
    private void DrawValidationAdornerIfNeeded(Graphics g, Rectangle cellBounds, object row, ViewGridColumn column)
    {
    ...
    private static string GetValidationKey(object rowObject, ViewGridColumn column)
...
public sealed class ViewGridValidationEventArgs : EventArgs
{
    public ViewGridValidationEventArgs(object rowObject, ViewGridColumn column)
    {
        RowObject = rowObject;
        Column = column;
    }
