...
{
    [Category("ViewGrid - State"), DefaultValue("")]
    ...
    [Category("ViewGrid - State"), DefaultValue(true)]
    ...
    public ViewGridState CaptureState(string name = "")
    {
        var state = new ViewGridState
        {
    ...
    public void ApplyState(ViewGridState state)
    {
        ...
        if (!string.IsNullOrWhiteSpace(state.Scenario) && Enum.TryParse<ViewGridScenario>(state.Scenario, out var scenario))
            ActiveScenario = scenario;

        if (!string.IsNullOrWhiteSpace(state.ViewMode) && Enum.TryParse<ViewGridMode>(state.ViewMode, out var viewMode))
            ViewMode = viewMode;
    ...
    public bool LoadState(string path)
    {
        ...
        ApplyState(ViewGridState.FromJson(File.ReadAllText(path)));
        return true;
    }
