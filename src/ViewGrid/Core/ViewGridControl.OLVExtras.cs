...
/// These helpers intentionally depend only on ViewGrid types; there is no BrightIdeasSoftware reference.
/// </summary>
public enum ViewGridSelectionRestoreMode
{
    None = 0,
    ByReference = 1,
    ByAspectValue = 2
...
public sealed class ViewGridSelectionSnapshot
{
    internal ViewGridSelectionSnapshot(List<object> references, List<object?> keys, string keyAspectName, int scrollY)
    {
...
public partial class ViewGridControl
{
    ...
    [Category("ViewGrid - Compatibility")]
    [DefaultValue(true)]
    [Description("OLV migration alias. ViewGrid header zaten tema uyumlu çizilir; false verilirse özel header renkleri korunur.")]
    ...
    public ViewGridSelectionSnapshot CaptureSelectionSnapshot(string keyAspectName = "")
    {
        ...
        var keys = !string.IsNullOrWhiteSpace(keyAspectName)
            ...
            : new List<object?>();
        return new ViewGridSelectionSnapshot(selected, keys, keyAspectName ?? string.Empty, _scrollY);
    }

    public void RestoreSelectionSnapshot(ViewGridSelectionSnapshot? snapshot, bool ensureVisible = true)
    {
