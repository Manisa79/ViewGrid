...
public sealed class FilterMenuForm : Form
{
    ...
    private readonly TextBox _search = new() { PlaceholderText = ViewGridText.SearchPlaceholder };
    ...
    private readonly CheckBox _selectAll = new() { Text = ViewGridText.SelectAll, AutoSize = true, ThreeState = true, AutoCheck = false };
    private readonly CheckBox _useTextFilter = new() { Text = ViewGridText.SearchTypedTextInColumn, AutoSize = true };
    ...
    private readonly Button _apply = new() { Text = ViewGridText.Apply, Height = 28 };
    ...
    private readonly Button _clearAll = new() { Text = "✕  " + ViewGridText.ClearAllFilters, Height = 26 };
    private readonly Button _sortAsc = new() { Text = "↑  " + ViewGridText.SortAscending, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _sortDesc = new() { Text = "↓  " + ViewGridText.SortDescending, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _unsort = new() { Text = "↕  " + ViewGridText.Unsort, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _groupBy = new() { Text = "▦  " + ViewGridText.GroupByColumn(""), Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    private readonly Button _clearGrouping = new() { Text = "▢  " + ViewGridText.ClearGrouping, Height = 26, TextAlign = ContentAlignment.MiddleLeft };
    ...
    public FilterMenuForm(ViewGridColumn column, IEnumerable<string> distinctValues, ViewGridColumnFilter? currentFilter = null, ViewGridTheme? theme = null, Func<string, IReadOnlyList<string>>? searchValueLoader = null, bool isFastPreview = false, int searchDebounceMs = 150, bool resizable = true, Size? defaultSize = null, Size? minimumSize = null, Size? maximumSize = null, bool rememberSize = true, string? sizeMemoryKey = null, bool showValueTooltips = true, bool autoWidthForLongValues = true)
    {
        ...
        _theme = theme ?? ViewGridTheme.LightTheme();
        ...
        MaximizeBox = false;
        MinimizeBox = false;
        ViewGridDialogChrome.ConfigureStandardDialog(this, _theme, MinimumSize, sizeable: resizable, iconKind: ViewGridDialogIconKind.Filter);
        Text = ViewGridText.ColumnFilterTitle(column.Header);
        _groupBy.Text = "▦  " + ViewGridText.GroupByColumn(column.Header);
    ...
    private void BuildLayout()
    {
        ...
        _apply.Width = 80;
        _limitedInfoLabel.Text = ViewGridText.FastPreviewLimitedInfo;
    ...
    private void ApplyTheme(ViewGridTheme theme)
    {
        ...
        Color formBack = ViewGridDialogThemeApplier.NormalizePanelBack(theme);
        Color controlBack = ViewGridDialogThemeApplier.NormalizeControlBack(theme);
        Color fore = ViewGridDialogThemeApplier.NormalizeControlFore(theme);
        Color panelFore = ViewGridDialogThemeApplier.NormalizePanelFore(theme);
        ...
        _limitedInfoPanel.BackColor = ViewGridDialogThemeApplier.Blend(controlBack, theme.AccentColor, theme.IsDark ? 0.36 : 0.18);
    ...
    private static void ApplyThemeRecursive(Control parent, ViewGridTheme theme, Color panelBack, Color controlBack, Color panelFore, Color controlFore)
    {
        foreach (Control control in parent.Controls)
        {
            switch (control)
            {
                ...
                case Button button:
                    ...
                    Color hoverBack = ViewGridDialogThemeApplier.Blend(controlBack, theme.AccentColor, theme.IsDark ? 0.24 : 0.10);
    ...
    private void RestoreChecks(ViewGridColumnFilter? currentFilter)
    {
        bool hasValueFilter = currentFilter?.Mode == ViewGridFilterMode.ValueList && currentFilter.SelectedValues != null;
    ...
    private void ApplyAndClose()
    {
        ...
        {
            Result = new ViewGridColumnFilter
            {
        ...
        else if (!_userChangedChecks && !string.IsNullOrWhiteSpace(searchText))
        {
            ...
            if (exact != null)
            {
                ...
                {
                    AspectName = _column.AspectName,
                    Mode = ViewGridFilterMode.ValueList,
        ...
        else
        {
            ...
            // the applied filter must be exact-value only. This matches Excel/ViewGrid user expectation
    ...
    private ViewGridColumnFilter? TryCreateExplicitFilter(string modeName, string searchText)
    {
        ViewGridFilterMode? mode = modeName switch
        {
            "Contains" => ViewGridFilterMode.Contains,
            "StartsWith" => ViewGridFilterMode.StartsWith,
            "Equals" => ViewGridFilterMode.Equals,
            "Blanks" => ViewGridFilterMode.IsEmpty,
            "Non-Blanks" => ViewGridFilterMode.IsNotEmpty,
            "GreaterThan" => ViewGridFilterMode.GreaterThan,
            "LessThan" => ViewGridFilterMode.LessThan,
            "Between" => ViewGridFilterMode.Between,
            _ => null
        };
        if (mode == null) return null;
        if (mode.Value == ViewGridFilterMode.IsEmpty || mode.Value == ViewGridFilterMode.IsNotEmpty)
        {
            return new ViewGridColumnFilter { AspectName = _column.AspectName, Mode = mode.Value, Enabled = true };
        }
