...
public sealed class ViewGridAdvancedFilterBuilderForm : Form
{
    ...
    private readonly List<ViewGridColumn> _columns;
    private readonly BindingSource _source = new();
    private readonly List<ViewGridColumnFilterPresetItem> _items = new();

    public ViewGridFilterPreset Preset { get; private set; }

    public ViewGridAdvancedFilterBuilderForm(IEnumerable<ViewGridColumn> columns, ViewGridFilterPreset initial, ViewGridTheme? theme)
    {
        _columns = columns.Where(x => x != null && !string.IsNullOrWhiteSpace(x.AspectName)).ToList();
        Preset = initial ?? new ViewGridFilterPreset();
        ...
        ViewGridDialogChrome.ConfigureStandardDialog(this, theme ?? WindowsThemeService.CurrentTheme(), new Size(780, 480), sizeable: true, iconKind: ViewGridDialogIconKind.Filter);
    ...
    private void BuildLayout()
    {
        ...
        _list.DisplayMember = nameof(ViewGridColumnFilterPresetItem.DisplayText);
    ...
    private void LoadInitial()
    {
        ...
        _mode.DataSource = Enum.GetValues(typeof(ViewGridFilterMode));
        _logic.DataSource = Enum.GetValues(typeof(ViewGridFilterLogic));
        _logic.SelectedItem = Preset.Logic;
        _global.Text = Preset.GlobalText ?? string.Empty;
        _items.Clear();
        if (Preset.Filters != null)
            _items.AddRange(Preset.Filters.Select(x => new ViewGridColumnFilterPresetItem
            {
    ...
    private void AddCondition()
    {
        ...
        var mode = _mode.SelectedItem is ViewGridFilterMode m ? m : ViewGridFilterMode.Contains;
        _items.Add(new ViewGridColumnFilterPresetItem
        {
            AspectName = aspect,
            Mode = mode,
            Text = _value1.Text,
    ...
    {
        if (_list.SelectedItem is ViewGridColumnFilterPresetItem item)
        {
            _items.Remove(item);
            RefreshList();
        }
    ...
    {
        Preset = new ViewGridFilterPreset
        {
            Name = Preset.Name,
            Description = Preset.Description,
            Logic = _logic.SelectedItem is ViewGridFilterLogic l ? l : ViewGridFilterLogic.And,
            GlobalText = _global.Text ?? string.Empty,
            Filters = _items.Select(x => new ViewGridColumnFilterPresetItem
            {
                AspectName = x.AspectName,
                Mode = x.Mode,
                Text = x.Text,
