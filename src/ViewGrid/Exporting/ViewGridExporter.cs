...
public static class ViewGridExporter
{
    public static string ToCsv(IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows, char separator = ';')
    {
    ...
    /// </summary>
    public static string SaveCsv(string path, IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows, char separator = ';')
    {
    ...
    public static string ToHtmlTable(IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows)
    {
        var cols = columns.Where(c => !c.PrivateColumn && c.Visible).ToList();
        var sb = new StringBuilder();
        sb.AppendLine("<table>");
    ...
    public static string ToJson(IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows)
    {
        var cols = columns.Where(c => !c.PrivateColumn && c.Visible).ToList();
    ...
    public static string SaveJson(string path, IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows)
    {
        if (string.IsNullOrWhiteSpace(path))
            path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "ViewGrid.json");
    ...
    /// For card/dashboard output use the ViewGridPdfExportOptions overload with Mode=Card.
    /// </summary>
    public static string SavePdf(string path, IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows, string title = "ViewGridControl")
    {
        return SavePdf(path, columns, rows, new ViewGridPdfExportOptions { Title = title, Mode = ViewGridPdfExportMode.Table });
    }
    ...
    /// </summary>
    public static string SavePdf(string path, IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows, ViewGridPdfExportOptions options)
    {
        if (string.IsNullOrWhiteSpace(path))
            path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "ViewGrid.pdf");
    ...
    private static byte[] BuildStyledPdf(IReadOnlyList<ViewGridColumn> columns, IReadOnlyList<object> rows, ViewGridPdfExportOptions options)
    {
        int pageWidth = options.Orientation == ViewGridPdfPageOrientation.Landscape ? 842 : 595;
        int pageHeight = options.Orientation == ViewGridPdfPageOrientation.Landscape ? 595 : 842;
        var pages = new List<PdfPage>();
        var mode = options.Mode == ViewGridPdfExportMode.Auto ? ViewGridPdfExportMode.Table : options.Mode;
        if (mode == ViewGridPdfExportMode.Card)
    ...
    private static void RenderPdfTable(List<PdfPage> pages, int pageWidth, int pageHeight, IReadOnlyList<ViewGridColumn> columns, IReadOnlyList<object> rows, ViewGridPdfExportOptions options)
    {
    ...
    private static void RenderPdfCards(List<PdfPage> pages, int pageWidth, int pageHeight, IReadOnlyList<ViewGridColumn> columns, IReadOnlyList<object> rows, ViewGridPdfExportOptions options)
    {
        int left = Math.Max(18, options.MarginLeft);
        int right = Math.Max(18, options.MarginRight);
        int top = Math.Max(18, options.MarginTop);
    ...
    private static PdfPage NewPdfPage(List<PdfPage> pages, int pageWidth, int pageHeight, ViewGridPdfExportOptions options, int pageNo)
    {
        ...
        {
            DrawPdfText(page, options.Title ?? "ViewGrid Export", options.MarginLeft, pageHeight - 30, 13, Color.FromArgb(20, 30, 45), true);
    ...
    private static void DrawPdfCard(PdfPage page, int x, int y, int w, int h, IReadOnlyList<ViewGridColumn> columns, object row, ViewGridPdfExportOptions options)
    {
        ViewGridCardVisualInfo? visual = null;
        ...
        IEnumerable<ViewGridColumn> cardColumns = columns.Where(c => c.VisibleInCard).OrderBy(c => c.CardOrder);
        ...
        foreach (ViewGridColumn column in cardColumns.Take(6))
        {
        ...
        {
            int bx = x + w - 12;
            foreach (ViewGridCardBadge badge in visual.Badges.Take(4))
            {
    ...
    private static int[] ComputePdfColumnWidths(IReadOnlyList<ViewGridColumn> columns, int usableWidth, bool fitToPage)
    {
    ...
    private static void DrawPdfFooter(PdfPage page, int pageWidth, int pageHeight, ViewGridPdfExportOptions options, int pageNo, int rowCount)
    {
    ...
    private static string GlyphText(ViewGridCardGlyph glyph)
    {
        return glyph switch
        {
            ViewGridCardGlyph.Warning => "!",
            ViewGridCardGlyph.Error => "X",
            ViewGridCardGlyph.Success => "OK",
            ViewGridCardGlyph.Message => "MSG",
            ViewGridCardGlyph.Attachment => "ATT",
            ViewGridCardGlyph.Pin => "PIN",
            ViewGridCardGlyph.Lock => "LOCK",
            ViewGridCardGlyph.Star => "*",
            ViewGridCardGlyph.Check => "OK",
            ViewGridCardGlyph.Clock => "TIME",
            ViewGridCardGlyph.Flag => "FLAG",
            ViewGridCardGlyph.Bell => "BELL",
            _ => "i"
        };
    }
    ...
    /// </summary>
    public static string SaveExcelWorkbook(string path, IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows, string worksheetName = "ViewGridControl")
    {
    ...
    /// </summary>
    public static string SaveExcelXml(string path, IEnumerable<ViewGridColumn> columns, IEnumerable<object> rows, string worksheetName = "ViewGridControl")
        => SaveExcelWorkbook(path, columns, rows, worksheetName);

    private static string NormalizeExcelPath(string path)
    {
        if (string.IsNullOrWhiteSpace(path))
            path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "ViewGrid.xlsx");
    ...
    private static string BuildWorksheetXml(IReadOnlyList<ViewGridColumn> cols, IReadOnlyList<object> rows)
    {
        var sb = new StringBuilder();
    ...
    private static string GetWritablePath(string path)
    {
        ...
        throw new UnauthorizedAccessException("ViewGridControl export için yazılabilir bir klasör bulunamadı. Lütfen farklı bir klasör seçin veya Windows Controlled Folder Access/antivirüs izinlerini kontrol edin.");
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(name)) return "ViewGridControl";
