#!/usr/bin/env python3
"""Lightweight static guard for ViewGrid partial-class growth.

It is not a compiler. It catches the most common regression seen during rapid
phase additions: duplicate ViewGridControl properties/events and sample code that
...
# Known nested classes in ViewGridControl*.cs may have properties named RowObject, Column, etc.
# These are intentionally ignored because they are not ViewGridControl members.
IGNORE_NAMES = {"RowObject", "RowIndex", "Column", "Model", "Name", "Position", "Rows", "TargetViewIndex", "ViewGridControl"}


def scan() -> int:
    props: dict[str, list[str]] = defaultdict(list)
    events: dict[str, list[str]] = defaultdict(list)
    for file in CORE.glob("ViewGridControl*.cs"):
        text = file.read_text(encoding="utf-8-sig", errors="ignore")
        marker = "partial class ViewGridControl"
    ...
    for title, data in (("property", props), ("event", events)):
        for name, files in sorted(data.items()):
            # ViewGridPlatformFeatureOptions is a separate options class that may share names.
    ...
    print("ViewGrid API guard completed. Review CHECK/WARNING lines before release.")
