...
public sealed class DocumentationMissingScreenshotsForm : PanoSampleFormBase
{
    ...
    public DocumentationMissingScreenshotsForm(DocumentationMissingScreenshotScenario scenario)
        : base($"Pano DOCX Missing Screenshot - {scenario.Title}", scenario.Description)
    {
        _scenario = scenario;
        Text = $"Pano Documentation Capture - {scenario.TargetHeading}";
        Width = 1280;
        Height = 800;
    ...
    private void ApplyScenario()
    {
        ...
        Pano.ClearGrouping();
        Pano.ClearSort();
        Pano.ShowHeader = true;
        Pano.ShowGridLines = true;
        Pano.FullRowSelect = true;
        Pano.MultiSelect = true;
        Pano.AllowEditAllCells = false;
        Pano.AllowColumnReorder = false;
        Pano.AllowRowDragDrop = false;
        Pano.ShowQuickFilterBar = false;
        Pano.ShowActiveFilterChips = false;
        Pano.CardVisualAdornments = false;
        Pano.AllowMultilineCells = false;
        Pano.MaxCellTextLines = 1;
        Pano.DetailsRowHeight = 30;
        Pano.EnableSearchEverywhere = false;
        Pano.EnableCommandPalette = false;
    ...
    {
        Pano.Columns.Add(new PanoColumn("✓", nameof(DocumentationFeatureRow.Checked), 44) { Kind = PanoColumnKind.CheckBox, HeaderCheckBox = true, HeaderCheckBoxThreeState = true });
        Pano.Columns.Add(new PanoColumn("Id", nameof(DocumentationFeatureRow.Id), 70));
        Pano.Columns.Add(new PanoColumn("Ad", nameof(DocumentationFeatureRow.Name), 230) { FillFreeSpace = true, Editable = true });
        Pano.Columns.Add(new PanoColumn("Kategori", nameof(DocumentationFeatureRow.Category), 150));
        Pano.Columns.Add(new PanoColumn("Durum", nameof(DocumentationFeatureRow.Status), 110) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("İlerleme", nameof(DocumentationFeatureRow.Progress), 130) { Kind = PanoColumnKind.ProgressBar });
        Pano.Columns.Add(new PanoColumn("Skor", nameof(DocumentationFeatureRow.Score), 90) { Kind = PanoColumnKind.Rating, MaxRating = 5 });
        if (includeSparkline)
            Pano.Columns.Add(new PanoColumn("Trend", nameof(DocumentationFeatureRow.Spark), 140) { Kind = PanoColumnKind.Sparkline });
        if (includeAction)
        {
            Pano.Columns.Add(new PanoColumn("Aksiyon", nameof(DocumentationFeatureRow.Action), 110) { Kind = PanoColumnKind.Button, ButtonText = "Aç" });
            Pano.Columns.Add(new PanoColumn("Link", nameof(DocumentationFeatureRow.Link), 120) { Kind = PanoColumnKind.Hyperlink });
        }
        Pano.Columns.Add(new PanoColumn("Notlar", nameof(DocumentationFeatureRow.Notes), 340) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
    }

    private void AddCapabilityColumns()
    {
        Pano.Columns.Add(new PanoColumn("Sıra", nameof(DocumentationFeatureRow.Id), 70));
        Pano.Columns.Add(new PanoColumn("Yetkinlik", nameof(DocumentationFeatureRow.Name), 260) { FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Kategori", nameof(DocumentationFeatureRow.Category), 160));
        Pano.Columns.Add(new PanoColumn("Durum", nameof(DocumentationFeatureRow.Status), 120) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Kapsam", nameof(DocumentationFeatureRow.Progress), 120) { Kind = PanoColumnKind.ProgressBar });
        Pano.Columns.Add(new PanoColumn("Çıktı", nameof(DocumentationFeatureRow.Action), 130));
        Pano.Columns.Add(new PanoColumn("Açıklama", nameof(DocumentationFeatureRow.Notes), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
    }

    private void ConfigureMixedColumnSystem()
    {
        AddStandardColumns(includeAction: true, includeSparkline: true);
        Pano.Columns.Add(new PanoColumn("Etiketler", nameof(DocumentationFeatureRow.Tags), 160) { Kind = PanoColumnKind.Tags });
        Pano.SetObjects(CreateRows("Column", 42));
        Pano.SortBy(Pano.Columns[nameof(DocumentationFeatureRow.Status)], false);
    }
    ...
    private void ConfigureDataBinding()
    {
        ...
        Pano.SetObjects(new[]
        {
            Row(1, "List<T> binding", "Binding", "Ready", 100, "SetObjects", "POCO nesne listesi doğrudan Pano içine bağlanır."),
    ...
    {
        AddCapabilityColumns();
        Pano.ShowQuickFilterBar = true;
        Pano.ShowActiveFilterChips = true;
        Pano.SetObjects(new[]
        {
        ...
        Pano.SetGlobalFilter("status open");
    }
    ...
    private void ConfigureQuickFilter()
    {
        ...
        Pano.ShowQuickFilterBar = true;
        Pano.ShowActiveFilterChips = true;
        Pano.SetObjects(CreateRows("Quick Filter", 80));
        Pano.SetGlobalFilter("Review");
    ...
    private void ConfigureSorting()
    {
        ...
        Pano.SetObjects(CreateRows("Sorting", 120));
        Pano.SortBy(Pano.Columns[nameof(DocumentationFeatureRow.Progress)], descending: true);
    ...
    {
        AddStandardColumns();
        Pano.SetObjects(CreateRows("Grouping", 120));
        Pano.SetGroupBy(nameof(DocumentationFeatureRow.Status));
    ...
    {
        AddStandardColumns();
        Pano.SetObjects(CreateRows("Quality", 80));
        Pano.RowBackColorGetter = row => row is DocumentationFeatureRow r && r.Status == "Fail" ? (Color?)Color.FromArgb(255, 235, 238) : null;
        Pano.RowForeColorGetter = row => row is DocumentationFeatureRow r && r.Status == "Fail" ? (Color?)Color.FromArgb(140, 0, 0) : null;
        Pano.ConditionalFormats.Clear();
        Pano.ConditionalFormats.Add(new global::Taylan.Pano.Formatting.PanoConditionalFormat
        {
            Column = Pano.Columns[nameof(DocumentationFeatureRow.Status)],
    ...
    private void ConfigureCardVisuals()
    {
        ...
        Pano.CardVisualAdornments = true;
        Pano.SetViewMode(PanoViewMode.DashboardCard);
        Pano.SetObjects(CreateRows("Card Visual", 28));
    }

    private void ConfigureCardActions()
    {
        AddStandardColumns(includeAction: true);
        Pano.SetViewMode(PanoViewMode.LargeCard);
        Pano.SetObjects(CreateRows("Card Action", 30));
        Pano.ButtonClick += (_, e) => Info.Text = $"Kart aksiyonu: {((DocumentationFeatureRow)e.RowObject).Name}";
        Pano.HyperlinkClick += (_, e) => Info.Text = $"Kart linki: {((DocumentationFeatureRow)e.RowObject).Link}";
    }

    private void ConfigureCheckboxLayout()
    {
        Pano.Columns.Add(new PanoColumn("Person", nameof(DocumentationFeatureRow.Name), 210) { CellCheckBox = true, HeaderCheckBox = true, HeaderCheckBoxThreeState = true, CheckBoxAspectName = nameof(DocumentationFeatureRow.Checked) });
        Pano.Columns.Add(new PanoColumn("Operasyon", nameof(DocumentationFeatureRow.Category), 160) { CellCheckBox = true, HeaderCheckBox = true, HeaderCheckBoxThreeState = true, CheckBoxAspectName = nameof(DocumentationFeatureRow.Approved) });
        Pano.Columns.Add(new PanoColumn("Onay", nameof(DocumentationFeatureRow.Status), 120) { CellCheckBox = true, HeaderCheckBox = true, HeaderCheckBoxThreeState = true, CheckBoxAspectName = nameof(DocumentationFeatureRow.Locked) });
        Pano.Columns.Add(new PanoColumn("İlerleme", nameof(DocumentationFeatureRow.Progress), 130) { Kind = PanoColumnKind.ProgressBar });
        Pano.Columns.Add(new PanoColumn("Notlar", nameof(DocumentationFeatureRow.Notes), 420) { FillFreeSpace = true });
        Pano.KeyboardSpaceTogglesCheckBoxes = true;
        Pano.SetObjects(CreateRows("Checkbox", 40));
    }
    ...
    private void ConfigureRowHeight()
    {
        ...
        Pano.MaxCellTextLines = 3;
        Pano.DetailsRowHeight = 44;
        Pano.SetObjects(CreateRows("Row Height", 45, longNotes: true));
    ...
    private void ConfigureVirtualMode(int count, string label)
    {
        ...
        Pano.SetVirtualProvider(new VirtualDemoRowProvider(count));
    ...
    {
        AddStandardColumns();
        Pano.DetailsRowHeight = 38;
        Pano.AllowMultilineCells = true;
        Pano.MaxCellTextLines = 2;
        Pano.SetObjects(CreateRows("High DPI", 60));
    ...
    {
        Pano.Columns.Add(new PanoColumn("Sabit Id", nameof(DocumentationFeatureRow.Id), 88));
        Pano.Columns.Add(new PanoColumn("Sabit Durum", nameof(DocumentationFeatureRow.Status), 120) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Serbest Ad", nameof(DocumentationFeatureRow.Name), 220) { FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Serbest Notlar", nameof(DocumentationFeatureRow.Notes), 320) { FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Progress", nameof(DocumentationFeatureRow.Progress), 130) { Kind = PanoColumnKind.ProgressBar });
        Pano.SetObjects(CreateRows("FixedFree", 55));
    ...
    {
        AddCapabilityColumns();
        Pano.EnableSearchEverywhere = true;
        Pano.EnableCommandPalette = true;
        Pano.ShowQuickFilterBar = true;
        Pano.SetObjects(new[]
        {
    ...
    private void ConfigureInlineEditing()
    {
        ...
        foreach (var column in Pano.Columns)
        ...
        Pano.SetObjects(CreateRows("Inline Edit", 50));
    ...
    private void ConfigureDragAndDrop()
    {
        ...
        Pano.AllowRowDragDrop = true;
        Pano.ShowDropIndicator = true;
        Pano.SetObjects(CreateRows("Drag Drop", 50));
    ...
    {
        AddStandardColumns(includeSparkline: true);
        Pano.SetViewMode(PanoViewMode.MasterDetail);
        Pano.SetObjects(CreateRows("Sync", 42));
    ...
    private static List<DocumentationFeatureRow> CreateRows(string prefix, int count, bool longNotes = false)
    {
        ...
        return Enumerable.Range(1, count).Select(i =>
        {
            ...
            string notes = longNotes
                ...
                : $"{prefix} senaryosu için Pano dokümantasyon satırı {i}.";
...
public static class DocumentationMissingScreenshotScenarios
{
    ...
    {
        new("kolon-sistemi", "Kolon Sistemi", "5. Kolon Sistemi", "kolon-sistemi", "PanoColumn türleri, header/cell checkbox, badge, progress, rating, button ve hyperlink kolonlarını gösterir."),
