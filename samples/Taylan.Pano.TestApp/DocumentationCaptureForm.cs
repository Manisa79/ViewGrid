...
public sealed partial class DocumentationCaptureForm : Form
{
    ...
    private void LoadItems()
    {
        ...
        AppendLog("  • pano-screenshot-manifest.json");
        AppendLog("  • pano-screenshots.md");
        AppendLog("  • pano-docx-insert-map.json");
    ...
    private async Task CaptureSelectedAsync()
    {
        ...
        if (selected.Count == 0)
        {
            MessageBox.Show(this, "En az bir örnek seçmelisin.", "Pano Documentation", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        ...
        {
            MessageBox.Show(this, "Çıktı klasörü boş olamaz.", "Pano Documentation", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
    ...
    private void WriteManifest(string outputFolder, List<DocumentationCaptureResult> results)
    {
        ...
        File.WriteAllText(Path.Combine(outputFolder, "pano-screenshot-manifest.json"), JsonSerializer.Serialize(results, jsonOptions), Encoding.UTF8);

        var md = new StringBuilder();
        md.AppendLine("# Taylan.Pano Documentation Screenshots");
        md.AppendLine();
        ...
        File.WriteAllText(Path.Combine(outputFolder, "pano-screenshots.md"), md.ToString(), Encoding.UTF8);
        ...
        File.WriteAllText(Path.Combine(outputFolder, "pano-docx-insert-map.json"), JsonSerializer.Serialize(insertMap, jsonOptions), Encoding.UTF8);
    }
    ...
    {
        // Overview / navigation
        Item("overview-example-center", "Overview", "Example Center Pro", "Ana ekran", "Tüm Pano örneklerini kategori ve arama ile bulma ekranı.", () => new ExampleCenterProForm()),
        ...
        Item("view-propertycard", "View Modes", "PropertyCard", "Özellik kartı", "Nesne özelliklerinin okunabilir kart biçiminde sunumu.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Property")),
        Item("view-groupcard", "View Modes", "GroupCard", "Gruplu kart", "Gruplu kart ve kategori bazlı sunum senaryosu.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Group")),
        Item("view-kanban", "View Modes", "Kanban", "Workflow", "Durum kolonları ve iş akışı kartları.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Kanban", "Workflow")),
        Item("view-heatmap", "View Modes", "HeatMap", "Yoğunluk", "Alarm, üretim veya metrik yoğunluğunu renklerle gösterme.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Heat", "HeatMap")),
        Item("view-kpi", "View Modes", "KPI Dashboard", "KPI", "Özet metrik kartları ve dashboard göstergeleri.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "KPI")),
        Item("view-minichart", "View Modes", "MiniChart", "Satır içi grafik", "Satır içinde mini grafik ve trend gösterimi.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Chart", "Mini")),
        ...
        Item("audix-pilot-overview", "Audix", "Audix Pilot", "Albüm görünümü", "Albüm kapağı, play/pause state, now playing rozeti ve medya overlay davranışı.", () => new PanoV51RealUsagePilotSampleForm()),
        Item("audix-now-playing", "Audix", "Audix Pilot", "Now Playing", "Çalan kartın ayrı görünmesi, pause overlay ve now playing rozeti.", () => new PanoV51RealUsagePilotSampleForm(), f => SelectScenario(f, "Now", "Playing", "Play")),
        Item("audix-filmstrip", "Audix", "Audix Pilot", "FilmStrip", "Audix için yatay albüm/şarkı şeridi.", () => new PanoV51RealUsagePilotSampleForm(), f => SelectScenario(f, "FilmStrip", "Film Strip")),
        Item("playback-playing", "Media Playback", "Media Playback State", "Playing", "Audio/video için Playing görsel durumu.", () => new PanoV41MediaPlaybackSampleForm(), f => SelectScenario(f, "Playing", "Play")),
        Item("playback-paused", "Media Playback", "Media Playback State", "Paused", "Audio/video için Paused görsel durumu.", () => new PanoV41MediaPlaybackSampleForm(), f => SelectScenario(f, "Paused", "Pause")),
        Item("playback-loading-error", "Media Playback", "Media Playback State", "Loading ve Error", "Loading, Error ve özel medya durumlarının dokümantasyon görüntüsü.", () => new PanoV41MediaPlaybackSampleForm(), f => SelectScenario(f, "Loading", "Error")),

        // Filtering and interaction
        Item("filter-popup-basic", "Filtering", "Filter Popup UX", "Temel popup", "Kolon filtre popup davranışı.", () => new PanoV2731FilterPopupUxForm()),
        Item("filter-popup-long-values", "Filtering", "Filter Popup UX", "Uzun değerler", "Uzun değerler, tooltip ve genişletilebilir popup davranışı.", () => new PanoV2731FilterPopupUxForm(), f => SelectScenario(f, "Long", "Uzun", "Tooltip")),
        Item("filter-popup-resize", "Filtering", "Filter Popup UX", "Resize", "Popup boyutlandırma ve hatırlama davranışı.", () => new PanoV2731FilterPopupUxForm(), f => SelectScenario(f, "Resize", "Boyut")),
        Item("popular-enterprise-overview", "Enterprise", "Popular Enterprise Features", "Presetler", "Column chooser, summary, conditional format ve advanced filter presetleri.", () => new PanoV279PopularEnterpriseFeaturesForm()),
        Item("popular-enterprise-filter", "Enterprise", "Popular Enterprise Features", "Advanced Filter", "Advanced filter presetleri ve hızlı filtre komutları.", () => new PanoV279PopularEnterpriseFeaturesForm(), f => SelectScenario(f, "Filter", "Advanced")),
        Item("popular-enterprise-summary", "Enterprise", "Popular Enterprise Features", "Summary", "Footer summary, aggregation ve sayısal özetler.", () => new PanoV279PopularEnterpriseFeaturesForm(), f => SelectScenario(f, "Summary", "Footer", "Aggregate")),
        ...
        Item("master-detail", "Interaction", "Master Detail", "Alt detay", "Üst kayıt ve alt detay Pano senaryosu.", () => new MasterDetailSampleForm()),
        ...
        Item("theme-audit-dark", "Theme", "Theme / Accessibility Audit", "Dark", "Dark tema okunurluğu ve güvenli varsayılanlar.", () => new PanoV502HardeningSampleForm(), f => SelectScenario(f, "Dark", "Koyu")),
        Item("theme-audit-light", "Theme", "Theme / Accessibility Audit", "Light", "Light tema okunurluğu ve güvenli varsayılanlar.", () => new PanoV502HardeningSampleForm(), f => SelectScenario(f, "Light", "Açık")),
        Item("theme-audit-contrast", "Theme", "Theme / Accessibility Audit", "High Contrast", "Kontrast, button, combo ve bilgi metni okunurluk testi.", () => new PanoV502HardeningSampleForm(), f => SelectScenario(f, "Contrast", "Accessibility", "Okunurluk")),
        Item("foundation-stability", "Foundation", "Pano Foundation", "Stability", "Pano 5.0 foundation, profile ve runtime stability akışı.", () => new PanoV50FoundationSampleForm()),
        Item("hardening-checks", "Foundation", "Build & Runtime Hardening", "Runtime check", "Tema, medya, layout ve interaction güvenli varsayılanları.", () => new PanoV502HardeningSampleForm(), f => SelectScenario(f, "Check", "Hardening")),
        ...
        Item("pro-experience-layout", "Pro Experience", "Pro Experience Suite", "Layout", "Layout, preset ve görünüm hafızası.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Layout")),
        Item("pro-experience-performance", "Pro Experience", "Pro Experience Suite", "Performance", "Performance Pro ve büyük veri hazırlıkları.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Performance", "Virtual")),
        Item("pro-experience-analytics", "Pro Experience", "Pro Experience Suite", "Analytics", "Visual analytics, heatmap, KPI ve mini chart bölümleri.", () => new PanoV37ToV40ProExperienceSampleForm(), f => SelectScenario(f, "Analytics", "Visual")),
    };
    ...
    string? TargetHeading = null)
{
    ...
    public string FileName => string.IsNullOrWhiteSpace(OutputFileName) ? $"pano-{SectionKey}" : OutputFileName;
