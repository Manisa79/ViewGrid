...
public sealed class PanoColumnDesignerEditorSmokeForm : PanoSampleFormBase
{
    ...
    private readonly Label _testSteps = new()
    {
        ...
        Text = "1) Audix Kolonu Ekle veya Designer Property Seti uygula.  2) Büyük 'Kolon Editor Aç' düğmesine bas.  3) Açılan Pano kolon editoründe ekle/sil/değiştir ve Tamam ya da İptal ile dön.  4) Sağdaki raporda propertylerin korunup korunmadığını kontrol et."
    };
    ...
    public PanoColumnDesignerEditorSmokeForm()
        : base(
            "Pano Kolon Designer Editor Smoke Test",
            "Gerçek PanoColumnCollectionEditor penceresini açar. Tamam/İptal, kolon ekle/sil ve property korunmasını burada hızlıca test edebilirsin.")
    {
        Width = 1280;
        Height = 760;
    ...
    {
        Pano.Columns.Clear();
        Pano.CheckBoxes = true;
        Pano.ShowGridLines = true;
        Pano.FullRowSelect = true;
        Pano.ViewMode = PanoViewMode.Details;

        Pano.Columns.Add(new GLVColumn("Person", nameof(DemoRow.Name), 190)
        {
            Name = "glvPerson",
            HeaderCheckBox = true,
        ...
        Pano.Columns.Add(new GLVColumn("Status", nameof(DemoRow.State), 120)
        {
            Name = "glvStatus",
        ...
        Pano.Columns.Add(new GLVColumn("Progress", nameof(DemoRow.Progress), 105)
        {
            Name = "glvProgress",
        ...
        Pano.Columns.Add(new GLVColumn("Notes", nameof(DemoRow.Notes), 300)
        {
            Name = "glvNotes",
            WordWrap = true,
            FillFreeSpace = true,
    ...
    private void OpenColumnEditor()
    {
        ...
        bool changed = InvokePanoColumnEditor();
        Pano.RebuildColumns();
    ...
    private bool InvokePanoColumnEditor()
    {
        var editorType = typeof(PanoControl).Assembly.GetType("Taylan.Pano.Design.PanoColumnCollectionEditor", throwOnError: true);
        ...
        var columnsProperty = TypeDescriptor.GetProperties(Pano)[nameof(Pano.Columns)];
    ...
    {
        var notes = Pano.Columns.ByName("glvNotes") ?? Pano.Columns.ByAspectName(nameof(DemoRow.Notes));
        if (notes != null)
        {
            notes.PrivateColumn = !notes.PrivateColumn;
            notes.Kind = PanoColumnKind.Text;
            notes.WordWrap = true;
        ...
        var status = Pano.Columns.ByName("glvStatus") ?? Pano.Columns.ByAspectName(nameof(DemoRow.State));
        if (status != null)
        {
            status.Kind = PanoColumnKind.Badge;
    ...
    private string CreateSnapshotText()
    {
        var lines = new List<string>
        {
            ...
            string.Empty,
            "Count=" + Pano.Columns.Count.ToString(System.Globalization.CultureInfo.InvariantCulture),
            "Visible=" + string.Join(", ", Pano.Columns.VisibleColumns.Select(c => c.Name)),
            string.Empty
        };

        for (int i = 0; i < Pano.Columns.Count; i++)
        {
            var c = Pano.Columns[i];
            lines.Add($"[{i}] {c.Name}");
    ...
    {
        if (Pano.Columns.ByName(baseName) == null)
            return baseName;

        int i = 2;
        while (Pano.Columns.ByName(baseName + i.ToString(System.Globalization.CultureInfo.InvariantCulture)) != null)
            i++;
