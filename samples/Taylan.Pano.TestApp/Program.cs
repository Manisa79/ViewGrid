...
using PanoToastKind = global::Taylan.Pano.Notifications.PanoToastKind;
using System.Data;
...
{
    public static PanoTheme AppTheme { get; set; } = WindowsThemeService.CurrentTheme();
    ...
    {
        string settingsFile = Path.Combine(Application.StartupPath, "settings", "pano-testapp-language.json");
        ...
        Taylan.Pano.Localization.PanoLocalization.Use(languageForm.SelectedLanguage);
    ...
    private static void InstallGlobalExceptionGuards()
    {
        ...
        {
            Debug.WriteLine("Pano UI exception: " + e.Exception);
        };

        AppDomain.CurrentDomain.UnhandledException += (_, e) =>
        {
            Debug.WriteLine("Pano domain exception: " + e.ExceptionObject);
        };

        TaskScheduler.UnobservedTaskException += (_, e) =>
        {
            Debug.WriteLine("Pano task exception: " + e.Exception);
            e.SetObserved();
        };
    }
}
...
{
    private readonly Taylan.Pano.Core.PanoControl _pano = new()
    {
        Dock = DockStyle.Fill,
    ...
    private PanoTheme _currentTheme = Program.AppTheme;
    private string _themeMode = "Windows";
    private bool _suppressSearchFilter;

    public MainForm()
    {
        Text = $"{PanoVersionInfo.SuggestedProjectName} Test App - {GetAppVersionText()}";
        Width = 1240;
        Height = 800;
        MinimumSize = new Size(980, 620);
        PanoWindowChrome.ApplyOnHandleCreated(this, () => _currentTheme, true);
        ...
        _search.TextChanged += (_,__) =>
        {
            ...
            _pano.SetGlobalFilter(_search.Text);
            if (!string.IsNullOrWhiteSpace(_search.Text)) _pano.JumpToFirstMatch(_search.Text);
        };
        _pano.ButtonClick += (_, e) => MessageBox.Show(this, $"Buton: {((DemoRow)e.RowObject).Name}", "PanoControl");
        _pano.CellValueChanged += (_, e) => _status.Text = $"Güncellendi: {e.Column.Header} = {e.NewValue}";
        _pano.ShowSummaryFooter = true;
        _pano.EnableRowDetails = true;
        _pano.FilterMenuMode = PanoFilterMenuMode.Both;
        ConfigureMillionRowFiltering(_pano);
        _pano.SetRowDetailsProvider(CreateDetailsProvider());
    ...
    internal static void ConfigureMillionRowFiltering(Taylan.Pano.Core.PanoControl grid)
    {
    ...
    private void ShowVersionInfo()
    {
        MessageBox.Show(this,
            $"Proje adı önerisi: {PanoVersionInfo.SuggestedProjectName}\n" +
            $"DLL: {PanoVersionInfo.DisplayText}\n" +
    ...
    private void BuildToolbar()
    {
        ...
        _tool.Items.Add(new ToolStripButton("Header Sağ Tık / Filtre Popup", null, (_,__) => _pano.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        _tool.Items.Add(new ToolStripButton("Ad filtre penceresi", null, (_,__) => _pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        _tool.Items.Add(new ToolStripButton("Kolon Seç", null, (_,__) => _pano.ShowColumnChooser()));

        var samplesMenu = new ToolStripDropDownButton("Pano");
        BuildProductPanoMenu(samplesMenu);
        _tool.Items.Add(samplesMenu);

        var viewMenu = new ToolStripDropDownButton("Görünüm");
        viewMenu.DropDownItems.Add("Kompakt", null, (_,__) => { _pano.SetViewMode(PanoViewMode.ExtraLargeIcons); _status.Text = "Görünüm: Kompakt"; });
        viewMenu.DropDownItems.Add("Standart", null, (_,__) => { _pano.SetViewMode(PanoViewMode.LargeIcons); _status.Text = "Görünüm: Standart"; });
        viewMenu.DropDownItems.Add("Geniş", null, (_,__) => { _pano.SetViewMode(PanoViewMode.MediumIcons); _status.Text = "Görünüm: Geniş"; });
        viewMenu.DropDownItems.Add("Liste", null, (_,__) => { _pano.SetViewMode(PanoViewMode.List); _status.Text = "Görünüm: Liste"; });
        viewMenu.DropDownItems.Add("Kart", null, (_,__) => { _pano.SetViewMode(PanoViewMode.Tile); _status.Text = "Görünüm: Kart"; });
        viewMenu.DropDownItems.Add("Geniş Kart", null, (_,__) => { _pano.SetViewMode(PanoViewMode.LargeCard); _status.Text = "Görünüm: Geniş Kart"; });
        viewMenu.DropDownItems.Add("Dashboard Kart", null, (_,__) => { _pano.SetViewMode(PanoViewMode.DashboardCard); _status.Text = "Görünüm: Dashboard Kart"; });
        viewMenu.DropDownItems.Add("Satır Kart", null, (_,__) => { _pano.SetViewMode(PanoViewMode.RowCard); _status.Text = "Görünüm: Satır Kart"; });
        viewMenu.DropDownItems.Add("DetailCard", null, (_,__) => { _pano.SetViewMode(PanoViewMode.DetailCard); _status.Text = "Görünüm: DetailCard"; });
        viewMenu.DropDownItems.Add("Poster", null, (_,__) => { _pano.TilePosterMode = true; _pano.SetViewMode(PanoViewMode.Poster); _status.Text = "Görünüm: Poster"; });
        viewMenu.DropDownItems.Add("MediaTile", null, (_,__) => { _pano.TilePosterMode = true; _pano.SetViewMode(PanoViewMode.MediaTile); _status.Text = "Görünüm: MediaTile"; });
        viewMenu.DropDownItems.Add("Gallery", null, (_,__) => { _pano.TilePosterMode = true; _pano.SetViewMode(PanoViewMode.Gallery); _status.Text = "Görünüm: Gallery"; });
        viewMenu.DropDownItems.Add("FilmStrip", null, (_,__) => { _pano.TilePosterMode = true; _pano.SetViewMode(PanoViewMode.FilmStrip); _status.Text = "Görünüm: FilmStrip"; });
        viewMenu.DropDownItems.Add("GroupCard", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.GroupCard); _status.Text = "Görünüm: GroupCard"; });
        viewMenu.DropDownItems.Add("PropertyCard", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.PropertyCard); _status.Text = "Görünüm: PropertyCard"; });
        viewMenu.DropDownItems.Add("KPI Dashboard", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.KpiDashboard); _status.Text = "Görünüm: KPI Dashboard"; });
        viewMenu.DropDownItems.Add("HeatMap", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.HeatMap); _status.Text = "Görünüm: HeatMap"; });
        viewMenu.DropDownItems.Add("MiniChart", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.MiniChart); _status.Text = "Görünüm: MiniChart"; });
        viewMenu.DropDownItems.Add("RowPreview", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.RowPreview); _status.Text = "Görünüm: RowPreview"; });
        viewMenu.DropDownItems.Add("MasterDetail", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.MasterDetail); _status.Text = "Görünüm: MasterDetail"; });
        viewMenu.DropDownItems.Add("Kanban", null, (_,__) => { _pano.TilePosterMode = false; _pano.SetViewMode(PanoViewMode.Kanban); _status.Text = "Görünüm: Kanban"; });
        viewMenu.DropDownItems.Add("Zaman Akışı", null, (_,__) => { _pano.SetViewMode(PanoViewMode.Timeline); _status.Text = "Görünüm: Zaman Akışı"; });
        viewMenu.DropDownItems.Add("Detay", null, (_,__) => { _pano.SetViewMode(PanoViewMode.Details); _status.Text = "Görünüm: Detay"; });
        _tool.Items.Add(viewMenu);

        var groupMenu = new ToolStripDropDownButton("Gruplama");
        groupMenu.DropDownItems.Add("Duruma göre grupla", null, (_,__) => { _pano.SetGroupBy(nameof(DemoRow.State)); _status.Text = "Gruplama: Durum"; });
        groupMenu.DropDownItems.Add("Mesleğe göre grupla", null, (_,__) => { _pano.SetGroupBy(nameof(DemoRow.Occupation)); _status.Text = "Gruplama: Meslek"; });
        groupMenu.DropDownItems.Add("Tüm grupları daralt", null, (_,__) => { _pano.CollapseAllGroups(); _status.Text = "Tüm gruplar daraltıldı"; });
        groupMenu.DropDownItems.Add("Tüm grupları aç", null, (_,__) => { _pano.ExpandAllGroups(); _status.Text = "Tüm gruplar açıldı"; });
        groupMenu.DropDownItems.Add("Gruplamayı kaldır", null, (_,__) => { _pano.ClearGrouping(); _status.Text = "Gruplama kapalı"; });
        _tool.Items.Add(groupMenu);

        var filterStyleMenu = new ToolStripDropDownButton("Filtre Stili");
        filterStyleMenu.DropDownItems.Add("Popup menü", null, (_,__) => { _pano.FilterMenuMode = PanoFilterMenuMode.PopupMenu; _status.Text = "Filtre stili: Popup menü"; });
        filterStyleMenu.DropDownItems.Add("Ayrı pencere", null, (_,__) => { _pano.FilterMenuMode = PanoFilterMenuMode.ModalWindow; _status.Text = "Filtre stili: Ayrı pencere"; });
        filterStyleMenu.DropDownItems.Add("İkisi birlikte", null, (_,__) => { _pano.FilterMenuMode = PanoFilterMenuMode.Both; _status.Text = "Filtre stili: İkisi birlikte"; });
        _tool.Items.Add(filterStyleMenu);

        var contextMenuMode = new ToolStripDropDownButton("Başlık Sağ Tık");
        contextMenuMode.DropDownItems.Add("Menü yok", null, (_,__) => { _pano.HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.None; _status.Text = "Başlık sağ tık: menü yok"; });
        contextMenuMode.DropDownItems.Add("Sadece filtre popup", null, (_,__) => { _pano.HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.FilterOnly; _status.Text = "Başlık sağ tık: sadece filtre"; });
        contextMenuMode.DropDownItems.Add("Tam menü", null, (_,__) => { _pano.HeaderContextMenuBehavior = PanoHeaderContextMenuBehavior.Full; _status.Text = "Başlık sağ tık: tam menü"; });
        contextMenuMode.DropDownItems.Add(new ToolStripSeparator());
        contextMenuMode.DropDownItems.Add("Sıralama öğelerini aç/kapat", null, (_,__) => { _pano.ShowHeaderMenuSortItems = !_pano.ShowHeaderMenuSortItems; _status.Text = $"Sıralama menüsü: {_pano.ShowHeaderMenuSortItems}"; });
        contextMenuMode.DropDownItems.Add("Layout öğelerini aç/kapat", null, (_,__) => { _pano.ShowHeaderMenuLayoutItems = !_pano.ShowHeaderMenuLayoutItems; _status.Text = $"Layout menüsü: {_pano.ShowHeaderMenuLayoutItems}"; });
        contextMenuMode.DropDownItems.Add("Gruplama öğelerini aç/kapat", null, (_,__) => { _pano.ShowHeaderMenuGroupingItems = !_pano.ShowHeaderMenuGroupingItems; _status.Text = $"Gruplama menüsü: {_pano.ShowHeaderMenuGroupingItems}"; });
        contextMenuMode.DropDownItems.Add(new ToolStripSeparator());
        contextMenuMode.DropDownItems.Add("Profil: Full", null, (_,__) => { _pano.ApplyMenuProfile(PanoMenuProfile.Full); _status.Text = "Menü profili: Full"; });
        contextMenuMode.DropDownItems.Add("Profil: Standard", null, (_,__) => { _pano.ApplyMenuProfile(PanoMenuProfile.Standard); _status.Text = "Menü profili: Standard"; });
        contextMenuMode.DropDownItems.Add("Profil: Minimal", null, (_,__) => { _pano.ApplyMenuProfile(PanoMenuProfile.Minimal); _status.Text = "Menü profili: Minimal"; });
        contextMenuMode.DropDownItems.Add("Profil: ReadOnly", null, (_,__) => { _pano.ApplyMenuProfile(PanoMenuProfile.ReadOnly); _status.Text = "Menü profili: ReadOnly"; });
        contextMenuMode.DropDownItems.Add("Profil: None", null, (_,__) => { _pano.ApplyMenuProfile(PanoMenuProfile.None); _status.Text = "Menü profili: None / Pano menüleri kapalı"; });
        contextMenuMode.DropDownItems.Add("Boş liste bilgisini aç/kapat", null, (_,__) => { _pano.ShowEmptyListMessage = !_pano.ShowEmptyListMessage; _pano.Invalidate(); _status.Text = $"Boş liste bilgisi: {_pano.ShowEmptyListMessage}"; });
        _tool.Items.Add(contextMenuMode);

        var colorMenu = new ToolStripDropDownButton("Satır Renkleri");
        void AddRowPreset(string text, PanoRowColorPreset preset, double strength = 0.18)
        {
            colorMenu.DropDownItems.Add(text, null, (_,__) =>
            {
                _pano.RowBackColorGetter = null;
                _pano.RowForeColorGetter = null;
                _pano.RowColorAspectName = nameof(DemoRow.State);
                _pano.RowColorStrength = strength;
                _pano.RowColorPreset = preset;
                _pano.CustomGroupBackColor = Color.Empty;
                _pano.Invalidate();
                _status.Text = "Satır renkleri: " + text;
            });
        }
        AddRowPreset("Tema varsayılanı", PanoRowColorPreset.ThemeDefault);
        AddRowPreset("Yumuşak zebra", PanoRowColorPreset.SubtleZebra, 0.12);
        AddRowPreset("Tema vurgu satırları", PanoRowColorPreset.SoftAccent, 0.13);
        AddRowPreset("AOI risk / durum uyumlu", PanoRowColorPreset.AOIRisk, 0.22);
        AddRowPreset("Durum bazlı pastel", PanoRowColorPreset.StatusPills, 0.18);
        AddRowPreset("Severity bantları", PanoRowColorPreset.SeverityBands, 0.24);
        AddRowPreset("Pastel kart görünümü", PanoRowColorPreset.PastelCards, 0.16);
        AddRowPreset("Focus glow", PanoRowColorPreset.FocusGlow, 0.18);
        colorMenu.DropDownItems.Add(new ToolStripSeparator());
        colorMenu.DropDownItems.Add("Özel: Fail yazısı vurgulu", null, (_,__) =>
        {
            _pano.RowColorPreset = PanoRowColorPreset.AOIRisk;
            _pano.RowColorAspectName = nameof(DemoRow.State);
            _pano.RowColorStrength = 0.24;
            _pano.RowForeColorGetter = row => ((DemoRow)row).State == "Fail" ? (_currentTheme.IsDark ? Color.FromArgb(255, 190, 190) : Color.FromArgb(140, 0, 0)) : null;
            _pano.Invalidate();
        ...
        {
            if (!_pano.FindNext(_search.Text))
        ...
        {
            if (!_pano.FindPrevious(_search.Text))
                _status.Text = "Eşleşme bulunamadı";
            else
        ...
        {
            var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Pano.csv");
            try
            {
                var actualPath = _pano.ExportVisibleCsv(path);
        ...
        {
            var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Pano.xlsx");
            try
            {
                var actualPath = _pano.ExportVisibleExcel(path, "PanoControl");
        ...
        _tool.Items.Add(new ToolStripButton("Yazdırma Önizleme", null, (_,__) => _pano.ShowPrintPreview("PanoControl Ana Örnek")));
        _tool.Items.Add(new ToolStripButton("Yazdır", null, (_,__) => _pano.Print("PanoControl Ana Örnek")));
        _tool.Items.Add(new ToolStripButton("Layout Kaydet", null, (_,__) => _pano.SaveLayout(Path.Combine(Application.StartupPath, "pano-layout.json"))));
        _tool.Items.Add(new ToolStripButton("Layout Yükle", null, (_,__) => _pano.LoadLayout(Path.Combine(Application.StartupPath, "pano-layout.json"))));
        ...
        themeMenu.DropDownItems.Add("Açık tema", null, (_,__) => { _themeMode = "Light"; ApplyAppTheme(PanoTheme.LightTheme(), "Açık tema"); });
        themeMenu.DropDownItems.Add("Koyu tema", null, (_,__) => { _themeMode = "Dark"; ApplyAppTheme(PanoTheme.DarkTheme(), "Koyu tema"); });
        themeMenu.DropDownItems.Add("Fluent açık + acrylic", null, (_,__) => { _themeMode = "FluentLight"; ApplyAppTheme(PanoTheme.FluentLightTheme(), "Fluent açık + acrylic"); });
        themeMenu.DropDownItems.Add("Fluent koyu + acrylic", null, (_,__) => { _themeMode = "FluentDark"; ApplyAppTheme(PanoTheme.FluentDarkTheme(), "Fluent koyu + acrylic"); });
        themeMenu.DropDownItems.Add(new ToolStripSeparator());
        themeMenu.DropDownItems.Add("Midnight Purple", null, (_,__) => { _themeMode = "MidnightPurple"; _pano.ThemePreset = PanoThemePreset.MidnightPurple; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.MidnightPurple), "Midnight Purple"); });
        themeMenu.DropDownItems.Add("Slate Blue", null, (_,__) => { _themeMode = "SlateBlue"; _pano.ThemePreset = PanoThemePreset.SlateBlue; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.SlateBlue), "Slate Blue"); });
        themeMenu.DropDownItems.Add("Graphite", null, (_,__) => { _themeMode = "Graphite"; _pano.ThemePreset = PanoThemePreset.Graphite; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.Graphite), "Graphite"); });
        themeMenu.DropDownItems.Add("Emerald", null, (_,__) => { _themeMode = "Emerald"; _pano.ThemePreset = PanoThemePreset.Emerald; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.Emerald), "Emerald"); });
        themeMenu.DropDownItems.Add("Warm Sand", null, (_,__) => { _themeMode = "WarmSand"; _pano.ThemePreset = PanoThemePreset.WarmSand; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.WarmSand), "Warm Sand"); });
        themeMenu.DropDownItems.Add("Ocean", null, (_,__) => { _themeMode = "Ocean"; _pano.ThemePreset = PanoThemePreset.Ocean; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.Ocean), "Ocean"); });
        themeMenu.DropDownItems.Add("Rose Quartz", null, (_,__) => { _themeMode = "RoseQuartz"; _pano.ThemePreset = PanoThemePreset.RoseQuartz; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.RoseQuartz), "Rose Quartz"); });
        themeMenu.DropDownItems.Add("Cyber Neon", null, (_,__) => { _themeMode = "CyberNeon"; _pano.ThemePreset = PanoThemePreset.CyberNeon; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.CyberNeon), "Cyber Neon"); });
        themeMenu.DropDownItems.Add("Nord", null, (_,__) => { _themeMode = "Nord"; _pano.ThemePreset = PanoThemePreset.Nord; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.Nord), "Nord"); });
        themeMenu.DropDownItems.Add("Olive", null, (_,__) => { _themeMode = "Olive"; _pano.ThemePreset = PanoThemePreset.Olive; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.Olive), "Olive"); });
        themeMenu.DropDownItems.Add("Steel", null, (_,__) => { _themeMode = "Steel"; _pano.ThemePreset = PanoThemePreset.Steel; ApplyAppTheme(PanoTheme.FromPreset(PanoThemePreset.Steel), "Steel"); });
        themeMenu.DropDownItems.Add(new ToolStripSeparator());
        ...
        _tool.Items.Add(new ToolStripSeparator());
        _versionLabel.Text = $"DLL: {PanoVersionInfo.InformationalVersion} | App: {GetAppVersionText()}";
    ...
    private void BuildProductPanoMenu(ToolStripDropDownButton root)
    {
        ...
        AddMenuItem(root.DropDownItems, "Audix Medya Pilot", () => new PanoV51RealUsagePilotSampleForm().Show());
        AddMenuItem(root.DropDownItems, "Theme Audit / Okunurluk", () => new PanoV51RealUsagePilotSampleForm().Show());
        AddMenuItem(root.DropDownItems, "Media Playback State", () => new PanoV41MediaPlaybackSampleForm().Show());
        ...
        BuildDeveloperPanoMenu(developer);
    }

    private void BuildDeveloperPanoMenu(ToolStripMenuItem developer)
    {
        ...
        AddMenuItem(latest.DropDownItems, "Pano 5.1 Audix Pilot / Theme Audit", () => new PanoV51RealUsagePilotSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "Pano v50.2 Build & Runtime Hardening", () => new PanoV502HardeningSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "Pano 5.0 Foundation / Stability", () => new PanoV50FoundationSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "V41 Media Playback State", () => new PanoV41MediaPlaybackSampleForm().Show());
        AddMenuItem(latest.DropDownItems, "V37-V40 Pro Experience", () => new PanoV37ToV40ProExperienceSampleForm().Show());
        ...
        AddMenuItem(designer.DropDownItems, "Kolon Designer Editor Smoke Test", () => new PanoColumnDesignerEditorSmokeForm().Show());
        ...
        AddMenuItem(designer.DropDownItems, "Pano Designer Smart Tag", () => new PanoDesignerSmartTagSampleForm().Show());
        AddMenuItem(designer.DropDownItems, "Pano/GLV uyumluluk / SetObjects", () => new PanoCompatSampleForm().Show());
        AddMenuItem(designer.DropDownItems, "Pano kolon özellikleri uyumluluk", () => new PanoColumnCompatibilitySampleForm().Show());
    ...
    private PanoRowDetailsProvider CreateDetailsProvider() => new()
    {
        PreferredHeight = 82,
    ...
    {
        _pano.Columns.Add(new PanoColumn("✓", nameof(DemoRow.Checked), 42)
        {
        ...
        _pano.Columns.Add(new PanoColumn("İşaret", nameof(DemoRow.NeedsReview), 58)
        {
            Kind = PanoColumnKind.CheckBox,
            AspectPutter = (row, value) => ((DemoRow)row).SetNeedsReview(Convert.ToBoolean(value))
        });
        _pano.Columns.Add(new PanoColumn("Id", nameof(DemoRow.Id), 70));
        _pano.Columns.Add(new PanoColumn("İkon", nameof(DemoRow.State), 70)
        {
            Kind = PanoColumnKind.Icon,
            ImageGetter = row => CreateStateIcon(((DemoRow)row).State),
            AspectGetter = row => ((DemoRow)row).State
        });
        _pano.Columns.Add(new PanoColumn("Ad", nameof(DemoRow.Name), 220) { Editable = true, FillFreeSpace = true });
        _pano.Columns.Add(new PanoColumn("Meslek", nameof(DemoRow.Occupation), 145));
        _pano.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 120) { Kind = PanoColumnKind.Badge });
        _pano.Columns.Add(new PanoColumn("İlerleme", nameof(DemoRow.Progress), 150) { Kind = PanoColumnKind.ProgressBar });
        _pano.Columns.Add(new PanoColumn("Puan", nameof(DemoRow.Rating), 120) { Kind = PanoColumnKind.Rating, MaxRating = 5 });
        _pano.Columns.Add(new PanoColumn("İşlem", nameof(DemoRow.ActionText), 100) { Kind = PanoColumnKind.Button });
        _pano.Summaries.Add(new PanoSummaryItem { Column = _pano.Columns[nameof(DemoRow.Id)]!, Type = PanoSummaryType.Count, Format = "Adet: {0}" });
        _pano.Summaries.Add(new PanoSummaryItem { Column = _pano.Columns[nameof(DemoRow.Progress)]!, Type = PanoSummaryType.Average, Format = "Ort: {0:0.0}" });
        _pano.ConditionalFormats.Add(new PanoConditionalFormat { Column = _pano.Columns[nameof(DemoRow.State)], BackColor = Color.FromArgb(60, 220, 60, 60), ForeColor = Color.DarkRed, Predicate = (_,_,v) => string.Equals(Convert.ToString(v), "Fail", StringComparison.OrdinalIgnoreCase) });
    }
    ...
    private void ApplyAppTheme(PanoTheme theme, string caption)
    {
        _currentTheme = EnsureReadable(theme);
        Program.AppTheme = _currentTheme;
        _pano.ApplyTheme(_currentTheme);
        _pano.SetRowDetailsProvider(CreateDetailsProvider());
        ...
        _themeInfo.Text = $"Tema: {caption}  |  DLL={PanoVersionInfo.InformationalVersion}  |  Accent={ColorTranslator.ToHtml(_currentTheme.AccentColor)}  |  Fluent={_currentTheme.UseFluentBackdrop}  |  Acrylic={_currentTheme.UseAcrylicEffect}  |  Animasyonlu seçim aktif";
        ...
        PanoWindowChrome.Apply(this, _currentTheme, true);
        _themePreview.Invalidate(true);
        Invalidate(true);
    }

    private static PanoTheme CreateAoiTheme()
    {
        var t = PanoTheme.FluentDarkTheme();
    ...
    private static PanoTheme CreateHighContrastTheme() => new()
    {
    ...
    private static PanoTheme EnsureReadable(PanoTheme t)
    {
    ...
    private async Task LoadRowsAsync(int rowCount, string caption)
    {
        ...
        try
        {
            var rows = Enumerable.Range(1, rowCount).Select(i => new DemoRow
                {
                    ...
                    NeedsReview = i % 18 == 0,
                    ActionText = "Aç",
                    Tags = i % 3 == 0 ? "AOI,Fail,Review" : i % 2 == 0 ? "Pano,Filter,Tile" : "Fast,Virtual,Theme",
            ...
            _search.Clear();
            if (rowCount <= 10_000)
                _pano.SetObjects(rows);
            else
                await _pano.SetObjectsAsync(_ => Task.FromResult<IEnumerable<DemoRow>>(rows));
            sw.Stop();
    ...
    private void LoadVirtualRows(int rowCount = 150_000_000)
    {
        ...
        _pano.ClearFilters();
        _pano.SetVirtualProvider(new VirtualDemoRowProvider(rowCount));
...
public sealed class DemoRow
{
    ...
    public string Description { get; set; } = "Pano örnek açıklama metni";
    public string Notes { get; set; } = "Pano örnek not metni";
    public DateTime Date { get; set; } = DateTime.Now;
    public int Progress { get; set; }
    public int Rating { get; set; }
    public string ActionText { get; set; } = "Aç";
    public string Tags { get; set; } = "AOI,Pano";
...
public sealed class VirtualDemoRowProvider : IQueryRowProvider, IAsyncPreloadRowProvider, IProviderChangeNotifier, IBulkCheckStateProvider
{
    ...
    private PanoFilterSet _filters = new();
    private PanoColumn[] _columns = Array.Empty<PanoColumn>();
    ...
    public void ApplyView(PanoFilterSet filters, PanoColumn[] columns, PanoColumn? sortColumn, bool sortDescending)
    {
        _filters = filters ?? new PanoFilterSet();
        _columns = columns ?? Array.Empty<PanoColumn>();
    ...
    public bool TryGetDistinctValues(PanoColumn column, PanoFilterSet filters, PanoColumn[] columns, int maxValues, string? searchText, out IReadOnlyList<string> values)
    {
    ...
    private IReadOnlyList<string> BuildVirtualNameDistinctValues(string? searchText, int maxValues)
    {
        ...
        // PanoControl to scan hundreds of millions of rows on the UI thread.
    ...
    private int MapViewIndexToRealIndex(int viewIndex)
    {
        ...
        // Sıralama da provider tarafında: PanoControl dev index listesi oluşturmaz.
    ...
    private static bool TryResolveExactIdentityFilter(PanoColumnFilter f, out int realIndex)
    {
        realIndex = -1;
        ...
        if (f.Mode == PanoFilterMode.ValueList && f.SelectedValues is { Count: 1 })
            return TryParseVirtualName(f.SelectedValues.First(), out realIndex);

        if ((f.Mode == PanoFilterMode.Equals || f.Mode == PanoFilterMode.Contains || f.Mode == PanoFilterMode.StartsWith) &&
    ...
    private DemoRow CreateRowByRealIndex(int realIndex)
    {
        return new DemoRow
        {
            ...
            ActionText = "Git",
            Tags = realIndex % 3 == 0 ? "AOI,Fail,Review" : realIndex % 2 == 0 ? "Pano,Filter,Tile" : "Fast,Virtual,Theme",
    ...
    public bool TrySetAllCheckStates(PanoColumn column, CheckState state)
    {
    ...
    public bool TryGetCheckStateSummary(PanoColumn column, out int checkedCount, out int uncheckedCount)
    {
        checkedCount = 0;
        uncheckedCount = 0;
        if (column == null) return false;
    ...
    private static bool TryGetNameExactIndex(PanoFilterSet filters, out int index)
    {
        index = -1;
...
public sealed class SampleHubForm : Form
{
    ...
    public SampleHubForm()
    {
        ...
        Text = "Pano Example Center Pro - Tek Merkez";
        Width = 1280;
        Height = 760;
        MinimumSize = new Size(980, 620);
        StartPosition = FormStartPosition.CenterScreen;

        PanoWindowChrome.ApplyOnHandleCreated(this, () => _theme, true);
        ApplyThemeToShell();
    ...
    {
        bool tr = PanoLocalization.EffectiveLanguage == PanoLanguage.Turkish;
        Text = tr ? "Pano Örnek Merkezi Pro - Tek Merkez" : "Pano Example Center Pro - Single Hub";
        _header.Text = tr ? "Pano Örnek Merkezi Pro" : "Pano Example Center Pro";
        _info.Text = tr
    ...
    private void BuildNavigation()
    {
        ...
        AddNav("Dil Değiştir", "TestApp açıkken Pano yerleşik menüleri, dialogları ve yeni açılan örnek ekranları için dili değiştirir.", () =>
        {
            if (Program.ShowLanguageSelectionDialog(this))
            {
                ...
                bool tr = PanoLocalization.EffectiveLanguage == PanoLanguage.Turkish;
                MessageBox.Show(this,
                    tr ? "Dil değiştirildi. Yeni açılan Pano örnekleri seçilen dili kullanacak." : "Language changed. Newly opened Pano samples will use the selected language.",
                    tr ? "Dil" : "Language",
        ...
        AddNav("Pano 5.1 Audix Pilot", "Audix gerçek kullanım pilotu: albüm kapağı, play/pause state, video preview, tema audit ve Example Center cleanup akışı.", () => new PanoV51RealUsagePilotSampleForm());
        AddNav("Pano v50.2 Hardening", "Build/API guard, runtime check, tema okunurluğu, Audix medya güvenli ayarları ve tak-çalıştır stabilizasyon ekranı.", () => new PanoV502HardeningSampleForm());
        AddNav("Pano 5.0 Foundation", "Stability, Theme Accessibility, Media Playback, modül profilleri ve hangi uygulamada hangi paket açılmalı sorusunu tek ekranda gösterir.", () => new PanoV50FoundationSampleForm());
        AddNav("V41 Media Playback State", "Audix ve video arşivleri için play/pause, şimdi çalıyor rozeti, equalizer ve video preview davranışını dene.", () => new PanoV41MediaPlaybackSampleForm());
        AddNav("V37-40 Pro Experience", "Enterprise Layout, Performance Pro, Interaction Pro ve Visual Analytics fazlarını tek ekranda dene.", () => new PanoV37ToV40ProExperienceSampleForm());
        ...
        AddNav("V31 Faz Merkezi", "Faz 31-37: Media Experience, Smart Views, Grouping, MasterDetail, Kanban Pro, Designer ve Export tek ekranda.", () => new PanoV31AllPhasesSampleForm());
        AddNav("Audix Albüm Kapakları", "Albüm kapağı, placeholder, FLAC/MP3 rozeti, hover play overlay ve Poster/Gallery/FilmStrip geçişleri.", () => new PanoV31AllPhasesSampleForm());
        ...
        AddNav("Tüm Görünüm Modları", "PanoViewMode listesindeki tüm yeni modları tek ekrandan dene.", () => new ViewModeShowcaseSampleForm());
        ...
        AddNav("PanoControl Mode", "Object / DataTable / Virtual / Tree / Tile modlarını tek ekranda test et", () => new PanoDataModeVerificationForm());
        ...
        AddNav("V31 Faz Merkezi", "Faz 31-37 tamamı: Audix medya, kullanıcı preset, gelişmiş gruplama, master detail, kanban, designer ve export akışı.", () => new PanoV31AllPhasesSampleForm());
        ...
        AddSection("Klasik Pano Örnekleri");
        AddNav("Filtreleme", "Popup filtre, metin/değer listesi, temizleme", () => new FilteringSampleForm());
        AddNav("Gruplama detay", "Pano/GLV tarzı grup davranışları", () => new GroupingSampleForm());
        ...
        AddNav("MasterData görünüm senaryoları", "SAP, BOM, ürün ağacı, program dosyası, makine seçimi ve log ekranları için hazır Pano senaryoları", () => new MasterDataScenarioSampleForm());
        AddNav("Pano v27 Product Core", "State Engine, renderer profilleri, range virtual provider ve MasterData hazır kullanım örneği", () => new PanoV27ProductSampleForm());
        AddNav("Example Center Pro / Kategori Bulucu", "Tüm fazları kategori + arama ile bulabileceğin yeni Pano 5.0 örnek merkezi", () => new ExampleCenterProForm());
        AddNav("v27.3 Renderer Showcase", "Badge, progress, icon+text, tag, button, hyperlink ve status renklerini tek tabloda gösterir", () => new PanoV273RendererShowcaseForm());
        AddNav("v27.3.1 Filter Popup UX", "Uzun SAP malzeme adı ve program yolu filtrelerinde resize, tooltip, auto-width ve boyut hatırlama davranışını gösterir", () => new PanoV2731FilterPopupUxForm());
        AddNav("v27.6 Design-Time Theme Sync", "Visual Studio designer açık tema önizlemesi, runtime temadan bağımsız design-time sync ve menü tema uyumu", () => new PanoV276DesignTimeThemeSampleForm());
        ...
        AddNav("v27.9 Popular Enterprise Features", "Arama paneli, özet/footer, conditional format, column chooser, frozen column, advanced filter ve presetleri tek ekranda toplar", () => new PanoV279PopularEnterpriseFeaturesForm());
        AddNav("v28 UX Polish", "Kart/poster/dashboard görünümlerinde global filtre butonunu üst bara taşır; best-practice UX presetlerini gösterir", () => new PanoV28UxPolishForm());
        AddNav("v28.1 Poster Mode", "PanoViewMode.Poster, otomatik poster ölçüleri, image scaling ve üst filtre UX davranışını gösterir", () => new PanoV28UxPolishForm());
        AddNav("PanoControl Mode", "Object / DataTable / Virtual / Tree / Tile ana çalışma modları", () => new PanoDataModeVerificationForm());
        ...
        AddNav("Kolon Designer Editor Smoke Test", "Son kolon editor düzeltmesini test eder: gerçek PanoColumnCollectionEditor, Tamam/İptal, ekle/sil ve Audix property korunması", () => new PanoColumnDesignerEditorSmokeForm());
        AddNav("Designer önizleme", "Design-time davranış vitrini", () => new DesignerPreviewSampleForm());
        AddNav("Designer + Menü Merge", "User ContextMenuStrip içinde Pano liste özellikleri alt menüsü", () => new DesignerMenuMergeSampleForm());
        ...
        AddNav("Pano/GLV uyumluluk / SetObjects", "SetObjects, AddObject, RemoveObject, seçim ve checked nesne uyumluluğu", () => new PanoCompatSampleForm());
        AddNav("Pano kolon özellikleri uyumluluk", "Pano kolon propertyleri, header checkbox, button sizing ve hyperlink davranışı", () => new PanoColumnCompatibilitySampleForm());
        ...
        AddNav("Pano Designer Smart Tag", "Visual Studio Designer hızlı görev menüsü ve Pano görevleri", () => new PanoDesignerSmartTagSampleForm());
        AddNav("Klavye erişimi / Shortcut testi", "Tüm Pano işlemlerine klavye ile erişim ve düzenleme kısayolları", () => new KeyboardAccessibilitySampleForm());
        AddNav("Menu Visibility Manager", "Örnek menüleri ve Pano yerleşik menülerini göster/gizle", () => new MenuVisibilityManagerSampleForm());
        ...
        AddNav("Master-detail", "Üst kayıt seçilince alt Pano detaylarını yükler", () => new MasterDetailSampleForm());
        ...
        AddNav("TreeView + TreeGrid", "TreeView master/detail ve hiyerarşik Pano", () => new TreeViewSampleForm());
        ...
        AddNav("Toast Notification Demo", "PanoToast info/success/warning/error bildirimi", () => new ToastNotificationSampleForm());
...
public abstract class PanoSampleFormBase : Form
{
    protected readonly Taylan.Pano.Core.PanoControl Pano = new()
    {
        Dock = DockStyle.Fill,
        EmptyListMessage = "Kayıt yok",
        CellEditActivationKey = Keys.F2,
    ...
    protected PanoSampleFormBase(string title, string info)
    {
        Text = title;
        Width = 980;
        Height = 620;
        ...
        PanoWindowChrome.ApplyOnHandleCreated(this, () => sampleTheme, true);
        ...
        Pano.ApplyTheme(sampleTheme);
        MainForm.ConfigureMillionRowFiltering(Pano);
        Info.Text = info;
        Controls.Add(Pano);
        Controls.Add(Info);
        Controls.Add(Tool);
        ...
        Pano.SetObjects(CreateRows(250));
    }

    protected void ApplySampleChrome(PanoTheme theme)
    {
        ...
        Tool.Renderer = new Taylan.Pano.Theming.SmartMenuRenderer(theme);
    ...
    private static void ApplyToolItemTheme(ToolStripItem item, PanoTheme theme)
    {
    ...
    private void AddSampleNavigation()
    {
        ...
        jump.DropDownItems.Add("V31 Faz Merkezi", null, (_,__) => new PanoV31AllPhasesSampleForm().Show());
        ...
        jump.DropDownItems.Add("Pano kolon özellikleri", null, (_,__) => new PanoColumnCompatibilitySampleForm().Show());
        ...
        jump.DropDownItems.Add("Pano Designer Smart Tag", null, (_,__) => new PanoDesignerSmartTagSampleForm().Show());
        ...
        themeMenu.DropDownItems.Add("Açık tema", null, (_,__) => ApplySampleChrome(PanoTheme.LightTheme()));
        themeMenu.DropDownItems.Add("Koyu tema", null, (_,__) => ApplySampleChrome(PanoTheme.DarkTheme()));
        themeMenu.DropDownItems.Add("Fluent açık", null, (_,__) => ApplySampleChrome(PanoTheme.FluentLightTheme()));
        themeMenu.DropDownItems.Add("Fluent koyu", null, (_,__) => ApplySampleChrome(PanoTheme.FluentDarkTheme()));
    ...
    {
        Pano.Columns.Add(new PanoColumn("✓", nameof(DemoRow.Checked), 42) { Kind = PanoColumnKind.CheckBox, AspectPutter = (row, value) => ((DemoRow)row).SetChecked(Convert.ToBoolean(value)) });
        Pano.Columns.Add(new PanoColumn("Id", nameof(DemoRow.Id), 70));
        Pano.Columns.Add(new PanoColumn("Ad", nameof(DemoRow.Name), 240) { Editable = true, FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Meslek", nameof(DemoRow.Occupation), 140));
        Pano.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 110) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("İlerleme", nameof(DemoRow.Progress), 140) { Kind = PanoColumnKind.ProgressBar });
        Pano.Columns.Add(new PanoColumn("Puan", nameof(DemoRow.Rating), 110) { Kind = PanoColumnKind.Rating, MaxRating = 5 });
    }
...
public sealed class MultiColumnCheckBoxSampleForm : PanoSampleFormBase
{
    ...
    {
        Pano.Columns.Clear();
        Pano.FullRowSelect = true;
        Pano.MultiSelect = true;
        Pano.ShowGridLines = true;
        Pano.KeyboardSpaceTogglesCheckBoxes = true;

        Pano.Columns.Add(new PanoColumn("Person", nameof(DemoRow.Name), 190)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Pano.Columns.Add(new PanoColumn("Operasyon", nameof(DemoRow.Occupation), 150)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Pano.Columns.Add(new PanoColumn("Onay", nameof(DemoRow.State), 120)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Pano.Columns.Add(new PanoColumn("Kilit", nameof(DemoRow.Tags), 180)
        {
            CellCheckBox = true,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Pano.Columns.Add(new PanoColumn("Action", nameof(DemoRow.ActionText), 110) { Kind = PanoColumnKind.Button, ButtonText = "Aç" });
        Pano.Columns.Add(new PanoColumn("Link", nameof(DemoRow.Link), 120) { Kind = PanoColumnKind.Hyperlink });
        Pano.Columns.Add(new PanoColumn("Notes", nameof(DemoRow.Tags), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });

        Pano.ButtonClick += (_, e) => Info.Text = $"Action tıklandı: {((DemoRow)e.RowObject).Name}";
        Pano.CellValueChanged += (_, e) =>
        {
        ...
        Pano.HyperlinkClick += (_, e) => Info.Text = $"Link tıklandı: {((DemoRow)e.RowObject).Link}";
        ...
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Pano.ShowColumnChooser()));

        var rows = CreateRows(80);
        for (int i = 0; i < rows.Count; i++)
        {
            rows[i].Name = i < 5 ? new[] { "Wilhelm Frat", "Alana Roderick", "Frank Price", "Eric", "Nicola Scotts" }[i] : "Person " + (i + 1);
            rows[i].Tags = "Pano kolon checkbox uyumluluğu test satırı " + (i + 1);
            rows[i].Approved = i % 3 == 0;
            rows[i].Locked = i % 7 == 0;
        }
        Pano.SetObjects(rows);
...
public sealed class TileCardCheckBoxSampleForm : PanoSampleFormBase
{
    public TileCardCheckBoxSampleForm() : base("Kart / Geniş Kart Checkbox", "Tile, Kart, Geniş Kart ve Poster görünümünde kart üzerinde checkbox overlay desteği; header checkbox ile senkron, designer propertyleri ve konum ayarı test edilir.")
    {
        ...
        Pano.FullRowSelect = true;
        Pano.MultiSelect = true;
        Pano.TileCheckBoxes = true;
        Pano.TileCheckBoxAspectName = nameof(DemoRow.Checked);
        Pano.TileCheckBoxPosition = PanoTileCheckBoxPosition.TopRight;
        Pano.TileCheckBoxSize = 20;
        Pano.TileCheckBoxMargin = 10;
        Pano.TilePreferredWidth = 280;
        Pano.TilePreferredHeight = 112;
        Pano.TileMaxTextLines = 4;
        Pano.LargeCardPreferredWidth = 560;
        Pano.LargeCardPreferredHeight = 176;
        Pano.LargeCardMaxTextLines = 8;
        Pano.TileShowAllVisibleTextColumns = true;
        Pano.KeyboardSpaceTogglesCheckBoxes = true;

        Pano.Columns.Add(new PanoColumn("Seç", nameof(DemoRow.Checked), 70)
        {
            Kind = PanoColumnKind.CheckBox,
            HeaderCheckBox = true,
            HeaderCheckBoxThreeState = true,
        ...
        Pano.Columns.Add(new PanoColumn("Ad", nameof(DemoRow.Name), 220) { Editable = true, FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 110) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Meslek", nameof(DemoRow.Occupation), 150));
        Pano.Columns.Add(new PanoColumn("Not", nameof(DemoRow.Tags), 330) { WordWrap = true, MaxTextLines = 3 });
        Pano.Columns.Add(new PanoColumn("İlerleme", nameof(DemoRow.Progress), 120) { Kind = PanoColumnKind.ProgressBar });

        Tool.Items.Add(new ToolStripButton("Detay", null, (_,__) => SetMode(PanoViewMode.Details)));
        Tool.Items.Add(new ToolStripButton("Kart", null, (_,__) => SetMode(PanoViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Geniş Kart", null, (_,__) => SetMode(PanoViewMode.LargeCard)));
        Tool.Items.Add(new ToolStripButton("Poster", null, (_,__) =>
        {
            Pano.TilePosterMode = !Pano.TilePosterMode;
            Pano.TilePosterImageHeight = 120;
            SetMode(PanoViewMode.Tile);
            Info.Text = "Poster modu = " + Pano.TilePosterMode;
        }));
        Tool.Items.Add(new ToolStripButton("Checkbox Aç/Kapat", null, (_,__) =>
        {
            Pano.TileCheckBoxes = !Pano.TileCheckBoxes;
            Info.Text = "TileCheckBoxes = " + Pano.TileCheckBoxes;
        }));
        Tool.Items.Add(new ToolStripButton("Sol Üst", null, (_,__) => SetCheckBoxPosition(PanoTileCheckBoxPosition.TopLeft)));
        Tool.Items.Add(new ToolStripButton("Sağ Üst", null, (_,__) => SetCheckBoxPosition(PanoTileCheckBoxPosition.TopRight)));
        Tool.Items.Add(new ToolStripButton("Sol Alt", null, (_,__) => SetCheckBoxPosition(PanoTileCheckBoxPosition.BottomLeft)));
        Tool.Items.Add(new ToolStripButton("Sağ Alt", null, (_,__) => SetCheckBoxPosition(PanoTileCheckBoxPosition.BottomRight)));
        ...
        Pano.SetObjects(rows);
        Pano.SetViewMode(PanoViewMode.Tile);
    ...
    {
        Pano.SetViewMode(mode);
        Info.Text = "Görünüm = " + mode + " / TileCheckBoxes = " + Pano.TileCheckBoxes + " / Konum = " + Pano.TileCheckBoxPosition;
    }

    private void SetCheckBoxPosition(PanoTileCheckBoxPosition position)
    {
        Pano.TileCheckBoxPosition = position;
        Pano.Invalidate();
        Info.Text = "TileCheckBoxPosition = " + Pano.TileCheckBoxPosition + " / Margin = " + Pano.TileCheckBoxMargin;
    }

    private void ChangeMargin(int delta)
    {
        Pano.TileCheckBoxMargin = Math.Max(0, Math.Min(48, Pano.TileCheckBoxMargin + delta));
        Pano.Invalidate();
        Info.Text = "TileCheckBoxPosition = " + Pano.TileCheckBoxPosition + " / Margin = " + Pano.TileCheckBoxMargin;
    }
...
public sealed class SampleCenterSearchDemoForm : PanoSampleFormBase
{
...
public sealed class ToastNotificationSampleForm : PanoSampleFormBase
{
    public ToastNotificationSampleForm() : base("Toast Notification Demo", "PanoToast bildirimleri host formun sağ altında tema uyumlu şekilde gösterir.")
    {
        ConfigureCommonColumns();
        Pano.SetObjects(CreateRows(45));
        Tool.Items.Add(new ToolStripButton("Info", null, (_,__) => Pano.ShowToast("Bilgilendirme bildirimi gösterildi.", PanoToastKind.Info)));
        Tool.Items.Add(new ToolStripButton("Success", null, (_,__) => Pano.ShowToast("İşlem başarıyla tamamlandı.", PanoToastKind.Success)));
        Tool.Items.Add(new ToolStripButton("Warning", null, (_,__) => Pano.ShowToast("Kontrol edilmesi gereken kayıtlar var.", PanoToastKind.Warning)));
        Tool.Items.Add(new ToolStripButton("Error", null, (_,__) => Pano.ShowToast("Örnek hata bildirimi.", PanoToastKind.Error)));
    }
}

public sealed class SearchPanelDemoForm : PanoSampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Pano.EnableModernSearchPanel = true;
        Pano.SearchPanelCanFilterResults = false;
        Pano.SetObjects(CreateRows(500));
        Tool.Items.Add(new ToolStripButton("Modern Arama Paneli", null, (_,__) => Pano.ShowModernSearchPanel()));
        Tool.Items.Add(new ToolStripButton("Panel filtre modunu aç/kapat", null, (_,__) =>
        {
            Pano.SearchPanelCanFilterResults = !Pano.SearchPanelCanFilterResults;
            Info.Text = "SearchPanelCanFilterResults = " + Pano.SearchPanelCanFilterResults;
        }));
    }
}

public sealed class AsyncFilterDemoForm : PanoSampleFormBase
{
    public AsyncFilterDemoForm() : base("Async Filter Demo", "Standart listelerde popup filtre değerleri daha hızlı açılsın diye async/full scan ayarları tek örnekte gösterilir.")
    {
        ConfigureCommonColumns();
        MainForm.ConfigureMillionRowFiltering(Pano);
        Pano.SetObjects(CreateRows(15000));
        Tool.Items.Add(new ToolStripButton("Ad filtresini aç", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Durum filtresini aç", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.State))));
...
public sealed class ThemePreviewDemoForm : PanoSampleFormBase
{
    public ThemePreviewDemoForm() : base("Theme Preview Demo", "Light, Dark ve Fluent temalar Pano + host kontrollerinde birlikte test edilir.")
    {
        ConfigureCommonColumns();
        Pano.SetObjects(CreateRows(120));
        Tool.Items.Add(new ToolStripButton("Light", null, (_,__) => ApplySampleChrome(PanoTheme.LightTheme())));
        Tool.Items.Add(new ToolStripButton("Dark", null, (_,__) => ApplySampleChrome(PanoTheme.DarkTheme())));
        Tool.Items.Add(new ToolStripButton("Fluent Light", null, (_,__) => ApplySampleChrome(PanoTheme.FluentLightTheme())));
        Tool.Items.Add(new ToolStripButton("Fluent Dark", null, (_,__) => ApplySampleChrome(PanoTheme.FluentDarkTheme())));
    }
}

public sealed class DialogShowcaseSampleForm : PanoSampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Pano.SetObjects(CreateRows(180));
        Tool.Items.Add(new ToolStripButton("Filtre Penceresi", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Kolon Seçici", null, (_,__) => Pano.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Arama Paneli", null, (_,__) => Pano.ShowModernSearchPanel()));
        Tool.Items.Add(new ToolStripButton("Toast", null, (_,__) => Pano.ShowToast("Dialog standardı örneği", PanoToastKind.Info)));
    }
}

public sealed class WindowChromeDemoForm : PanoSampleFormBase
{
    ...
    private void ShowChromeDialog(bool sizable)
    {
        ...
        {
            Text = sizable ? "Pano sizeable themed dialog" : "Pano fixed themed dialog",
            StartPosition = FormStartPosition.CenterParent,
            Size = new Size(520, 340),
            MinimumSize = new Size(420, 260),
            Font = Font
        };
        PanoDialogChrome.ConfigureStandardDialog(form, Program.AppTheme, new Size(520, 340), sizable);
        ...
        PanoDialogThemeApplier.Apply(form, Program.AppTheme);
        form.ShowDialog(this);
    }
}

public sealed class TileAdvancedDemoForm : PanoSampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Pano.SetObjects(CreateRows(240));
        Pano.SetViewMode(PanoViewMode.Tile);
        Tool.Items.Add(new ToolStripButton("Tile", null, (_,__) => Pano.SetViewMode(PanoViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Details", null, (_,__) => Pano.SetViewMode(PanoViewMode.Details)));
        Tool.Items.Add(new ToolStripButton("Large Icons", null, (_,__) => Pano.SetViewMode(PanoViewMode.LargeIcons)));
    }
}

public sealed class VirtualizationStressDemoForm : PanoSampleFormBase
{
    public VirtualizationStressDemoForm() : base("Virtualization Stress Test", "1M+ satır senaryosunda provider tabanlı liste, hızlı filtre ve scroll davranışı test edilir.")
    {
        ConfigureCommonColumns();
        MainForm.ConfigureMillionRowFiltering(Pano);
        Pano.SetVirtualProvider(new VirtualDemoRowProvider(1_500_000));
        Tool.Items.Add(new ToolStripButton("Ad filtresi", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Durum filtresi", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("999M provider", null, (_,__) => Pano.SetVirtualProvider(new VirtualDemoRowProvider(999_000_000))));
    }
}

public sealed class LayoutProfilesDemoForm : PanoSampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Pano.SetObjects(CreateRows(100));
        Tool.Items.Add(new ToolStripButton("Layout Kaydet", null, (_,__) => Pano.SaveLayout(Path.Combine(Application.StartupPath, "pano-sample-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Layout Yükle", null, (_,__) => Pano.LoadLayout(Path.Combine(Application.StartupPath, "pano-sample-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Kolon Seçici", null, (_,__) => Pano.ShowColumnChooser()));
    }
}

public sealed class KeyboardNavigationDemoForm : PanoSampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Pano.CellEditActivationKey = Keys.F2;
        Pano.AllowEditAllCells = true;
        Pano.SetObjects(CreateRows(70));
        ...
        Tool.Items.Add(new ToolStripButton("Ctrl+F Panel", null, (_,__) => Pano.ShowModernSearchPanel()));
    }
}

public sealed class ExcelFilterDemoForm : PanoSampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Pano.FilterMenuMode = PanoFilterMenuMode.Both;
        Pano.SetObjects(CreateRows(320));
        Tool.Items.Add(new ToolStripButton("Ad filtre", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Meslek filtre", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Occupation))));
        Tool.Items.Add(new ToolStripButton("Temizle", null, (_,__) => Pano.ClearFilters()));
    }
}

public sealed class DesignerPreviewHubDemoForm : PanoSampleFormBase
{
    ...
    {
        ConfigureCommonColumns();
        Pano.DesignTimeSampleData = false;
        Pano.SetObjects(CreateRows(36));
        Tool.Items.Add(new ToolStripButton("Kolon düzenleyici", null, (_,__) => Pano.ShowColumnChooser()));
...
public sealed class ProgressBarSampleForm : PanoSampleFormBase
{
    public ProgressBarSampleForm() : base("Pano Modern ProgressBar Örneği", "ProgressBar animasyonu varsayılan kapalıdır. Görünüm tema, satır rengi ve accent renkle uyumludur; istenirse kod/designer ile açılır.")
    {
        Pano.ProgressBarAnimated = false;
        Pano.ProgressBarUseGradient = true;
        Pano.ProgressBarShowText = true;
        Pano.ProgressBarUseAccentColor = true;
        Pano.RowColorPreset = PanoRowColorPreset.StatusPills;
        Pano.RowColorAspectName = nameof(DemoRow.State);
        Pano.RowColorStrength = 0.16;
        Tool.Items.Add(new ToolStripButton("Animasyon Aç/Kapat", null, (_,__) => { Pano.ProgressBarAnimated = !Pano.ProgressBarAnimated; Pano.Invalidate(); Info.Text = "ProgressBarAnimated = " + Pano.ProgressBarAnimated + "  (büyük listelerde kapalı önerilir)"; }));
        Tool.Items.Add(new ToolStripButton("Accent renk", null, (_,__) => { Pano.ProgressBarUseAccentColor = true; Pano.Invalidate(); }));
        Tool.Items.Add(new ToolStripButton("Düşük/orta/yüksek renk", null, (_,__) => { Pano.ProgressBarUseAccentColor = false; Pano.Invalidate(); }));
        Tool.Items.Add(new ToolStripButton("Gradient Aç/Kapat", null, (_,__) => { Pano.ProgressBarUseGradient = !Pano.ProgressBarUseGradient; Pano.Invalidate(); }));
    }
}

public sealed class PrintPreviewSampleForm : PanoSampleFormBase
{
    public PrintPreviewSampleForm() : base("Pano Yazdırma / Önizleme Örneği", "Pano/GLV tarzı önizleme desteği: tüm görünür satırlar, seçili satırlar, PageSetup, PrintPreview ve Print.")
    {
        Pano.PrintTitle = "PanoControl Yazdırma Önizleme Örneği";
        Pano.PrintMaxRows = 5000;
        Tool.Items.Add(new ToolStripButton("Önizleme", null, (_,__) => Pano.ShowPrintPreview("PanoControl Yazdırma Önizleme Örneği")));
        Tool.Items.Add(new ToolStripButton("Yazdır", null, (_,__) => Pano.Print("PanoControl Yazdırma Önizleme Örneği")));
        Tool.Items.Add(new ToolStripButton("Sayfa Ayarı", null, (_,__) => Pano.ShowPageSetup("PanoControl Yazdırma Önizleme Örneği")));
        Tool.Items.Add(new ToolStripButton("Sadece Seçili Satırlar", null, (_,__) => { Pano.PrintSelectedRowsOnly = !Pano.PrintSelectedRowsOnly; Info.Text = "PrintSelectedRowsOnly = " + Pano.PrintSelectedRowsOnly + " | Çoklu seçim yapıp önizleme açabilirsin."; }));
        Tool.Items.Add(new ToolStripButton("Tema Rengi Aç/Kapat", null, (_,__) => { Pano.PrintUseThemeColors = !Pano.PrintUseThemeColors; Info.Text = "PrintUseThemeColors = " + Pano.PrintUseThemeColors + " | Profesyonel çıktı için kapalı önerilir."; }));
        Tool.Items.Add(new ToolStripButton("Fit to page width", null, (_,__) => { Pano.PrintFitToPageWidth = !Pano.PrintFitToPageWidth; Info.Text = "PrintFitToPageWidth = " + Pano.PrintFitToPageWidth; }));
        Tool.Items.Add(new ToolStripButton("Grid Aç/Kapat", null, (_,__) => { Pano.PrintShowGrid = !Pano.PrintShowGrid; Info.Text = "PrintShowGrid = " + Pano.PrintShowGrid; }));
        Tool.Items.Add(new ToolStripButton("Zebra Aç/Kapat", null, (_,__) => { Pano.PrintZebraRows = !Pano.PrintZebraRows; Info.Text = "PrintZebraRows = " + Pano.PrintZebraRows; }));
    }
}

public sealed class FilteringSampleForm : PanoSampleFormBase
{
    public FilteringSampleForm() : base("Pano Filtreleme Detay Örneği", "Popup filtre, ayrı filtre penceresi, hızlı metin filtreleme ve tüm filtreleri temizleme örneği.")
    {
        Pano.FilterMenuMode = PanoFilterMenuMode.Both;
        var search = new ToolStripTextBox { Width = 180 };
        search.TextChanged += (_,__) => Pano.SetGlobalFilter(search.Text ?? "");
        Tool.Items.Add(new ToolStripLabel("Ara:"));
        Tool.Items.Add(search);
        Tool.Items.Add(new ToolStripButton("Ad popup filtre", null, (_,__) => Pano.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Popup stil", null, (_,__) => Pano.FilterMenuMode = PanoFilterMenuMode.PopupMenu));
        Tool.Items.Add(new ToolStripButton("İkisi birlikte", null, (_,__) => Pano.FilterMenuMode = PanoFilterMenuMode.Both));
        Tool.Items.Add(new ToolStripButton("Ad filtre penceresi", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Filtreleri temizle", null, (_,__) => { search.Text = ""; Pano.ClearFilters(); }));
    }
}

public sealed class GroupingSampleForm : PanoSampleFormBase
{
    public GroupingSampleForm() : base("Pano Gruplama Detay Örneği", "Pano/GLV tarzı gruplama: başlığa tıkla aç/kapat, tüm grupları toplu daralt/aç.")
    {
        Tool.Items.Add(new ToolStripButton("Duruma göre grupla", null, (_,__) => Pano.SetGroupBy(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("Mesleğe göre grupla", null, (_,__) => Pano.SetGroupBy(nameof(DemoRow.Occupation))));
        Tool.Items.Add(new ToolStripButton("Tüm grupları daralt", null, (_,__) => Pano.CollapseAllGroups()));
        Tool.Items.Add(new ToolStripButton("Tüm grupları aç", null, (_,__) => Pano.ExpandAllGroups()));
        Tool.Items.Add(new ToolStripButton("Gruplamayı kaldır", null, (_,__) => Pano.ClearGrouping()));
        Tool.Items.Add(new ToolStripButton("Grup başlığı rengi", null, (_,__) => { Pano.CustomGroupBackColor = Color.FromArgb(235, 229, 250); Pano.Invalidate(); }));
        Pano.SetGroupBy(nameof(DemoRow.State));
    }
}

public sealed class ViewModesSampleForm : PanoSampleFormBase
{
    public ViewModesSampleForm() : base("Pano Görünüm Modları Detay Örneği", "Kompakt, Standart, Geniş, Liste, Kart, Geniş Kart ve Detay görünümleri aynı veriyle denenebilir.")
    {
        AddMode("Kompakt", PanoViewMode.ExtraLargeIcons);
        AddMode("Standart", PanoViewMode.LargeIcons);
        AddMode("Geniş", PanoViewMode.MediumIcons);
        AddMode("Liste", PanoViewMode.List);
        AddMode("Kart", PanoViewMode.Tile);
        AddMode("Geniş Kart", PanoViewMode.LargeCard);
        AddMode("Detay", PanoViewMode.Details);
    }
    private void AddMode(string text, PanoViewMode mode) => Tool.Items.Add(new ToolStripButton(text, null, (_,__) => Pano.SetViewMode(mode)));
}

public sealed class TileSampleForm : PanoSampleFormBase
{
    public TileSampleForm() : base("Pano Kart Detay Örneği", "Pano tile görünümüne benzer kart dizilim örneği. Kartlarda ad, meslek, tarih/durum gibi alanlar kompakt gösterilir.")
    {
        Pano.TilePreferredHeight = 96;
        Pano.TileMaxTextLines = 4;
        Pano.SetViewMode(PanoViewMode.Tile);
        Tool.Items.Add(new ToolStripButton("Kart görünümü", null, (_,__) => Pano.SetViewMode(PanoViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Detay görünümü", null, (_,__) => Pano.SetViewMode(PanoViewMode.Details)));
        Tool.Items.Add(new ToolStripButton("Mesleğe göre grupla", null, (_,__) => Pano.SetGroupBy(nameof(DemoRow.Occupation))));
        Tool.Items.Add(new ToolStripButton("Kart yüksekliği +", null, (_,__) => { Pano.TilePreferredHeight += 12; Pano.SetViewMode(PanoViewMode.Tile); }));
        Tool.Items.Add(new ToolStripButton("Kart yüksekliği -", null, (_,__) => { Pano.TilePreferredHeight = Math.Max(72, Pano.TilePreferredHeight - 12); Pano.SetViewMode(PanoViewMode.Tile); }));
        Tool.Items.Add(new ToolStripButton("Satır +", null, (_,__) => { Pano.TileMaxTextLines = Math.Min(12, Pano.TileMaxTextLines + 1); Pano.Invalidate(); }));
        Tool.Items.Add(new ToolStripButton("Satır -", null, (_,__) => { Pano.TileMaxTextLines = Math.Max(1, Pano.TileMaxTextLines - 1); Pano.Invalidate(); }));
    }
}

public sealed class PosterGallerySampleForm : PanoSampleFormBase
{
    ...
    public PosterGallerySampleForm() : base("Pano Poster / Resimli İçerik Örneği", "Plex/Netflix tarzı katalog: büyük görsel, başlık, yıl, tür, rating ve responsive poster kartları.")
    {
        _posters = CreatePosterImages();

        Pano.Columns.Add(new PanoColumn("Poster", nameof(DemoRow.PosterIndex), 92)
        {
            Kind = PanoColumnKind.Image,
            ImageGetter = row => _posters[((DemoRow)row).PosterIndex % _posters.Length]
        });
        Pano.Columns.Add(new PanoColumn("Başlık", nameof(DemoRow.Name), 230) { FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Tür", nameof(DemoRow.Occupation), 115) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Yıl", nameof(DemoRow.Year), 80));
        Pano.Columns.Add(new PanoColumn("Rating", nameof(DemoRow.Rating), 100) { Kind = PanoColumnKind.Rating });
        Pano.Columns.Add(new PanoColumn("İzleme", nameof(DemoRow.Progress), 110) { Kind = PanoColumnKind.ProgressBar });
        Pano.SetViewMode(PanoViewMode.Poster);
        Pano.TilePosterMode = true;
        Pano.TilePreferredWidth = 214;
        Pano.TilePreferredHeight = 292;
        Pano.TilePosterImageHeight = 168;
        Pano.EnableModernEmptyState = true;
        Pano.EmptyListMessage = "Bu filtreye uygun poster bulunamadı";
        Pano.AutoSizeTileWidthToContent = false;
        Pano.MediaImageScaleMode = PanoMediaImageScaleMode.Cover;

        _rows = CreatePosterRows();
        Pano.SetObjects(_rows);
        ...
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(new ToolStripButton("Poster", null, (_,__) => { Pano.TilePosterMode = true; Pano.SetViewMode(PanoViewMode.Poster); }));
        Tool.Items.Add(new ToolStripButton("MediaTile", null, (_,__) => { Pano.TilePosterMode = true; Pano.SetViewMode(PanoViewMode.MediaTile); }));
        Tool.Items.Add(new ToolStripButton("FilmStrip", null, (_,__) => { Pano.TilePosterMode = true; Pano.SetViewMode(PanoViewMode.FilmStrip); }));
        Tool.Items.Add(new ToolStripButton("Detay", null, (_,__) => { Pano.TilePosterMode = false; Pano.SetViewMode(PanoViewMode.Details); }));
        Tool.Items.Add(new ToolStripButton("Görsel +", null, (_,__) => { Pano.TilePosterImageHeight += 12; Pano.TilePreferredHeight += 12; Pano.RefreshView(); }));
        Tool.Items.Add(new ToolStripButton("Görsel -", null, (_,__) => { Pano.TilePosterImageHeight = Math.Max(90, Pano.TilePosterImageHeight - 12); Pano.TilePreferredHeight = Math.Max(190, Pano.TilePreferredHeight - 12); Pano.RefreshView(); }));
        Tool.Items.Add(new ToolStripButton("Yıla göre grupla", null, (_,__) => Pano.SetGroupBy(nameof(DemoRow.Year))));
        Tool.Items.Add(new ToolStripButton("Grupları temizle", null, (_,__) => Pano.ClearGrouping()));
        Tool.Items.Add(new ToolStripButton("Filtre menüsü", null, (_,__) => Pano.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripSeparator());
    ...
    private static Bitmap CreatePosterBitmap(Color baseColor, int number)
    {
        ...
        TextRenderer.DrawText(g, "PANO POSTER", small, new Rectangle(0, 248, bmp.Width, 26), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
...
public sealed class MediaLibrarySampleForm : PanoSampleFormBase
{
    ...
    public MediaLibrarySampleForm() : base("Pano Media Library Showcase", "Audix, film arşivi, fotoğraf galerisi, makine kataloğu ve AOI hata görseli gibi senaryolarda aynı Pano verisini Poster / MediaTile / FilmStrip / DetailCard olarak gösterir.")
    {
        _covers = CreateMediaImages();
        _items = CreateMediaItems();

        Pano.Columns.Add(new PanoColumn("Kapak", nameof(MediaItem.ImageIndex), 120)
        {
            Kind = PanoColumnKind.Image,
            ImageGetter = row => _covers[((MediaItem)row).ImageIndex % _covers.Length]
        });
        Pano.Columns.Add(new PanoColumn("Başlık", nameof(MediaItem.Title), 240) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        Pano.Columns.Add(new PanoColumn("Alt Bilgi", nameof(MediaItem.Subtitle), 210) { FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Tür", nameof(MediaItem.MediaType), 95) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Albüm / Koleksiyon", nameof(MediaItem.Collection), 170));
        Pano.Columns.Add(new PanoColumn("Süre", nameof(MediaItem.Duration), 70));
        Pano.Columns.Add(new PanoColumn("Durum", nameof(MediaItem.State), 90) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Açıklama", nameof(MediaItem.Description), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });

        Pano.SetObjects(_items);
        Pano.TilePosterMode = true;
        Pano.TilePreferredWidth = 218;
        Pano.TilePreferredHeight = 292;
        Pano.TilePosterImageHeight = 166;
        Pano.TileMaxTextLines = 5;
        Pano.PosterPreferredWidth = 220;
        Pano.PosterPreferredHeight = 306;
        Pano.PosterImageHeight = 182;
        Pano.MediaImageScaleMode = PanoMediaImageScaleMode.Cover;
        Pano.MediaImageRoundedCorners = true;
        Pano.EnableModernEmptyState = true;
        Pano.EmptyListMessage = "Bu filtreye uygun medya kaydı yok";
        Pano.SetViewMode(PanoViewMode.Poster);
        ...
        _scale.Items.Add(PanoMediaImageScaleMode.Cover);
        _scale.Items.Add(PanoMediaImageScaleMode.Contain);
        _scale.Items.Add(PanoMediaImageScaleMode.Stretch);
        _scale.SelectedItem = PanoMediaImageScaleMode.Cover;

        _search.TextChanged += (_, __) => ApplyFilter();
        _type.SelectedIndexChanged += (_, __) => ApplyFilter();
        _scale.SelectedIndexChanged += (_, __) =>
        {
            if (_scale.SelectedItem is PanoMediaImageScaleMode mode)
            {
                Pano.MediaImageScaleMode = mode;
                Pano.RefreshView();
            }
        };
        ...
        Tool.Items.Add(new ToolStripButton("KPI", null, (_, __) => { Pano.TilePosterMode = false; Pano.TilePreferredWidth = 240; Pano.TilePreferredHeight = 140; Pano.SetViewMode(PanoViewMode.KpiDashboard); }));
        Tool.Items.Add(new ToolStripButton("HeatMap", null, (_, __) => { Pano.TilePosterMode = false; Pano.TilePreferredWidth = 190; Pano.TilePreferredHeight = 126; Pano.SetViewMode(PanoViewMode.HeatMap); }));
        Tool.Items.Add(new ToolStripButton("MiniChart", null, (_, __) => { Pano.TilePosterMode = false; Pano.TilePreferredWidth = 320; Pano.TilePreferredHeight = 118; Pano.SetViewMode(PanoViewMode.MiniChart); }));
        Tool.Items.Add(new ToolStripButton("Property", null, (_, __) => { Pano.TilePosterMode = false; Pano.SetViewMode(PanoViewMode.PropertyCard); }));
        Tool.Items.Add(new ToolStripButton("DetailCard", null, (_, __) => { Pano.TilePosterMode = false; Pano.SetViewMode(PanoViewMode.DetailCard); }));
        Tool.Items.Add(new ToolStripButton("Detay Liste", null, (_, __) => { Pano.TilePosterMode = false; Pano.SetViewMode(PanoViewMode.Details); }));
        Tool.Items.Add(new ToolStripButton("Koleksiyona göre grupla", null, (_, __) => Pano.SetGroupBy(nameof(MediaItem.MediaType))));
        Tool.Items.Add(new ToolStripButton("Grupları temizle", null, (_, __) => Pano.ClearGrouping()));
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(_status);
    ...
    {
        Pano.TilePosterMode = true;
        Pano.TilePreferredWidth = 210;
        Pano.TilePreferredHeight = 250;
        Pano.TilePosterImageHeight = 148;
        Pano.TileMaxTextLines = 5;
    ...
    {
        Pano.TilePosterMode = true;
        Pano.TilePreferredWidth = 190;
        Pano.TilePreferredHeight = 236;
        Pano.TilePosterImageHeight = 128;
        Pano.TileMaxTextLines = 4;
    ...
    {
        Pano.TilePosterMode = true;
        Pano.TilePreferredWidth = 920;
        Pano.TilePreferredHeight = 164;
        Pano.TilePosterImageHeight = 116;
        Pano.TileMaxTextLines = 7;
    ...
    private static Bitmap CreateMediaBitmap(Color baseColor, string label, int seed)
    {
        ...
        TextRenderer.DrawText(g, "PANO MEDIA", small, new Rectangle(0, 392, bmp.Width, 30), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
...
public sealed class PanoV31AllPhasesSampleForm : PanoSampleFormBase
{
    ...
    public PanoV31AllPhasesSampleForm() : base("Pano v31 Faz Merkezi", "Faz 31-37 tek ekranda: Audix medya deneyimi, smart view presetleri, gelişmiş grouping, master-detail, kanban, designer ve export akışı.")
    {
        _images = CreatePhaseImages();
        _items = CreateFeatureItems();

        Pano.Columns.Clear();
        Pano.Columns.Add(new PanoColumn("Görsel", nameof(V31FeatureItem.ImageIndex), 96)
        {
            Kind = PanoColumnKind.Image,
            ImageGetter = row => _images[((V31FeatureItem)row).ImageIndex % _images.Length]
        });
        Pano.Columns.Add(new PanoColumn("Özellik", nameof(V31FeatureItem.Title), 220) { Editable = true, FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        Pano.Columns.Add(new PanoColumn("Faz", nameof(V31FeatureItem.Phase), 90) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Proje", nameof(V31FeatureItem.Project), 120) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Pano Modu", nameof(V31FeatureItem.ViewMode), 140));
        Pano.Columns.Add(new PanoColumn("Kullanım", nameof(V31FeatureItem.Usage), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
        Pano.Columns.Add(new PanoColumn("Rozet", nameof(V31FeatureItem.Badge), 85) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("Konum", nameof(V31FeatureItem.WhereToFind), 260) { WordWrap = true, MaxTextLines = 3 });

        Pano.SetObjects(_items);
        Pano.TilePosterMode = true;
        Pano.TilePreferredWidth = 245;
        Pano.TilePreferredHeight = 314;
        Pano.TilePosterImageHeight = 150;
        Pano.TileMaxTextLines = 6;
        Pano.MediaImageScaleMode = PanoMediaImageScaleMode.Cover;
        Pano.MediaImageRoundedCorners = true;
        Pano.MediaPlaceholderImage = _images[0];
        Pano.ShowMediaOverlayButton = true;
        Pano.MediaOverlayButtonText = "▶";
        Pano.MediaQualityBadgeAspectName = nameof(V31FeatureItem.Badge);
        Pano.MediaQualityBadgeGetter = row => ((V31FeatureItem)row).Badge;
        Pano.EnableModernEmptyState = true;
        Pano.EmptyListMessage = "Bu arama/filtre için özellik bulunamadı";
        Pano.SetViewMode(PanoViewMode.Poster);
        ...
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(new ToolStripButton("Audix Poster", null, (_, __) => ApplyMedia(PanoViewMode.Poster, 245, 314, 150)));
        Tool.Items.Add(new ToolStripButton("Gallery", null, (_, __) => ApplyMedia(PanoViewMode.Gallery, 218, 265, 132)));
        Tool.Items.Add(new ToolStripButton("MediaTile", null, (_, __) => ApplyMedia(PanoViewMode.MediaTile, 198, 238, 112)));
        Tool.Items.Add(new ToolStripButton("FilmStrip", null, (_, __) => ApplyMedia(PanoViewMode.FilmStrip, 900, 168, 110)));
        Tool.Items.Add(new ToolStripButton("Feature Finder", null, (_, __) => { Pano.TilePosterMode = false; Pano.SetViewMode(PanoViewMode.RowPreview); }));
        Tool.Items.Add(new ToolStripButton("Kanban Pro", null, (_, __) => { Pano.TilePosterMode = false; Pano.TilePreferredWidth = 330; Pano.TilePreferredHeight = 188; Pano.SetViewMode(PanoViewMode.Kanban); Pano.SetGroupBy(nameof(V31FeatureItem.Project)); }));
        Tool.Items.Add(new ToolStripButton("MasterDetail", null, (_, __) => { Pano.TilePosterMode = false; Pano.SetViewMode(PanoViewMode.MasterDetail); }));
        Tool.Items.Add(new ToolStripButton("KPI", null, (_, __) => { Pano.TilePosterMode = false; Pano.TilePreferredWidth = 240; Pano.TilePreferredHeight = 140; Pano.SetViewMode(PanoViewMode.KpiDashboard); }));
        Tool.Items.Add(new ToolStripButton("Export/Print", null, (_, __) => { Pano.TilePosterMode = false; Pano.SetViewMode(PanoViewMode.DetailCard); }));
        Tool.Items.Add(new ToolStripButton("Grupları temizle", null, (_, __) => Pano.ClearGrouping()));
        Tool.Items.Add(new ToolStripSeparator());
        Tool.Items.Add(_status);
        UpdateStatus(_items.Count);
    ...
    private void ApplyMedia(PanoViewMode mode, int width, int height, int imageHeight)
    {
        Pano.ClearGrouping();
        Pano.TilePosterMode = true;
        Pano.TilePreferredWidth = width;
        Pano.TilePreferredHeight = height;
        Pano.TilePosterImageHeight = imageHeight;
        Pano.PosterPreferredWidth = width;
        Pano.PosterPreferredHeight = height;
        Pano.PosterImageHeight = imageHeight;
        Pano.SetViewMode(mode);
    }
    ...
    private static List<V31FeatureItem> CreateFeatureItems()
    {
        ...
        {
            new("31", "Audix", "Albüm kapağı lazy-load + disk cache", "Poster / Gallery / MediaTile", "ImageGetter içinde dosyadan kapak oku, disk cache ile sakla, Pano MediaPlaceholderImage ile eksik kapakları düzgün göster.", "Example Center > V31 Faz Merkezi / Media Library", "CACHE", 0),
            ...
            new("31", "Audix", "FLAC / MP3 / 320kbps rozeti", "Poster / Gallery", "MediaQualityBadgeAspectName veya MediaQualityBadgeGetter ile kalite rozeti kapak üzerinde gösterilir.", "Pano - Media Experience property grubu", "FLAC", 2),
            new("31", "Genel", "Eksik görsel placeholder", "Poster / MediaTile", "MediaPlaceholderImage ile albüm kapağı, öğrenci fotoğrafı, makine görseli veya AOI karesi yoksa boş alan yerine kurumsal placeholder gösterilir.", "Pano - Media Experience > MediaPlaceholderImage", "SAFE", 3),
            ...
            new("32", "Genel", "Layout export/import", "Details", "Pano layout profilini dışarı aktarma ve başka kullanıcıya/PC’ye taşıma senaryosu.", "Example Center > Layout Profiles Demo", "JSON", 6),
            ...
            new("35", "LineWorkspace", "Duruma göre kart taşıma hazırlığı", "Kanban", "Sürükle-bırak iş akışına hazır görünüm; şimdilik grup + kart mantığıyla pratik takip sağlar.", "PanoViewMode.Kanban", "FLOW", 12),

            new("36", "Genel", "Designer friendly property grupları", "Tüm modlar", "Medya ayarları Pano - Media Experience kategorisinde görünür; WinForms Designer’da ayarlanabilir property yaklaşımı korundu.", "PropertyGrid > Pano - Media Experience", "DESIGN", 13),
    ...
    private static Bitmap CreatePhaseBitmap(Color baseColor, string label, int seed)
    {
        ...
        using var small = new Font("Segoe UI", 11, FontStyle.Bold);
        TextRenderer.DrawText(g, "PANO V31", small, new Rectangle(0, 392, bmp.Width, 28), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
...
public sealed class HighlightSearchSampleForm : PanoSampleFormBase
{
    private readonly ToolStripTextBox _search = new() { Width = 180 };
    public HighlightSearchSampleForm() : base("Pano Highlight Search Detay Örneği", "Arama metni satır içinde sarı highlight ile gösterilir, sonraki/önceki eşleşmeye atlanabilir.")
    {
        _search.Text = "kayıt 25";
        _search.TextChanged += (_,__) => { Pano.SetGlobalFilter(_search.Text ?? ""); Pano.JumpToFirstMatch(_search.Text ?? ""); };
        Tool.Items.Add(new ToolStripLabel("Highlight ara:"));
        Tool.Items.Add(_search);
        Tool.Items.Add(new ToolStripButton("Sonraki", null, (_,__) => Pano.FindNext(_search.Text ?? "")));
        Tool.Items.Add(new ToolStripButton("Önceki", null, (_,__) => Pano.FindPrevious(_search.Text ?? "")));
        Tool.Items.Add(new ToolStripButton("Temizle", null, (_,__) => { _search.Text = ""; Pano.ClearFilters(); }));
        Shown += (_,__) => { Pano.SetGlobalFilter(_search.Text ?? ""); Pano.JumpToFirstMatch(_search.Text ?? ""); };
    }
}

public sealed class ColoringSampleForm : PanoSampleFormBase
{
    ...
    public ColoringSampleForm() : base("Pano Satır Renklendirme Detay Örneği", "Satır renkleri seçilen temaya göre otomatik uyarlanır. Fail/Review/OK gibi durumlar açık ve koyu temada okunabilir kalır.")
    {
        foreach (PanoThemePreset preset in Enum.GetValues(typeof(PanoThemePreset)))
            _themeBox.Items.Add(preset);
        foreach (PanoRowColorPreset preset in Enum.GetValues(typeof(PanoRowColorPreset)))
            _rowColorBox.Items.Add(preset);

        _themeBox.SelectedItem = PanoThemePreset.MidnightPurple;
        _rowColorBox.SelectedItem = PanoRowColorPreset.AOIRisk;
        ...
        {
            Pano.RowForeColorGetter = row => ((DemoRow)row).State == "Fail" ? (PanoTheme.FromPreset((PanoThemePreset)_themeBox.SelectedItem!).IsDark ? Color.FromArgb(255, 190, 190) : Color.FromArgb(140, 0, 0)) : null;
            Pano.Invalidate();
        }));
        Tool.Items.Add(new ToolStripButton("Koşullu format ekle", null, (_,__) =>
        {
            Pano.ConditionalFormats.Clear();
            Pano.ConditionalFormats.Add(new PanoConditionalFormat { Column = Pano.Columns[nameof(DemoRow.State)], BackColor = Color.FromArgb(80, 220, 60, 60), ForeColor = Color.DarkRed, Predicate = (_,_,v) => Convert.ToString(v) == "Fail" });
            Pano.Invalidate();
        }));
        Tool.Items.Add(new ToolStripButton("Sıfırla", null, (_,__) =>
        {
            Pano.RowBackColorGetter = null;
            Pano.RowForeColorGetter = null;
            Pano.ConditionalFormats.Clear();
            _rowColorBox.SelectedItem = PanoRowColorPreset.ThemeDefault;
            ApplyPreview();
        }));
    ...
    {
        var themePreset = _themeBox.SelectedItem is PanoThemePreset tp ? tp : PanoThemePreset.System;
        var rowPreset = _rowColorBox.SelectedItem is PanoRowColorPreset rp ? rp : PanoRowColorPreset.ThemeDefault;
        Pano.ThemePreset = themePreset;
        Pano.RowColorPreset = rowPreset;
        Pano.RowColorAspectName = nameof(DemoRow.State);
        Pano.RowColorStrength = rowPreset == PanoRowColorPreset.AOIRisk || rowPreset == PanoRowColorPreset.SeverityBands ? 0.23 : 0.16;
        Pano.RowBackColorGetter = null;
        Pano.RowForeColorGetter = null;
        Pano.Invalidate();
        Info.Text = $"Tema: {themePreset} | Satır renkleri: {rowPreset} | Aspect: {Pano.RowColorAspectName}";
    }
}

public sealed class ImageComboSampleForm : PanoSampleFormBase
{
    private static readonly string[] StatusValues = { "OK", "Review", "Fail" };

    public ImageComboSampleForm() : base("Pano Image + Combo Hücre Örneği", "İkon/görsel kolon + ikonlu ComboBox hücre düzenleme. Combo hücreye tek tıklayınca liste direkt açılır; F2 de desteklenir.")
    {
        Pano.Columns.Clear();
        Pano.RowHeight = 34;
        Pano.CellEditActivationKey = Keys.F2;
        Pano.AllowEditAllCells = true;
        Pano.EnableCellEditing = true;

        Pano.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 76)
        {
            Kind = PanoColumnKind.Icon,
        ...
        Pano.Columns.Add(new PanoColumn("Ad", nameof(DemoRow.Name), 240) { Editable = true, FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Kategori", nameof(DemoRow.Occupation), 170)
        {
            Kind = PanoColumnKind.ComboBox,
            Editable = true,
            ComboBoxItems = DemoData.Occupations.ToList(),
            ComboBoxImageGetter = CreateOccupationIcon,
            Editor = new Taylan.Pano.Editing.ComboBoxCellEditor(DemoData.Occupations)
        });
        Pano.Columns.Add(new PanoColumn("Karar", nameof(DemoRow.State), 130)
        {
            Kind = PanoColumnKind.ComboBox,
            Editable = true,
            ComboBoxItems = StatusValues.ToList(),
            ComboBoxImageGetter = CreateStatusIcon,
            Editor = new Taylan.Pano.Editing.ComboBoxCellEditor(StatusValues)
        });
        Pano.Columns.Add(new PanoColumn("Puan", nameof(DemoRow.Rating), 110) { Kind = PanoColumnKind.Rating, MaxRating = 5 });
        Pano.Columns.Add(new PanoColumn("İlerleme", nameof(DemoRow.Progress), 140) { Kind = PanoColumnKind.ProgressBar });

        Tool.Items.Add(new ToolStripButton("Karar filtresi", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("Kategori filtresi", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Occupation))));
...
public sealed class ExportSampleForm : PanoSampleFormBase
{
    public ExportSampleForm() : base("Pano Export Detay Örneği", "Görünen satırları CSV veya Excel .xlsx olarak dışarı aktarır.")
    {
    ...
    {
        var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Pano.Sample.csv");
        try
        {
            var actualPath = Pano.ExportVisibleCsv(path);
    ...
    {
        var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Pano.Sample.xlsx");
        try
        {
            var actualPath = Pano.ExportVisibleExcel(path, "PanoControlSample");
...
public sealed class MillionRowsSampleForm : PanoSampleFormBase
{
    public MillionRowsSampleForm() : base("Pano 1M Sanal Liste Detay Örneği", "999.000.000 satır virtual/query provider ile simüle edilir; büyük veri RAM'e alınmaz, filtre/sort provider tarafında çalışır.")
    {
        Tool.Items.Add(new ToolStripButton("999M Virtual yükle", null, (_,__) => Pano.SetVirtualProvider(new VirtualDemoRowProvider(999_000_000))));
        Tool.Items.Add(new ToolStripButton("1.500.000 normal/async yükle", null, async (_,__) => await Pano.SetObjectsAsync<DemoRow>(async ct => { await Task.Delay(25, ct); return CreateRows(1_500_000); })));
        Tool.Items.Add(new ToolStripButton("Ad filtre popup", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Hızlı filtre modu", null, (_,__) => { Pano.FastFilterMenuForHugeLists = true; MainForm.ConfigureMillionRowFiltering(Pano); Info.Text = "999M filtre menüsü hızlı açılır; yazılan metin provider tarafında tüm sanal veri üzerinde filtrelenir."; }));
        Tool.Items.Add(new ToolStripButton("Tam tarama modu", null, (_,__) => { Pano.FastFilterMenuForHugeLists = false; Info.Text = "Dev veride tüm distinct değerleri listelemek yerine arama/provider filtresi kullanılmalıdır."; }));
        Shown += (_,__) => Pano.SetVirtualProvider(new VirtualDemoRowProvider(999_000_000));
    }
}

public sealed class DesignerApiSampleForm : PanoSampleFormBase
{
    public DesignerApiSampleForm() : base("Pano Designer / API Detay Örneği", "DLL projeye eklendiğinde designer tarafında değiştirilebilen özelliklerin ve kodla ayarlanabilen API'lerin kısa vitrini.")
    {
        Tool.Items.Add(new ToolStripButton("Designer özelliklerini göster", null, (_,__) => MessageBox.Show(this,
            ...
            "• Columns koleksiyonu: CheckBox, Badge, ProgressBar, Rating, Button", "Pano Designer/API")));
        Tool.Items.Add(new ToolStripButton("Header filtre menüsü", null, (_,__) => Pano.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Pano.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Layout kaydet", null, (_,__) => Pano.SaveLayout(Path.Combine(Application.StartupPath, "sample-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Layout yükle", null, (_,__) => Pano.LoadLayout(Path.Combine(Application.StartupPath, "sample-layout.json"))));
    }
}



public sealed class ColumnManagerSampleForm : PanoSampleFormBase
{
    private readonly string _layoutPath = Path.Combine(Application.StartupPath, "column-manager-sample-layout.json");

    public ColumnManagerSampleForm() : base("Pano Column Manager Detay Örneği", "Kolon başlığını tutup sağa/sola sürükleyerek yer değiştir. Genişlik değişiklikleri ve kolon görünürlüğü layout olarak kaydedilebilir.")
    {
        Pano.Name = "ColumnManagerSamplePano";
        Pano.AllowColumnReorder = true;
        Pano.ShowColumnReorderPreview = true;
        Pano.AutoSaveColumnLayout = false;
        Pano.ColumnLayoutStorageKey = "ColumnManagerSample";

        Tool.Items.Add(new ToolStripButton("Sürükle-bırak aktif", null, (_,__) => { Pano.AllowColumnReorder = true; Info.Text = "Kolon başlığını tutup sağa/sola sürükle."; }));
        Tool.Items.Add(new ToolStripButton("Otomatik kaydet Aç/Kapat", null, (_,__) => { Pano.AutoSaveColumnLayout = !Pano.AutoSaveColumnLayout; Info.Text = "AutoSaveColumnLayout = " + Pano.AutoSaveColumnLayout; }));
        Tool.Items.Add(new ToolStripButton("Layout kaydet", null, (_,__) => { Pano.SaveLayout(_layoutPath); Info.Text = "Kaydedildi: " + _layoutPath; }));
        Tool.Items.Add(new ToolStripButton("Layout yükle", null, (_,__) => { Pano.LoadLayout(_layoutPath); Info.Text = "Yüklendi: " + _layoutPath; }));
        Tool.Items.Add(new ToolStripButton("Varsayılana dön", null, (_,__) => { Pano.ResetColumnLayout(); Info.Text = "Kolon düzeni varsayılana döndü."; }));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Pano.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("İçeriğe sığdır", null, (_,__) => Pano.AutoResizeColumnsToContent()));
...
public sealed class LocalizationSampleForm : PanoSampleFormBase
{
    private readonly ToolStripComboBox _language = new() { DropDownStyle = ComboBoxStyle.DropDownList, Width = 170 };

    public LocalizationSampleForm() : base("Pano Dil / Localization Test Formu", "Menü, filtre popup, kolon seçimi ve boş liste metinlerinin Windows dili veya seçilen dile göre değişmesini test eder.")
    {
        foreach (PanoLanguage lang in Enum.GetValues(typeof(PanoLanguage)))
            _language.Items.Add(lang);
        _language.SelectedItem = PanoLanguage.Auto;
        _language.SelectedIndexChanged += (_,__) => ApplySelectedLanguage();

        Tool.Items.Add(new ToolStripLabel("Dil:"));
        Tool.Items.Add(_language);
        Tool.Items.Add(new ToolStripButton("Header filtre", null, (_,__) => Pano.ShowHeaderContextMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Filtre penceresi", null, (_,__) => Pano.ShowFilterMenuForAspect(nameof(DemoRow.Name))));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Pano.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Boş liste metni", null, (_,__) => Pano.SetObjects(Array.Empty<DemoRow>())));
        Tool.Items.Add(new ToolStripButton("Veriyi geri yükle", null, (_,__) => Pano.SetObjects(CreateRows(250))));

        Pano.Language = PanoLanguage.Auto;
        Pano.FilterMenuMode = PanoFilterMenuMode.Both;
    ...
    {
        if (_language.SelectedItem is not PanoLanguage lang) return;
        Pano.Language = lang;
        Pano.EmptyListMessage = PanoText.EmptyList;
        Info.Text = $"Aktif dil: {lang} / Effective: {PanoText.EffectiveLanguage}. Sağ tık menüsünü veya filtre popup'ını açarak metinleri kontrol et.";
        Pano.Invalidate();
    }
}

public sealed class GLVFeaturePackSampleForm : PanoSampleFormBase
{
    public GLVFeaturePackSampleForm() : base("Pano GLV Feature Pack", "GLV'de sık kullanılan liste özelliklerinin Pano karşılıkları: hyperlink, toggle, tags, sparkline, color swatch, seçim/kolon/layout işlemleri.")
    {
        Pano.Columns.Clear();
        Pano.Columns.Add(new PanoColumn("✓", nameof(DemoRow.Checked), 42) { Kind = PanoColumnKind.CheckBox, AspectPutter = (row, value) => ((DemoRow)row).SetChecked(Convert.ToBoolean(value)) });
        Pano.Columns.Add(new PanoColumn("Toggle", nameof(DemoRow.NeedsReview), 76) { Kind = PanoColumnKind.ToggleSwitch, AspectPutter = (row, value) => ((DemoRow)row).SetNeedsReview(Convert.ToBoolean(value)) });
        Pano.Columns.Add(new PanoColumn("Id", nameof(DemoRow.Id), 70));
        Pano.Columns.Add(new PanoColumn("Ad", nameof(DemoRow.Name), 210) { Editable = true, FillFreeSpace = true });
        Pano.Columns.Add(new PanoColumn("Link", nameof(DemoRow.Link), 90) { Kind = PanoColumnKind.Hyperlink });
        Pano.Columns.Add(new PanoColumn("Etiketler", nameof(DemoRow.Tags), 180) { Kind = PanoColumnKind.Tags });
        Pano.Columns.Add(new PanoColumn("Mini grafik", nameof(DemoRow.Spark), 130) { Kind = PanoColumnKind.Sparkline });
        Pano.Columns.Add(new PanoColumn("Renk", nameof(DemoRow.RowColor), 105) { Kind = PanoColumnKind.ColorSwatch });
        Pano.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 110) { Kind = PanoColumnKind.Badge });
        Pano.Columns.Add(new PanoColumn("İlerleme", nameof(DemoRow.Progress), 140) { Kind = PanoColumnKind.ProgressBar });
        Pano.Columns.Add(new PanoColumn("Puan", nameof(DemoRow.Rating), 110) { Kind = PanoColumnKind.Rating });
        Pano.SetObjects(CreateRows(500));
        Pano.HyperlinkClick += (_, e) => MessageBox.Show(this, $"Hyperlink tıklandı: {((DemoRow)e.RowObject).Name}", "Pano Hyperlink");
        Pano.ItemActivate += (_, e) => Info.Text = $"Aktif satır: {((DemoRow)e.RowObject).Name}";

        Tool.Items.Add(new ToolStripButton("Tümünü seç", null, (_,__) => Pano.SelectAllRows()));
        Tool.Items.Add(new ToolStripButton("Seçimi temizle", null, (_,__) => Pano.ClearSelection()));
        Tool.Items.Add(new ToolStripButton("Seçimi ters çevir", null, (_,__) => Pano.InvertSelection()));
        Tool.Items.Add(new ToolStripButton("Kolonları içeriğe göre ayarla", null, (_,__) => Pano.AutoResizeColumnsToContent()));
        Tool.Items.Add(new ToolStripButton("Üste git", null, (_,__) => Pano.ScrollToTop()));
        Tool.Items.Add(new ToolStripButton("Alta git", null, (_,__) => Pano.ScrollToBottom()));
        Tool.Items.Add(new ToolStripButton("Layout kaydet", null, (_,__) => Pano.SaveLayout(Path.Combine(Application.StartupPath, "olv-feature-pack-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Layout yükle", null, (_,__) => Pano.LoadLayout(Path.Combine(Application.StartupPath, "olv-feature-pack-layout.json"))));
        Tool.Items.Add(new ToolStripButton("Kart görünümü", null, (_,__) => Pano.SetViewMode(PanoViewMode.Tile)));
        Tool.Items.Add(new ToolStripButton("Detay", null, (_,__) => Pano.SetViewMode(PanoViewMode.Details)));
    }
}
...
public sealed class DatabaseSampleForm : Form
{
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new PanoColumn("Id", "", 70) { AspectGetter = row => ((DataRow)row)["Id"] });
        _grid.Columns.Add(new PanoColumn("Barkod", "", 160) { AspectGetter = row => ((DataRow)row)["Barcode"] });
        _grid.Columns.Add(new PanoColumn("Makine", "", 130) { AspectGetter = row => ((DataRow)row)["MachineName"] });
        _grid.Columns.Add(new PanoColumn("Assembly", "", 220) { AspectGetter = row => ((DataRow)row)["AssemblyName"] });
        _grid.Columns.Add(new PanoColumn("AOI", "", 90) { AspectGetter = row => ((DataRow)row)["AoiResult"] });
        _grid.Columns.Add(new PanoColumn("AI", "", 90) { AspectGetter = row => ((DataRow)row)["AIResult"] });
        _grid.Columns.Add(new PanoColumn("Bekleyen", "", 90) { AspectGetter = row => ((DataRow)row)["WaitingDecision"] });
        _grid.Columns.Add(new PanoColumn("Tarih", "", 160) { AspectGetter = row => ((DateTime)((DataRow)row)["AoiTestTime"]).ToString("yyyy-MM-dd HH:mm") });
    }
...
// panoView1.SetObjects(rows);
// panoView1.FastFilterMenuForHugeLists = true;
// panoView1.AsyncLoadFullFilterValues = true;
"""";
}
...
public sealed class TreeViewSampleForm : Form
{
    ...
    private readonly PanoControl _detailGrid = new()
    {
        Dock = DockStyle.Fill,
        FullRowSelect = true,
        ShowGridLines = true,
    ...
    private readonly TreePanoControl _treeGrid = new()
    {
        Dock = DockStyle.Fill,
        FullRowSelect = true,
        ShowGridLines = true,
        ...
        TreeSearchBehavior = TreePanoSearchBehavior.ExpandAncestorsAndDescendants,
    ...
    {
        Text = "Pano TreeView + TreeGrid Örnekleri";
        Width = 1260;
        Height = 760;
        MinimumSize = new Size(980, 620);
        StartPosition = FormStartPosition.CenterParent;
        BackColor = Program.AppTheme.BackColor;
        ForeColor = Program.AppTheme.ForeColor;
        PanoDialogChrome.ConfigureStandardDialog(this, Program.AppTheme, new Size(980, 620), sizeable: true);
        PanoDialogThemeApplier.Apply(this, Program.AppTheme);
        ...
        var tabs = new TabControl { Dock = DockStyle.Fill };
        var masterDetail = new TabPage("TreeView + Pano detay") { BackColor = Program.AppTheme.BackColor };
        var treeGridTab = new TabPage("TreePanoControl") { BackColor = Program.AppTheme.BackColor };
    ...
    private void ConfigureDetailGrid()
    {
        ...
        _detailGrid.Columns.Add(new PanoColumn("Seviye", "Level", 70));
        _detailGrid.Columns.Add(new PanoColumn("Tip", "Kind", 100));
        _detailGrid.Columns.Add(new PanoColumn("Kod", "Code", 150));
        _detailGrid.Columns.Add(new PanoColumn("Ad", "Name", 260) { FillFreeSpace = true });
        _detailGrid.Columns.Add(new PanoColumn("Durum", "State", 100) { Kind = PanoColumnKind.Badge });
        _detailGrid.Columns.Add(new PanoColumn("Adet", "Count", 80));
    }

    private void ConfigureTreeGrid()
    {
        _treeGrid.ApplyTheme(Program.AppTheme);
        _treeGrid.Columns.Add(new PanoColumn("Ağaç", "Name", 360) { FillFreeSpace = true, AspectGetter = row => new string(' ', ((TreeDemoNode)row).Level * 4) + (((TreeDemoNode)row).Children.Count > 0 ? (_treeGrid.IsExpanded(row) ? "▾ " : "▸ ") : "  ") + ((TreeDemoNode)row).Name });
        _treeGrid.Columns.Add(new PanoColumn("Tip", "Kind", 110));
        _treeGrid.Columns.Add(new PanoColumn("Kod", "Code", 150));
        _treeGrid.Columns.Add(new PanoColumn("Durum", "State", 100) { Kind = PanoColumnKind.Badge });
        _treeGrid.Columns.Add(new PanoColumn("Adet", "Count", 80));
...
{
    private readonly PanoTheme _theme = Program.AppTheme;
    ...
    public DialogThemeResizeShowcaseForm()
    {
        ...
        PanoDialogChrome.ConfigureStandardDialog(this, _theme, new Size(640, 420), sizeable: true);
        ...
        var grid = new PanoControl { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true };
        grid.ApplyTheme(_theme);
        grid.Columns.Add(new PanoColumn("Ad", nameof(DemoRow.Name), 180) { FillFreeSpace = true });
        grid.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 100));
        grid.Columns.Add(new PanoColumn("Yıl", nameof(DemoRow.Year), 80));
        ...
        Controls.Add(panel);
        Controls.Add(info);
        PanoDialogThemeApplier.Apply(this, _theme);
    }
}

public sealed class PanoDataModeVerificationForm : Form
{
    public PanoDataModeVerificationForm()
    {
        Text = "PanoControl Mode Doğrulama - Object / DataTable / Virtual / Tree / Tile";
        Width = 1180;
        Height = 720;
        StartPosition = FormStartPosition.CenterParent;
        BackColor = Program.AppTheme.BackColor;
    ...
    {
        var grid = CreateDemoGrid(PanoDataMode.Object);
        grid.SetObjects(CreateRows(300));
        return Wrap("Mode = Object", "POCO / model listesi ile klasik Pano/GLV benzeri kullanım.", grid);
    }

    private static TabPage CreateDataTableTab()
    {
        var grid = CreateDemoGrid(PanoDataMode.DataTable);
    ...
    {
        var grid = CreateDemoGrid(PanoDataMode.Virtual);
    ...
    {
        var grid = new TreePanoControl
        {
            Dock = DockStyle.Fill,
            Mode = PanoDataMode.Tree,
            FullRowSelect = true,
            ShowGridLines = true,
            EnableModernEmptyState = true,
            EmptyListMessage = "Tree kayıt yok"
        };
        grid.Columns.Add(new PanoColumn("Ad", "Name", 260));
        grid.Columns.Add(new PanoColumn("Tür", "State", 130));
        grid.Columns.Add(new PanoColumn("Yıl", "Year", 80));
        ...
        grid.Mode = PanoDataMode.Tree;
        return Wrap("Mode = Tree", "TreePanoControl ile hiyerarşik veri senaryosu. Aynı Pano kolon/render altyapısını kullanır.", grid);
    }

    private static TabPage CreateTileTab()
    {
        var grid = CreateDemoGrid(PanoDataMode.Tile);
        grid.TileMinWidth = 260;
        grid.TilePreferredHeight = 110;
        grid.SetObjects(CreateRows(150));
        grid.Mode = PanoDataMode.Tile;
        return Wrap("Mode = Tile", "Kart/tile görünüm odaklı kullanım. Mode=Tile otomatik Tile ViewMode uygular.", grid);
    }

    private static PanoControl CreateDemoGrid(PanoDataMode mode)
    {
        ...
        grid.Columns.Add(new PanoColumn("Id", nameof(DemoRow.Id), 70));
        grid.Columns.Add(new PanoColumn("Ad", nameof(DemoRow.Name), 240));
        grid.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 110));
        grid.Columns.Add(new PanoColumn("Meslek", nameof(DemoRow.Occupation), 150));
        grid.Columns.Add(new PanoColumn("Yıl", nameof(DemoRow.Year), 80));
        grid.Columns.Add(new PanoColumn("Progress", nameof(DemoRow.Progress), 110) { Kind = PanoColumnKind.ProgressBar });
        return grid;
    }
    ...
    private static List<DemoRow> CreateRows(int count)
    {
        ...
        for (int i = 1; i <= count; i++)
        {
            list.Add(new DemoRow
            {
                Id = i,
                RealIndex = i,
                Name = "Pano Mode satır " + i.ToString("0000"),
                State = i % 7 == 0 ? "Fail" : i % 5 == 0 ? "Review" : "OK",
                Occupation = DemoData.Occupations[i % DemoData.Occupations.Length],
                Year = 2020 + (i % 7),
                Progress = (i * 7) % 101,
    ...
    {
        var t = new DataTable("PanoDataModeDataTable");
...
public sealed class FeatureVerificationSuiteForm : Form
{
    public FeatureVerificationSuiteForm()
    {
        ...
        Text = "Pano Özellik Test Merkezi";
        Width = 920;
        Height = 620;
        MinimumSize = new Size(760, 480);
        BackColor = theme.BackColor;
        ForeColor = theme.ForeColor;
        PanoWindowChrome.ApplyOnHandleCreated(this, () => theme, true);
        ...
        var title = new Label
        {
            ...
            Text = "Pano özellik doğrulama örnekleri",
            Font = new Font("Segoe UI", 14F, FontStyle.Bold),
        ...
        AddSuiteCard(grid, theme, "8) PanoControl Mode", "Object/DataTable/Virtual/Tree/Tile modlarını ayrı sekmelerde doğrular.", () => new PanoDataModeVerificationForm().Show());
    ...
    private static void AddSuiteCard(TableLayoutPanel grid, PanoTheme theme, string header, string description, Action open)
    {
...
public sealed class LayoutManagerVerificationForm : PanoSampleFormBase
{
    private readonly string _layoutPath = Path.Combine(Application.StartupPath, "pano-v2487-layout-test.json");
    ...
    {
        Pano.ColumnLayoutStorageKey = "PanoV2487LayoutTest";
        Pano.AutoSaveColumnLayout = false;
        Tool.Items.Add(new ToolStripButton("1 Kaydet", null, (_,__) => { Pano.SaveLayout(_layoutPath); Info.Text = "Layout kaydedildi: " + _layoutPath; }));
        Tool.Items.Add(new ToolStripButton("2 Kolonları boz", null, (_,__) => { var progressCol = Pano.Columns[nameof(DemoRow.Progress)]; if (progressCol != null) progressCol.Visible = false; var nameCol = Pano.Columns[nameof(DemoRow.Name)]; if (nameCol != null) nameCol.Width = 420; Pano.SetGroupBy(nameof(DemoRow.State)); Pano.RefreshView(); Info.Text = "Kolon görünürlüğü/genişliği ve grup değiştirildi. Şimdi Yükle ile geri al."; }));
        Tool.Items.Add(new ToolStripButton("3 Yükle", null, (_,__) => { Pano.LoadLayout(_layoutPath); Info.Text = "Layout yüklendi."; }));
        Tool.Items.Add(new ToolStripButton("4 Reset", null, (_,__) => { Pano.ResetColumnLayout(); Info.Text = "Layout varsayılana döndü."; }));
        Tool.Items.Add(new ToolStripButton("Kolon seçici", null, (_,__) => Pano.ShowColumnChooser()));
        Tool.Items.Add(new ToolStripButton("Duruma göre grupla", null, (_,__) => Pano.SetGroupBy(nameof(DemoRow.State))));
        Tool.Items.Add(new ToolStripButton("Grubu kaldır", null, (_,__) => Pano.ClearGrouping()));
    }
}

public sealed class IncrementalSearchHighlightVerificationForm : PanoSampleFormBase
{
    ...
    {
        Pano.HighlightSearchText = true;
        Pano.HighlightGlobalFilterText = true;
        Pano.EnableIncrementalSearch = true;
        Pano.DefaultHighlightDurationMs = 4500;
        Pano.SetObjects(CreateRows(1000));
        Tool.Items.Add(new ToolStripLabel("Ara:"));
        ...
        Tool.Items.Add(new ToolStripButton("Seçiliyi vurgula", null, (_,__) => { if (Pano.SelectedObject != null) Pano.HighlightObject(Pano.SelectedObject, Color.Orange, 6000, "manual"); }));
        Tool.Items.Add(new ToolStripButton("Vurguları temizle", null, (_,__) => Pano.ClearRowHighlights()));
        _find.TextChanged += (_,__) => { if (!string.IsNullOrWhiteSpace(_find.Text)) Pano.JumpToFirstMatch(_find.Text); else Pano.ClearSearchHighlight(); };
    }

    private void DoFindNext()
    {
        bool ok = Pano.FindNext(_find.Text);
        Info.Text = ok ? "Sonraki eşleşmeye gidildi." : "Eşleşme bulunamadı.";
    }

    private void DoFindPrevious()
    {
        bool ok = Pano.FindPrevious(_find.Text);
    ...
    {
        int highlighted = 0;
        for (int i = 0; i < Pano.ViewCount && highlighted < 25; i++)
        {
            if (Pano.GetModelObject(i) is DemoRow row && row.State == "Fail")
            {
                Pano.HighlightRow(i, Color.FromArgb(255, 193, 7), 7000, "Fail");
                highlighted++;
            }
        }
...
public sealed class AdvancedCopyExportVerificationForm : PanoSampleFormBase
{
    private readonly string _folder = Path.Combine(Application.StartupPath, "PanoExportTests");
    ...
    public AdvancedCopyExportVerificationForm() : base("Advanced Copy / Export", "Test: birkaç satır seç; hücre/satır/JSON kopyala. CSV, Excel XML ve JSON dosyaları test klasörüne yazılır.")
    {
        ...
        Pano.MultiSelect = true;
        Pano.SelectObject(Pano.GetModelObject(4));
        Pano.SelectObject(Pano.GetModelObject(9), addToSelection: true);
        Tool.Items.Add(new ToolStripButton("Seçimi kopyala", null, (_,__) => { Pano.CopySelectionToClipboard(); Info.Text = "Seçim TSV olarak panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("Hücreyi kopyala", null, (_,__) => { Pano.CopySelectedCellToClipboard(); Info.Text = "Aktif hücre panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("JSON kopyala", null, (_,__) => { Pano.CopySelectionAsJsonToClipboard(); Info.Text = "Seçim JSON olarak panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("CSV export", null, (_,__) => Export(() => Pano.ExportVisibleCsv(Path.Combine(_folder, "visible.csv")))));
        Tool.Items.Add(new ToolStripButton("Excel export", null, (_,__) => Export(() => Pano.ExportVisibleExcel(Path.Combine(_folder, "visible.xlsx"), "PanoTest"))));
        Tool.Items.Add(new ToolStripButton("JSON export", null, (_,__) => Export(() => Pano.ExportVisibleJson(Path.Combine(_folder, "visible.json")))));
        Tool.Items.Add(new ToolStripButton("Seçili JSON export", null, (_,__) => Export(() => Pano.ExportSelectedJson(Path.Combine(_folder, "selected.json")))));
    }
...
public sealed class GroupingVerificationForm : PanoSampleFormBase
{
    ...
    {
        Pano.EnableGrouping = true;
        Pano.SetObjects(CreateRows(800));
        Tool.Items.Add(new ToolStripButton("Durum", null, (_,__) => { Pano.SetGroupBy(nameof(DemoRow.State)); Info.Text = "Duruma göre gruplandı."; }));
        Tool.Items.Add(new ToolStripButton("Meslek", null, (_,__) => { Pano.SetGroupBy(nameof(DemoRow.Occupation)); Info.Text = "Mesleğe göre gruplandı."; }));
        Tool.Items.Add(new ToolStripButton("Yıl", null, (_,__) => { Pano.SetGroupBy(nameof(DemoRow.Year)); Info.Text = "Yıla göre gruplandı."; }));
        Tool.Items.Add(new ToolStripButton("Daralt", null, (_,__) => { Pano.CollapseAllGroups(); Info.Text = "Tüm gruplar daraltıldı."; }));
        Tool.Items.Add(new ToolStripButton("Aç", null, (_,__) => { Pano.ExpandAllGroups(); Info.Text = "Tüm gruplar açıldı."; }));
        Tool.Items.Add(new ToolStripButton("Renk ver", null, (_,__) => { Pano.CustomGroupBackColor = Color.FromArgb(235, 229, 250); Pano.Invalidate(); Info.Text = "Grup başlığı rengi değiştirildi."; }));
        Tool.Items.Add(new ToolStripButton("Temizle", null, (_,__) => { Pano.ClearGrouping(); Info.Text = "Gruplama kaldırıldı."; }));
        Pano.SetGroupBy(nameof(DemoRow.State));
    }
}

public sealed class MiniAnalyticsVerificationForm : PanoSampleFormBase
{
    ...
    public MiniAnalyticsVerificationForm() : base("Mini Analytics", "Test: kolon bazlı Row/Blank/Distinct/Top Values raporunu üret. Filtre uygula ve raporu tekrar yenileyerek görünür veri analitiğini kontrol et.")
    {
        ...
        Pano.SetObjects(CreateRows(1200));
        ...
        Tool.Items.Add(new ToolStripButton("Panoya kopyala", null, (_,__) => { Pano.CopyMiniAnalyticsToClipboard(); Info.Text = "Mini analytics panoya kopyalandı."; }));
        Tool.Items.Add(new ToolStripButton("Fail filtresi", null, (_,__) => { Pano.SetGlobalFilter("Fail"); RefreshAnalytics(); }));
        Tool.Items.Add(new ToolStripButton("Filtre temizle", null, (_,__) => { Pano.ClearFilters(); RefreshAnalytics(); }));
        RefreshAnalytics();
    }
    ...
    private void RefreshAnalytics()
    {
        ...
        foreach (var item in Pano.GetVisibleAnalytics(maxDistinctPerColumn: 5, maxRows: 50000))
        {
    ...
    {
        var item = Pano.GetColumnAnalytics(aspectName, maxDistinct: 10, maxRows: 50000);
        if (item == null) return;
...
public sealed class DesignerPreviewSampleForm : PanoSampleFormBase
{
    private readonly ToolStripComboBox _preview = new() { DropDownStyle = ComboBoxStyle.DropDownList, Width = 130 };

    public DesignerPreviewSampleForm() : base("Pano Designer Önizleme", "DesignTimeThemePreview, AutoThemeFromParent ve örnek veri davranışını runtime'da da test eder.")
    {
        foreach (PanoDesignTimeThemePreview value in Enum.GetValues(typeof(PanoDesignTimeThemePreview)))
            _preview.Items.Add(value);
        _preview.SelectedItem = PanoDesignTimeThemePreview.Auto;
        ...
        Tool.Items.Add(new ToolStripButton("Örnek veri yenile", null, (_,__) => { Pano.DesignTimeSampleData = true; Pano.RefreshDesignTimePreview(); }));

        Pano.AutoThemeFromParent = true;
        Pano.DesignTimeSampleData = true;
        ApplyPreviewMode();
    }

    private void ApplyPreviewMode()
    {
        if (_preview.SelectedItem is not PanoDesignTimeThemePreview mode) return;
        Pano.DesignTimeThemePreview = mode;
        ApplyParentPreview();
    }

    private void ApplyParentPreview()
    {
        var mode = Pano.DesignTimeThemePreview;
        var theme = mode switch
        {
            ...
            PanoDesignTimeThemePreview.Fluent => BackColor.GetBrightness() < 0.45f ? PanoTheme.FluentDarkTheme() : PanoTheme.FluentLightTheme(),
            _ => PanoTheme.FromParentColor(BackColor, ForeColor)
        };
        Pano.ApplyTheme(theme);
        Info.BackColor = theme.PanelBackColor;
        Info.ForeColor = theme.ForeColor;
        Tool.BackColor = theme.HeaderBackColor;
        Tool.ForeColor = theme.HeaderForeColor;
        Info.Text = $"Designer önizleme modu: {mode} | AutoThemeFromParent={Pano.AutoThemeFromParent} | Form rengi={ColorTranslator.ToHtml(BackColor)}";
    }
}
...
public sealed class DatabaseCrudEditorsSampleForm : Form
{
    ...
    private void ConfigureGeneratedColumns()
    {
        ...
        if (barcode != null) { barcode.EditorType = Taylan.Pano.Editing.PanoCellEditorKind.TextBox; barcode.Required = true; barcode.MaxLength = 30; barcode.Width = 160; }

        var active = _grid.GetColumn("IsActive");
        if (active != null) { active.Header = "Aktif"; active.Kind = PanoColumnKind.CheckBox; active.EditorType = Taylan.Pano.Editing.PanoCellEditorKind.CheckBox; active.Width = 80; }

        var result = _grid.GetColumn("Result");
        if (result != null) { result.Header = "Sonuç"; result.Kind = PanoColumnKind.ComboBox; result.EditorType = Taylan.Pano.Editing.PanoCellEditorKind.ComboBox; result.ComboBoxItems = new List<string> { "PASS", "FAIL", "WAIT", "REPAIR" }; result.Width = 110; }

        var qty = _grid.GetColumn("Quantity");
        if (qty != null) { qty.Header = "Adet"; qty.Kind = PanoColumnKind.Numeric; qty.EditorType = Taylan.Pano.Editing.PanoCellEditorKind.Numeric; qty.NumericMinimum = 0; qty.NumericMaximum = 999999; qty.Width = 90; }

        var score = _grid.GetColumn("Score");
        if (score != null) { score.Header = "Puan"; score.Kind = PanoColumnKind.Numeric; score.EditorType = Taylan.Pano.Editing.PanoCellEditorKind.Numeric; score.NumericMinimum = 0; score.NumericMaximum = 100; score.Width = 90; }

        var date = _grid.GetColumn("TestDate");
        if (date != null) { date.Header = "Test Tarihi"; date.Kind = PanoColumnKind.Date; date.EditorType = Taylan.Pano.Editing.PanoCellEditorKind.DateTime; date.DateTimeFormat = Taylan.Pano.Editing.PanoDateTimeEditorFormat.Custom; date.DateTimeCustomFormat = "dd.MM.yyyy HH:mm"; date.Width = 150; }
    ...
    private void DeleteRows()
    {
        ...
        MessageBox.Show(this, deleted + " satır silinmek üzere işaretlendi.", "Pano CRUD");
        RefreshStatus();
    }
    ...
    private void SaveChangesDemo()
    {
        ...
        MessageBox.Show(this, "Demo DataTable modunda SaveChanges AcceptChanges yapar. SQL adapter bağlıysa INSERT/UPDATE/DELETE komutları çalışır. Affected: " + affected, "Pano CRUD");
        RefreshStatus();
    }
...
{
    private readonly Taylan.Pano.Core.PanoControl _grid = new()
    {
        Dock = DockStyle.Fill,
    ...
    private readonly Label _info = new()
    {
        ...
        TextAlign = ContentAlignment.MiddleLeft,
        Text = "Pano üzerinde sağ tıkla: kullanıcı ContextMenuStrip menüsünün altında 'Liste özellikleri' olarak Pano'nin kendi hızlı menüsü birleşir."
    };
    ...
    {
        Text = "Pano Designer + ContextMenu Merge Örneği";
        Width = 980;
        Height = 640;
        StartPosition = FormStartPosition.CenterScreen;
        ...
        _grid.Columns.Add(new PanoColumn("Id", nameof(Row.Id), 70));
        _grid.Columns.Add(new PanoColumn("Ad", nameof(Row.Name), 180) { FillFreeSpace = true });
        _grid.Columns.Add(new PanoColumn("Durum", nameof(Row.State), 110));
        _grid.Columns.Add(new PanoColumn("Oran", nameof(Row.Rate), 90) { TextAlign = ContentAlignment.MiddleRight });
        ...
        _top.Items.Add(new ToolStripButton("Mode=Object", null, (_,__) => _grid.Mode = PanoDataMode.Object));
        _top.Items.Add(new ToolStripButton("Mode=Virtual", null, (_,__) => _grid.Mode = PanoDataMode.Virtual));
        _top.Items.Add(new ToolStripButton("Mode=Tree", null, (_,__) => _grid.Mode = PanoDataMode.Tree));
        _top.Items.Add(new ToolStripButton("Mode=Tile", null, (_,__) => _grid.Mode = PanoDataMode.Tile));
...
// not only as detached code snippets under samples/Pano.FeatureSamples.
public sealed class PdfExportSuiteSampleForm : PanoSampleFormBase
{
    public PdfExportSuiteSampleForm() : base("v28.7 PDF Export Suite", "Details/Table ve Card/Dashboard PDF export örneği. Masaüstüne Pano_Table_Export.pdf ve Pano_Card_Export.pdf üretir.")
    {
        Pano.Columns.Clear();
        Pano.Columns.Add(new PanoColumn("Durum", nameof(DemoRow.State), 120) { CardRole = "Title", CardOrder = 0 });
        Pano.Columns.Add(new PanoColumn("Makine", nameof(DemoRow.Machine), 150) { CardOrder = 1 });
        Pano.Columns.Add(new PanoColumn("Meslek", nameof(DemoRow.Occupation), 140) { CardOrder = 2 });
        Pano.Columns.Add(new PanoColumn("Açıklama", nameof(DemoRow.Description), 260) { CardOrder = 3, CardShowCaption = true });
        Pano.Columns.Add(new PanoColumn("Puan", nameof(DemoRow.Rating), 80) { CardOrder = 4 });
        Pano.SetObjects(CreateRows(60));
        Pano.CardVisualInfoGetter = row =>
        {
            ...
            return new PanoCardVisualInfo
            {
                ...
                Badges =
                {
                    new PanoCardBadge { Text = r.State.Contains("Fail", StringComparison.OrdinalIgnoreCase) ? "FAIL" : "OK", BackColor = statusColor, ForeColor = Color.White }
                }
            };
        };
        Tool.Items.Add(new ToolStripSeparator());
        ...
        Tool.Items.Add(new ToolStripButton("Card görünümü", null, (_,__) => Pano.SetViewMode(PanoViewMode.DashboardCard)));
    }

    private void ExportPdf(bool card)
    {
        string file = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), card ? "Pano_Card_Export.pdf" : "Pano_Table_Export.pdf");
        string actual = Pano.ExportVisiblePdf(file, new global::Taylan.Pano.Exporting.PanoPdfExportOptions
        {
            Title = card ? "Pano Card PDF Export" : "Pano Table PDF Export",
            Subtitle = "v28.7 export smoke sample",
            Mode = card ? global::Taylan.Pano.Exporting.PanoPdfExportMode.Card : global::Taylan.Pano.Exporting.PanoPdfExportMode.Table,
            Orientation = card ? global::Taylan.Pano.Exporting.PanoPdfPageOrientation.Portrait : global::Taylan.Pano.Exporting.PanoPdfPageOrientation.Landscape,
            FitToPageWidth = true,
            ShowGridLines = true,
            ZebraRows = true,
            CardColumns = 2,
        ...
        MessageBox.Show(this, "PDF oluşturuldu:\n" + actual, "Pano PDF Export", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
}

public sealed class CellOverflowScrollSampleForm : PanoSampleFormBase
{
    public CellOverflowScrollSampleForm() : base("v28.8 Hücre İçi Scroll", "Uzun açıklama/not kolonlarında satır yüksekliği büyümeden hücre içinde mouse wheel ile kaydırma ve çift tık okuyucu popup test edilir.")
    {
        Pano.Columns.Clear();
        Pano.RowHeight = 62;
        Pano.DetailsRowHeight = 62;
        Pano.AllowMultilineCells = true;
        Pano.AutoRowHeightForMultilineCells = false;
        Pano.EnableCellOverflowScroll = true;
        Pano.ShowCellOverflowScrollBars = true;
        Pano.EnableCellOverflowDetailsPopup = true;
        Pano.Columns.Add(new PanoColumn("Id", nameof(DemoRow.Id), 70));
        Pano.Columns.Add(new PanoColumn("Makine", nameof(DemoRow.Machine), 150));
        Pano.Columns.Add(new PanoColumn("Kısa Durum", nameof(DemoRow.State), 120));
        Pano.Columns.Add(new PanoColumn("Mesaj / Açıklama", nameof(DemoRow.Description), 300)
        {
            WordWrap = true,
        ...
        Pano.Columns.Add(new PanoColumn("Teknisyen Notu", nameof(DemoRow.Notes), 280)
        {
            WordWrap = true,
            AllowCellScroll = true,
            CellScrollMaxVisibleLines = 3,
        ...
        Pano.Columns.Add(new PanoColumn("Zaman", nameof(DemoRow.Date), 140));
        var rows = CreateRows(80);
