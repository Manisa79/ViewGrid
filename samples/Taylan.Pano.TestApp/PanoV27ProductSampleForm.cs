...
public sealed class PanoV27ProductSampleForm : Form
{
    ...
    private string StateFile => Path.Combine(Application.StartupPath, "pano-v27-product-state.json");

    public PanoV27ProductSampleForm()
    {
        Text = "Pano v27 - Product Core / MasterData Hazırlığı";
        Width = 1320;
        Height = 820;
        ...
        LoadMasterDataRows();
        _grid.ApplyScenario(PanoScenario.BomPositions);
    ...
    {
        _grid.Columns.Add(new PanoColumn("Seç", nameof(V27Row.Selected), 52) { Kind = PanoColumnKind.CheckBox, CellCheckBox = true, HeaderCheckBox = true });
        _grid.Columns.Add(new PanoColumn("Id", nameof(V27Row.Id), 70));
        _grid.Columns.Add(new PanoColumn("Tip", nameof(V27Row.Type), 105).ApplyVisualProfile(PanoCellVisualProfile.Badge));
        _grid.Columns.Add(new PanoColumn("Kod", nameof(V27Row.Code), 150));
        _grid.Columns.Add(new PanoColumn("Açıklama", nameof(V27Row.Name), 280) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
        _grid.Columns.Add(new PanoColumn("Makine", nameof(V27Row.Machine), 110));
        _grid.Columns.Add(new PanoColumn("RefDes", nameof(V27Row.RefDes), 130));
        _grid.Columns.Add(new PanoColumn("İlerleme", nameof(V27Row.Progress), 95).ApplyVisualProfile(PanoCellVisualProfile.Progress));
        _grid.Columns.Add(new PanoColumn("Durum", nameof(V27Row.Status), 115).ApplyVisualProfile(PanoCellVisualProfile.WarningStatus));
        _grid.Columns.Add(new PanoColumn("Not", nameof(V27Row.Note), 360) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
    }

    private void ConfigureToolbar()
    {
        _tool.Items.Add(new ToolStripButton("MasterData BOM", null, (_, __) => ApplyScenario(PanoScenario.BomPositions)));
        _tool.Items.Add(new ToolStripButton("Ticket Dashboard", null, (_, __) => ApplyScenario(PanoScenario.TicketBoard)));
        _tool.Items.Add(new ToolStripButton("Timeline", null, (_, __) => ApplyScenario(PanoScenario.Timeline)));
    ...
    private void ApplyScenario(PanoScenario scenario)
    {
        ...
        if (scenario == PanoScenario.TicketBoard)
            _grid.SetGroupBy(nameof(V27Row.Status));
        else if (scenario == PanoScenario.Timeline)
    ...
    private void LoadMasterDataRows()
    {
        ...
        for (int i = 1; i <= 180; i++)
        {
            ...
            _rows.Add(new V27Row
            {
                ...
                Note = "MasterData için BOM, SAP ürün ağacı, program dosyası ve pozisyon kontrolü aynı Pano üzerinde denenir."
            });
        }
    }
    ...
    private void LoadRangeVirtualDemo()
    {
        ...
        var provider = new PanoRangeVirtualProvider(
            () => total,
        ...
        _grid.ApplyScenario(PanoScenario.DenseData);
