...
public sealed class KeyboardAccessibilitySampleForm : Form
{
    private readonly Label _help = new()
    {
        ...
        Text = "Pano klavye erişimi: Ok/Page/Home/End satır gezinme, Space seçili satır checkbox durumunu değiştirir, çoklu seçimde Space seçili satırları birlikte işaretler/kaldırır, Enter checkbox toggle yapmaz; button/default aksiyon içindir. Ctrl+Shift+Space header checkbox ile tümünü seçer. Ctrl+Left/Right aktif kolon, Alt+Down kolon menüsü, Ctrl+Shift+F filtre, Ctrl+Shift+L kolon seçici, Ctrl+Shift+R aktif kolon sığdır, Ctrl+Shift+Plus tüm kolonları sığdır, Ctrl+G grupla, Ctrl+Shift+G gruplamayı temizle, F3/Shift+F3 arama. Bu örnekte hücre edit açıkça F2 ile etkinleştirilmiştir."
    };
    ...
    {
        Text = "Pano Klavye Erişimi Örneği";
        Width = 1100;
        Height = 680;
        MinimumSize = new Size(840, 520);

        var name = new PanoColumn("Person", nameof(KeyboardRow.Person), 180)
        {
            HeaderCheckBox = true,
        ...
        _grid.Columns.Add(new PanoColumn("Occupation", nameof(KeyboardRow.Occupation), 160));
        _grid.Columns.Add(new PanoColumn("Status", nameof(KeyboardRow.Status), 120));
        _grid.Columns.Add(new PanoColumn("Note", nameof(KeyboardRow.Note), 360) { FillsFreeSpace = true });
        _grid.Columns.Add(new PanoColumn("Action", nameof(KeyboardRow.Action), 100) { Kind = PanoColumnKind.Button, ButtonText = "Aç" });
        ...
        _grid.ButtonClick += (_, e) => MessageBox.Show(this, $"Klavye/mouse action: {((KeyboardRow)e.RowObject).Person}", "Pano");
