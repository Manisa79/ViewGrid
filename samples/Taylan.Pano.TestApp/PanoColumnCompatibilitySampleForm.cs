...
public sealed class PanoColumnCompatibilitySampleForm : Form
{
    private readonly PanoControl _grid = new()
    {
        Dock = DockStyle.Fill,
        EmptyListMessage = "Pano kolon uyumluluk örneği boş",
    ...
    private readonly Label _info = new()
    {
        Dock = DockStyle.Fill,
        AutoEllipsis = true,
        Text = "Pano uyumlu checkbox görünümü: CheckBoxes=True yeni kolon eklemez; checkbox ve header checkbox ilk görünür veri kolonunun içinde çalışır. Header checkbox tüm görünen satırları işaretler/kaldırır."
    };
    ...
    public PanoColumnCompatibilitySampleForm()
    {
        Text = "Pano Uyumlu Kolon / Checkbox Örneği";
        Width = 1180;
        Height = 720;
    ...
    private void BuildToolbar()
    {
        ...
        _tool.Items.Add(new ToolStripButton("Light", null, (_, __) => ApplyTheme(Taylan.Pano.Theming.PanoTheme.LightTheme())));
        _tool.Items.Add(new ToolStripButton("Dark", null, (_, __) => ApplyTheme(Taylan.Pano.Theming.PanoTheme.DarkTheme())));
    }
    ...
    private void ConfigureColumns()
    {
        ...
        _grid.Columns.Add(new PanoColumn("Person", nameof(CompatRow.Person), 220)
        {
            HeaderCheckBox = true,
        ...
        _grid.Columns.Add(new PanoColumn("Occupation", nameof(CompatRow.Occupation), 150)
        {
        ...
        _grid.Columns.Add(new PanoColumn("Action", nameof(CompatRow.Action), 110)
        {
            Kind = PanoColumnKind.Button,
            ButtonSizing = PanoColumnButtonSizing.TextBounds,
            ButtonMaxWidth = 90,
            ButtonPadding = new Padding(2),
            EnableButtonWhenItemIsDisabled = true
        });

        _grid.Columns.Add(new PanoColumn("Link", nameof(CompatRow.Url), 170)
        {
            Hyperlink = true,
            HeaderTextAlign = ContentAlignment.MiddleCenter
        });

        _grid.Columns.Add(new PanoColumn("Notes", nameof(CompatRow.Notes), 260)
        {
            WordWrap = true,
        ...
        _grid.Columns.Add(new PanoColumn("Runtime Hidden", nameof(CompatRow.HiddenValue), 120)
        {
            Visible = false,
    ...
    private void LoadRows()
    {
        var rows = Enumerable.Range(1, 100).Select(i => new CompatRow
        {
            ...
            Action = "Aç",
            Url = "Detay " + i,
            Notes = "Pano kolon property uyumluluğu test satırı " + i,
    ...
    private void CycleButtonSizing()
    {
        ...
        {
            PanoColumnButtonSizing.TextBounds => PanoColumnButtonSizing.CellBounds,
            PanoColumnButtonSizing.CellBounds => PanoColumnButtonSizing.FixedBounds,
            _ => PanoColumnButtonSizing.TextBounds
        };

        _grid.RefreshView();
    }

    private void ApplyTheme(Taylan.Pano.Theming.PanoTheme theme)
    {
