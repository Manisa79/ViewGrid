...
using Taylan.Pano.Presets;

namespace Taylan.Pano.TestApp;

public sealed class ExcelFilterRowSampleForm : Form
{
    private readonly PanoControl _grid = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true, FastFilterMenuForHugeLists = true, AsyncLoadFullFilterValues = true };
    private PanoExcelFilterRowPanel? _filterRow;
    ...
    public ExcelFilterRowSampleForm()
    {
        ...
        _grid.Columns.Add(new PanoColumn("Barkod", "Barcode", 160) { FillFreeSpace = true });
        _grid.Columns.Add(new PanoColumn("Makine", "Machine", 120));
        _grid.Columns.Add(new PanoColumn("Sonuç", "Result", 90) { Kind = PanoColumnKind.Badge });
        _grid.Columns.Add(new PanoColumn("Aktif", "Active", 70) { Kind = PanoColumnKind.CheckBox });
        _grid.Columns.Add(new PanoColumn("Puan", "Score", 80) { Kind = PanoColumnKind.Numeric, TextAlign = ContentAlignment.MiddleRight });
        _grid.Columns.Add(new PanoColumn("Tarih", "TestDate", 150) { Kind = PanoColumnKind.Date });
        _grid.SetObjects(CreateRows(2000));

        _filterRow = new PanoExcelFilterRowPanel(_grid);
...
{
    private readonly PanoControl _master = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true };
    private readonly PanoControl _detail = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true, EmptyListMessage = "Üstten kayıt seçin" };
    private readonly List<TestResultRow> _masters;
    private readonly List<FailDetailRow> _details;

    public MasterDetailSampleForm()
    {
        Text = "Master-detail Pano";
        Width = 1280;
        Height = 760;
        StartPosition = FormStartPosition.CenterParent;
        BackColor = Program.AppTheme.BackColor;
        ...
        PanoPresets.ApplyAoiFailList(_master);
        _master.Columns.Clear();
        _master.Columns.Add(new PanoColumn("Id", "Id", 70));
        _master.Columns.Add(new PanoColumn("Barkod", "Barcode", 150) { FillFreeSpace = true });
        _master.Columns.Add(new PanoColumn("Makine", "Machine", 120));
        _master.Columns.Add(new PanoColumn("Program", "AssemblyName", 220) { FillFreeSpace = true });
        _master.Columns.Add(new PanoColumn("Sonuç", "Result", 90) { Kind = PanoColumnKind.Badge });
        _master.Columns.Add(new PanoColumn("Tarih", "TestDate", 150));
        _master.FrozenColumnCount = 2;

        _detail.Columns.Add(new PanoColumn("FailId", "Id", 70));
        _detail.Columns.Add(new PanoColumn("Layer", "Layer", 80));
        _detail.Columns.Add(new PanoColumn("Board", "Board", 80));
        _detail.Columns.Add(new PanoColumn("RefDes", "RefDes", 120));
        _detail.Columns.Add(new PanoColumn("Feature", "Feature", 160) { FillFreeSpace = true });
        _detail.Columns.Add(new PanoColumn("PassRate", "PassRate", 100) { Kind = PanoColumnKind.ProgressBar });
        _detail.Columns.Add(new PanoColumn("Decision", "Decision", 120) { Kind = PanoColumnKind.ComboBox, Editable = true, ComboBoxItems = new List<string>{"Bekliyor", "FalseCall", "Gerçek Hata"} });
...
{
    private readonly PanoControl _grid = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true, EnableRowDetails = true };
    ...
    public FrozenCommandDetailsSampleForm()
    {
        ...
        _grid.Columns.Add(new PanoColumn("Aç", "Open", 70) { Kind = PanoColumnKind.Button, AspectGetter = _ => "Aç" });
        _grid.Columns.Add(new PanoColumn("Barkod", "Barcode", 160) { FillFreeSpace = true });
        _grid.Columns.Add(new PanoColumn("Makine", "Machine", 120));
        _grid.Columns.Add(new PanoColumn("Sonuç", "Result", 90) { Kind = PanoColumnKind.Badge });
        _grid.Columns.Add(new PanoColumn("Skor", "Score", 100) { Kind = PanoColumnKind.ProgressBar });
        _grid.Columns.Add(new PanoColumn("Not", "Note", 240) { FillFreeSpace = true });
        ...
        _grid.SetRowDetailsProvider(new PanoRowDetailsProvider
        {
...
{
    private readonly PanoControl _grid = new() { Dock = DockStyle.Fill, FullRowSelect = true, ShowGridLines = true };
    ...
    public ConditionalFormattingSampleForm()
    {
        ...
        _grid.Columns.Add(new PanoColumn("Sonuç", "Result", 90) { Kind = PanoColumnKind.Badge });
        _grid.Columns.Add(new PanoColumn("Aktif", "Active", 70) { Kind = PanoColumnKind.CheckBox });
        _grid.Columns.Add(new PanoColumn("Skor", "Score", 110) { Kind = PanoColumnKind.ProgressBar });
        _grid.Columns.Add(new PanoColumn("Tarih", "TestDate", 150));
        ...
        _grid.ConditionalFormats.Add(new PanoConditionalFormat
        {
        ...
        var tool = new ToolStrip { Dock = DockStyle.Top, GripStyle = ToolStripGripStyle.Hidden };
        tool.Items.Add(new ToolStripButton("Sadece FAIL", null, (_,__) => _grid.SetColumnFilter(new PanoColumnFilter { AspectName = "Result", Mode = PanoFilterMode.Equals, Text = "FAIL" })));
