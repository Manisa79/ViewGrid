...
public sealed class ViewGridV51RealUsagePilotSampleForm : Form
{
    ...
    public ViewGridV51RealUsagePilotSampleForm()
    {
        Text = "ViewGrid 5.1 - Audix Pilot + Theme Audit + Example Cleanup";
        Width = 1360;
        Height = 820;
        ...
        LoadRows();
        ApplyTheme(true);
        UpdateDetails("ViewGrid 5.1 gerçek kullanım pilotu açıldı.");
    }
    ...
    {
        _grid.Columns.Clear();
        var cover = new ViewGridColumn("Kapak", nameof(AudixTrackRow.Cover), 96) { Kind = ViewGridColumnKind.Image };
        cover.ImageGetter = row => (row as AudixTrackRow)?.Cover;
        _grid.Columns.Add(cover);
        _grid.Columns.Add(new ViewGridColumn("Şarkı", nameof(AudixTrackRow.Title), 240) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        _grid.Columns.Add(new ViewGridColumn("Sanatçı", nameof(AudixTrackRow.Artist), 170));
        _grid.Columns.Add(new ViewGridColumn("Albüm", nameof(AudixTrackRow.Album), 180));
        _grid.Columns.Add(new ViewGridColumn("Tür", nameof(AudixTrackRow.MediaKind), 80) { Kind = ViewGridColumnKind.Badge });
        _grid.Columns.Add(new ViewGridColumn("Kalite", nameof(AudixTrackRow.Quality), 80) { Kind = ViewGridColumnKind.Badge });
        _grid.Columns.Add(new ViewGridColumn("Durum", nameof(AudixTrackRow.PlaybackState), 110) { Kind = ViewGridColumnKind.Badge });
    }

    private void ConfigureToolbar()
    {
        _tool.Items.Add(new ToolStripButton("Poster", null, (_, __) => SetMode(ViewGridMode.Poster)));
        _tool.Items.Add(new ToolStripButton("MediaTile", null, (_, __) => SetMode(ViewGridMode.MediaTile)));
        _tool.Items.Add(new ToolStripButton("Gallery", null, (_, __) => SetMode(ViewGridMode.Gallery)));
        _tool.Items.Add(new ToolStripButton("FilmStrip", null, (_, __) => SetMode(ViewGridMode.FilmStrip)));
    ...
    private void ConfigureMedia()
    {
        ...
        _grid.MediaKindGetter = row => (row as AudixTrackRow)?.MediaKind ?? ViewGridMediaKind.Audio;
        _grid.MediaPlaybackStateGetter = row => (row as AudixTrackRow)?.PlaybackState ?? ViewGridMediaPlaybackState.None;
    ...
    private void SetMode(ViewGridMode mode)
    {
        ...
        _grid.TilePosterMode = mode is ViewGridMode.Poster or ViewGridMode.Gallery or ViewGridMode.MediaTile or ViewGridMode.FilmStrip;
        UpdateDetails($"Görünüm modu değişti: {mode}. Audix'te albüm kapağı için en iyi modlar Poster / MediaTile / Gallery / FilmStrip.");
    }

    private void TogglePlayback(AudixTrackRow? clicked, ViewGridMediaPlaybackState currentState)
    {
        if (clicked == null) return;

        foreach (var row in _rows)
        {
            if (!ReferenceEquals(row, clicked) && row.PlaybackState == ViewGridMediaPlaybackState.Playing)
                row.PlaybackState = ViewGridMediaPlaybackState.None;
        }

        clicked.PlaybackState = currentState == ViewGridMediaPlaybackState.Playing
            ? ViewGridMediaPlaybackState.Paused
            : ViewGridMediaPlaybackState.Playing;

        UpdateDetails(clicked.MediaKind == ViewGridMediaKind.Video
    ...
    {
        _grid.ApplyTheme51AuditDefaults(_dark ? ViewGridThemeStudioPreset.AudixDark : ViewGridThemeStudioPreset.AudixLight);
    ...
    {
        var lines = _grid.RunViewGrid51UsageChecks()
            .Select(c => (c.Passed ? "✓ " : "! ") + c.Profile + " / " + c.Check + " - " + c.Message);
        MessageBox.Show(this, string.Join(Environment.NewLine, lines), "ViewGrid 5.1 Usage Check", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }

    private void ApplyTheme(bool dark)
    {
        _dark = dark;
        var preset = dark ? ViewGridThemeStudioPreset.AudixDark : ViewGridThemeStudioPreset.AudixLight;
        ...
        _header.Text = dark
            ? "ViewGrid 5.1 Audix Pilot - Koyu tema okunurluk testi"
            : "ViewGrid 5.1 Audix Pilot - Açık tema okunurluk testi";
    }

    private void LoadRows()
    {
        _rows.Clear();
        _rows.Add(new AudixTrackRow("Me Malone O Pateras Mou", "Antzela Dimitriou", "Greatest Hits", ViewGridMediaKind.Audio, "MP3", "03:28", CreateCover(Color.SteelBlue, "A1")));
        _rows.Add(new AudixTrackRow("Mia Litropi Agapi", "Antzela Dimitriou", "Greatest Hits", ViewGridMediaKind.Audio, "FLAC", "04:12", CreateCover(Color.MediumPurple, "A2")) { PlaybackState = ViewGridMediaPlaybackState.Playing });
        _rows.Add(new AudixTrackRow("Na Giriseis Xana", "Antzela Dimitriou", "This Is Greece", ViewGridMediaKind.Audio, "320", "03:57", CreateCover(Color.SeaGreen, "A3")));
        _rows.Add(new AudixTrackRow("Factory Clip 01", "Video Archive", "Factory Videos", ViewGridMediaKind.Video, "1080p", "01:14", CreateCover(Color.DarkOrange, "V1")));
        _rows.Add(new AudixTrackRow("AOI Defect Review", "Inspection Video", "AOI Review", ViewGridMediaKind.Video, "4K", "00:48", CreateCover(Color.Crimson, "V2")));
        _rows.Add(new AudixTrackRow("Kapak Eksik Örneği", "Unknown Artist", "Missing Covers", ViewGridMediaKind.Audio, "MISSING", "02:49", CreatePlaceholderCover()));
    ...
    private void UpdateSelectionDetails()
    {
        ...
        UpdateDetails("Seçilen medya" + Environment.NewLine +
            ...
            "Audix entegrasyon notu: Modelde Cover/Image property, PlaybackState, Quality ve MediaKind alanları olursa ViewGrid medya kartı state'i kendi çizebilir.");
    }

    private void UpdateDetails(string message)
    {
        _details.Text = message + Environment.NewLine + Environment.NewLine +
            "ViewGrid 5.1 hedefi:" + Environment.NewLine +
    ...
    private static Bitmap CreateCover(Color baseColor, string text)
    {
        ...
        TextRenderer.DrawText(g, "AUDIX / VIEWGRID", small, new Rectangle(0, 394, bmp.Width, 32), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
    ...
    {
        public AudixTrackRow(string title, string artist, string album, ViewGridMediaKind mediaKind, string quality, string duration, Image cover)
        {
            Title = title;
            Artist = artist;
            Album = album;
