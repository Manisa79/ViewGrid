...
public static class GLVSmartDragDropSamples
{
    ...
    public static FastViewGridControl CreateSmartDragDropGrid()
    {
        var grid = new FastViewGridControl
        {
            ...
            UserLayoutKey = "ViewGrid.TestApp.SmartDragDrop"
        };
        ...
        grid.RowDropValidating += (_, e) =>
        {
            ...
            if (e.TargetViewIndex < 0) e.Position = ViewGridDropPosition.After;
        };
        ...
        {
            if (e.Kind == ViewGridExternalDropKind.Files && e.Payload is string[] files)
                MessageBox.Show("Bırakılan dosya sayısı: " + files.Length);
            else if (e.Kind == ViewGridExternalDropKind.Text)
