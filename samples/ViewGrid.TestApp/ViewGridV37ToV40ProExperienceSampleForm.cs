...
public sealed class ViewGridV37ToV40ProExperienceSampleForm : Form
{
    ...
    public ViewGridV37ToV40ProExperienceSampleForm()
    {
        Text = "ViewGrid v37-v40 Pro Experience - Layout + Performance + Interaction + Analytics";
        Width = 1360;
        Height = 820;
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new ViewGridColumn("Tip", nameof(ProExperienceRow.Type), 120) { Kind = ViewGridColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Kod", nameof(ProExperienceRow.Code), 130));
        _grid.Columns.Add(new ViewGridColumn("Başlık", nameof(ProExperienceRow.Title), 260) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 2 });
        _grid.Columns.Add(new ViewGridColumn("Sahip", nameof(ProExperienceRow.Owner), 130) { AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Durum", nameof(ProExperienceRow.Status), 120) { Kind = ViewGridColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Değer", nameof(ProExperienceRow.Value), 90) { Kind = ViewGridColumnKind.ProgressBar });
        _grid.Columns.Add(new ViewGridColumn("Açıklama", nameof(ProExperienceRow.Detail), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
    }
    ...
    private void ApplyPhase(int phase)
    {
        switch (phase)
        {
            case 37:
                ...
                break;
            case 38:
                _grid.ApplyV38PerformanceProfile(ViewGridV38PerformancePreset.MediaLibrary);
            ...
            case 39:
                ...
                break;
            case 40:
                _grid.ApplyV40AnalyticsProfile(ViewGridV40AnalyticsPreset.FactoryOverview);
    ...
    private void LoadRows()
    {
        ...
        for (int i = 1; i <= 80; i++)
        {
            rows.Add(new ProExperienceRow
            {
                ...
                Title = "ViewGrid Pro Experience faz örneği " + i,
