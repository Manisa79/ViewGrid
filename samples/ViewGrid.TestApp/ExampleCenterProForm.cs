...
public sealed class ExampleCenterProForm : Form
{
    ...
    private string StateFile => Path.Combine(Application.StartupPath, "viewgrid-example-center-pro-state.json");

    public ExampleCenterProForm()
    {
        Text = "ViewGrid Example Center Pro - Özellik Rehberi";
        Width = 1440;
        Height = 900;
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new ViewGridColumn("Seç", nameof(ProRow.Selected), 48) { Kind = ViewGridColumnKind.CheckBox });
        _grid.Columns.Add(new ViewGridColumn("Tip", nameof(ProRow.Type), 110) { Kind = ViewGridColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Kod / Ticket", nameof(ProRow.Code), 150));
        _grid.Columns.Add(new ViewGridColumn("Başlık", nameof(ProRow.Title), 260) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
        _grid.Columns.Add(new ViewGridColumn("Hat / Makine", nameof(ProRow.Machine), 130) { AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Ref / Katman", nameof(ProRow.RefDes), 120));
        _grid.Columns.Add(new ViewGridColumn("İlerleme", nameof(ProRow.Progress), 90) { Kind = ViewGridColumnKind.ProgressBar });
        _grid.Columns.Add(new ViewGridColumn("Durum", nameof(ProRow.Status), 120) { Kind = ViewGridColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Açıklama", nameof(ProRow.Detail), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 5 });
    }

    private void ConfigureToolbar()
    {
        _tool.Items.Add(new ToolStripButton("State Kaydet", null, (_, __) => { _grid.AutoStateFilePath = StateFile; _grid.SaveStateToDefaultPath("Example Center Pro"); MessageBox.Show(this, "Grid state kaydedildi.", "ViewGrid", MessageBoxButtons.OK, MessageBoxIcon.Information); }));
        ...
        _tool.Items.Add(new ToolStripButton("Advanced Gizle/Göster", null, (_, __) => { bool hide = (_grid.HiddenMenuItems & ViewGridMenuItemKeys.AdvancedFilter) == 0; _grid.SetMenuItemVisible(ViewGridMenuItemKeys.AdvancedFilter, !hide); _grid.RefreshView(); }));
    }

    private void BuildScenarios()
    {
        _scenarios.Add(new ScenarioInfo("01  Ticket Dashboard", ViewGridScenario.TicketBoard, "AOI Support Desk ticketleri için durum, önem ve mesaj özeti odaklı kart vitrini.", "Açık/Bekliyor/Çözüldü ticket akışında teknisyenin en hızlı karar vereceği görünüm. Dashboard Kart, badge kolonları ve state engine birlikte gösterilir."));
        _scenarios.Add(new ScenarioInfo("02  Kanban Takip", ViewGridScenario.TicketBoard, "Ticket veya görevleri Kanban mantığında takip etmek için kart ağırlıklı görünüm.", "Open / In Progress / Waiting / Done benzeri süreçlerde kullanılabilir. ViewGrid burada gerçek kanban kolon motoruna geçmeden önce kart + grup yaklaşımı ile pratik bir kullanım sunar."));
        _scenarios.Add(new ScenarioInfo("03  MasterData BOM", ViewGridScenario.BomPositions, "BOM, SAP pozisyonları ve ürün ağacı detayları için yoğun ama okunabilir tablo.", "MasterData tarafında RefDes, malzeme kodu, katman, miktar ve durum gibi kolonlar yoğun listede hızlı taranır. Excel benzeri kullanım hedeflenir."));
        _scenarios.Add(new ScenarioInfo("04  Program Dosyaları", ViewGridScenario.ProgramFiles, "Dizgi programı, makine programı ve çıktı dosyaları için kart görünümü.", "Flex, QX, Axial, Radial gibi makine klasörleri ve program çıktılarında dosya odaklı seçim ekranı için uygundur."));
        _scenarios.Add(new ScenarioInfo("05  Makine / Hat Seçimi", ViewGridScenario.MachineOrLinePicker, "Hat, makine, feeder veya istasyon seçimi için ikon/kart ağırlıklı görünüm.", "AOI Support Desk, MasterData ve üretim araçlarında operatör veya teknisyen için hızlı seçim ekranı olarak kullanılabilir."));
        _scenarios.Add(new ScenarioInfo("06  İşlem Geçmişi", ViewGridScenario.Timeline, "Log, karar akışı, ticket mesajları ve operasyon geçmişi için zaman akışı.", "AOI false-call kararları, SAP sorgu adımları, server bağlantı logları veya kullanıcı aksiyonları için daha okunaklı bir geçmiş görünümü sağlar."));
        _scenarios.Add(new ScenarioInfo("07  Master-Detail", ViewGridScenario.MasterDetail, "Üst kayıt + alt detay mantığındaki bakım ekranları için profesyonel görünüm.", "MasterData ürün ağacı, Support Desk ticket mesajları veya makine bakım ekranlarında tek kayıt seçilip alt panelde uzun açıklama gösterilebilir."));
        _scenarios.Add(new ScenarioInfo("08  Yoğun Veri", ViewGridScenario.DenseData, "1M+ satır hedefli SQL/SAP/API listeleri için yoğun grid görünümü.", "Çok satır, az boşluk, hızlı filtre/sıralama ve kaydedilebilir kullanıcı layoutu hedeflenir."));
        _scenarios.Add(new ScenarioInfo("09  Filter Popup UX", ViewGridScenario.DenseData, "Uzun SAP malzeme adı ve program yolu filtrelerinde resize, tooltip, auto-width ve boyut hatırlama davranışı.", "MasterData, AOI Support Desk ve program dosyası seçim ekranlarında satıra sığmayan filtre değerlerini görmek için popup kenardan büyütülebilir; uzun değerlerde tooltip ve otomatik genişlik kullanılabilir."));
        _scenarios.Add(new ScenarioInfo("10  Advanced Filter & Preset", ViewGridScenario.DenseData, "Çok kolonlu AND/OR filtre oluşturucu, hızlı filtreler ve kaydedilebilir filtre presetleri.", "MasterData BOM, SAP pozisyonları, program yolu arama ve AOI ticket listelerinde kullanıcıların sık kullandığı filtreleri preset olarak saklaması için hazırlanmıştır. Sağ tık menüsünden Gelişmiş Filtre veya toolbar butonlarından hızlı preset akışı denenebilir."));
        _scenarios.Add(new ScenarioInfo("11  Menu & Icon Customization", ViewGridScenario.DenseData, "v27.5 menü grupları, item bazlı görünürlük, built-in menü kapatma/merge ve özel ikon kullanımını tek yerde gösterir.", "Designer tarafında MenuOptions ve MenuIcons genişletilebilir property olarak görünür. Runtime tarafında SetMenuGroupVisible, SetMenuItemVisible, SetCustomMenuIconFolder ve SetCustomMenuIconImageList ile proje bazlı menü/ikon kimliği verilebilir."));
        _scenarios.Add(new ScenarioInfo("12  Card Filter UX", ViewGridScenario.TicketBoard, "v27.7 kart, dashboard, kanban ve poster görünümlerinde header olmadan filtreye hızlı erişim.", "Üst hızlı filtre barı, floating filtre butonu ve aktif filtre chipleri büyük görünümlerde kullanıcıyı header menüsüne bağımlı bırakmaz. AOI ticket kartları, MasterData program kartları ve makine seçim ekranları için önerilir."));
        _scenarios.Add(new ScenarioInfo("13  Popular Enterprise Features", ViewGridScenario.DenseData, "v27.9 popüler enterprise grid davranışları: arama paneli, özet/footer, frozen column, conditional format, column chooser ve gelişmiş filtre presetleri.", "DevExpress/Telerik/Syncfusion tarzı gridlerde kullanıcıların en çok beklediği davranışları ViewGrid'de tek preset altında toplar. MasterData, Support Desk, büyük veri inceleme ve veri giriş ekranları için hazır ayarlar denenebilir."));
        _scenarios.Add(new ScenarioInfo("14  v28 UX Polish", ViewGridScenario.TicketBoard, "Kart/poster/dashboard görünümlerinde global filtreyi üst bara taşıyan ve popüler grid davranışlarını tek preset altında toplayan final UX vitrini.", "DevExpress, Telerik ve Syncfusion gibi güçlü gridlerdeki arama/filtre/preset/kolon seçici/özet davranışları ViewGrid'de hafif ve designer dostu property'ler olarak toparlanır. Bu senaryoda filtre butonu kart alanında değil, üst hızlı filtre barındadır."));
        _scenarios.Add(new ScenarioInfo("15  v28.1 Poster Mode", ViewGridScenario.TicketBoard, "ExtraLargeIcons yerine akılda kalıcı Poster görünüm modu, otomatik poster ölçüleri ve üst filtre UX'i.", "Poster görünümü büyük görsel kart senaryoları için tasarlandı. TilePosterMode altyapısını güvenli şekilde kullanır, ama kullanıcı tarafında ViewGridMode.Poster ismiyle seçilir. PosterPreferredWidth, PosterPreferredHeight, PosterImageHeight ve PosterModeAutoLayout property'leri designer/runtime tarafında ayarlanabilir."));
        _scenarios.Add(new ScenarioInfo("16  v31 Audix Media Temel Örnek", ViewGridScenario.ProgramFiles, "Albüm kapağı, medya rozeti, hover play overlay ve poster/gallery/filmstrip akışı.", "Audix için albüm kapağı gösterimi, FLAC/MP3 rozeti, placeholder ve media tile ayarları tek örnekte toplanır. Poster, Gallery, MediaTile ve FilmStrip modları bu senaryoda denenebilir."));
        _scenarios.Add(new ScenarioInfo("17  v32 Faz Merkezi / Nerede Bulurum", ViewGridScenario.DenseData, "Faz 38-48 özelliklerinin kısa açıklama, hedef proje ve görünüm modu ile tek listede bulunması.", "Example Center kalabalıklaştığı için bu merkez hangi özelliğin nerede kullanılacağını gösterir: UX Intelligence, Factory Intelligence, Timeline Engine, Document Explorer, Virtualization Pro, Search Everywhere, Command Palette, Layout Studio, Dashboard Builder, AI Layer ve Ecosystem."));
        _scenarios.Add(new ScenarioInfo("18  v33 Theme Lab / Okunurluk", ViewGridScenario.DenseData, "Koyu/açık tema, buton, combobox, bilgi metni, badge ve kart metinlerinin kontrast test merkezi.", "Theme Accessibility Engine; Fore/Back, Panel, Control, Border, Muted, Empty, Accent ve Selection renklerini normalize eder. Audix, AOI Support Desk, MasterData ve Line Workspace koyu/açık temalarında yazı kaybolmasını engellemek için hazırlanmıştır."));
        _scenarios.Add(new ScenarioInfo("19  v32 Factory Intelligence", ViewGridScenario.MachineOrLinePicker, "Makine durum overlay, üretim heatmap ve canlı dashboard hazırlığı.", "Line Workspace ve Factory Navigator için Running/Waiting/Fault/Offline durumları HeatMap/KPI kartlarına taşınır. SQL/SignalR/API canlı veri akışı için host tarafına hazır property ve helperlar eklendi."));
        _scenarios.Add(new ScenarioInfo("20  v32 Timeline Engine", ViewGridScenario.Timeline, "Ticket, makine ve işlem geçmişleri için daha güçlü zaman akışı.", "AOI Support Desk ticket geçmişi, makine alarm/start/stop kayıtları ve operatör-teknisyen mesajları Timeline modunda okunabilir hale getirilir."));
        _scenarios.Add(new ScenarioInfo("21  v32 Document Explorer", ViewGridScenario.ProgramFiles, "PDF, görsel, CAD, video ve klasör önizleme mantığı için Explorer tarzı galeri.", "Windows Explorer / medya arşivi mantığında dosya kartları, thumbnail, tür rozeti ve lazy-load senaryoları denenir. Audix ve teknik doküman arşivi için uygundur."));
        _scenarios.Add(new ScenarioInfo("22  v32 Virtualization Pro", ViewGridScenario.DenseData, "100K+ ve 1M+ satır hedefli yoğun veri ve tile virtualization hazırlığı.", "Çok büyük veri kaynaklarında DenseList, sanal provider ve hızlı arama davranışlarının nasıl konumlanacağı gösterilir."));
        _scenarios.Add(new ScenarioInfo("23  v32 Search Everywhere + Command Palette", ViewGridScenario.DenseData, "Ctrl+K / Ctrl+Shift+P tarzı arama ve komut merkezine hazır akış.", "Satır, kolon, filtre, görünüm ve export komutlarını tek merkezden bulmaya yönelik API ve örnek kullanım notları içerir."));
        _scenarios.Add(new ScenarioInfo("24  v32 Layout Studio", ViewGridScenario.MasterDetail, "Kullanıcı görünüm tasarlama, kaydetme ve yeniden yükleme akışı.", "Kolon, kart, dashboard ve filtre düzeninin host uygulama tarafından saklanabilmesi için SaveExperienceSnapshot ve ilgili propertyler gösterilir."));
        _scenarios.Add(new ScenarioInfo("25  v32 Dashboard Builder", ViewGridScenario.TicketBoard, "KPI, chart, table, card, heatmap, timeline ve gallery widget fikrini tek ViewGrid içinde toplar.", "Power BI benzeri hafif dashboard mantığı için widget tanımları, KPI kartları ve heatmap görünümü örneklenir."));
        _scenarios.Add(new ScenarioInfo("26  v32 AI Layer + Ecosystem", ViewGridScenario.DenseData, "Host uygulamanın ürettiği akıllı öneri/uyarı kartlarını ViewGrid ile birlikte gösterme hazırlığı.", "Eksik kapak, artan AOI hata oranı, PCB varyant benzerliği veya riskli makine durumları gibi öneriler ViewGridAiInsight listesinde taşınır."));
        _scenarios.Add(new ScenarioInfo("27  v34 Build Quality / Stability", ViewGridScenario.DenseData, "API çakışma kontrolü, compile-safe örnek merkezi ve geriye uyumluluk kalite taraması.", "Duplicate property/class risklerini azaltmak, eski projeleri kırmadan yeni fazları açmak ve Example Center içindeki senaryoları kalite raporu ile denetlemek için hazırlanmıştır."));
        _scenarios.Add(new ScenarioInfo("28  v35 Theme Studio", ViewGridScenario.DenseData, "Audix, AOI Support Desk, FactoryOS, SmokeWhite ve yüksek kontrast tema presetleri.", "Koyu/açık temada canlı önizleme, okunurluk normalizasyonu ve proje bazlı hazır tema paletleri tek merkezde denenir."));
        _scenarios.Add(new ScenarioInfo("29  v36 Media Pro / Cache & Overlay", ViewGridScenario.ProgramFiles, "Audix için albüm kapağı cache, placeholder, overlay, kalite rozeti ve media gruplama akışı.", "Poster, Gallery, MediaTile ve FilmStrip görünümlerinde albüm kapaklarını daha profesyonel göstermek için disk/bellek cache, eksik kapak davranışı ve seçili kart glow ayarları eklenmiştir."));
        _scenarios.Add(new ScenarioInfo("30  v37-v40 Pro Experience", ViewGridScenario.DenseData, "Enterprise Layout, Performance Pro, Interaction Pro ve Visual Analytics fazları tek merkezde.", "Layout kaydet/yükle, medya ve büyük veri performans profilleri, Command Palette/Search Everywhere, kısayollar, KPI/HeatMap/Timeline analiz akışları ViewGridV37ToV40ProExperienceSampleForm içinde canlı örneklenir."));
        _scenarios.Add(new ScenarioInfo("31  v50 Foundation / Modül Profilleri", ViewGridScenario.DenseData, "ViewGrid 5.0 modül profilleri, runtime stability check ve güvenli varsayılanlar.", "Audix, AOI Support Desk, Factory Intelligence, MasterData ve Bilge Defter için hazır modül profilleri; tema erişilebilirliği, medya playback ve komut paleti kontrolleri tek merkezden doğrulanır."));
        _scenarios.Add(new ScenarioInfo("32  v50.1 Example Center Navigator", ViewGridScenario.DenseData, "Örnek merkezinde kaybolmamak için kategori + arama destekli bulucu ekranı.", "Example Center artık Media/Audix, Theme/Okunurluk, Stability/Build, Performance, Analytics/Dashboard, Factory/AOI, Layout/Interaction ve Timeline/Kanban kategorileriyle filtrelenebilir."));
        _scenarios.Add(new ScenarioInfo("33  v50.2 Runtime Hardening Kontrolleri", ViewGridScenario.DenseData, "Derleme/API guard, runtime checks, tema okunurluğu ve Audix medya güvenli varsayılanları.", "Yeni faz eklemek yerine mevcut fazları projelerde daha güvenilir kullanmak için hazırlanmıştır. Duplicate property/event riskleri, medya cache/playback ayarları, tema erişilebilirliği ve Example Center bulucu akışı tek merkezde doğrulanır."));
        _scenarios.Add(new ScenarioInfo("34  ViewGrid 5.1 Audix Pilot / Theme Audit", ViewGridScenario.ProgramFiles, "Audix gerçek kullanım pilotu: albüm kapağı, playback state, video preview, tema audit ve Example Center cleanup.", "ViewGrid 5.1 artık yeni özellik ekleme yerine gerçek projeye tak-çalıştır kalitesine odaklanır. Audix tarafında Poster/MediaTile/Gallery/FilmStrip görünümleri, kapak cache, placeholder, play/pause state, now-playing rozeti ve video preview davranışı birlikte denenir. Theme Audit koyu/açık/yüksek kontrast okunurluğunu korur; Example Center cleanup ise özellikleri kategori ve arama ile bulunabilir hale getirir."));
        _scenarios.Add(new ScenarioInfo("35  Kolon Designer Editor Smoke Test", ViewGridScenario.DenseData, "Son kolon editor düzeltmesini test eder: gerçek ViewGridColumnCollectionEditor, Tamam/İptal, ekle/sil ve property korunması.", "Sample Hub içindeki 'Kolon Designer Editor Smoke Test' formu gerçek ViewGrid kolon editor penceresini açar. Audix benzeri Image/Card/Tooltip/SearchAlias propertyleri, PrivateColumn, Visible, Width ve Name/AspectName değerleri editor turundan sonra raporda karşılaştırılabilir. Arama: designer, kolon, editor, Audix column."));
        _scenarios.Add(new ScenarioInfo("36  Runtime View Menu Modern", ViewGridScenario.ProgramFiles, "Sağ tık menüsündeki modern görünüm grupları, medya presetleri, dashboard editor ve performance özetini test eder.", "ViewGrid runtime menüsü artık Akıllı presetler, Media Smart Preset, Dashboard preset editor, View Mode Memory ve Performance summary içerir. Sağ tık > Görünüm içinden Müzik/Film/Fotoğraf/Doküman presetleri, KPI/HeatMap/MiniChart toggle, medya cache/lazy image profili ve aktif görünüm bilgisi denenebilir. Arama: menü, view mode, dashboard, audix, media, filmstrip, performance."));
        _scenarios.Add(new ScenarioInfo("37  ViewGrid Diagnostics Center", ViewGridScenario.DenseData, "Build quality, runtime hardening, media, dashboard, view mode memory ve paint performansını tek raporda toplar.", "ViewGrid Diagnostics Center; v34 Build Quality, v50 Foundation, v50.2 Hardening, v51 Real Usage, Media Smart Preset, Dashboard Preset Editor, View Mode Memory ve Performance pass kontrollerini tek metin raporda birleştirir. Sağ tık > Görünüm > Dashboard preset editor > Diagnostics Center ile pencere açılabilir; rapor kopyalanabilir ve yenilenebilir. Arama: diagnostics, kalite, performance, dashboard, media, view mode."));
    }
    ...
    private void ApplySelectedScenario()
    {
        ...
        {
            _grid.ApplyPopularFeaturePack(ViewGridPopularFeaturePreset.MasterData);
            _grid.AddSemanticStatusConditionalFormat(nameof(ProRow.Status));
            _grid.AddCountSummary(nameof(ProRow.Code));
            _grid.AddNumericSummary(nameof(ProRow.Progress), ViewGrid.Summary.ViewGridSummaryType.Average, "Ort. %{0:0}");
        }
        else if (scenario.Title.Contains("Card Filter UX", StringComparison.OrdinalIgnoreCase))
        {
            _grid.SetViewMode(ViewGridMode.DashboardCard);
        ...
        {
            _grid.SetViewMode(ViewGridMode.Kanban);
            _grid.SetGroupBy(nameof(ProRow.Status));
        }
        else if (scenario.Scenario == ViewGridScenario.MachineOrLinePicker)
        {
            _grid.SetGroupBy(nameof(ProRow.Type));
        }
        else if (scenario.Scenario == ViewGridScenario.Timeline)
        {
        ...
        else if (scenario.Title.Contains("Faz Merkezi", StringComparison.OrdinalIgnoreCase))
        {
            ...
            _grid.SetViewMode(ViewGridMode.RowPreview);
        }
        else if (scenario.Title.Contains("Theme Lab", StringComparison.OrdinalIgnoreCase))
        {
            ...
            _grid.ApplyTheme(ViewGridThemeAccessibility.Normalize(ViewGridTheme.FromParentColor(BackColor, ForeColor)));
        }
        else if (scenario.Title.Contains("Factory Intelligence", StringComparison.OrdinalIgnoreCase))
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.FactoryIntelligence);
        ...
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.TimelineEngine);
        }
        else if (scenario.Title.Contains("Document Explorer", StringComparison.OrdinalIgnoreCase))
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.DocumentExplorer);
            _grid.ShowMediaQualityBadge = true;
        ...
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.VirtualizationPro);
        }
        else if (scenario.Title.Contains("Search Everywhere", StringComparison.OrdinalIgnoreCase))
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.SearchEverywhere);
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.CommandPalette);
        }
        else if (scenario.Title.Contains("Layout Studio", StringComparison.OrdinalIgnoreCase))
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.LayoutStudio);
            _grid.SetViewMode(ViewGridMode.MasterDetail);
        }
        else if (scenario.Title.Contains("Dashboard Builder", StringComparison.OrdinalIgnoreCase))
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.DashboardBuilder);
        }
        else if (scenario.Title.Contains("AI Layer", StringComparison.OrdinalIgnoreCase))
        {
            _grid.ApplyV32ExperiencePhase(ViewGridExperiencePhase.AiLayer);
        ...
        {
            _grid.ApplyV35ThemeStudioPack(ViewGridThemeStudioPreset.FactoryOsDark);
            _grid.SetViewMode(ViewGridMode.KpiDashboard);
        }
        else if (scenario.Title.Contains("v37-v40 Pro Experience", StringComparison.OrdinalIgnoreCase))
        {
            ...
            _grid.ApplyV38PerformanceProfile(ViewGridV38PerformancePreset.Balanced);
            _grid.ApplyV39InteractionProfile(ViewGridV39InteractionPreset.PowerUser);
            _grid.ApplyV40AnalyticsProfile(ViewGridV40AnalyticsPreset.KpiDashboard);
            _grid.SetViewMode(ViewGridMode.KpiDashboard);
        }
        else if (scenario.Title.Contains("ViewGrid 5.1 Audix", StringComparison.OrdinalIgnoreCase))
        {
        ...
        {
            _grid.ApplyMediaSmartPreset(ViewGridMediaSmartPreset.Music);
            _grid.MediaQualityBadgeAspectName = nameof(ProRow.Type);
            _grid.SetViewMode(ViewGridMode.MediaTile);
            ...
            _grid.HeaderContextMenuBehavior = ViewGridHeaderContextMenuBehavior.Full;
        }
        else if (scenario.Title.Contains("Diagnostics Center", StringComparison.OrdinalIgnoreCase))
        {
            _grid.ApplyViewGrid51RealUsageDefaults();
            _grid.ApplyMediaSmartPreset(ViewGridMediaSmartPreset.Music);
            _grid.MediaQualityBadgeAspectName = nameof(ProRow.Status);
            _grid.EnableViewGridDiagnosticsCenter = true;
            _grid.EnablePaintPerformanceMetrics = true;
            _grid.RememberViewModePerScenario = true;
            _grid.EnsureDefaultDashboardWidgets();
            _grid.SetDashboardWidgetEnabled(ViewGridDashboardWidgetKind.Kpi, true);
            _grid.SetDashboardWidgetEnabled(ViewGridDashboardWidgetKind.HeatMap, true);
            _grid.SetDashboardWidgetEnabled(ViewGridDashboardWidgetKind.Chart, true);
            _grid.SetViewMode(ViewGridMode.RowPreview);
            _grid.HeaderContextMenuBehavior = ViewGridHeaderContextMenuBehavior.Full;
        }
    ...
    private static ScenarioDoc BuildScenarioDoc(ScenarioInfo scenario)
    {
        ...
        if (title.Contains("Ticket Dashboard", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Ticket durumlarını kart/dashboard dilinde hızlı okumayı anlatır.", "Satırları durum veya makineye göre grupla; sağ tık menüsünden kart ve dashboard görünümleri arasında geç.", "Ticket tipi, durum rozeti ve ilerleme bilgisi aynı kartta okunur.", "ApplyScenario(TicketBoard), ViewGridMode.DashboardCard, Badge, ProgressBar", "ticket, dashboard, card, badge, support desk");

        if (title.Contains("Kanban", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "İş akışını durum gruplarıyla kanban mantığına yaklaştırır.", "Duruma göre gruplamayı ve Kanban görünümünü dene.", "Open/In Progress/Waiting/Done akışı görsel olarak ayrılır.", "ViewGridMode.Kanban, SetGroupBy(Status)", "kanban, workflow, ticket, status");

        if (title.Contains("MasterData BOM", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Yoğun teknik veride klasik tablo verimliliğini gösterir.", "Filtre, sıralama, kolon genişliği ve State Kaydet/Yükle akışını dene.", "BOM/SAP satırları dar alanda okunur ve kullanıcı düzeni korunur.", "ViewGridScenario.BomPositions, DenseData, FilterMenuMode", "bom, sap, dense, table, masterdata");

        if (title.Contains("Program Dosyaları", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Dosya/program seçimi için kart odaklı görünümü anlatır.", "Kart, liste ve sağ tık görünüm modları arasında geç.", "Program klasörleri tablo yerine daha seçilebilir kartlar halinde görünür.", "ViewGridScenario.ProgramFiles, Tile, RowCard", "program, file, folder, machine");
        ...
        if (title.Contains("İşlem Geçmişi", StringComparison.OrdinalIgnoreCase) || title.Contains("Timeline Engine", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Log, ticket mesajı ve makine olaylarını zaman akışı gibi okumayı anlatır.", "Timeline görünümüne geç ve makineye göre gruplamayı dene.", "Olay kayıtları klasik tablodan daha kronolojik ve taranabilir görünür.", "ViewGridMode.Timeline, ViewGridExperiencePhase.TimelineEngine", "timeline, log, history, event");

        if (title.Contains("Master-Detail", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Üst kayıt ve detay açıklama düzenini gösterir.", "Bir satır seç; MasterDetail ve DetailCard görünümlerini karşılaştır.", "Seçilen kayıt ana bilgileriyle birlikte uzun detayını kaybetmeden okunur.", "ViewGridMode.MasterDetail, DetailCard", "master detail, property, detail");
        ...
        if (title.Contains("Yoğun Veri", StringComparison.OrdinalIgnoreCase) || title.Contains("Virtualization", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Büyük listelerde yoğun görünüm, filtre ve performans yaklaşımını anlatır.", "DenseList/Details arasında geç; arama ve filtreyi yoğun satırda dene.", "Çok satırlı veri az boşlukla, hızlı taranabilir şekilde görünür.", "ViewGridMode.DenseList, V38PerformancePreset, ViewCount", "performance, dense, virtual, big data");
        ...
        if (title.Contains("Poster Mode", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Görsel ağırlıklı poster kart ölçülerini ve medya kart davranışını gösterir.", "Poster görünümüne geç; poster genişlik/yükseklik ve image height etkisini izle.", "ExtraLargeIcons yerine daha anlaşılır Poster modu kullanılır.", "ViewGridMode.Poster, PosterPreferredWidth, PosterImageHeight", "poster, image, card");
        ...
        if (title.Contains("Faz Merkezi", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "ViewGrid fazlarının hangi projede ne işe yaradığını bir indeks gibi açıklar.", "Satırları faz, hedef proje ve durum alanlarına göre filtrele.", "Özelliklerin isimleri değil, kullanım alanları anlaşılır hale gelir.", "ApplyV32UltimateExperiencePack, ViewGridExperiencePhase", "roadmap, phase, feature index");

        if (title.Contains("Theme", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Koyu/açık/yüksek kontrast tema okunurluğunu ve hazır paletleri gösterir.", "Açık Tema/Koyu Tema butonlarını dene; yazı, kart, badge ve buton kontrastını gözle.", "Metinler koyu ve açık temada kaybolmadan okunur.", "ViewGridThemeAccessibility, ApplyV35ThemeStudioPack, EnforceThemeAccessibility", "theme, contrast, readability");
        ...
        if (title.Contains("AI Layer", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Host uygulamanın ürettiği öneri/uyarıları ViewGrid içinde göstermeye hazırlık yapar.", "AI insight satırlarını filtrele; dashboard/timeline ile nasıl birleşebileceğini incele.", "ViewGrid karar vermez, host uygulamanın ürettiği sinyalleri okunur hale getirir.", "AddAiInsight, ViewGridAiInsight", "ai, insight, suggestion");
        ...
        if (title.Contains("Build Quality", StringComparison.OrdinalIgnoreCase) || title.Contains("Hardening", StringComparison.OrdinalIgnoreCase) || title.Contains("Foundation", StringComparison.OrdinalIgnoreCase) || title.Contains("Diagnostics", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "API, runtime, tema, medya ve dashboard stabilitesini kontrol eden güvenlik/kalite yüzeyini anlatır.", "Sağ tık > Görünüm > Dashboard preset editor > Diagnostics Center aç; raporu yenile ve kopyala.", "Riskli ayarlar uyarı olarak görünür, aktif guard ve presetler doğrulanır.", "RunBuildQualityDiagnostics, RunViewGrid502RuntimeHardeningChecks, RunViewGridDiagnosticsCenterText", "diagnostics, quality, stability, hardening");

        if (title.Contains("Kolon Designer", StringComparison.OrdinalIgnoreCase))
            return ScenarioDoc.Create(scenario, "Visual Studio designer tarafındaki ViewGrid kolon editörünün stabil çalışmasını test eder.", "Sample Hub içindeki Kolon Designer Editor Smoke Test formunu aç; ekle/sil/tamam/iptal davranışını dene.", "ViewGrid kendi kolon editörünü açar, property değerleri korunur ve gereksiz field uyarıları azalır.", "ViewGridColumnCollectionEditor, ViewGridControlDesigner, GLVColumn", "designer, column, editor, visual studio");

        return ScenarioDoc.Create(scenario, "Bu örnek seçilen ViewGrid senaryosunun temel kullanımını açıklar.", "Sağ tık menüsü, toolbar butonları ve görünüm modlarını deneyerek davranışı karşılaştır.", "Seçilen senaryonun görünüm, filtre, state ve data sunumu netleşir.", "ApplyScenario, SetViewMode, ViewGridScenario", "viewgrid, example, scenario");
    }
    ...
    private void LoadRows(ScenarioInfo scenario)
    {
        ...
        else if (scenario.Scenario == ViewGridScenario.BomPositions || scenario.Scenario == ViewGridScenario.MasterDetail || scenario.Scenario == ViewGridScenario.DenseData)
        {
            AddBomRows();
        }
        else if (scenario.Scenario == ViewGridScenario.ProgramFiles)
        {
            AddProgramRows();
        }
        else if (scenario.Scenario == ViewGridScenario.MachineOrLinePicker)
    ...
    {
        _grid.MenuIconMode = ViewGridMenuIconMode.BuiltInThenCustom;
        _grid.MenuIconSize = ViewGridMenuIconSize.Medium20;
        _grid.MenuProfile = ViewGridMenuProfile.Custom;
        _grid.MenuOptions.HeaderGroups = ViewGridMenuGroups.Filter | ViewGridMenuGroups.Sort | ViewGridMenuGroups.ColumnChooser | ViewGridMenuGroups.ViewMode | ViewGridMenuGroups.State | ViewGridMenuGroups.Scenario;
        _grid.MenuOptions.BodyGroups = ViewGridMenuGroups.Clipboard | ViewGridMenuGroups.Filter | ViewGridMenuGroups.ViewMode | ViewGridMenuGroups.Theme;
        _grid.MenuOptions.MergedGroups = ViewGridMenuGroups.Filter | ViewGridMenuGroups.Sort | ViewGridMenuGroups.ColumnChooser | ViewGridMenuGroups.ViewMode;
        _grid.VisibleMenuItems = ViewGridMenuItemKeys.All;
        _grid.HiddenMenuItems = ViewGridMenuItemKeys.None;
    }
    ...
    private void AddBomRows()
    {
        ...
        for (int i = 1; i <= 80; i++)
        {
            _rows.Add(new ProRow
            {
                ...
                Progress = 40 + (i % 60),
                Status = statuses[i % statuses.Length],
                Detail = "MasterData ekranlarında SAP, ürün ağacı, BOM, RefDes, katman ve program eşleşmesi aynı ViewGrid senaryosu ile gösterilebilir."
            });
        }
    }
    ...
    private void AddAudixMediaRows()
    {
        ...
        for (int i = 1; i <= 42; i++)
        {
            _rows.Add(new ProRow
            {
                ...
                Detail = "Albüm kapağı ImageGetter ile bağlanır; ViewGrid Poster/Gallery/MediaTile/FilmStrip modlarında aynı data farklı sunulur."
            });
        }
    }
    ...
    private void AddV32PhaseRows()
    {
        ...
        AddPhaseRow(47, "AI Layer", "Host uygulamanın ürettiği akıllı önerileri ViewGrid ile gösterme", "Audix / AOI / MasterData");
        AddPhaseRow(48, "ViewGrid Ecosystem", "ViewGridKanban, ViewGridTimeline, ViewGridDashboard gibi ortak çekirdek yaklaşımı", "Uzun vadeli platform");
    ...
    private void AddPhaseRow(int id, string title, string detail, string target)
    {
        _rows.Add(new ProRow
        {
            ...
            Title = title,
            Machine = target,
            RefDes = "ViewGridExperiencePhase." + title.Replace(" ", string.Empty),
    ...
    {
        AddPhaseRow(34, "Build Quality", "Duplicate property/class riskleri, public API taraması, kolon/view/theme/media kalite kontrol raporu", "ViewGrid çekirdeği / Example Center");
    ...
    {
        _rows.Add(new ProRow { Id = 37001, Type = "Diagnostics", Code = "CENTER", Title = "ViewGrid Diagnostics Center", Machine = "Runtime Menu", RefDes = "ShowViewGridDiagnosticsCenter", Progress = 100, Status = "Ready", Detail = "Sağ tık > Görünüm > Dashboard preset editor > Diagnostics Center penceresi build quality, runtime hardening, media, dashboard ve performance kontrollerini tek raporda toplar." });
        _rows.Add(new ProRow { Id = 37002, Type = "Quality", Code = "V34", Title = "Build Quality Aggregation", Machine = "ViewGrid.Core", RefDes = "RunBuildQualityDiagnostics", Progress = 100, Status = "OK", Detail = "Public API, kolon, aktif görünüm, tema kontrastı ve medya kaynak uyarıları tek rapora aktarılır." });
        _rows.Add(new ProRow { Id = 37003, Type = "Runtime", Code = "V50", Title = "Foundation Runtime Checks", Machine = "ViewGrid.Core", RefDes = "RunViewGrid5RuntimeChecks", Progress = 100, Status = "OK", Detail = "Tema erişilebilirliği, media playback state, command palette ve modül profili hızlı kontrol edilir." });
        _rows.Add(new ProRow { Id = 37004, Type = "Hardening", Code = "V50.2", Title = "Runtime Hardening Checks", Machine = "ViewGrid.Core", RefDes = "RunViewGrid502RuntimeHardeningChecks", Progress = 100, Status = "OK", Detail = "Tema, medya cache/lazy loading, playback overlay, search/command ve layout studio guard değerleri doğrulanır." });
    ...
    private void AddDocumentExplorerRows()
    {
        ...
        for (int i = 1; i <= 48; i++)
        {
            _rows.Add(new ProRow
            {
                ...
                Detail = "ViewGridDocumentPreviewKind ile host uygulama thumbnail üretimini yönetebilir. Gallery görünümü Explorer hissi verir."
            });
        }
    }
    ...
    private void AddDashboardBuilderRows()
    {
        ...
        for (int i = 0; i < widgets.Length; i++)
        {
            _rows.Add(new ProRow
            {
                ...
                RefDes = "ViewGridDashboardWidgetKind." + widgets[i],
                Progress = 70 + i * 3,
                Status = i % 2 == 0 ? "Aktif" : "Hazır",
                Detail = "DashboardWidgets koleksiyonu ile host uygulama ViewGrid içinde hafif dashboard düzeni hazırlayabilir."
            });
        }
    }
    ...
    {
        _rows.Add(new ProRow { Id = 5021, Type = "Build", Code = "API-GUARD", Title = "Statik API Guard", Machine = "tools/viewgrid_api_guard.py", RefDes = "Duplicate/Namespace", Progress = 94, Status = "OK", Detail = "Duplicate property/event, eksik namespace ve örnek merkez referans riskleri için guard akışı güçlendirildi." });
        _rows.Add(new ProRow { Id = 5022, Type = "Runtime", Code = "CHECKS", Title = "RunViewGrid502RuntimeHardeningChecks", Machine = "ViewGrid.Core", RefDes = "Theme/Media/Layout", Progress = 100, Status = "OK", Detail = "Host uygulama açılışında tema, medya, interaction ve layout güvenliğini tek listede doğrulayabilir." });
        ...
        _rows.Add(new ProRow { Id = 5025, Type = "Performance", Code = "CACHE", Title = "Media Cache Limit Guard", Machine = "ViewGrid.Media", RefDes = "512 Audix", Progress = 90, Status = "Watch", Detail = "Audix profili medya memory cache limitini yükseltir; büyük arşivlerde lazy loading ile birlikte kullanılmalıdır." });
    ...
    {
        _rows.Add(new ProRow { Id = 5001, Type = "Stability", Code = "CHECK-THEME", Title = "Theme Accessibility Guard", Machine = "ViewGrid.Core", RefDes = "Dark/Light", Progress = 100, Status = "OK", Detail = "Koyu/açık tema geçişlerinde metin, buton, combo, kart ve boş mesaj kontrastı merkezi olarak normalize edilir." });
        _rows.Add(new ProRow { Id = 5002, Type = "Stability", Code = "CHECK-MEDIA", Title = "Media Playback State Guard", Machine = "ViewGrid.Media", RefDes = "Audio/Video", Progress = 100, Status = "OK", Detail = "Audix gibi uygulamalarda play tuşuna basıldığında kart playing/paused/loading/error durumunu görsel olarak gösterir." });
        ...
        _rows.Add(new ProRow { Id = 5006, Type = "Guard", Code = "API-GUARD", Title = "API Guard / Duplicate Risk Control", Machine = "Build", RefDes = "Property", Progress = 88, Status = "Watch", Detail = "ViewGridColumnKind, EnableCommandPalette, Size overload gibi önceki hataları yakalamak için kalite kontrol akışı dokümante edildi." });
    }
    ...
    {
        public ScenarioInfo(string title, ViewGridScenario scenario, string description, string longDescription)
        {
            Title = title;
            Scenario = scenario;
        ...
        public ViewGridScenario Scenario { get; }
        ...
        private static string ResolveScope(string title)
        {
            ...
            if (title.Contains("ViewGrid 5.1", StringComparison.OrdinalIgnoreCase)) return "Pilot";
        ...
        private static string ResolveCategory(string title, ViewGridScenario scenario)
        {
            ...
            if (scenario == ViewGridScenario.Timeline) return "Workflow / Kanban / Timeline";
