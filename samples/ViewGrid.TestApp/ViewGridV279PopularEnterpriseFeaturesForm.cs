...
public sealed class ViewGridV279PopularEnterpriseFeaturesForm : Form
{
    ...
    private readonly ViewGridControl _grid = new()
    {
        ...
        PopularFeaturePreset = ViewGridPopularFeaturePreset.MasterData,
    ...
    public ViewGridV279PopularEnterpriseFeaturesForm()
    {
        Text = "ViewGrid v27.9 - Popular Enterprise Features";
        Width = 1380;
        Height = 820;
    ...
    {
        _tool.Items.Add(new ToolStripButton("Balanced", null, (_, __) => ApplyPreset(ViewGridPopularFeaturePreset.Balanced)));
        _tool.Items.Add(new ToolStripButton("MasterData", null, (_, __) => ApplyPreset(ViewGridPopularFeaturePreset.MasterData)));
        _tool.Items.Add(new ToolStripButton("SupportDesk", null, (_, __) => ApplyPreset(ViewGridPopularFeaturePreset.SupportDesk)));
        _tool.Items.Add(new ToolStripButton("Large Data", null, (_, __) => ApplyPreset(ViewGridPopularFeaturePreset.LargeDataReview)));
        _tool.Items.Add(new ToolStripButton("Data Entry", null, (_, __) => ApplyPreset(ViewGridPopularFeaturePreset.DataEntry)));
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new ViewGridColumn("ID", nameof(EnterpriseRow.Id), 58));
        _grid.Columns.Add(new ViewGridColumn("Tip", nameof(EnterpriseRow.Type), 105) { Kind = ViewGridColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Kod", nameof(EnterpriseRow.Code), 130));
        _grid.Columns.Add(new ViewGridColumn("Başlık / Malzeme", nameof(EnterpriseRow.Title), 280) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 3 });
        _grid.Columns.Add(new ViewGridColumn("Makine", nameof(EnterpriseRow.Machine), 130) { AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Durum", nameof(EnterpriseRow.Status), 126) { Kind = ViewGridColumnKind.Badge, AllowGroup = true });
        _grid.Columns.Add(new ViewGridColumn("Miktar", nameof(EnterpriseRow.Quantity), 90));
        _grid.Columns.Add(new ViewGridColumn("İlerleme", nameof(EnterpriseRow.Progress), 96) { Kind = ViewGridColumnKind.ProgressBar });
        _grid.Columns.Add(new ViewGridColumn("Etiketler", nameof(EnterpriseRow.Tags), 230) { Kind = ViewGridColumnKind.Tags });
        _grid.Columns.Add(new ViewGridColumn("Açıklama / Yol", nameof(EnterpriseRow.Description), 420) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
    }
    ...
    private void ApplyDemoEnhancements()
    {
        ...
        _grid.AddNumericSummary(nameof(EnterpriseRow.Quantity), ViewGridSummaryType.Sum, "Toplam {0}");
        _grid.AddNumericSummary(nameof(EnterpriseRow.Progress), ViewGridSummaryType.Average, "Ort. %{0:0}");
    ...
    private void ApplyPreset(ViewGridPopularFeaturePreset preset)
    {
    ...
    {
        _dark = dark;
        var theme = dark ? ViewGridTheme.DarkTheme() : ViewGridTheme.LightTheme();
