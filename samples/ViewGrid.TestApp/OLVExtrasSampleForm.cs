...
public sealed class OLVExtrasSampleForm : Form
{
    ...
    {
        Text = "ViewGrid OLV Ekstra Uyumluluk ve Faydalı API Örneği";
        Width = 1200;
        Height = 720;
        StartPosition = FormStartPosition.CenterScreen;

        _grid.Columns.Add(new ViewGridColumn("Person", nameof(OlvExtraRow.Person), 180));
        _grid.Columns.Add(new ViewGridColumn("Occupation", nameof(OlvExtraRow.Occupation), 140));
        _grid.Columns.Add(new ViewGridColumn("Status", nameof(OlvExtraRow.Status), 120));
        _grid.Columns.Add(new ViewGridColumn("Key", nameof(OlvExtraRow.Key), 90));
        _grid.Columns.Add(new ViewGridColumn("Notes", nameof(OlvExtraRow.Notes), 460) { FillFreeSpace = true, WordWrap = true, MaxTextLines = 4 });
